...
/**
 * จำนวนเส้นที่ติ๊กให้ตั้งแต่เปิดหน้า — **น้อยกว่าเพดาน** โดยตั้งใจ
 *
 * 🛑 ลดจาก 5 → 3 (2026-08-29 หลัง user เปิดของจริงแล้วบอกว่า "chart ดูยาก")
 * ต้นเหตุไม่ใช่เฉดสี แต่คือ **ปริมาณข้อมูลต่อพื้นที่**: ค่าอยู่ในช่วง 0–6 (แคบมาก) × 31 วัน
 * × 5 เส้น ⇒ เส้นกองทับกันในแถบแนวตั้งบาง ๆ และตัดกันตลอดทั้งกราฟ
 * ลดจำนวนเส้นคือตัวลดจุดตัดที่ได้ผลตรงที่สุดโดยไม่เสียความสามารถ — ผู้ใช้ยังติ๊กเพิ่มได้ถึง 6
 */
export const DEFAULT_CHART_SERIES = 3
 ...
 */
/**
 * ลำดับสำคัญกว่ารายชื่อ — ตำแหน่งในอาเรย์กำหนดว่าสินค้าอันดับที่เท่าไรได้สีอะไร
 * และกำหนดว่า **ตัวไหนถูกดันไปเป็นสีสำรอง** (ตำแหน่งท้าย = ต้องติ๊กเพิ่มเองถึงจะเห็น)
 *
 * 🛑 เรียงใหม่ 2026-08-29: ของเดิม `primary → secondary → alpha → beta → delta` ทำให้
 * 3 ใน 5 สีตั้งต้นอยู่ในตระกูลน้ำเงิน-ม่วง และที่แย่ที่สุดคือ **`chart-primary` (213°) กับ
 * `chart-delta` (193°) ห่างกันแค่ 20° — ใกล้ที่สุดในทั้ง 6 โทเคน** แต่กลับอยู่ตำแหน่ง 1 กับ 5
 * ของค่าตั้งต้นพอดี (user รายงานเองว่า "ฟ้าอ่อนกับน้ำเงินแยกกันแทบไม่ออก")
 *
 * ลำดับใหม่ดัน `delta` ไปเป็นสีสำรองท้ายสุด แล้วเรียงที่เหลือให้ห่างกันมากที่สุด:
 *   primary 213° → beta 345° (ห่าง 132°) → zeta 27° → alpha 170° (ห่าง 143°) → secondary 245°
 * คู่ที่อ่อนสุดที่เหลือคือ primary-secondary (32°) ซึ่ง **แก้ต่อไม่ได้โดยไม่ทำให้แย่ลง**
 * (ดัน secondary ออกแทน จะได้คู่ primary-delta 20° ซึ่งแย่กว่า) — ชดเชยด้วยการวางไว้
 * ตำแหน่ง 1 กับ 5 ไม่ติดกัน และอยู่คนละกลุ่ม dash
 *
 * 🛑 **`chart-gamma` ยังถูกตัดออกต่อไป และเหตุผลแข็งกว่าเดิม** — ตรวจ hex จริงแล้ว
 * `--color-chart-gamma` กับ `--color-warning` เป็น **`#f9bf59` เหมือนกันทุกตัวอักษร**
 * (ไม่ใช่แค่โทนใกล้กัน) ⇒ สินค้าที่ติดป้าย "เงียบมาแล้ว N วัน" (badge warning) ถ้าบังเอิญ
 * ได้เส้นสี gamma ด้วย ผู้ใช้จะอ่านว่า "เส้นสีนี้ = สินค้าที่มีปัญหา" ทั้งที่สีเส้นถูกแจกตาม
 * อันดับยอดขาย ไม่เกี่ยวกับสถานะเลย — หลักการเดียวกับ Verified-Means-Green
 * (สีต้องสื่อความหมายเดียวกันทุกที่ในหน้า) แม้จะเป็นคนละสีจากเขียว
 */
export const CHART_COLOR_TOKENS = [
  'chart-primary', // 213° น้ำเงิน
  'chart-beta', // 345° ชมพูแดง
  'chart-zeta', // 27° ส้มอิฐ
  'chart-alpha', // 170° เขียวมิ้นต์
  'chart-secondary', // 245° ม่วง
  'chart-delta', // 193° ฟ้าอ่อน — สีสำรอง ใกล้ primary ที่สุด จึงไม่อยู่ในชุดตั้งต้น
] as const
