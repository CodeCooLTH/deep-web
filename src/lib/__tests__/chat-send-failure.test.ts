...
/**
 * [blocker] — บับเบิลที่ล้มเพราะ "อีกแอปถือสิทธิ์คุมเธรด" ต้องมีทางไปต่อ
 *
 * user รายงาน 2026-09-10: เธรดที่ไม่มี marker ของ Meta AI จะไม่เข้าเงื่อนไขแถบ manual-override
 * ⇒ ร้านเห็นแต่บับเบิลแดง + "ยกเลิก" ทั้งที่ retryable=false (กดลองใหม่ไม่มีวันผ่าน) = ทางตัน
 * ทางออกเดียวที่ได้ผลจริงคือไปกดรับดูแลที่ Business Suite
 */
describe('[blocker] ทางออกของสาเหตุ "อีกแอปถือสิทธิ์คุมเธรด"', () => {
  const RAW_AI = '(#10) Message failed to send because another app is controlling this thread now.'

  it('ติดธง action=BUSINESS_SUITE และยังกดลองใหม่ไม่ได้', () => {
    const d = describeSendFailure(RAW_AI)
    expect(d.known).toBe(true)
    expect(d.retryable).toBe(false)
    expect(d.action).toBe('BUSINESS_SUITE')
  })

  /** สาเหตุอื่นต้องไม่ติดธงนี้ ไม่งั้นปุ่มจะไปโผล่ในที่ที่ Business Suite แก้ไม่ได้ */
  it('สาเหตุอื่นไม่ติดธง', () => {
    for (const raw of [
      '(#10) This message is sent outside of allowed window.',
      "(#100) Cannot tag messages with 'HUMAN_AGENT' without prior approval.",
      "(#551) This person isn't available right now.",
      'อะไรก็ไม่รู้ที่เราไม่รู้จัก',
      '',
    ]) {
      expect(describeSendFailure(raw).action, raw).toBeNull()
    }
  })

  it('UI อ่านธงจาก failDetail ไม่ได้ match ถ้อยคำเองซ้ำ (HR16)', async () => {
    const fs = await import('fs')
    const p = 'src/app/(paces)/seller/(chat)/inbox/[conversationId]/components/ChatThread.tsx'
    const code = fs
      .readFileSync(p, 'utf8')
      .split('\n')
      .filter((l) => !l.trim().startsWith('//') && !l.trim().startsWith('*'))
      .join('\n')
    expect(code).toMatch(/failDetail\?\.action === 'BUSINESS_SUITE'/)
    // ห้ามมีการ match ข้อความของ Meta ซ้ำที่ชั้น UI
    expect(code).not.toMatch(/another app is controlling/i)
  })
})
