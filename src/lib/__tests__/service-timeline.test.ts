...
describe('getServiceTimeline — บรรทัดอธิบาย (note)', () => {
  ...
  it('[blocker] เวลาของขั้นที่ไม่มีข้อมูล ต้องไม่ส่งมาเลย ห้ามส่งขีดหรือค่าเดา', () => {
    /* ขั้น "ร้านให้บริการ" ไม่มีคอลัมน์เก็บเวลาในระบบเลย (ตรวจ schema 2026-08-28)
       ส่งค่าเดา = จอบอกเวลาที่ไม่มีใครยืนยันได้ · ส่งขีด = ตัวเรนเดอร์ต้องรู้จักขีด */
    const t = run({ appointmentStatus: 'COMPLETED', createdAtIso: '2026-08-14T03:00:00.000Z' })
    expect(t.find((s) => s.label === BOOKED)?.atIso).toBe('2026-08-14T03:00:00.000Z')
    expect(t.find((s) => s.label === SERVED)?.atIso).toBeUndefined()
    expect(t.find((s) => s.label === DONE)?.atIso).toBeUndefined()
  })

  it('[blocker] ยืนยันนัดแล้ว → ขั้น 2 ต้องมีเวลาที่กด', () => {
    const t = run({ buyerConfirmedAt: '2026-08-15T03:00:00.000Z' })
    expect(t.find((s) => s.label === BUYER_OK)?.atIso).toBe('2026-08-15T03:00:00.000Z')
  })
})
