...
describe('[blocker] ทางเข้าหน้าจ่ายเงินต้องมีด่าน app-shell ทุกจุด', () => {
  ...
  it('[blocker] ShopQuickLinks ต้องมีกลไกกรองที่ใช้งานจริง ไม่ใช่แค่รับ prop', () => {
    /**
    ...
    /**
     * 🛑 **กฎเปลี่ยนที่ feature 00064** — เดิมข้อนี้บังคับว่า `/subscriptions` ต้องอยู่ในชุดกรอง
     * เพราะตอนนั้นมันพาไปหน้าที่ **หักกระเป๋าเงิน** ซึ่งผิด 3.1.1
     *
     * ตอนนี้ `/subscriptions` ในแอปเด้งไป `/business/subscribe` ที่ **ซื้อผ่าน Apple** ⇒ ถูกกฎแล้ว
     * และ **ห้ามกรองทิ้ง** เพราะการ์ดนี้คือเมนูฝั่งร้าน **ทางเดียวบนมือถือ** (sidebar เป็นของ
     * เดสก์ท็อป) ⇒ กรองทิ้ง = แอป iOS ไม่มีทางไปถึงหน้าซื้อ รวมทั้งทีมรีวิวของ Apple
     *
     * ข้อบังคับฝั่งตรงข้ามย้ายไปอยู่ที่ `iap-entry-point-in-app.test.ts` แล้ว —
     * ที่นี่เหลือหน้าที่เดียว: **กลไกกรองต้องยังทำงาน** สำหรับลิงก์จ่ายเงินที่ไม่ผ่าน Apple
     * ที่จะเพิ่มเข้ามาในอนาคต (เช่น เติมเงินกระเป๋า)
     */
    /**
