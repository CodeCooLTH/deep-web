...
export const APPOINTMENT_SUMMARY_LABEL: Record<AppointmentSummaryKey, string> = {
   ...
   * ค่านี้คือ `Order.depositAmount` ซึ่งเป็น **ข้อตกลง** ไม่ใช่ข้อเท็จจริงว่าเงินเข้าแล้ว
   *
   * อัปเดต 2026-08-15 (feature 00050): ระบบ**รู้แล้ว**ว่ารับเงินไปเท่าไร — อยู่ที่ตาราง
   * `OrderPayment` (ไม่ใช่คอลัมน์ `depositReceivedAt` อย่างที่คอมเมนต์เดิมคาดไว้)
   * **แต่ป้ายนี้ยังต้องเป็น "มัดจำที่ตกลงไว้" เหมือนเดิม** เพราะมันติดป้ายให้ค่า
   * `depositAmount` ซึ่งความหมายไม่เปลี่ยน — ถ้าอยากแสดง "รับแล้วเท่าไร" ต้องเพิ่มบรรทัด
   * ใหม่จาก `computeOrderMoney()` ไม่ใช่เปลี่ยนความหมายของบรรทัดนี้ (BR-SQ-02)
   */
