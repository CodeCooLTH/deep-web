...
/**
 * 🛑 [blocker] — regex เบอร์เดิม (`/0\d(?:[ \-.]?\d){8}/`) รับแค่ เว้นวรรค/ขีด/จุด
 * ⇒ `_` และ `+66` ทำให้ปุ่ม "วางจากแชท" **คืนเบอร์ว่างแบบเงียบ ๆ** ช่องเบอร์ไม่ถูกเติม
 * ไม่มี error ไม่มี log — ร้านเห็นแค่ "กดแล้วไม่มีอะไรขึ้น" (ext 2026-08-21)
 *
 * และเบอร์ถูกใช้ 2 งานในไฟล์นี้: **ดึงออกมา** + **ลบออกจากบรรทัดก่อนหาที่อยู่**
 * ถ้าแก้แค่งานแรก จะได้ที่อยู่ที่มีเบอร์ค้างอยู่ข้างใน ⇒ เทสด้านล่างเช็คทั้งสองงานเสมอ
 */
describe('[blocker] รูปแบบเบอร์ที่ regex เดิมมองไม่เห็น', () => {
  it('ขีดล่างคั่น — ดึงเบอร์ได้', () => {
    const r = parseOrderMessage('สมชาย ใจดี\n99/9 ม.5 ต.บางรัก อ.เมือง จ.ชลบุรี 20000\nโทร 0_9_2_0791649')
    expect(r.phone).toBe('0920791649')
    expect(r.addressLine).toBe('99/9 ม.5')
  })
  ...
  /**
   * 🛑 2 เคสนี้คือตัวที่พิสูจน์ว่า "การลบ" ถูกแก้ด้วย ไม่ใช่แค่ "การดึง" —
   * ถ้า stripPhone ยังใช้ regex เก่า เบอร์ที่ลบไม่ออกจะ **กลายเป็นที่อยู่** ทั้งดุ้น
   * (ยืนยันด้วย mutation: คืน stripPhone ไปใช้ regex เดิม → 2 เคสนี้แดง)
   */
  it('บรรทัดที่มีแต่เบอร์ (ขีดล่างคั่น) ต้องไม่กลายเป็นที่อยู่', () => {
    const r = parseOrderMessage('สมชาย ใจดี\n0_9_2_0791649\nกรุงเทพ 10110')
    expect(r.phone).toBe('0920791649')
    expect(r.addressLine ?? '').not.toMatch(/0_9_2/)
  })

  it('เบอร์นำหน้าชื่อ (ขีดล่างคั่น) ต้องไม่ถูกลากไปเป็นที่อยู่', () => {
    const r = parseOrderMessage('0_9_2_0791649 สมชาย ใจดี')
    expect(r.phone).toBe('0920791649')
    expect(r.addressLine ?? '').not.toMatch(/0_9_2/)
  })

  it('เว้นวรรคทีละหลัก (เคสจากภาพหน้าจอ user)', () => {
    expect(parseOrderMessage('สมชาย ใจดี\nโทร 0 8 6 5 3 5 2960').phone).toBe('0865352960')
  })

  it('เบอร์ที่ไม่ใช่มือถือ ต้องไม่ถูกหยิบมาเป็นเบอร์ลูกค้า', () => {
    expect(parseOrderMessage('ร้านค้า\nโทร 021234567').phone).toBeUndefined()
  })
})
