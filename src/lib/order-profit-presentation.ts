...
/**
 * ป้ายอธิบายว่า "กำไร" บนหน้ารายการหมายถึงตัวไหน — โผล่ **ครั้งเดียวต่อหน้า** ไม่ใช่ต่อแถว
 * (หัวคอลัมน์เดสก์ท็อป + แคปชั่นเหนือการ์ดมือถือ ใช้สตริงเดียวกันเป๊ะจากที่นี่)
 *
 * มีเพราะ Hard Rule 16: ระบบมี "กำไร" 2 นิยามที่ต่างกันจริง — `GROSS_PROFIT_FORMULA`
 * (ตัวที่หน้านี้โชว์: ยอดที่ลูกค้าจ่าย − ต้นทุนสินค้า) กับ `NET_PROFIT_FORMULA` ที่ `/sales`
 * ผู้ขายที่บวกเลขรายใบทั้งเดือนแล้วเทียบกับหน้ายอดขายจะได้ไม่ตรงเสมอ กฎบังคับว่าต้อง
 * **บอกบนหน้าจอ ไม่ใช่แค่คอมเมนต์** — และ user สั่งห้ามมีคำต่อแถว ที่นี่จึงเป็นทางออก
 */
export const profitColumnCaption = (orderNoun: string) => `ยอด${orderNoun} · กำไรขั้นต้น`

/**
 * เวอร์ชันสำหรับแถว/การ์ดในหน้ารายการ — บังคับ `hasMissingCost: false` ก่อนเรียกตัวเต็ม
 *
 * user เคาะ 2026-08-08 ว่า "โชว์เลขเหมือนกันหมด" หลังถูกถามตรง ๆ ว่าใบที่ตั้งต้นทุนไม่ครบ
 * ควรทำยังไง (รับทราบแล้วว่าเลขของใบพวกนั้นสูงกว่าความจริง เพราะรายการที่ยังไม่ตั้งต้นทุน
 * ถูกข้าม ไม่ได้นับเป็น 0)
 *
 * ที่ทำเป็น delegate ไม่ใช่เขียน switch ใหม่ — จะได้ icon/label/tone ชุดเดียวกับตัวเต็มเสมอ
 * ไม่มีทางเพี้ยนจากกันเมื่อมีคนแก้ตัวใดตัวหนึ่งทีหลัง
 *
 * [สำคัญ] ความเสี่ยงที่ user รับไปแล้ว: ใบต้นทุนไม่ครบที่คำนวณได้ "บวก" จะขึ้นเขียวเหมือน
 * ยืนยันแล้ว ทั้งที่ต้นทุนที่ยังไม่กรอกอาจพลิกเป็นขาดทุนได้ (ฝั่งลบไม่มีปัญหานี้ — ต่อให้
 * ต้นทุนที่ขาดเป็น 0 ก็ยังขาดทุนอยู่ดี ทิศทางจึงจริงแน่นอน)
 */
export function presentOrderProfitCompact(profit: OrderProfit, orderNoun: string): ProfitPresentation {
  return presentOrderProfit({ ...profit, hasMissingCost: false }, orderNoun)
}
