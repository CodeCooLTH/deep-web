...
//
// ==========================================================================
// ระวังที่สุดในไฟล์นี้ — ขาเข้ากลับด้านกับขาออก — `dst_district` คนละความหมายกันคนละทิศทาง
// (ต่อยอด BR-ISHIP-31)
// --------------------------------------------------------------------------
// ขาออก (create_order):        dst_district = ตำบล    dst_amphure = อำเภอ
// ขาเข้า (query_orders/get_order): dst_district = อำเภอ   dst_area    = ตำบล
//
// พิสูจน์กับ prod แล้ว 2026-08-07 ด้วย GET /api/get_order ของพัสดุจริง 2 ใบ:
//   TH066536981258 → dst_district=กาญจนดิษฐ์  dst_area=ช้างซ้าย
//   TH681090UM471E → dst_district=พนม         dst_area=พังกาญจน์
// เทียบกับไฟล์ที่อยู่ที่ iShip กำหนดให้ใช้ (public/data/iship-address.json):
//   "สุราษฎร์ธานี, กาญจนดิษฐ์(อำเภอ), ช้างซ้าย(ตำบล), 84160"
//   "สุราษฎร์ธานี, พนม(อำเภอ),        พังกาญจน์(ตำบล), 84250"
//
// เดิมไฟล์นี้เดาว่าขาเข้าสมมาตรกับขาออก (ตำบล ← dst_district) โดยรับความเสี่ยงไว้ด้วย
// เหตุผลว่า "การผูกพัสดุมีขั้นเทียบที่อยู่ให้ร้านตรวจด้วยตาก่อนกดยืนยัน" — ด่านนั้น
// ไม่ทำงานจริง: 23 ออเดอร์บน prod ถูกเขียนตำบล/อำเภอสลับกันโดยไม่มีใครทัก แล้วไปโผล่
// ตอนเปิดพัสดุใบถัดไปของลูกค้าคนเดิมว่า "ที่อยู่ผู้รับไม่ผ่านการตรวจสอบของขนส่ง"
// (เพราะคู่ที่สลับแล้วไม่มีอยู่ในไฟล์ที่อยู่ของ iShip) — ห่างจากต้นเหตุคนละหน้าจอ
//
// บทเรียน: ชื่อฟิลด์ที่เหมือนกันสองทิศทางไม่ได้แปลว่าความหมายเหมือนกัน และ "ให้คนตรวจ
// ด้วยตา" ไม่ใช่ด่านกันข้อมูลเพี้ยน — ต้องพิสูจน์ด้วย payload จริงจากปลายทางเท่านั้น
// ==========================================================================
...
/**
 * parseReceiverLocality — ตัดสินว่าช่องไหนคือตำบล ช่องไหนคืออำเภอ (อ่านหมายเหตุหัวไฟล์ก่อนแก้)
 *
 * เกณฑ์ตัดสินคือ "มี dst_area ไหม" ไม่ใช่ชื่อฟิลด์เดี่ยว ๆ:
 *   - มี dst_area  = รูปของ **ขาเข้า** (query_orders/get_order) → area=ตำบล, district=อำเภอ
 *   - ไม่มี        = รูปของ **ขาออก** (payload ที่เราประกอบเอง / echo กลับมา) → district=ตำบล
 * dst_amphure ชนะเสมอเมื่อมี เพราะเป็นชื่อที่ iShip ใช้เรียกอำเภอฝั่งขาออกตรง ๆ
 *
 * แยกเป็นฟังก์ชันเพราะกติกานี้ต้องเทสได้เดี่ยว ๆ — คู่ที่สลับกันไม่ทำให้อะไรพังเสียงดัง
 * มันแค่ไปโผล่เป็น "ที่อยู่ไม่ผ่านการตรวจสอบของขนส่ง" อีกหลายวันให้หลังที่หน้าจออื่น
 */
export function parseReceiverLocality(o: Record<string, unknown>): {
  subdistrict: string | null;
  district: string | null;
} {
  const dstDistrict = str(o, "dst_district");
  const dstArea = str(o, "dst_area");
  const dstAmphure = str(o, "dst_amphure");

  if (dstArea !== null) {
    return { subdistrict: dstArea, district: dstAmphure ?? dstDistrict };
  }
  return { subdistrict: dstDistrict, district: dstAmphure };
}
...
export function parseParcelRow(raw: unknown): IShipParcel | null {
  ...
  return {
    ...
    // ห้ามแก้สองบรรทัด subdistrict/district ก่อนอ่านหมายเหตุหัวไฟล์
    receiver: {
      name: str(o, "dst_name"),
      phone: str(o, "dst_phone"),
      line1: str(o, "dst_address"),
      ...parseReceiverLocality(o),
