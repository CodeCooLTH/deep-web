...
export interface IShipOrderRow {
  ...
  /** ค่าธรรมเนียมที่ขนส่งหักจากยอด COD ("12.63") — คนละก้อนกับ discount_price ไม่ทับซ้อน */
  cod_fee?: string | number | null;
  /**
   * 🛑 **ค่าส่งที่ขนส่งคิดจริง — ไม่ใช่ "ส่วนลด" ตามที่ชื่อฟิลด์บอก**
   *
   * นี่คือเงินที่ถูกหักจากเครดิตร้านจริง และเป็นตัวเดียวในทั้ง payload ขาเข้าที่บอกค่าส่งได้
   * (`price`/`total_price` ไม่มีทั้งใน query_orders และ get_order — มีเฉพาะใน payload ของ webhook
   * ที่ไม่เคยเปิดใช้บน prod และใน response ของ check-price)
   *
   * ยืนยัน 2026-08-09 กับบัญชีจริง: ยิง check-price ที่ `actual_weight` + ขนาดจริงรายใบ แล้วเทียบกับ
   * ค่านี้ → ตรงกัน 55/56 ใบ · ใบที่ 56 (`TH066536981258` ShopeeExpress) ค่านี้ = 38 ขณะที่ quote ที่
   * 4.13 กก. ได้ 41 แต่ quote ที่ 4.0 กก. ได้ 38 พอดี = iShip คิดตามน้ำหนักที่บันทึกไว้ตอนนั้น
   *
   * ⇒ **ห้ามคำนวณค่าส่งย้อนหลังด้วย check-price** จะได้ราคาวันที่ยิง ไม่ใช่เงินที่ถูกหักจริง
   */
  discount_price?: string | number | null;
  /**
   * น้ำหนักที่ขนส่งชั่งได้จริง (kg) — **ต่างจาก `weight` ที่ร้านแจ้งตอนสร้างพัสดุ**
   *
   * ข้อมูลจริง 2026-08-09: 92 จาก 151 ใบชั่งได้หนักกว่าที่แจ้ง ⇒ ค่าส่งจริงสูงกว่าที่ร้านคาด
   * (มีเฉพาะใน `query_orders` — `get_order` มีแต่ `weight`)
   */
