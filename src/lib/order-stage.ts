...
export type ShippingStageKey =
  ...
  | 'SHIPPING'
  /**
   * พัสดุจบเส้นทางแล้ว (ถึงมือผู้ซื้อ/ตีกลับถึงร้าน) แต่ **ออเดอร์ยังไม่ปิดการขาย**
   *
   * เพิ่ม 2026-08-04 หลัง user รายงานว่าออเดอร์หายไปจากทุกไทล์ทันทีที่ผูกพัสดุ iShip:
   * ใบ DP2569085F97153B (COD, ผู้ซื้อยังไม่กดยืนยันรับของ) ผูกพัสดุที่ขนส่งส่งถึงไปแล้วเมื่อวาน
   * → carrierStatus='delivered' → เดิมตกเข้า DONE ทันทีทั้งที่ร้านยังไม่ได้เงินและยังปิดงานไม่ได้
   *
   * ไม่ใช่บั๊กของการผูกพัสดุอย่างเดียว — ออเดอร์ไหนที่ webhook แจ้ง delivered แล้วผู้ซื้อไม่เคย
   * กดยืนยัน ก็หายเงียบแบบเดียวกันมาตลอด การผูกพัสดุแค่ทำให้เห็นทันทีในวินาทีเดียว
   */
  | 'AWAITING_CLOSE'
  | 'PROBLEM'
...
export function deriveShippingStage(o: ShippingStageInput): ShippingStageKey {
  ...
  if (o.hasShipment) {
    ...
    if (o.carrierStatus && TERMINAL_CARRIER.includes(o.carrierStatus)) {
      // พัสดุจบเส้นทางแล้ว ≠ ออเดอร์จบแล้ว — ตัวตัดสิน DONE ต้องเป็นสถานะของ "ออเดอร์"
      // ไม่ใช่ของ "พัสดุ" (เดิม return 'DONE' ตรงนี้เลย = ต้นเหตุที่ใบหายจากทุกไทล์)
      // ครอบทั้ง delivered (รอผู้ซื้อกดยืนยัน/รอเก็บเงินปลายทาง) และ return_success
      // (ของตีกลับถึงร้านแล้ว ร้านต้องตัดสินใจคืนเงิน/ส่งใหม่) — ทั้งคู่ยังเป็นงานค้างของร้าน
      return o.status === 'CONFIRMED' ? 'DONE' : 'AWAITING_CLOSE'
    }
...
export const SHIPPING_STAGE_LABEL: Record<Exclude<ShippingStageKey, 'DONE'>, string> = {
  ...
  // ตั้งใจไม่ใช้คำว่า "ส่งถึงแล้ว" — คำนั้นรายงานสถานะพัสดุ ซึ่งเป็นความเข้าใจผิดตัวเดียวกับที่
  // ทำให้เกิดบั๊กนี้ (เห็น delivered แล้วนึกว่าจบ) และชนกับแท็บ "จัดส่งแล้ว" ที่อยู่หน้าเดียวกัน
  // "รอปิดงาน" บอกว่าร้านยังต้องทำอะไรต่อ โดยครอบทั้งกรณีรอผู้ซื้อยืนยัน และกรณีของตีกลับ
  AWAITING_CLOSE: 'รอปิดงาน',
