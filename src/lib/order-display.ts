 ...
 */
/**
 * 🛑 `DRAFTED` (feature 00061) = **ยังไม่ใช่ออเดอร์** — ระบบแกะข้อความที่ร้านพิมพ์ในแชทแล้ว
 * ข้อมูลไม่ครบพอสร้างจริง ⇒ ยอดเป็น ฿0 · ไม่มีเลขคำสั่งซื้อ · ไม่มีรายการสินค้าสักแถว
 * ทุกหน้าที่แสดงค่าเหล่านี้ต้องแตกสาขาให้มัน ไม่งั้นผู้ขายจะเห็น "ออเดอร์พัง" แทนที่จะเห็น "ร่าง"
 */
...
> = {
  ...
  /**
   * feature 00061 — ร่างจากแชทที่ข้อมูลยังไม่ครบ
   *
   * โทน `warning` ไม่ใช่ `danger`: มันคือ **งานที่รอผู้ขายเติมให้ครบ** ไม่ใช่สิ่งที่ผิดพลาด
   * (ระบบทำงานถูกแล้ว มันแค่บอกว่าข้อความยังขาดอะไร) · ไอคอน `file-alert` ตัวเดียวกับ
   * badge ในรายการห้องแชท เพื่อให้ผู้ขายเชื่อมโยงได้ว่าเป็นเรื่องเดียวกัน
   */
  DRAFTED: { label: 'ร่าง', cls: 'bg-warning/15 text-warning-ink', icon: 'file-alert', tone: 'warning' },
...
export function getOrderTimeline(
  ...
  switch (status) {
    /**
     * feature 00061 — ร่างยังไม่ได้เดินเข้าเส้นทางออเดอร์เลยแม้แต่ขั้นแรก
     *
     * 🛑 ห้ามคืน `สั่งซื้อแล้ว: done` เหมือนสถานะอื่น — ร่างไม่มีการสั่งซื้อเกิดขึ้นจริง
     * ยังไม่มีเลขคำสั่งซื้อ ไม่ตัดสต๊อก ลูกค้าไม่เคยเห็นอะไรเลย ⇒ ขั้นแรกต้องเป็น `cur`
     * (กำลังทำอยู่) ไม่ใช่ `done` ไม่งั้นไทม์ไลน์จะยืนยันสิ่งที่ยังไม่เกิด
     */
    case 'DRAFTED':
      return [
        { label: 'ร่างจากแชท',   state: 'cur'  },
        { label: 'สั่งซื้อแล้ว',   state: 'up'   },
        { label: isShipping ? 'จัดส่ง' : 'ยืนยันรับ', state: 'up' },
      ]
