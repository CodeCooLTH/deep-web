 ...
 *
 * 🛑 **แก้ไขข้อเท็จจริง 2026-09-18: ไฟล์นี้ไม่ใช่ตัวแก้บั๊กนั้น** — ต้นเหตุจริงคือ
 * `callbacks.redirect` คืน path ไม่มีโดเมน ทำให้ `signIn()` ของ next-auth throw ก่อนโค้ดหลังล็อกอิน
 * จะได้รัน (บรรทัดข้างล่างนี้ไม่เคยถูกเรียกเลยในเคสนั้น) · แก้ที่ `post-auth-redirect.ts`
 * เหตุผลสองข้อด้านล่างเป็นการเดาจากบั๊กเก่า **ไม่เคยพิสูจน์ว่าเกิดจริง** · ตัวกลางนี้ยังเก็บไว้
 * เพราะรวม 8 จุดไว้ที่เดียวและ hard-navigate ปลอดภัยกว่า แต่อย่าอ้างว่ามันแก้บั๊กนี้
 *
