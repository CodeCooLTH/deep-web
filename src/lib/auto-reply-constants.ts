 ...
 */
export const DEFAULT_GUARDRAILS: { rule: string; denyPhrases: string[]; mode?: 'BLOCK' | 'AVOID' }[] = [
  // AVOID = ยังตอบอยู่ แต่ห้ามพูดแบบนั้น — ข้อนี้มาจากปัญหาจริง (user 2026-08-01):
  // บอทตอบว่า "ทางร้านไม่มีข้อมูลเรื่องนี้ค่ะ" ซึ่งจบบทสนทนาทิ้งทั้งที่ยังขายต่อได้
  // ใช้ BLOCK กับเรื่องนี้ไม่ได้ เพราะการเงียบยิ่งแย่กว่าการพูดผิดที่
  {
    rule: 'ห้ามตอบว่า "ไม่มีข้อมูล" หรือ "ร้านไม่ได้ระบุไว้" แล้วจบ — ให้บอกว่าจะเช็คให้/ให้แอดมินยืนยันอีกที แล้วชวนคุยเรื่องสินค้าต่อ',
    denyPhrases: [],
    mode: 'AVOID',
  },
  {
