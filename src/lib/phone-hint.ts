...
export function chipsHeadline(count: number, blocked: boolean): string {
  if (blocked) return 'เบอร์นี้ยังบันทึกไม่ได้ — กดเบอร์ด้านล่างเพื่อแก้ให้ถูกต้อง'
  if (count > 1) return `เจอ ${count} เบอร์ในข้อความ — กดเลือกเบอร์ของลูกค้า`
  return 'ตัดช่องว่างและเครื่องหมายออกให้แล้ว — กดเพื่อใช้และค้นหาลูกค้าเดิม'
}
