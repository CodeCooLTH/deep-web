...
/**
  * 🛑 คำชุดนี้ต้องเข้าคู่กับ `MOBILE_RULE_TEXT` (src/lib/phone.ts) — ห้ามผันคำ
  * ผู้ใช้คนเดียวกันเห็นทั้งข้อความนี้ตอนพิมพ์ และเห็น `MOBILE_RULE_TEXT` ตอนกดบันทึก
  * ถ้าเขียนคนละแบบจะอ่านเหมือนเป็นกฎคนละข้อ (PRODUCT.md: กลุ่มเป้าหมายรวมผู้สูงวัย/
  * digital-literacy ต่ำ — ศัพท์ที่ไม่คงที่คือภาระคิดที่เพิ่มมาฟรี ๆ)
  */
const lengthMessage = (n: number) => `เบอร์นี้มี ${n} หลัก — เบอร์มือถือต้องมี 10 หลัก`

const NOT_MOBILE_MESSAGE = 'เบอร์นี้ไม่ใช่เบอร์มือถือ — ต้องขึ้นต้นด้วย 06, 08 หรือ 09'

/**
 * วลีเดียวกันในรูปที่เอาไป **ต่อท้ายประโยคอื่น** ได้
 *
 * 🛑 ห้ามเอา `lengthMessage`/`NOT_MOBILE_MESSAGE` ไปต่อท้ายตรง ๆ — จะได้ขีดคั่นซ้อนกัน
 * สองอันในประโยคเดียว ("ไม่พบลูกค้าเดิม — เบอร์นี้มี 8 หลัก — เบอร์มือถือต้องมี 10 หลัก")
 * ซึ่งอ่านเหมือนสามประโยคชนกัน. คำศัพท์ยังชุดเดียวกันทุกตัว เปลี่ยนแค่รูปประโยค
 */
const lengthClause = (n: number) => `เบอร์ที่พิมพ์มี ${n} หลัก เบอร์มือถือต้องมี 10 หลัก`

const NOT_MOBILE_CLAUSE = 'เบอร์มือถือต้องขึ้นต้นด้วย 06, 08 หรือ 09'
 ...
 *
 * 🛑 **ต้องจบใน 1 บรรทัดที่ 375px** (user สั่งย่อ 2026-08-21) — ที่ `text-xs` ในการ์ดกว้าง
 * 331px ได้ราว 53 ตัวอักษร เกินจากนั้นตกบรรทัดแล้วบล็อกนี้สูงขึ้นอีก ~18px ทุกครั้งที่ขึ้น
 *
 * ข้อเท็จจริง "ระบบตัดให้แล้ว" ถูกตัดออกจากคำโดยตั้งใจ — มันมองเห็นอยู่แล้วจากการที่
 * ช่องด้านบนมีเบอร์เพี้ยนและปุ่มด้านล่างมีเบอร์สะอาด สิ่งที่ตาเห็นไม่ต้องเขียนซ้ำ
 * ส่วนที่เขียนไม่ได้แล้วผู้ใช้ไม่มีทางรู้เองคือ **กดแล้วจะค้นหาลูกค้าเดิมให้ด้วย** จึงเก็บไว้
 *
...
export function chipsHeadline(count: number, blocked: boolean): string {
  if (blocked) return 'ยังบันทึกไม่ได้ — กดเบอร์ด้านล่างเพื่อแก้'
  if (count > 1) return `เจอ ${count} เบอร์ — กดเลือกเบอร์ของลูกค้า`
  return 'กดเพื่อใช้เบอร์นี้และค้นหาลูกค้าเดิม'
}
...
export function emptyStateMessage(raw: string): string {
  ...
  // ใช้คำชุดเดียวกับที่ขึ้นใต้ช่องกรอก — ผู้ใช้เห็นทั้งสองที่ในนาทีเดียวกัน
  if ((reason === 'too-short' && digitCount > 0) || reason === 'too-long') {
    return `ไม่พบลูกค้าเดิม — ${lengthClause(digitCount)}`
  }
  if (reason === 'not-mobile') return `ไม่พบลูกค้าเดิม — ${NOT_MOBILE_CLAUSE}`
