...
export const CONNECT_SCOPES = [
   ...
   *
   * 🛑 **ใส่กลับ 2026-09-08** หลังเพิ่มเข้า use case ใน App Dashboard + ใบร่าง App Review
   * `1739136693971389` — คอมเมนต์เดิมด้านล่างเขียนลำดับไว้ว่า "App Review ผ่าน → ค่อยใส่บรรทัดกลับ"
   * ซึ่ง **เป็นไปไม่ได้**: ขั้น `api_precheck` ของ Meta ต้องเห็น API call จริง *ก่อน* ยื่น
   * แต่ call เกิดไม่ได้ถ้า scope ไม่อยู่ในบรรทัดนี้ ⇒ ลำดับที่ถูกคือ
   * เปิดใน App Dashboard → ใส่ scope → เชื่อมเพจใหม่ → ยิง insights → precheck ผ่าน → ยื่น
   *
   * อันตรายที่กันไปแล้ว: เดิม scope นี้ทำหน้า login ตีกลับ `Invalid Scopes` = **เชื่อมเพจไม่ได้
   * ทั้งกระบวนการทุกร้าน** (พังบน prod 2026-07-26) ⇒ ทดสอบ `dialog/oauth` ตรง ๆ พร้อมตัวควบคุม
   * ก่อนใส่กลับ: ชุดนี้ + insights = หน้า login ปกติ 455 KB · ชุดนี้ + scope ปลอม = หน้า error
   * 1.5 KB "Sorry, something went wrong" ⇒ dialog รับ scope นี้แล้วจริง
   * 🛑 หลักฐานสุดท้ายคือ **กดเชื่อมเพจจริง 1 เพจ** (ตัวทดสอบไม่ได้ล็อกอิน พิสูจน์ได้แค่ว่า
   * dialog ไม่ปฏิเสธตั้งแต่ต้นทาง) — ถ้าเชื่อมแล้วพัง ให้ถอดบรรทัดนี้ออกทันที ไม่ต้องรอสรุปอื่น
   *
   * ร้านที่เชื่อมไว้ก่อนหน้านี้ไม่ได้ scope นี้อัตโนมัติ (scope ผูกกับ token ตอนกดอนุญาต ไม่ใช่
   * ตอนแอปประกาศ) ต้องกดเชื่อมเพจใหม่ครั้งเดียวถึงจะได้ยอดวิว — ไม่มีร้านไหนพังระหว่างนั้นเพราะ
   * `shop-video.service` ทนกับ token ที่ไม่มี scope นี้อยู่แล้ว (คืนยอดวิวเป็น null)
   */
  'instagram_manage_insights',
  // ── บันทึกเดิมก่อน 2026-09-08 (เก็บไว้เป็นที่มาของข้อควรระวังข้างบน) ────────────────
