...
export type RecentProductMessage = {
   ...
   *
   * (CR 2026-08-23, E-11) เพิ่มค่าที่ควรรู้จัก: `'QUEUED'` = เขียนแถวแล้วแต่ยังไม่ยิงออก —
   * **บล็อกเหมือน `'SENT'`** เพราะมันจะถูกยิงออกไปแน่นอนในอีกไม่กี่วินาที ปล่อยผ่าน = ลูกค้าได้
   * การ์ดซ้ำจากการกดรัวเพียงครั้งเดียว ซึ่งเป็นเหตุผลทั้งหมดที่ด่านนี้มีอยู่
   */
...
export function isDuplicateProductSend(
  ...
  return candidate.every((m, i) => {
    ...
    //
    // 🛑 `FAILED` เป็นค่า **เดียว** ที่ปล่อยผ่าน (allow-list กลับด้าน) — ค่าอื่นทั้งหมดรวม `QUEUED`
    // ที่เพิ่งเข้ามาปี 2026-08-23 ถือว่า "ถึงลูกค้าแล้วหรือกำลังจะถึงแน่ ๆ" จึงบล็อก. เขียนเป็น
    // deny-list (`!== 'SENT'` ฯลฯ) ไม่ได้: ค่าใหม่ที่โผล่มาวันหลังจะหลุดด่านเงียบ ๆ
