...
/**
 * 🛑 **เลิกจองความสูงของบล็อกคำโปรย (2026-08-30)** — เคยจองไว้ 2 บรรทัดเสมอ
 *
 * เหตุผลเดิม (2026-08-23 user: "ตอนเปลี่ยนมันจะได้ไม่แว็บ ๆ"): ปกจัด **กึ่งกลางแนวตั้ง**
 * ⇒ เนื้อหาสูงเท่าไหร่ ทุกอย่างเลื่อนตามครึ่งหนึ่งของส่วนต่าง ร้านที่เขียน 1 กับ 2 บรรทัด
 * ชื่อร้านอยู่คนละที่ห่างกัน ~17px
 *
 * เหตุผลนั้น **หมดไปพร้อมกับ `alignItems:'center'`** — ตอนนี้จัดชิดบน ของที่อยู่เหนือคำโปรย
 * (โลโก้ · ชื่อ · ฝั่งติดต่อ) ไม่ขยับตามความยาวคำโปรยอีกต่อไป ⇒ ไม่ต้องจองก็ไม่แว็บ
 * ส่วนการจองที่เหลืออยู่กลายเป็นช่องว่างเปล่าในร้านที่ไม่มีคำโปรย ซึ่งเป็นสิ่งที่หัวหน้าทัก
 *
 * 🛑 ถ้าวันไหนกลับไป `center` ต้องเอาการจองกลับมาด้วย ไม่งั้นบั๊ก "แว็บ ๆ" กลับมาทันที
 */
const BIO_RESERVED_BLOCK_SIZE = 'auto'

/**
 * 🛑 **เลิกจองความสูงแล้ว (2026-08-30)** — ค่านี้เคยเป็น 260 เพื่อให้ชื่อร้านอยู่ y เดิมเสมอ
 * ไม่ว่าชื่อจะ 1 หรือ 2 บรรทัด ตอนที่ hero ยังจัด **กึ่งกลางแนวตั้ง**
 *
 * พอเปลี่ยนเป็น `alignItems:'start'` เหตุผลนั้นหมดไปเอง: ของที่อยู่บนไม่ขยับตามความสูงของ
 * สิ่งที่อยู่ล่างอีกต่อไป ⇒ การจองกลายเป็นแค่ **ช่องว่างเปล่า** ในร้านที่ไม่มีคำโปรย
 * (หัวหน้าเห็นบนจอจริง: "ถ้าไม่มีข้อมูลก็ไม่เว้นแบบนั้นสิ มันดูโล่ง ๆ ให้ไหลตาม detail")
 *
 * 0 = ไม่จอง · เก็บค่าคงที่ไว้เป็นจุดเดียวเผื่อวันที่ต้องกลับไปจองอีก จะได้ไม่ต้องไล่หา
 */
const IDENTITY_MIN_BLOCK_SIZE = 0
...
const HERO_INNER_SX = {
  ...
  gap: { sm: '20px', md: '28px' },
  /**
   * 🛑 **`start` ไม่ใช่ `center`** (หัวหน้าสั่ง 2026-08-30: "logo เอาไว้ข้างบนตรงชื่อสิ ·
   * ฝั่งติดต่อร้านด้วย เอาขึ้นข้างบน")
   *
   * `center` ทำให้ทั้งสามคอลัมน์ถูกจัดกึ่งกลางเทียบกับคอลัมน์ที่สูงที่สุด (คอลัมน์ตัวตน)
   * ⇒ วัดจากจอจริงได้ **ชื่อร้าน y=42 · โลโก้ y=87 · การ์ดคะแนน y=136** — สามอย่างที่ควร
   * เริ่มพร้อมกันกลับเริ่มคนละที่ อ่านเป็นของที่ลอยกันคนละระดับ
   *
   * `start` แก้ทั้งสามข้อพร้อมกัน และยังทำให้ **การจองความสูงไม่จำเป็นอีกต่อไป** —
   * เนื้อหาที่ยาวขึ้นดันลงล่างอย่างเดียว ของที่อยู่บนไม่ขยับ (ดูเหตุผลเต็มที่ `BIO_CLAMP_LINES`)
   */
  alignItems: 'start',
...
const SCORE_CARD_SX = {
   ...
   */
  minBlockSize: { md: 118 },
  /* การ์ด = 20px ค่าเดียวทั้งระบบ (DESIGN.md §Cards) — เดิม 13/19px ซึ่งไม่ตรงกับ
     การ์ดใบไหนในระบบเลย และแคบกว่าการ์ดที่อยู่ใต้มันบนหน้าเดียวกัน 7px */
  padding: '20px',
