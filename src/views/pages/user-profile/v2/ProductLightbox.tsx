    ...
    <Box sx={{ position: 'relative', inlineSize: '100%', blockSize: '100%', display: 'flex' }}>
      <Box
        sx={{
          ...
          /* 🛑 ต้องเป็น `blockSize: 100%` ไม่ใช่ `minBlockSize` — รูปข้างในเป็น absolute จึงไม่มี
             ความสูงตามธรรมชาติให้กล่องนี้เลย ค่า min จึงกลายเป็นความสูง *จริง* ทั้งที่ตั้งใจให้เป็น
             แค่พื้นขั้นต่ำ (เดิม 420px ตายตัวบนทุกจอ = ที่มาของ "รูปเล็กมาก" ที่ user เจอ)
             ตอนนี้เปลือกใน ProfileLightbox ถือความสูง 88dvh แล้ว ที่นี่แค่ยืดเต็มตาม */
          blockSize: { md: '100%' },
        }}
      >
        {src ? (
