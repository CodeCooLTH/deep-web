...
/**
 * จำนวน "ขายแล้ว" ต่อสินค้า — ใช้โชว์บนหน้าร้านสาธารณะ
 *
 * 🛑 **ใช้เกณฑ์เดียวกับหลังร้าน** (user สั่ง 2026-08-11 "อยากให้เป็นจำนวนเดียวกับหลังบ้าน"):
 * นับ **ทุกสถานะยกเว้น CANCELLED** และนับเป็น **จำนวนชิ้น** (`sum(qty)`) เหมือน
 * `getBestSellerProducts()` เป๊ะ — ผู้ขายเปิดหลังร้านกับหน้าร้านพร้อมกันแล้วเห็นเลขไม่ตรง
 * คือสิ่งที่ทำให้เขาเลิกเชื่อตัวเลขทั้งหน้า ซึ่งเสียหายกว่าความเข้มงวดของเกณฑ์
 *
 * 🛑 **สิ่งที่แลกไป และต้องรู้ไว้:** ตัวเลขนี้ **ร้านสร้างเองได้** แล้ว — สร้างออเดอร์ทิ้งไว้โดยไม่ต้อง
 * มีผู้ซื้อยืนยันหรือขนส่งขยับพัสดุ ก็ดัน "ขายแล้ว" ขึ้นได้ทันที ต่างจากเดิมที่ผูกกับ BR-POC
 * (ผู้ซื้อกดยืนยันเอง หรือขนส่งขยับจริง) ซึ่งเป็นหลักฐานที่ร้านสร้างเองไม่ได้
 *
 * ⚠️ ตัวเลขชุดอื่นบนหน้าเดียวกัน (ออเดอร์ · ลูกค้า · ซื้อซ้ำ · อัตราสำเร็จ ในแถบหลักฐาน)
 * **ยังใช้ BR-POC ตามเดิม ไม่ได้เปลี่ยนตามรอบนี้** — ผลคือผลรวม "ขายแล้ว" ของทุกสินค้า
 * จะมากกว่าเลข "ออเดอร์" ในแถบหลักฐานเสมอ (คนละเกณฑ์ + คนละหน่วย: ชิ้น vs ใบ)
 * ถ้าวันหนึ่งมีคนถามว่าทำไมสองเลขนี้ไม่ตรงกัน คำตอบอยู่ตรงนี้ ไม่ใช่บั๊ก (Hard Rule 16)
 */
