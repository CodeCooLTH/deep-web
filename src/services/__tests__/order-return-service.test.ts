...
describe('[blocker] ตำแหน่งปุ่มคืนของในห้องแชท', () => {
  ...
  it('[blocker] หน้ารายละเอียดออเดอร์ต้องเป็นชีตจากเมนู ⋮ ไม่ใช่การ์ดในหน้า', () => {
    ...
    /**
     * 🛑 เดิมเช็ค `toContain('asSheet')` ซึ่งผูกกับ **วิธีเขียน** — พอ prop นั้นถูกลบทิ้ง
     * (2026-08-26 · โหมดการ์ดเป็นโค้ดตายที่ไม่มีวันถูกเรียก) ด่านก็แดงทั้งที่กฎยังครบ
     * กฎที่ยังเป็นไปได้จริงคือ "เปิดเป็นชีตที่คุมด้วย state" ⇒ ผูกกับ `sheetOpen` แทน
     */
    expect(client).toMatch(/<ReturnPanel[\s\S]{0,200}?sheetOpen=/)
  })
  ...
  it('ต้องอยู่ในเมนู ⋮ ของออเดอร์ ไม่ใช่การ์ดคงที่', () => {
    ...
    expect(panel).toMatch(/<ReturnPanel[\s\S]{0,200}?sheetOpen=/)
   ...
   * ใบเดียวกลางจอคือคลิกที่ไม่ได้ตัดสินใจอะไรเพิ่ม
   *
   * 🛑 **ด่านนี้ถูกเขียนใหม่ 2 รอบเพราะผูกกับ *วิธีเขียน* ทั้งสองรอบ**
   *   รอบแรกเช็คสตริง `!form && !asSheet ?` ตรงตัว → แดงตอนยกเงื่อนไขเป็นตัวแปรร่วม `formOpen`
   *   รอบสองเช็ค `const formOpen = form || asSheet` → แดงตอนลบโหมดการ์ดทิ้ง (2026-08-26)
   * ทั้งสองรอบ **กฎยังครบทุกตัวอักษร มีแต่ด่านที่พัง** (คลาสเดียวกับด่าน `provider="apple"` 08-12)
   *
   * รอบนี้ผูกกับสิ่งที่ยัง *เป็นไปได้จริง*: ตอนนี้โค้ดเหลือทางเดียว (ชีตเสมอ) ⇒ สิ่งที่ต้องกัน
   * ไม่ใช่ "สาขาที่สองกลับมา" อีกต่อไป แต่คือ **ปุ่มคั่นกลับมา** และ **สาขาโหมดการ์ดกลับมา**
   */
  it('[blocker] เปิดชีตแล้วต้องเจอฟอร์มเลย ไม่มีปุ่มคั่น และไม่มีสาขาโหมดการ์ด', () => {
    const rp = strip(
      'src/app/(paces)/seller/(dashboard)/orders/[token]/components/ReturnPanel.tsx',
    )
    // ปุ่มคั่นต้องไม่มีอยู่เลย ไม่ใช่ "มีแต่ถูกกั้น"
    expect(rp).not.toContain('เปิดเรื่องคืนของ')
    // prop ที่เคยแบกสาขาโหมดการ์ดต้องไม่กลับมา (call site ทั้งสองส่งค่าคงที่มาตลอด)
    for (const dead of ['asSheet', 'compact', 'initialCount']) {
      expect(rp, dead).not.toMatch(new RegExp(`\\b${dead}\\b`))
    }
    // และต้องยังเป็นชีตที่คุมด้วย sheetOpen เหมือนเดิม
    expect(rp).toMatch(/if \(!sheetOpen\) return null/)
  })
