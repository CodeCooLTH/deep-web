...
export type OrderRow = {
  ...
  shipment?: {
     ...
     * (กองงาน) ส่วนแถบต้องรู้ระดับ carrierStatus: `return` กับ `return_success` เป็นกอง
     * RETURNED เหมือนกัน แยกจากกันไม่ได้ถ้ามีแต่ stage (user เจอบน prod 2026-08-24:
     * hover ขึ้น "กำลังจัดส่ง" ตัวหนา ให้พัสดุที่กลับมาถึงร้านแล้ว)
     *
     * ตั้งแต่ 2026-08-25 ทั้งคู่อยู่จุดที่ 4 ของแถว 1 เหมือนกัน (= "ส่งไม่สำเร็จ" — ขาไป
     * จบยังไง) แล้วแยกกันบน **แถวที่ 2** แทน ดู `describeReturnLeg()`
     */
     ...
     *
     * 🛑 `null` = **ขนส่งไม่ได้แจ้งเวลา ไม่ใช่ "ไม่เกิด"** — จุดสว่างบนแถว 2 ตัดสินจาก
     * `carrierStatus` ไม่ใช่จากสองช่องนี้ (6 จาก 12 ใบตีกลับบน prod ถึงร้านแล้วแต่ไม่มีเวลา
     * เพราะสถานะมาจากรอบ poll ที่ไม่ผ่าน ShipmentEvent) ⇒ ห้ามเอาไปใช้ตัดสินว่าถึงหรือยัง
     *
     * เป็น string เพราะข้ามเส้น RSC มา (Date serialize ไม่ได้ตรง ๆ)
     */
