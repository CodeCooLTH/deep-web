...
export default function ShopForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      {/*
        ปุ่มบันทึก — มือถือเท่านั้น (เดสก์ท็อปมีปุ่มอยู่ท้ายการ์ดแล้ว)

        🛑 **เคยเป็นแถบ `fixed` ลอยเหนือเมนูล่าง — ถอดออกแล้ว 2026-08-15 ตามที่ user สั่ง**
        เหตุผลที่ user ให้: *"มันลอยแบบนี้กลัวเข้าใจผิด ปุ่มเหมือนออกจากระบบเลย แต่คนละสี"*
        ซึ่งตรงประเด็น — ปุ่มที่ลอยค้างอยู่ตำแหน่งเดียวกับปุ่มออกจากระบบตลอดเวลา อ่านเป็นของ
        ชิ้นเดียวกันโดยที่สีเป็นตัวแยกอย่างเดียว และสีคือสิ่งที่คนอ่านผิดง่ายที่สุดตอนรีบ
        (Impeccable: สีไม่ควรเป็นตัวแบกความหมายตัวเดียว)

        ตอนนี้อยู่ในสายเนื้อหาปกติ ต่อจากการ์ดฟอร์ม (จบด้วย "คำอธิบายร้าน") และอยู่ **ก่อน**
        การ์ด "จัดการร้าน"/"ออกจากระบบ" ที่ page.tsx render ต่อจากฟอร์ม ⇒ ลำดับการอ่านตรงกับ
        ลำดับการทำงาน: กรอกเสร็จ → บันทึก → ค่อยไปเรื่องอื่น

        ประวัติที่ยังต้องรู้ (อย่ากลับไปทำซ้ำ): เคยลองเป็น `sticky` ในการ์ดแล้วพัง — มันลอยทับ
        ช่องกรอกที่เหลือกลางฟอร์ม เพราะ sticky ยังนับเป็นลูกของการ์ด จึงเลื่อนทับพี่น้องตัวเอง
        (สกรีนช็อตจริง 2026-08-01) ⇒ ถ้าวันหนึ่งอยากให้ปุ่มตามจอ ต้องเป็น fixed นอกการ์ดเท่านั้น
        และต้องแก้เรื่องที่มันอ่านปนกับปุ่มออกจากระบบก่อน
      */}
