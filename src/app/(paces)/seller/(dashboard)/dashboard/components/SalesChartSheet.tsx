 ...
 *
 * ── v8 2026-08-23 (user รายงานจาก prod 3 รอบในวันเดียว) ─────────────────────────────────
 * - 🛑 **เพิ่มป้ายของ "เส้น" ใต้กราฟ** — กราฟนี้วาดสองหน่วยบนสองแกนที่ `show: false` ทั้งคู่
 *   (แท่ง = บาท แกนซ้าย · เส้น = จำนวนใบ แกนขวา) และแถบสมการเหนือกราฟรับเฉพาะก้อนเงิน
 *   โดยเจตนา ⇒ เส้นซึ่งเด่นที่สุดบนกราฟ **ไม่มีชื่อกำกับที่ไหนเลย** ผู้อ่านจึงเดาว่าเป็นยอดขาย
 *   แล้วสรุปว่ากราฟขัดกับตาราง (วันที่ 13 เส้นสูงสุด 3 ใบ แต่ ฿902 · วันที่ 23 ฿20,890 แต่ 2 ใบ)
...
export default function SalesChartSheet({ initialSeries, onClose }: Props) {
  ...
  const detailRows = series.labels
    ...
    /* 🛑 ไม่ต้องนับ `received` เข้าเงื่อนไข "แถวนี้มีอะไรไหม" — เงินทุกก้อนลงแถวตาม *วันที่ขาย*
       ...
       `received > 0` ต้องมี `value > 0` เสมอโดยโครงสร้าง · เงื่อนไขเพิ่มจึงเป็นโค้ดตายที่
       ชวนให้เข้าใจผิดว่ามีแถวชนิดที่ไม่มีอยู่จริง
       (คอมเมนต์เดิมตรงนี้อธิบายแกน `receivedAt` ซึ่งเป็นร่างแรกที่ถูกเปลี่ยนไปแล้ว) */
        ...
        <div className="mx-auto w-full max-w-lg py-4">
          ...
          {error ? (
            ...
            <>
              ...
              />
              {/**
                * ป้ายของ "เส้น" — 🛑 จำเป็น ไม่ใช่ของประดับ
                *
                * กราฟนี้วาด **สองหน่วยบนสองแกนที่ซ่อนทั้งคู่**: แท่ง = บาท (แกนซ้าย) ·
                * เส้น = จำนวนใบ (แกนขวา `opposite: true`) และ `yaxis` ทุกตัวตั้ง `show: false`
                * ตามมติ v3 ("ตัวเลขเป๊ะอยู่ในตารางข้างล่างแล้ว")
                *
                * ผลคือเส้นซึ่งเป็นสิ่งที่เด่นที่สุดบนกราฟ (ลากยาวทั้งเดือน วาดทับแท่ง)
                * **ไม่มีชื่อกำกับที่ไหนเลย** — แถบสมการเหนือกราฟมีแต่ก้อนเงิน (รอยืนยัน/ยืนยันแล้ว/
                * ต้นทุน/ค่าส่ง) และจงใจไม่รับของที่ไม่ใช่เงินเข้ามา เพราะมันเป็นสมการที่ต้องบวกลบ
                * ตามได้ ⇒ ผู้อ่านจึงเหลือทางเดียวคือเดาว่าเส้นเป็นยอดขาย
                *
                * แล้วเส้นกับแท่งจะดู "ขัดกัน" ทันทีเมื่อวันที่ขายเยอะไม่ใช่วันที่ออเดอร์เยอะ —
                * user รายงานเองบน prod 2026-08-23: วันที่ 13 เส้นสูงสุด (3 ใบ) แต่ยอดขายแค่ ฿902
                * ส่วนวันที่ 23 ยอด ฿20,890 กลับมีเส้นต่ำกว่า (2 ใบ) · ตัวเลขถูกทุกช่อง
                * สิ่งที่ขาดคือคำว่า "เส้นนี้นับใบ ไม่ใช่นับเงิน"
                *
                * วางใต้กราฟไม่ใช่ในแถบสมการ — คนละชนิดข้อมูลกัน ยัดเข้าไปจะทำให้แถบนั้น
                * เลิกอ่านเป็นสมการ (เหตุผลเดียวกับที่ "ต้นทุนสินค้า" ไม่มีจุดสี ดู v5 หัวไฟล์)
                */}
              <p className="flex items-center justify-center gap-1.5 pb-1 text-2xs text-default-700">
          ...
          {!loading && !error && !isEmpty && detailRows.length > 0 && (
            <div className="mt-5 overflow-x-auto">
                      ...
                      {isService && (
                        <>
                          ...
                          {/**
                            ...
                            *
                            * 🛑 **สีเตือนเฉพาะตอนที่ "รู้ว่าค้าง" จริง** คือแถวที่มีบันทึกรับเงินแล้ว
                            * บางส่วน · แถวที่ทุกใบยังไม่เคยบันทึกรับเงินเลย (`unrecorded === value`)
                            * ระบบไม่มีทางรู้ว่าเก็บเงินไปหรือยัง — ร้านอาจเก็บสดแล้วไม่ได้กดบันทึก
                            * ⇒ ใส่สีส้มให้ = **ยืนยันสิ่งที่ไม่รู้** ซึ่งอ่านว่า "ลูกค้ายังไม่จ่าย"
                            * (user เจอเองบน prod 2026-08-23: ใบวันที่ 1 ส.ค. สถานะ CONFIRMED
                            *  งานปิดไปแล้ว แต่ขึ้น "ค้างรับ ฿300" สีส้ม เพราะไม่มีแถว OrderPayment)
                            * ตัวเลขยังแสดงอยู่ (ซ่อน = ปิดข้อมูล) แต่เป็นสีปกติ + มีหมายเหตุใต้ตาราง
                            * (`partial-data-must-be-labeled-or-filled.md` — เลือกทาง "บอก")
                            */}
                          <span
                            className={`min-w-14 flex-1 basis-0 text-end font-semibold tabular-nums ${
                              r.value - r.received > 0 && r.received > 0
                                ? 'text-warning-ink'
                                : 'text-default-700'
                            }`}
                          >
              ...
              {/**
                * หมายเหตุใต้ตาราง — มีเฉพาะร้านบริการ
                *
                ...
                *    บังคับให้ต้อง "บอก" เมื่อเติมข้อมูลให้เต็มไม่ได้)
                *
                * ประโยคที่ 2 ขึ้นเฉพาะเมื่อมีใบแบบนั้นจริงในช่วงที่ดูอยู่ — ขึ้นตลอดเวลาจะกลายเป็น
                * ข้อความประจำที่ไม่มีใครอ่าน แล้ววันที่มันสำคัญจริงก็จะถูกมองข้ามไปด้วย
                */}
              {isService && (
              <div className="border-t border-dashed border-default-300 px-4 py-2.5 lg:px-5">
                <p className="text-2xs leading-relaxed text-default-700">
                  ...
                  {hasUnrecorded && (
                    <>
                      {' · '}
                      บางรายการยังไม่ได้บันทึกการรับเงิน ยอด &ldquo;ค้างรับ&rdquo; ของวันนั้นจึงเป็นยอดสูงสุดที่เป็นไปได้
                      ไม่ใช่ยอดที่ยืนยันแล้ว
                    </>
                  )}
                </p>
              </div>
