    ...
    : STATUSES.map((st) =>
        ...
        st.key === 'SHIPPED' && appointmentToday !== undefined
          ? {
              ...
              showBadge: true,
              /**
               * `?apptDay=` = แกน "วันของนัด" ของหน้า /orders (user สั่ง 2026-08-10 "กดตรง
               * รอดำเนินการ นัดวันนี้ สำเร็จ ยกเลิก → ไปที่รายการ Order Lists") — เดิมพาไป
               * /queues เพราะยังไม่มีตัวกรองแกนนี้ ทำให้ไทล์เดียวในแถบพาไปคนละที่กับเพื่อน
               *
               * ไม่ใช่ `?appt=` ซึ่งเป็นแกน *สถานะนัด* — เอาค่าวันที่ไปปนแกนสถานะคือคลาสบั๊ก
               * เดียวกับที่ 00028 เจ็บมาแล้ว (ค่าที่ 3 ตกเข้า branch ผิดโดยไม่มีอะไรฟ้อง)
               *
               * เลขบนไทล์กับจำนวนแถวที่กรองได้ตรงกันเพราะทั้งคู่ประกอบจาก lib/appointment-day.ts
               */
              href: '/orders?apptDay=today',
            }
          : {
              key: st.key,
              label: t.dashboard[st.labelKey],
