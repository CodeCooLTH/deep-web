...
describe('ThreadContextBar — ลำดับความสำคัญของที่มา', () => {
  ...
  it('ประกอบที่มาที่ยุบเป็นชิปครบ (ชื่อร้าน / คอมเมนต์)', () => {
    expect(posOf('shop')).toBeGreaterThan(-1)
    expect(posOf('comment')).toBeGreaterThan(-1)
  })

  it('ชื่อร้านถูก push ก่อนคอมเมนต์เสมอ (ตัวแรก = ตัวที่โชว์ตอนยุบ)', () => {
    expect(posOf('shop')).toBeLessThan(posOf('comment'))
  })

  /**
   * 🛑 2026-09-09 — user สั่งถอน "โฆษณา" ออกจากแถบชิป ให้กลับไปเป็นแบนเนอร์เต็มแถบเหมือนเดิม
   * ("อยากให้แสดงแบบเดิม ... ไม่ชอบ pill แบบปัจจุบัน") ⇒ ข้อที่เคยตรึงลำดับ `ad` ในแถบถูกถอด
   *
   * นี่คือการ **กลับบางส่วน** ของมติ 2026-08-11 ที่ยุบ 3 แถบเป็นแถบเดียว (ซึ่งก็มาจากคำสั่ง user
   * เหมือนกัน: "panel ด้านบน มันซ้อนกันเยอะ จนใช้ยาก") — ที่มาอื่นยังยุบอยู่ตามเดิม ถอนเฉพาะโฆษณา
   * เพราะผู้ขายต้องอ่าน *เนื้อโฆษณา* เพื่อรู้ว่าลูกค้าเห็นข้อเสนออะไรมาก่อนทัก ต่างจากที่มาอื่นที่
   * ตอบแค่ "ห้องนี้มาจากไหน" ซึ่งดูครั้งเดียวก็พอ
   *
   * สิ่งที่ยังต้องตรึงไว้คือ **แบนเนอร์ต้องถูก render จริง** ไม่ใช่ประกอบไว้แล้วลืมวาง —
   * ตัวแปรที่ถูกสร้างแล้วไม่มีใครใช้คือรูปร่างของบั๊กที่ tsc มองไม่เห็น (ค่ายังถูกต้องทุกบรรทัด)
   */
  it('[blocker] โฆษณาเป็นแบนเนอร์เดี่ยว ไม่ใช่ชิปในแถบ และต้องถูก render จริง', () => {
    expect(src).not.toMatch(/key: 'ad'/)
    expect(src).toMatch(/adBanner = \(/)
    expect(src).toMatch(/\{adBanner\}/)
  })
