...
// isQuotable ย้ายไป `@/services/chat-thread-messages.service` แล้ว (2026-09-10) —
// หน้าเธรด RSC ต้องใช้ตัวเดียวกัน ห้ามมีสองนิยาม (HR16)
...
export async function GET(
  ...
  try {
     ...
     *
     * เรียกเมื่อไร (อัปเดต 2026-09-14 ส่วนขยาย 00018 — เดิมเรียกทุกคำขอที่ไม่มี cursor รวม poll):
     *   · คำขอหน้าแรกธรรมดา (ไม่มี cursor ไม่ใช่ delta) — `loadInitial` ของ hook / R13 โหลดหน้าแรกใหม่ /
     *     poll ที่ store ไม่มี watermark แล้ว
     *   · คำขอ delta ที่ส่ง `sync=1` — hook ส่งครั้งเดียวหลังเปิดห้อง (R7)
     *   · **ไม่เรียก** กับ poll/realtime ที่เป็น delta (ทุก 12 วินาที) และการเลื่อนดูของเก่า (มี cursor)
     * throttle 5 นาทีต่อเธรดในตัวฟังก์ชันยังเป็นด่านความถี่อีกชั้น
     *
     * ข้อความที่เติมทีหลังไปถึงจอผ่าน trigger `chat_message_realtime_broadcast_trigger` (AFTER INSERT
     * FOR EACH ROW) → hook ยิง delta → แถวที่เติมได้ seq ใหม่จึงเข้าเงื่อนไข delta (chat-delta-query.ts)
     * (แถวที่เก่ากว่าช่วงที่จอโหลดไว้ไม่ถูกแทรกเข้าจอ — loadOlder ดึงจาก DB เอง ดู planDeltaApply)
     * · poll 12 วินาทีเป็น safety-net ของ realtime ที่หลุด
     *
     * แลกไป 2 ข้อ: (1) ข้อความที่เติมไม่อยู่ใน response เดียวกัน มาช้ากว่านั้นหนึ่งรอบ realtime/poll
     * (2) ข้อความที่ webhook ไม่ส่ง **ซึ่งเกิดขึ้นระหว่างที่ห้องเปิดค้างอยู่แล้ว** ไม่มีใครสั่ง sync ให้
     * (poll ไม่ sync) ⇒ โผล่ตอนเปิดห้องครั้งถัดไป
     */
    /**
     ...
     * (2026-09-10) — หน้าเธรด (RSC) ต้องส่งข้อความชุดแรกไปกับ HTML ได้ด้วย ไม่งั้นเปิดห้อง
     * 1 ครั้งต้องไป-กลับเซิร์ฟเวอร์ 2 รอบเรียงกัน แล้วผู้ใช้เห็นสเกเลตัน 2 ช่วงซ้อน
     * 🛑 ห้ามก็อปตรรกะกลับมาที่นี่ ทั้งสองทางเข้าต้องประกอบข้อความด้วยกฎชุดเดียวกัน (HR16)
     */
