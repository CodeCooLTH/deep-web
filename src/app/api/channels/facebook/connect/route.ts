...
// คนนอก role → `config_id` ของ Facebook Login for Business (ส่ง scope ตรง ๆ = "ฟีเจอร์ไม่พร้อมใช้งาน"
// ดู CONNECT_CONFIG_ID) · บัญชีทีมใน FB_CHAT_ROLE_USER_IDS → scope ครบรวมตัวที่ยังรอรีวิว
// (role ขอสิทธิ์ standard ได้ แต่ Configuration ใส่ได้เฉพาะตัวที่เลือกไว้)
// ponytail: รายชื่อใน env ต้องตรงกับ role บน App Dashboard เอง — ผ่านรีวิวครบแล้วลบทิ้งทั้งกลไก
export function connectAuthParam(userId: string | null): [string, string] {
  const roleUsers = (process.env.FB_CHAT_ROLE_USER_IDS ?? '').split(',').map((s) => s.trim()).filter(Boolean)
  return userId && roleUsers.includes(userId)
    ? ['scope', `${CONNECT_SCOPES},${PENDING_REVIEW_SCOPES}`]
    : ['config_id', CONNECT_CONFIG_ID]
}
...
export async function GET(request: NextRequest) {
  ...
  authorizeUrl.searchParams.set(...connectAuthParam(sessionUserId(session)))
