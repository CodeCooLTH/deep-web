  ...
  it('ไม่มีฟิลด์นอก allow-list ติดมา', () => {
    ...
    expect(Object.keys(out).sort()).toEqual(
      [
        ...
        'latestReview',
        /**
         * ช่องทาง+ชื่อเพจต้นทาง (2026-08-30) — เพิ่มเข้า allow-list โดยตั้งใจ
         *
         * ไม่ใช่ PII ของผู้ซื้อ: เป็นชื่อ **เพจของร้าน** ซึ่งอยู่ในบล็อกหลักฐานบนจอเดียวกันแล้ว
         * (`channels[].name`) — ค่านี้แค่ชี้ว่าใบนี้เกิดที่เพจไหนในบรรดาเพจที่โชว์อยู่แล้ว
         * ใช้ติดป้าย "คุยกันที่นี่" แทนบรรทัดที่พิมพ์ชื่อซ้ำ
         */
        'originChannel',
        ...
        'serviceEndIso',
        /**
         * เงินที่ร้านรับจริงของ **บิลใบนี้** (2026-08-31) — เพิ่มเข้า allow-list โดยตั้งใจ
         * (user เคาะเอง · ทางเลือก ก)
         *
         * ไม่ใช่ PII ของผู้ซื้อ และไม่ใช่ตัวเลขระดับร้าน — เป็นยอดของใบที่ผู้ถือลิงก์ถืออยู่
         * ซึ่ง **ร้านเป็นคนส่งลิงก์นี้ให้ลูกค้าเอง** และลูกค้าคือคนที่ต้องรู้ว่ายังค้างเท่าไร
         * (เหตุผลเดียวกับ `payoutSnapshot` ที่อยู่ในลิสต์นี้แล้ว — เลขบัญชีร้านยังส่งได้)
         *
         * ทำไมต้องมี: ป้ายสถานะการชำระเงินบนจอนี้ derive จาก `Order.status` ซึ่ง `CONFIRMED`
         * แปลว่า *ผู้ซื้อยืนยันว่าได้รับบริการแล้ว* ไม่ได้แปลว่าจ่ายเงินแล้ว ⇒ บิลบริการที่ปิดงาน
         * แล้วแต่ยังค้างเงินขึ้นเขียว "ชำระแล้ว" มาตลอด · จอที่ล็อกอินแล้วมีตัวเลขนี้อยู่ก่อนแล้ว
         * **แก้ข้างเดียวไม่ได้** — บิลใบเดียวกันจะขึ้นป้ายคนละอย่างก่อน/หลังล็อกอิน
         *
         * 🛑 รูปร่างจำกัดไว้ 4 คีย์ (`totalAmount`/`totalReceived`/`outstanding`/`fullyPaid`)
         * **ไม่มี `entries`** — รายการรับเงินทีละก้อน (วิธีชำระ/เวลา/บันทึกภายใน) เป็นของฝั่งร้าน
         * มีเทส `[blocker]` แยกคุมข้อนี้ที่ `buyer-seller-payment-parity.test.ts`
         */
        'money',
        /**
        ...
        'returnedAt',
        /**
         * "พัสดุใบนี้เคยมีปัญหาครั้งแรกเมื่อไร" (2026-09-14) — เพิ่มเข้า allow-list โดยตั้งใจ
         *
         * ไม่ใช่ PII ด้วยเหตุผลเดียวกับสองช่องข้างบน: เป็นเวลาที่ *ขนส่ง* ทำให้เกิด ไม่ได้บอก
         * ตัวตน/สถานที่/พฤติกรรมของใคร · ผู้ซื้อคือคนที่ควรรู้ที่สุดว่าของที่เขารออยู่เคยส่ง
         * ไม่สำเร็จมาแล้ว — ถ้าซ่อน เขาจะไปทวงร้านว่า "ทำไมของช้า" โดยไม่มีใครบอกความจริง
         * (หลักเดียวกับที่ ParcelTimeline เขียนไว้เรื่องพัสดุตีกลับ)
         */
        'problemAt',
