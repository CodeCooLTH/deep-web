...
**buyer / Vuexy:** 6 / 8 / 12px + `full`.

| บทบาท | ค่า | นิยามอยู่ที่ |
|---|---|---|
| ปุ่ม · อินพุต · ของที่กดได้ | **6px** | `theme.shape.borderRadius` |
| ↳ ปุ่ม `size='small'` / `'large'` | 4px / 8px | `overrides/button.ts` — ธีมไล่ตามขนาดปุ่ม ไม่ใช่ค่าหลุด |
| อวตาร · รูปย่อ (`variant='rounded'`) | **8px** | `@core/theme/overrides/avatar.ts` |
| **การ์ด · แผง · ภาชนะ** | **12px** | `@core/theme/overrides/card.ts` · `accordion.tsx` |
| chip · พิล | `full` | — |

ความต่าง **6 vs 8 vs 12** คือสิ่งที่แยก "ของที่กดได้" ออกจาก "รูป" ออกจาก "ภาชนะ" — อย่ายุบให้เท่ากัน. ยิ่งเป็นภาชนะยิ่งกลม.

🛑 **การ์ดคือ 12px ไม่ใช่ 8px** (แก้ 2026-08-30, user เคาะ). ก่อนหน้านี้เอกสารประกาศ 8 แต่ `<Card>` ของธีมให้ 6 มาตลอด ขณะที่หน้าโปรไฟล์ร้าน (`/b`, `/u`) ประกอบการ์ดเองด้วย `div` แล้วตั้ง 12 ⇒ **ผู้ซื้อที่เดินสลับระหว่างหน้าร้านกับหน้าออเดอร์เห็นการ์ดกลมไม่เท่ากันเป็นเท่าตัว** และไม่มีตัวเลขไหนตรงกับเอกสารสักตัว. ตัดสินให้ยึด 12 เพราะเป็นค่าที่ surface สาธารณะใช้จริงมาก่อน — เอกสารเดินตามของจริง ไม่ใช่ให้ของจริงเดินตามเอกสารที่ไม่เคยมีใครทำตาม.

🛑 **ห้ามแก้ค่านี้ด้วยการขยับ `shape.borderRadius`** — มันเป็น **ตัวคูณ** ของ `borderRadius: N` ทุกตัวใน `sx` ทั้งระบบ (`borderRadius: 2` = 12px วันนี้ จะกลายเป็น 24px ทันที). ที่ถูกคือ override เฉพาะคอมโพเนนต์นั้น. และ `<Card>` **ห้ามใส่คลาสรัศมีทับ** (`rounded-2xl` ฯลฯ) — ให้รับจากธีมจุดเดียวเสมอ.

**โทเคนที่ใช้เขียน** (`marketing.css` remap Tailwind ให้ตรง MUI ramp — ห้ามใช้ค่า arbitrary):

| เขียน | ได้ | ใช้กับ |
|---|---|---|
| `rounded` / `rounded-md` | 6px | ปุ่ม · อินพุต |
| `rounded-lg` | 8px | แผ่นไอคอน · อวตาร · รูปย่อ |
| `rounded-2xl` | **12px** | การ์ด · แผง · กล่อง · **แถว/ไทล์** |
| `rounded-full` | pill | chip |

🛑 **ห้าม** `rounded-xl` (10) · `rounded-3xl` (16) · `rounded-4xl` (24) · `rounded-xs` (2) · `rounded-sm` (4 — สงวนให้ธีมตั้งเองกับปุ่ม `size='small'`) · `rounded-[Npx]` ทุกค่า.

**แถว/ไทล์ที่กดได้ = 12px ไม่ใช่ 6px.** "ของที่กดได้ 6px" หมายถึง **ปุ่มกับอินพุต** — แถวรายการหรือไทล์ที่บังเอิญกดได้ยังเป็น *ภาชนะ* รูปทรงเดินตามสิ่งที่มันเป็น ไม่ใช่ตามว่ากดได้ไหม (แถวออเดอร์บน `/dashboard` เคยเป็น 6px ด้วยเหตุผลนี้ผิด ๆ — แก้แล้ว 2026-08-30).

**รูปทรงในตัวของคอมโพเนนต์ไม่อยู่ใต้บันไดนี้** — `MuiBadge` (20px) · `MuiLinearProgress` (48px) · `MuiSwitch` track (10px) เป็นรูปทรงที่ธีมกำหนดให้ *ตัวคอมโพเนนต์นั้น* คลาสเดียวกับ chip ที่เป็นพิล ไม่ใช่การ์ดหรือกล่อง — ยุบให้เป็น 12px แล้วมันพัง.

**รูปทรงของ section ไม่อยู่ใต้บันไดนี้** — แถบโค้งใหญ่บนหน้า landing (`rounded-3xl md:rounded-[60px]`) เป็นภาษาของหน้า ไม่ใช่การ์ด ใช้ได้แต่ต้อง **เขียนคอมเมนต์กำกับบรรทัดนั้น** (ท่า carve-out เดียวกับ HR7) ไม่งั้น `theme-guard.sh` บล็อก.

ขอบเขต: ตารางนี้เป็นของ **`(marketing)/**` เท่านั้น**. ธีม MUI ชุดนี้ถูก mount ที่ `src/app/(marketing)/layout.tsx` ที่เดียว — `(paces)/**` ใช้ Preline/CSS คนละชุด ค่าด้านล่างไม่เปลี่ยนตาม.
...
### Cards / Containers
- **Corner:** **12px** ฝั่ง buyer (นิยามเดียวที่ `@core/theme/overrides/card.ts` — ดู §Shapes); 4px (`--radius`) ฝั่ง Paces.
...
- **Do** ผ่าน WCAG 2.1 AA: body contrast ≥4.5:1, focus state ชัด, tap target ≥44px
  ...
  - 🛑 **กฎนี้ไม่ได้ผูกกับ MUI** — ของที่กดได้ซึ่งเขียนด้วย Tailwind ล้วนต้องใช้ `h-11`/`size-11`
    (44px) **ห้าม** `h-8`/`h-9`/`h-10`/`size-9`/`size-10` ต่อให้ไม่ได้แตะ `sx` เลยสักตัว
  - 🛑 **และไม่ได้ผูกกับสกิน buyer ด้วย** — `seller`/`admin` (Paces) อยู่ใต้กฎเดียวกัน
    รูปแบบต่างกันตรงเดียว: **44px เฉพาะมือถือ (<1024px) · เดสก์ท็อปคงค่าเดิมของธีม**
    เพราะคอนโซลผู้ขายเป็น Operate surface ที่ความหนาแน่นมีค่า และเกณฑ์ 44px มาจาก "นิ้วโป้ง"
    ไม่ใช่บริบทของเมาส์ (WCAG 2.5.8 สำหรับเมาส์ = 24px ซึ่งค่าเดิม 37px ผ่านสบาย)
    นิยามเดียวอยู่ที่ `safepay-overrides.css` §"เป้าที่กดได้ = 44px บนมือถือ" ครอบ
    `.btn` · `.btn-icon` · `.page-link` · `badge` ที่กดได้ · `form-switch` · `form-radio` ·
    คำอธิบายกราฟ — ของที่ประกอบเองใช้ `min-h-11 lg:min-h-0` / `size-11 lg:size-N`
    🛑 ค่าตั้งต้นของธีม Paces เตี้ยกว่าเกณฑ์**ทุกตัว**: `.btn` 37 · `.btn-icon` 37 ·
    `.btn-sm.btn-icon` 30 · `.page-link` 30 · `.form-switch` 20 · `.form-radio` 18
    ⇒ แก้รายจุดไม่มีวันจบ ต้องยกที่ธีมเสมอ
  ...
  🛑 **และรอบนั้นยังนับไม่ครบ** — "48 จุด" มาจากการวัด 16 หน้าที่เขียนด้วย MUI
  เชลล์มือถือ `/m/**` ซึ่งเขียน Tailwind ล้วนไม่ได้อยู่ในชุดที่วัด และเหลือของเตี้ยอีก **74 จุด**
  (แถบบนที่ติดทุกหน้า · ปุ่มย้อนกลับ 36px 7 หน้า · ชิปกรอง 32px · "เลือกไฟล์" **17px**)
  วัดใหม่ครบ 35 หน้า 2026-08-31 แล้วเก็บหมด — **เหลือ 0**

  บทเรียนที่ตั้งเป็นกฎ: *ด่านที่เขียนจากคำศัพท์ของหน้าที่เพิ่งตรวจ จะตาบอดกับหน้าที่เขียนด้วย
  คำศัพท์อื่น* — `[TAP]` ตัวแรกเฝ้าแต่ `minBlockSize`/`min-bs-*` ซึ่งเป็นคำของ MUI จึงมองไม่เห็น
  `/m` ทั้งเชลล์ ตอนนี้ด่านเฝ้าทั้งสองคำศัพท์

  🛑 **แล้วมันเกิดซ้ำอีกครั้งที่ฝั่งผู้ขาย** (วัด 2026-09-01) — คราวนี้ไม่ใช่ *คำศัพท์* ที่ตาบอด
  แต่เป็น *ขอบเขต*: กฎทุกข้อใน `theme-guard.sh` เขียน `if [ "$is_paces" = false ]`
  ⇒ `seller`/`admin` ไม่เคยถูกวัดเลยสักครั้ง และมีเป้าต่ำกว่า 44px **223 จุด · 30 จาก 32 หน้า**
  เยอะกว่าฝั่ง buyer เกือบเท่าตัว ทั้งที่โค้ดเขียนไว้เองว่า "seller ใช้งาน mobile เป็นหลัก"

  ⇒ กติกาใหม่: **กฎที่เขียนว่า "ทั้งระบบ" ต้องมีด่านที่ครอบทั้งระบบ** ถ้าด่านครอบไม่ได้ทุกสกิน
  ต้องเขียนขอบเขตจริงลงในกฎ ไม่ใช่ปล่อยให้กฎอ้างกว้างกว่าที่บังคับได้
