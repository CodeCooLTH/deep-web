...
# 🛑 **ไฟล์ธีมไม่อยู่ใต้กฎที่เขียนไว้สำหรับผู้ใช้ธีม** — `src/assets/css/**` คือที่ที่ token
# ถูก *นิยาม* (hex ของ sidenav · `rounded-sm` ของฟอร์ม · ความสูงตั้งต้นของปุ่ม) ส่วนกฎในไฟล์นี้
# ทั้งหมดเขียนไว้สำหรับ *โค้ดที่บริโภค token* ⇒ เอามาตัดสินกันจะแดงถาวรโดยไม่มีทางแก้ให้ผ่าน
#
# พิสูจน์แล้วทั้งสองทิศ (2026-09-01): ปล่อยไว้ = `[SHAPE]` ของ buyer จับ `rounded-sm` ใน
# `_forms.css` แดงมาตลอด · ตีเป็น paces = `[HR7]` จับ hex ของ `--sidenav-*` ซึ่งเป็นที่ที่
# hex ต้องอยู่ ⇒ ไม่ใช่ทั้งสองฝั่ง ต้องข้าม
#
# กติกาของไฟล์ธีมถูกคุมด้วยเทส `[blocker]` แทน (`paces-tap-target.test.ts` ·
# `buyer-card-radius.test.ts`) ซึ่งตรวจ *เจตนา* ได้ ต่างจาก grep ที่ตรวจได้แค่รูปแบบ
case "$file" in src/assets/css/*) exit 0 ;; esac
...
# ── tap target: ของที่ "ไม่ได้ใช้ธีม" ──────────────────────────────────────
#
# 🛑 รูที่ทำให้เชลล์ `/m` ทั้งเชลล์หลุดรอบ 2026-08-30 ไปได้
#
# ด่าน [TAP] ข้างบนจับแต่ `minHeight`/`minBlockSize`/`min-bs-*` ซึ่งเป็นคำศัพท์ของ **MUI**
# แต่ `/m` เป็นเชลล์ที่เขียนด้วย Tailwind ล้วน ปุ่มคือ `<button className='size-9 …'>`
# และลิงก์คือ `<Link className='h-8 …'>` — ไม่มีคำว่า min-bs- สักตัว ด่านจึงมองไม่เห็นเลย
# ผลคือรายงานรอบนั้นเขียนว่า "เป้าต่ำกว่า 44px = 0" ทั้งที่ยังเหลืออีก 74 จุดใน `/m`
#
# บทเรียน: ด่านที่เขียนจากคำศัพท์ของหน้าที่เพิ่งตรวจ จะตาบอดกับหน้าที่เขียนด้วยคำศัพท์อื่น
#
# จับ `h-N`/`size-N` (N ≤ 10 = ต่ำกว่า 44px) เฉพาะบน className ที่มีร่องรอยว่า **กดได้**
# (`cursor-pointer` · `no-underline` = ลิงก์ · `active:` = สถานะตอนกด)
# วัดแล้วรีโปนี้ให้ 2 hit และเป็นของจริงทั้งคู่ — ไม่ใช่ด่านที่ร้องมั่ว
if [ "$is_paces" = false ]; then
  m=$(scan_nocomment "\b(h|size)-([1-9]|10)\b[^'\"]*(cursor-pointer|no-underline|active:)|(cursor-pointer|no-underline|active:)[^'\"]*\b(h|size)-([1-9]|10)\b")
  [ -n "$m" ] && add "[TAP] เป้าที่กดได้เขียนด้วย Tailwind ก็ต้อง ≥44px — ใช้ \`h-11\`/\`size-11\` ไม่ใช่ \`h-8\`/\`h-9\`/\`h-10\`/\`size-9\`/\`size-10\` (DESIGN.md §Do's). ถ้าตาเห็นเล็กแต่พื้นที่แตะใหญ่จริง (ลูก absolute ทับ / ::after inset ติดลบ) ให้เขียนคอมเมนต์กำกับบรรทัด:
$m"
fi

# ── tap target ≥ 44px — สกิน Paces (seller/admin) ───────────────────────────
#
# 🛑 รูที่ทำให้ **ทั้งฝั่งผู้ขายไม่เคยถูกวัดเลยสักครั้ง**
#
# กฎ 4 ข้อข้างบน ([SHAPE] [SPACE] [TAP] [SX]) เขียน `if [ "$is_paces" = false ]` ทุกข้อ
# ขณะที่ `DESIGN.md` §Do's ประกาศ "tap target ≥44px" ไว้เป็นเกณฑ์ **WCAG ของทั้งระบบ**
# ⇒ กฎเขียนกว้าง แต่ตัวบังคับแคบ (รอยเดียวกับ `/m` เมื่อ 2026-08-31)
#
# วัดจอจริง 390px (2026-09-01 · 40 หน้า): ฝั่งผู้ขายมีเป้าต่ำกว่า 44px **223 จุด · 30/32 หน้า**
# — เยอะกว่าฝั่ง buyer เกือบเท่าตัว และโค้ดเขียนไว้เองว่า "seller ใช้งาน mobile เป็นหลัก"
#
# พื้นถูกยกที่ธีมแล้ว (`safepay-overrides.css` — `.btn`/`.btn-icon`/`.page-link`/`badge`/
# `form-switch`/`form-radio` สูง 44px เมื่อ <1024px) ⇒ ด่านนี้จับ **ของที่ประกอบเอง**
# ซึ่งไม่ได้เดินผ่าน primitive เหล่านั้น
#
# 🛑 ต้องมี `lg:` คู่เสมอ — ฝั่งผู้ขายเป็น Operate surface ที่ความหนาแน่นบนเดสก์ท็อปมีค่า
# กฎคือ "44 บนมือถือ · ค่าเดิมบนจอใหญ่" ไม่ใช่ "44 ทุกจอ" (เหตุผลเต็มที่ `_forms.css`)
#
# 🛑 **ต้องไม่จับตัวที่มี prefix breakpoint** (`lg:size-9` `lg:min-h-10`) — นั่นคือ *ค่าเดสก์ท็อป*
# ซึ่งเป็นครึ่งหนึ่งของกฎ ไม่ใช่การละเมิด · ร่างแรกจับด้วย `\b` เฉย ๆ แล้วแดงใส่ไฟล์ที่เพิ่งแก้ถูก
# ทั้ง 4 ไฟล์ในนาทีเดียวกัน (ด่านที่ลงโทษคนทำตามกฎ = ด่านที่จะถูกปิดทิ้ง)
# ⇒ บังคับให้ตัวคลาสนำหน้าด้วยช่องว่าง/quote/backtick เท่านั้น ไม่ใช่ `:`
#
# และข้ามไฟล์ `.css` — ที่นั่นคือ *ตัวธีมที่นิยามค่าตั้งต้น* ซึ่งต้องมีค่าเดิมอยู่เพื่อให้ `lg:` ใช้ได้
case "$file" in *.css) : ;; *)
if [ "$is_paces" = true ]; then
  m=$(scan_nocomment "[[:space:]\"'\`{]((h|size|min-h)-([1-9]|10))\b[^'\"]*(cursor-pointer|onClick|hover:bg-|hover:text-)|(cursor-pointer|onClick|hover:bg-|hover:text-)[^'\"]*[[:space:]\"'\`{]((h|size|min-h)-([1-9]|10))\b")
  [ -n "$m" ] && add "[TAP] เป้าที่กดได้ฝั่งผู้ขายต้อง ≥44px บนมือถือ — ใช้ \`min-h-11 lg:min-h-0\` หรือ \`size-11 lg:size-N\` (DESIGN.md §Do's · พื้นอยู่ที่ safepay-overrides.css). ถ้าเป็นป้ายไอคอนที่กดไม่ได้ หรือพื้นที่แตะใหญ่จริงด้วย ::after ให้เขียนคอมเมนต์กำกับบรรทัด:
$m"
fi
;; esac
