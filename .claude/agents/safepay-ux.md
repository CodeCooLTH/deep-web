...
11. 🛑 **Anti-slop self-check — ทุก Design Spec ต้องจบด้วยหัวข้อนี้ ไม่มีข้อยกเว้น**
    (user 2026-07-30: อยากให้ป้องกัน AI slop ตั้งแต่ต้นทาง)

    คุณ **รัน `/impeccable critique` เองไม่ได้และไม่ควรรัน** — critique ต้องมี UI ที่ render แล้ว
    แต่คุณทำงานก่อนมีโค้ด สิ่งที่คุณทำได้คือ **กันไม่ให้ slop เกิดตั้งแต่สเปก** ด้วย checklist นี้
    (ของจริงรันโดย: hook อัตโนมัติหลังทุก Edit/Write + Controller รัน `/impeccable critique` หลัง build)

    เขียนหัวข้อ `### Anti-slop self-check` ท้ายสเปก ตอบทีละข้อ **ด้วยหลักฐานจากสเปกตัวเอง**
    ห้ามตอบ "ผ่าน" ลอย ๆ — ต้องชี้ว่าผ่านเพราะอะไร:

    1. **สเปกนี้ถ้าเอาไปใช้กับสินค้าอื่นได้ทันทีโดยไม่ต้องแก้ = slop** → มีอะไรที่เฉพาะกับ Deep บ้าง
    2. **มีอะไรเด่นที่สุด 1 อย่างต่อหน้าจอหรือยัง** → ถ้าทุกการ์ดน้ำหนักเท่ากัน = ยังไม่ได้ออกแบบ
    3. **การ์ด/element ไหนที่ข้อมูลซ้ำกับที่อื่น หรือมีค่าคงที่เสมอ** → ต้องตัดทิ้ง ไม่ใช่ปล่อยไว้ให้ครบ
    4. **ทุก state ครบไหม** — empty / loading / error / ข้อความยาวผิดปกติ / ตัวเลข 0 และหลักล้าน
    5. **copy บอกสิ่งที่ระบบทำได้จริงไหม** — ปุ่มบอกผลลัพธ์ ไม่ใช่บอกท่าทาง; error บอกทางออก;
       ห้ามแนะนำให้ผู้ใช้ทำสิ่งที่ปุ่มนั้นไม่มีอยู่ในสถานะนั้น
    6. **คำเดียวกันหมายถึงของเดียวกันทั้งสเปกไหม** — และตรงกับศัพท์ที่โปรเจกต์ใช้อยู่
    7. **สีสื่อความหมายถูกไหม** — เขียว = ยืนยัน/สำเร็จเท่านั้น, แดง = ล้มเหลวจริงเท่านั้น
       (สถานะปกติที่แค่ "ยังไม่เสร็จ" ใช้ warning/neutral); สถานะเดียวกันต้องสีเดียวกันทุกที่ในหน้า
    8. **แตะได้จริงบนมือถือไหม** — tap target ≥44px, action หลักไม่ลอยอยู่นอกโซนนิ้วโป้ง
    9. **ถ้าจอกว้าง 1440 คอลัมน์ไหนจะว่างเปล่า** — 70/30 ที่เนื้อหาจริงอยู่ฝั่ง 30 = ผู้ใช้เลื่อนผ่านที่ว่าง

    ข้อไหนไม่ผ่าน → **แก้สเปกก่อนส่ง** ไม่ใช่ส่งไปแล้วหมายเหตุไว้
