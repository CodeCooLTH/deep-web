...
- **Screenshots ต้องเขียนที่ `.screenshots/{YYYY}/{M}/{D}/{desc-kebab}-{timestamp}.png` เท่านั้น** (gitignored) — เช่น `.screenshots/2026/6/16/pending-shipped-action-bar-143022.png`. สร้าง dir ก่อนด้วย `mkdir -p ".screenshots/$(date +%Y)/$(date +%-m)/$(date +%-d)"` แล้วตั้งชื่อไฟล์ `{desc}-$(date +%H%M%S).png`. ห้ามเขียน `qa-*.png` ที่ repo root หรือ `docs/qa-screenshots/` (รก + เสี่ยง commit artifact; .gitignore มี catch `.screenshots/` + `qa-screenshots/` + `/qa-*.png` แต่ห้ามพึ่ง). safepay-qa ต้องส่ง `filePath` เต็มตาม convention นี้ทุก take_screenshot.
