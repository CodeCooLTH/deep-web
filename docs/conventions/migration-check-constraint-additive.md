...
กฎข้างบนพูดถึงการ **แก้** CHECK ให้ปลอดภัย แต่เคสที่เกิดจริงรอบสองคือการ **ไม่แก้เลย**

feature 00039 เพิ่ม `BUYER_NO_SHOW` / `BUYER_NO_PAYMENT` เข้า `CANCEL_REASONS_BY_VERTICAL` แล้ว `DATABASE.md` ของตัวเองเขียนไว้ว่า "ห้ามใส่ CHECK แบบระบุรายชื่อค่าให้ `cancelReason`" — ประโยคนั้นเขียนราวกับว่ายังไม่มี constraint ทั้งที่ `Order_cancel_reason` อยู่บนตารางมาตั้งแต่ 2026-07-22 ผลคือค่าใหม่ทั้งสองตัวถูก DB ปฏิเสธด้วย `23514` เต็มหน้าจอผู้ขายบน prod

**สิ่งที่ไม่มีทางจับได้ด้วยเครื่องมือที่มี:** `tsc` · build · เทสหน่วย · grep — ผ่านหมด เพราะค่าที่โค้ดส่งออกไปถูกต้องทุกตัวอักษร สิ่งที่ผิดคือ *อีกฝั่งไม่รู้จักมัน* และไม่มีอะไรในรีโปเชื่อมสองฝั่งนี้เข้าหากัน

**สิ่งที่ต้องทำ:** ทุก enum ที่เป็น `String + CHECK` ต้องมีเทสที่อ่านค่าจาก **ซอร์สของ TypeScript** แล้วยืนยันว่ามันปรากฏใน `prisma/migrations/**` ด้วย — ตัวอย่าง `src/lib/__tests__/cancel-reason-db-constraint.test.ts`. เทสแบบนี้ราคาถูกและเป็นด่านเดียวที่จับคลาสนี้ได้ก่อนขึ้น prod
