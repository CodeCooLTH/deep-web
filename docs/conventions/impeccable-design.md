...
- 🛑 **ฝั่งผู้ขาย/แอดมิน (Paces) ก็อยู่ใต้กฎนี้** → 44px **เฉพาะมือถือ (<1024px)**
  เดสก์ท็อปคงค่าเดิมของธีม (Operate surface — ความหนาแน่นมีค่า · เมาส์ไม่ใช่บริบทของ 44px)
  นิยามเดียวที่ `safepay-overrides.css` §"เป้าที่กดได้ = 44px บนมือถือ"
  ของที่ประกอบเองใช้ `min-h-11 lg:min-h-0` / `size-11 lg:size-N`

  ที่มา: วัด 2026-09-01 พบ **223 จุด · 30/32 หน้า** เพราะกฎทุกข้อใน `theme-guard.sh` เขียน
  `if [ "$is_paces" = false ]` ⇒ ฝั่งผู้ขายไม่เคยถูกวัดเลย (คลาสเดียวกับ `/m` แต่ตาบอดที่
  *ขอบเขต* ไม่ใช่ *คำศัพท์*)

- 🛑 **ของที่กดได้ซึ่งเขียนด้วย Tailwind ล้วน (เชลล์ `/m`) ก็อยู่ใต้กฎนี้** → `h-11` / `size-11`
  ห้าม `h-8` `h-9` `h-10` `size-9` `size-10` — ต่อให้ไม่ได้แตะ `sx` เลยสักตัว

  ที่มา: `[TAP]` ตัวแรกเฝ้าแต่ `minBlockSize`/`min-bs-*` ซึ่งเป็น *คำศัพท์ของ MUI* จึงมองไม่เห็น
  `/m` ทั้งเชลล์ — รายงาน 2026-08-30 สรุปว่า "เหลือ 0" ทั้งที่ยังเตี้ยอยู่อีก 74 จุด
  (แถบบนทุกหน้า · ปุ่มย้อนกลับ 36px · ชิปกรอง 32px · "เลือกไฟล์" 17px)

  **ด่านที่เขียนจากคำศัพท์ของหน้าที่เพิ่งตรวจ จะตาบอดกับหน้าที่เขียนด้วยคำศัพท์อื่น**
