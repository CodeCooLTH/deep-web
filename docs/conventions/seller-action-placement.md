...
### 3.1 🛑 action ที่ "ส่งถึงลูกค้าจริง" แต่ไม่ destructive — เกณฑ์คือ **แตะพลาดได้ไหม** ไม่ใช่ **ออกไปข้างนอกไหม**

คำถามนี้กลับมาถามซ้ำทุกครั้งที่มีปุ่มส่งของเข้าแชทตัวใหม่ (ล่าสุด 2026-08-11 โหมด "ส่งการ์ดสินค้า") — **มติคือไม่ใส่ confirm** และนี่คือเกณฑ์ที่ใช้ตัดสิน ไม่ต้องเปิดประเด็นใหม่:

| เข้าเงื่อนไข | ใส่ `pacesConfirm.question` | ตัวอย่างในโปรเจกต์ |
|---|---|---|
| ปุ่มส่ง **ซ้อนอยู่บนของที่ affordance หลักเป็นอย่างอื่น** หรือเป็น **1 แตะจากรายการที่ลอยอยู่** (mis-tap ได้จริง) | **ใส่** | `CustomerPanel.tsx` ปุ่ม hover "ส่งเข้าแชท" บนการ์ดออเดอร์ — การ์ดใบเดียวกันแตะแล้วเปิดโมดัลแก้ไข |
| ปุ่มส่งเป็น **ขั้นสุดท้ายของ flow ที่ผู้ขายตั้งใจเดินเข้ามาเอง** และปุ่มเขียนผลลัพธ์ไว้ตรง ๆ | **ไม่ใส่** | `ProductPickerPanel` โหมด "ส่งการ์ดสินค้า" + ปุ่ม `ส่ง N ข้อความ (M รายการ)` |

**ทำไมไม่ใช้เกณฑ์ "outward-facing = ต้องยืนยัน"** (ซึ่งเป็นคำที่คอมเมนต์ของ `CustomerPanel` เขียนไว้): เกณฑ์นั้น**พาไปถึงปุ่มส่งของช่องพิมพ์เองด้วย** — ข้อความ/รูป/สติกเกอร์/อิโมจิ ออกถึงลูกค้าทันทีทุกตัวและไม่มีอันไหนถาม ถ้าเกณฑ์นี้ถูกจริงต้องใส่ Swal ทั้งกล่องแชท ซึ่งไม่มีใครเสนอ ⇒ มันไม่ใช่เส้นที่แยกสองเคสข้างบนออกจากกัน

เหตุผลประกอบอีก 2 ข้อ:
- แชทเป็น **งานความถี่สูง** — Swal ที่เด้งทุกครั้งจะถูกกดผ่านโดยไม่อ่านภายในวันเดียว แล้วมันกินความน่าเชื่อถือของ Swal ตัวที่ควรหยุดคนจริง ๆ (`pacesConfirm.danger` ของการลบ) confirm มีต้นทุนกับ *ตัวอื่น* เสมอ ไม่ใช่แค่กับปุ่มที่มันแปะอยู่
- สิ่งที่ต้องมีแทน confirm คือ **ทำให้ปุ่มโกหกไม่ได้**: แยกกลุ่ม "ส่งออกทันที" ออกจากกลุ่มที่เติมช่องพิมพ์ (เส้นคั่น + หัวข้อกลุ่ม), hint บอกผลลัพธ์ ("ส่งทันทีเมื่อกด"), ปุ่มระบุจำนวนที่จะออกไป, และ **ส่งซ้ำต้องไม่ยิงซ้ำ** (207 ติ๊กใบที่ถึงลูกค้าแล้วออกให้เอง — `3549f4eb`)

🛑 **ห้ามใช้ข้อนี้ไปถอด confirm ของ `CustomerPanel` ออก** — มันอยู่คนละฝั่งของตาราง ถ้าจะถอดต้องย้ายปุ่มออกจากพื้นผิวที่แตะแล้วเปิดโมดัลก่อน
...
- [ ] ปุ่มที่ส่งของถึงลูกค้าจริงแต่ไม่ destructive → ตัดสินด้วยตาราง §3.1 (mis-tap ได้ = ถาม · ขั้นสุดท้ายของ flow = ไม่ถาม) **ห้ามเปิดประเด็นใหม่รายปุ่ม**
