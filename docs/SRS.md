...
| `paymentMethod` | String? | Phase B — วิธีชำระเงิน (FR-6.11) · ค่าบน prod มี `COD` / `CASH` / `TRANSFER` แต่เป็น free text (ร้านพิมพ์เองได้) ⇒ การตัดสินว่า "ใบนี้เก็บเงินปลายทางไหม" ต้องผ่าน `isCODPayment()` (`src/lib/order-display.ts`) ห้ามเทียบ `=== 'COD'` เอง. 🛑 **มีผู้เขียน 2 ราย**: ร้านตอนสร้าง/แก้ออเดอร์ และ `syncOrderPaymentToParcel()` ที่เขียนเป็น `COD` ให้เมื่อพัสดุ iShip เก็บเงินปลายทางจริง (event `PAYMENT_METHOD_SYNCED`) — ตัวหลังเชื่อยอด COD บนพัสดุ ดังนั้น **ยอด COD ของพัสดุที่ผิดจะกลายเป็นวิธีชำระเงินที่ผิดทันที** ยอดนั้นต้องมาจาก `resolveDefaultCodAmount()` ที่เดียว (00022 SRS §24 · BR-ISHIP-89/90/91) |
...
#### ChatMessage.updatedAt / Conversation.metaBackfilledAt (ส่วนขยาย 2026-09-14, "ห้องแชทเปิดแล้วเห็นทันที")

> `docs/20 - Features/00018 - Facebook Chat Integration/EXTENSIONS-2026-09-14.md` — additive ล้วน ไม่แตะคอลัมน์เดิม

| Field | Type | หมายเหตุ |
|-------|------|---------|
| `ChatMessage.updatedAt` | `DateTime @updatedAt @default(now())` | watermark แกนที่ 2 ของ delta (`GET .../messages?afterUpdatedAt=`) — Prisma เขียนให้เองทุก `update`/`updateMany` 🛑 **ห้ามเขียนมือ/raw SQL** (ด่าน `chat-message-no-raw-write.test.ts` สแกนทั้ง `src/`) 🛑 **แถวที่มีอยู่ก่อน migration `20260914150000` มีค่า `1970-01-01`** (fast default ของ Postgres 11+ — เขียนแบบนี้เพื่อไม่ต้อง `UPDATE`/rewrite ตารางที่ใหญ่ที่สุดในระบบ ~106k แถวตอนเขียน) ⇒ **ห้ามอ่านคอลัมน์นี้ตรง ๆ เป็น "เวลาแก้ล่าสุด" ต้องใช้ `max(createdAt, updatedAt)` เสมอ** (`src/lib/chat-message-store.ts::watermarksOf`) |
| `Conversation.metaBackfilledAt` | `DateTime?` | `null` = ยังไม่เคยไล่ดึงข้อความย้อนหลังจาก Meta ครบถึง `conversation.createdAt` (เปิดห้องครั้งถัดไปไล่ต่อ) · มีค่า = ครบแล้ว รอบถัดไปดึงหน้าเดียวพอ. 🛑 เธรดที่มีข้อความเกิน ~2,000 ใบ (limit 100 × เพดาน 20 หน้า/รอบ) ไม่มีวันได้ธง เพราะไม่เก็บ cursor ข้ามรอบ (known-gap ที่เปิดเผยแล้ว — ดู EXTENSIONS §8) |

**Index ใหม่:** `[conversationId, updatedAt]` (`ChatMessage_conversationId_updatedAt_idx`) — แกน `updatedAt` ของ delta ไม่ full-scan
...
| delta ของ `GET /api/chat/conversations/[id]/messages` (ส่วนขยาย 2026-09-14) | รับ `afterSeq` (int ≥0) และ/หรือ `afterUpdatedAt` (ISO timestamp) เพิ่มจากพารามิเตอร์เดิม (`cursor`/`take`) — มีอย่างน้อยหนึ่งตัว = โหมด delta: `WHERE` เป็น `OR` ของ `seq > afterSeq` กับ `updatedAt > afterUpdatedAt` (`src/lib/chat-delta-query.ts`), `take` ถูก cap ที่ `Math.min(take,100)` เสมอ, และ 🛑 **`nextCursor` เป็น `null` เสมอ** (delta ตอบ "มีอะไรเปลี่ยนตั้งแต่ watermark นี้" ไม่มีแนวคิดหน้าถัดไป ห้ามใช้ต่อเป็น cursor ของ pagination) · `afterSeq=0` เป็นค่าที่ถูกต้อง (ห้องยังไม่มีข้อความ) ต้องเช็คด้วย `!== undefined`. พารามิเตอร์ `sync=v.literal('1')` (ใหม่เช่นกัน) สั่งให้ backend ไล่เก็บข้อความที่ webhook ไม่ส่ง (`syncMissingMessagesFromMeta`) แม้คำขอเป็น delta — route trigger เมื่อ `!cursor && (!isDeltaRequest \|\| sync==='1')` เท่านั้น (ไม่ trigger ทุกรอบ poll) response ทุกโหมดมี `asOf` (ISO เวลาฝั่ง server ที่จับ **ก่อน** query ข้อความ — field เสริม) ให้ client ยก watermark แกน updatedAt ถึงค่านี้ ห้องที่เงียบจึง poll ได้ 0 แถว (`nextWatermarks()` ใน `src/lib/chat-message-store.ts`). ไม่ระบุพารามิเตอร์ใหม่ทั้งสามเลย = พฤติกรรมเดิมทุกประการ (ChatWidget ใช้ hook เดียวกันแต่ไม่แตะโค้ด delta) — รายละเอียด+rulings ที่เปลี่ยนพฤติกรรม (R1-R27) ดู `docs/20 - Features/00018 …/EXTENSIONS-2026-09-14.md` |
