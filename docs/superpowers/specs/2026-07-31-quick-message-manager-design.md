...
### แก้ทิศทางรอบ 5 — จากการใช้งานจริงบน prod (2026-08-01)

| อาการที่ user เจอ | สาเหตุ | แก้ |
|---|---|---|
| ป้าย "DeepBot" ทะลุขึ้นมาบนแผง | แผงใช้ `z-10` แต่ `AutoReplyTag` เป็น `z-20` (tooltip `z-30`) | แผงขึ้นเป็น `z-40` — สูงกว่าทุกอย่างที่ลอยในพื้นที่ข้อความ ยังต่ำกว่าโมดัลจัดการ (`z-90`) |
| คำอธิบายบนการ์ดยาว 10 บรรทัด การ์ดสูงผิดปกติ | `line-clamp-2 block` — `block` ทับ `display:-webkit-box` ที่ `line-clamp` ต้องใช้ (**บั๊กเดิมที่เคยแก้ในตารางแล้ว แต่พลาดซ้ำตอนแปลงการ์ดเป็น `<span>`**) | `line-clamp-3` (user ขอ 3 บรรทัด) + `min-h-10.5`; ตัด `block` ออกจากหัวข้อด้วย; ใส่คอมเมนต์กำกับกันพลาดรอบสาม |
| การ์ดบน desktop สูงจนกินพื้นที่ทั้งแผง | รูปใช้ `aspect-square` → ความสูงผูกกับความกว้างคอลัมน์ ยิ่งจอกว้างยิ่งสูง | `h-28` คงที่ทั้งการ์ดมีรูปและการ์ดที่ใช้ข้อความแทนรูป |
| **"จะเรียงลำดับหมวดยังไง"** | เดิม sort ตามตัวอักษรไทยทับ ผู้ใช้คุมไม่ได้ | **ลำดับกลุ่ม = ลำดับที่ผู้ใช้ลากเอง** — กลุ่มไหนมีข้อความอันดับต้นสุด กลุ่มนั้นขึ้นก่อน (`Map` เก็บลำดับ insert + list เรียงตาม `sortOrder` อยู่แล้ว แค่เอา `sort` ออก). ผลพลอยได้: ไม่ต้องมี UI จัดลำดับกลุ่มแยก โหมดจัดลำดับเดิมคุมได้ทั้งกลุ่มและรายการ |

**🛑 บทเรียนสำคัญของรอบนี้:** commit `d507acba` วาง JSX comment `{/* */}` ไว้ในกิ่งของ ternary (ตำแหน่ง expression — ต้องใช้ `//` เท่านั้น) ทำให้ **prod build ล้ม** และ user ต้องตามแก้เองใน `46e1ad50` สาเหตุรากคือรัน `npm run build` กับ `git push` ในคำสั่งเดียวโดยไม่ได้ gate ด้วย `&&` → build ล้มแต่ push เดินต่อ ดู retro `docs/retro/2026-08-01-quick-message-panel-retrospective.md`
