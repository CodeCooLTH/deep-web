...
### ผลวัดจริง 2026-08-16 (P0)

| ขอบเขต | ผล |
|---|---|
| `vitest run src/` — 252 ไฟล์ | **เขียวหมด 2,945/2,945** |
| `vitest run tests/` — 12 ไฟล์ | แดง 8 ไฟล์ / 80 เทส |

🛑 **แดงทั้ง 80 มาจากสาเหตุเดียว และไม่ใช่บั๊กของโปรดักต์:** ตอนเขียน Hard Rule 13
(หลังฐาน prod ถูกล้าง 2026-07-31) มีคนถอด `cleanDatabase()` ออกอย่างถูกต้อง แต่ไม่ได้ตามไปแก้
ผู้เรียกทั้ง 8 ไฟล์ ⇒ ทุกเทสตายที่บรรทัดแรกใน 0ms พร้อมข้อความบอกทางแก้ไว้ให้แล้ว
(`deleteTestData({ userIds, shopIds })`)

⇒ **นี่คืองาน 1 ชิ้นที่ทำค้าง ไม่ใช่หนี้ 80 ข้อ** (`docs/conventions/known-limitation-vs-unfinished.md`)

⚠️ CLAUDE.md บันทึกสาเหตุไว้ผิดว่า "prisma mock ไม่ครบ" — คำอธิบายผิดตัวนี้ถูกคัดลอกต่อกันมา
ตั้งแต่ 2026-08-07 (18 ข้อ → 98 → 80) และเป็นเหตุผลที่ไม่มีใครไปแตะ ต้องแก้บันทึกด้วย

### สรุปด่าน (แก้จากร่างแรก)

`known-failures.txt` ที่ร่างแรกออกแบบไว้ **ถูกถอดทิ้ง** — มันมีไว้โอบรับหนี้ 98 ข้อที่ไม่มีอยู่จริง
การเก็บไว้จะกลายเป็นที่ซ่อนของแบบเดียวกับ `ADVISORY_PATTERN` ที่เราตั้งใจหนี

| ด่าน | ขอบเขต | สถานะ |
|---|---|---|
| `tsc --noEmit` | ทั้งรีโป | บล็อกได้ทันที |
| `vitest run src/` | 252 ไฟล์ ไม่แตะ DB | **บล็อกได้ทันที — เขียวอยู่แล้ว** |
| `theme-guard.sh` | ไฟล์ที่ PR แตะ | บล็อกได้ทันที |
| `vitest run tests/` | ต้องมี Postgres | **P0.1 — แก้ 8 ไฟล์ก่อน แล้วค่อยเข้าด่าน** (CI ต้องมี service container `postgres:16`) |

**ห้ามตั้ง `tests/` เป็น advisory ระหว่างรอ** — ปล่อยไว้นอกด่านไปก่อนอย่างเปิดเผย
แล้วปิดงาน P0.1 ให้จบ ดีกว่ามีด่านที่ประกาศตัวว่าไม่บล็อก
...
|---|---|---|---|
| R-1 | ~~เทสแดงค้างจำนวนไม่ทราบ~~ | — | ✅ **ปิดแล้ว 2026-08-16**: `src/` เขียวหมด · `tests/` แดง 80 จากสาเหตุเดียว (§6.1) |
...
|---|---|---|
| **P0** | ~~รันเทสเต็ม บันทึกเลขจริง~~ ✅ **เสร็จ 2026-08-16** | — |
| **P0.1** | แก้ 8 ไฟล์ใน `tests/` ที่ยังเรียก `cleanDatabase()` → `deleteTestData({ userIds, shopIds })` + แก้บันทึกที่ผิดใน CLAUDE.md | P0 |
| **P1** | `verify.yml` ทำงานบน PR (`tsc` + `vitest src/` + `theme-guard`) | P0 — **ทำได้เลย ไม่ต้องรอ P0.1** |
