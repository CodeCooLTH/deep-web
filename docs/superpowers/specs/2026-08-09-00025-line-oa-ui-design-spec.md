...
- ~~เป็น "ข้อมูล ไม่ใช่การนับถอยหลัง" ตาม BRD AC-005-05 — **ไม่มีตัวเลขวินาทีนับถอยหลังเลย**~~ 🔄 **แก้ 2026-08-10:** ปุ่มส่งนับถอยหลังวินาทีจริงแล้ว (`ส่ง · ฟรี 45 วิ`) · **`AC-005-05` เป็น ID ของ feature 00019 ไม่ใช่ของ 00025** — ของจริงคือ bullet ใต้ `FR-LINE-05` ซึ่งข้อที่อยู่ติดกันขอให้แสดง "เหลือเวลาเท่าไร" ด้วยซ้ำ. ข้อห้ามที่เหลือคือห้ามมี**ตัวเร่งความเครียด** (แดง/กะพริบ/ขยายเมื่อใกล้ 0) ไม่ใช่ห้ามตัวเลข
...
1. ~~**เกณฑ์ "โควตาเหลือน้อย"**~~ — ✅ **ปิดแล้ว 2026-08-09: user เคาะ `≤20%` ของโควตาทั้งหมด** (แพ็กเกจ 300 → เตือนเมื่อเหลือ ≤60) เดิม BRD บอกว่า "เกณฑ์กำหนดใน SRS" แต่ grep ทั้งไฟล์แล้ว **SRS ไม่เคยระบุตัวเลขจริง** — ตัวเลขนี้จึงมีที่มาจากมติของ user โดยตรง ไม่ใช่จากเอกสารเดิม
   - implement: ประกาศเป็นค่าคงที่ชื่อสื่อความใน `src/lib/line/constants.ts` (เช่น `QUOTA_LOW_RATIO = 0.2`) 🛑 **ห้าม hardcode `0.2` กระจายใน component** — S-9 (service) และ S-14 (UI) ต้องอ่านจากค่าเดียวกัน
   - `type: "unlimited"` ไม่มีสถานะนี้เลย · อ่านโควตาไม่ได้ (`stale`) ไม่นับเป็น "เหลือน้อย"
