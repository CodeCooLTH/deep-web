...
|---|---|---|
| `ORDER_CREATED` | **วันที่ที่ผู้ขายเลือก** (ไทม์ไลน์อ่านแล้วสมเหตุสมผล) | เมื่อ ≠ เวลาจริง เพิ่ม `keyedInAt` = เวลาจริงที่คีย์ |
| `ORDER_DATE_CHANGED` *(ชนิดใหม่)* | เวลาจริงที่กดแก้ | `{ from, to }` (ISO ทั้งคู่) |

แสดงผลใน Activity Log: *"ลงวันที่ 28 ก.ค. 2569 · คีย์เข้าระบบ 6 ส.ค. 2569 09:12"* — ความจริงไม่หายไปไหน
...
- `POST /api/orders` พร้อม `createdAt` ย้อนหลัง → ตรวจ **3 ค่าพร้อมกัน**: `Order.createdAt`, `Order.orderNo`, `OrderEvent(ORDER_CREATED).occurredAt`
