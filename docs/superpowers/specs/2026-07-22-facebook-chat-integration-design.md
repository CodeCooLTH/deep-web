...
### 8.1 Layout reference ที่ user กำหนด (2026-07-22)

user ส่งภาพหน้าจอ CRM ของ **12Tees** (`crm-v2.12tees.com`) มาเป็น reference พร้อมกำกับว่า
**"ใช้ Theme ตามกฎ แต่ layout ประมาณนี้"** — ตรงกับ Hard Rule 6 พอดี: เอา **IA/layout ตาม ref**
แต่ **skin/สี/component = Paces ปัจจุบัน** (น้ำเงิน `#236dc9` ไม่ใช่เขียวของ 12Tees)

โครง 3 คอลัมน์ที่อ่านได้จาก reference:

| คอลัมน์ | องค์ประกอบ |
|---|---|
| ซ้าย (รายการแชท) | ช่องค้นหาบนสุด ("ค้นหาชื่อ / ลูกค้า / เบอร์ / ข้อความในแชท") · แถบ tab ฟิลเตอร์ตามช่องทาง (ทั้งหมด / LINE / Messenger / Gmail) · ปุ่ม "ตัวกรอง" พร้อม badge นับ · chip แสดงฟิลเตอร์ที่ใช้อยู่ (`เปิดอยู่ ✕`) · แถวแชท = avatar + ชื่อ + tag ผู้ดูแล + preview ข้อความ + เวลา + ปุ่มซ่อน/ปักหมุด |
| กลาง (เธรด) | header ชื่อลูกค้า + ไอคอนช่องทาง + ปุ่มปิดงาน · เส้นคั่นวันที่ · bubble ข้อความ · **การ์ดแนบแบบ structured** (ในภาพคือใบเสนอราคา: ชื่อ/เลขที่/จำนวนรายการ/ยอดสุทธิ/ปุ่มดู PDF) + สถานะ "ส่งแล้ว ✓ + เวลา" · composer ล่างสุดพร้อมแถวไอคอน (ข้อความด่วน / emoji / แนบไฟล์ / โน้ตภายใน) |
| ขวา (แผงลูกค้า) | avatar + ชื่อ + ไอคอนช่องทาง · tab (ลูกค้า / ใบเสนอราคา / ออเดอร์ / แท็ก / Note) · ชื่อที่แสดงในแชท · การ์ดลูกค้าที่ผูกไว้ (รหัส + ผู้ติดต่อ + เบอร์ + ลิงก์ออก + ปุ่มยกเลิกการผูก) · ช่องเบอร์โทร · dropdown ผู้ดูแล · dropdown สถานะการขาย |

**สิ่งที่ตรงกับ MVP อยู่แล้ว:** โครง 3 คอลัมน์, tab ฟิลเตอร์ตามช่องทาง, ค้นหา,
แผงขวาที่ผูก ExternalContact → Customer ด้วยเบอร์ (= S-4), ไอคอนช่องทางบนแถวแชท

#### ผลตัดสิน Q-4 (user, 2026-07-22)

| จาก reference | ผล | หมายเหตุ |
|---|---|---|
| โครง 3 คอลัมน์ + filter ช่องทาง + ค้นหา | ✅ เข้า MVP | ตรงกับ S-1/S-5 เดิม |
| **ปักหมุด / ซ่อนแชท / ปุ่มปิดงาน** | ✅ **เพิ่มเข้า MVP** | ต้องเพิ่มคอลัมน์บน `Conversation` → รวมเข้า migration เดียวกัน (S-7) |
| **tab ใบเสนอราคา / ออเดอร์ / แท็ก / Note ในแผงขวา** | ✅ **เพิ่มเข้า MVP** | ต้องมี model แท็ก + โน้ต (S-8) |
| มอบหมายผู้ดูแล (assignee) + สถานะการขาย | ❌ ไม่เอา | ต้องผูกระบบพนักงาน (00012) + นิยาม sales pipeline ที่ยังไม่มี |
| ช่องทาง Gmail | ❌ ไม่เอา | นอก scope feature นี้ |

🛑 **ช่องว่างที่ต้องตัดสินก่อนทำ tab แผงขวา:** ระบบ Deep **ยังไม่มีฟีเจอร์ใบเสนอราคา (quotation)
เลย** — ไม่มี model, ไม่มีหน้าจอ, ไม่มี PDF ในภาพ reference การ์ดใบเสนอราคาคือแกนหลักของ
12Tees แต่ของเราไม่มีของจริงมารองรับ ทางเลือก: (ก) ตัด tab ใบเสนอราคาออก เหลือ ออเดอร์/แท็ก/Note
(ข) ทำใบเสนอราคาเป็น feature แยกก่อน แล้วค่อยต่อ tab นี้ทีหลัง — **ยังไม่ตัดสิน ดู §14 Q-5**

**เพิ่มเข้า scope MVP จากผลตัดสินนี้:**

| # | รายการ |
|---|---|
| S-7 | ปักหมุด / ซ่อน / ปิดงาน ต่อเธรด (`isPinned`, `isHidden`, `resolvedAt` บน `Conversation`) |
| S-8 | แท็กและโน้ตภายในต่อเธรด/ลูกค้า + tab ออเดอร์ในแผงขวา |
...
> ผ่าน `safepay-ux` ก่อนตาม Hard Rule 8 mockup จะออกในขั้น UI plan ไม่ใช่ที่นี่
...
| Q-4 | ฟีเจอร์ใน layout reference ที่เกิน MVP เอาเข้าตัวไหนบ้าง | ✅ **ปิดแล้ว** — เอา ปักหมุด/ซ่อน/ปิดงาน (S-7) + tab แผงขวา (S-8); ไม่เอา assignee/สถานะการขาย/Gmail (§8.1) |
| Q-5 | tab "ใบเสนอราคา" ในแผงขวา — ระบบยังไม่มีฟีเจอร์ใบเสนอราคาเลย จะตัดออกหรือทำ feature ใบเสนอราคาก่อน | รอ user ตัดสิน — บล็อกเฉพาะ tab นั้น ไม่บล็อกส่วนอื่น |
