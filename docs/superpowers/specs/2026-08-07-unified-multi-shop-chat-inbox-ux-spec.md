...
## 12. 🛑 Open questions — ต้องให้ user เคาะก่อน implement

1. **ย้ายสวิตช์เข้า dropdown แทน pill ในหัวแชท** — ไม่ตรงกับตัวอย่าง `ร้านทั้งหมด ▾` ใน architecture spec §6.1 แม้แก้ปัญหางบพื้นที่ได้จริง trade-off คือ **ผู้ใช้ไม่เห็นโหมดปัจจุบันเป็นข้อความตลอดเวลา** เห็นแค่ไอคอนบน badge ของปุ่ม avatar ที่เปลี่ยนไป ยอมรับได้ไหม หรืออยากยุบช่องค้นหาให้แคบลงเพื่อแลกกับ pill จริงในหัวแชท
2. **สีของ badge ร้านเมื่อไม่มีโลโก้** — ใช้ `bg-primary/10 text-primary` เหมือน `PageAvatar` (ทุกร้านสีเดียวกัน) หรือสุ่มสีต่อร้านเพื่อแยกด้วยตาง่ายขึ้น (ยังไม่มี palette สุ่มสีที่ registered ใน Paces token — ถ้าเอาต้องคุยเพิ่ม)
3. **แถบบริบทร้านใต้หัวเธรด (§6)** ควรกดได้ไหม (กดแล้วเปิด `ChatShopSwitcher` ลัด) หรือ display-only ตามที่ออกแบบไว้
