...
| | โปรไฟล์ ↗ | ลิงก์ภายนอก | — | ย้ายมาจาก dropdown ("เปิดหน้าร้าน") |

**หายไปทั้งกลุ่ม:** `บัญชีของฉัน` — `/account` ย้ายไปอยู่ใน dropdown อย่างเดียว

**"โปรไฟล์ ↗"** ไม่ใช่ route ภายใน — ชี้ไปหน้าร้านจริงบนโดเมนผู้ซื้อ
`BUSINESS + slug` → `/b/{slug}` · อื่น ๆ → `/u/{username}` เปิดแท็บใหม่ พร้อมไอคอน `external-link`
(ต้องประกอบ URL ข้าม subdomain แบบเดียวกับ `public-profile/page.tsx` ที่ตัด `seller.` ออกจาก host
ไม่งั้น proxy จะ rewrite เป็น `/seller/b/{slug}` → 404 — บั๊กเดิมที่เคยเจอมาแล้ว)

ป้าย "โปรไฟล์" (ลิงก์ออก) กับ "ตั้งค่าหน้าร้าน" (`/public-profile`) อยู่กลุ่มเดียวกันโดยตั้งใจ —
อันหลังคือที่ตั้งว่า "หน้าร้านจะโชว์อะไร" อันแรกคือ "ไปดูของจริง" จึงเปลี่ยนป้ายอันหลัง
ไม่ให้ทั้งคู่ขึ้นต้นด้วยคำว่าโปรไฟล์
...
1. **slug ของรายการห้ามเปลี่ยน** — `ShopShortcut` ในฐานข้อมูลเก็บ slug ไว้ ถ้าเปลี่ยนแล้ว
