...
| D2 | บัญชี provider ซ้ำ | **บล็อก + บอกวิธีแก้** ที่ `/settings` — ตรวจภายหลังพบว่าระบบมี 2 พฤติกรรมตาม entry point และ user ตัดสิน 2026-08-02 ให้ **คงแยกตามบริบท** (order-claim ต้องลื่นเพราะกำลังจะบล็อกการซื้อ; การตั้งค่าบัญชีเป็นการกระทำที่ตั้งใจ ไม่รีบ) |
...
> **แก้ไขรอบ 2 (2026-08-02 หลัง implement):** หัวข้อนี้เคยสรุปว่า "ส่วน C มีครบแล้ว ไม่ต้องทำ"
> ซึ่ง**ยังผิดอยู่ดี** — ที่มีอยู่คือการเชื่อม OAuth เท่านั้น **ส่วน username/password ไม่เคยมี**
> (การ์ดใช้ `hasPassword` แค่ขึ้นข้อความเตือน ไม่มีปุ่มให้ตั้ง) สร้างเพิ่มแล้วในรอบนี้: แถว
> "รหัสผ่าน" + `POST /api/account/otp-for-password` + `POST /api/account/set-password-otp`
> บทเรียน: การเห็นชื่อไฟล์/โฟลเดอร์ไม่เท่ากับรู้ว่ามันทำอะไร — ต้องเปิดอ่านก่อนสรุปว่ามีแล้ว
