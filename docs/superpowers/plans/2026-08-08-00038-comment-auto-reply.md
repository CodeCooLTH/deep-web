...
เพิ่มในไฟล์เทสเดิม — mock `replyToComment` และ `sendPrivateReplyToCommentById` แล้วยืนยัน 3 พฤติกรรม:

```ts
it('เปิดทั้ง 2 สวิตช์ -> เรียกทั้งตอบใต้คอมเมนต์และทักแชท ตามลำดับ', async () => { /* ... */ })
it('ตอบใต้คอมเมนต์ล้มเหลว -> ยังทักแชทต่อ (ไม่ผูกกันแบบ all-or-nothing)', async () => { /* ... */ })
it('ด่านไม่ผ่าน -> เขียน log พร้อม skipReason และไม่เรียกตัวส่งเลยสักตัว', async () => { /* ... */ })
