...
- **BR-AI-09-EX (ข้อยกเว้นเดียวของ BR-AI-09 — ผู้ใช้อนุมัติ 2026-08-04):** ร้านกดปุ่ม
  "ใช้ AI ช่วยอ่าน" ในชีตกระจายที่อยู่เอง แล้วส่ง **ข้อความก้อนที่ร้านเลือก** ไปให้ AI แยกเป็น
  ชื่อ/เบอร์/ที่อยู่ ได้ ภายใต้เงื่อนไขครบทุกข้อ:
  1. ต้องเป็นการกดของคนในร้านต่อครั้ง — ห้ามเป็น fallback อัตโนมัติเมื่อ parser ปกติแยกไม่ครบ
  2. ส่งได้เฉพาะข้อความก้อนนั้น ห้ามแนบบริบทอื่นเลย (ไม่มีชื่อร้าน สินค้า ราคา ประวัติแชท
     หรือข้อมูลลูกค้าจากฐาน) — ห้ามเรียกผ่าน `ai-context.service.ts` ซึ่งเป็นเส้นทางของ
     "AI ที่รู้จักร้าน" และยังอยู่ใต้ BR-AI-09 เต็มรูปแบบ
  3. ห้าม log เนื้อความ และจำกัดความยาวคำขอ (2,000 ตัวอักษร)
  4. ผลจาก AI ใช้เติมเฉพาะช่องที่ parser rule-based แยกไม่ได้ — parser ยังเป็นค่าตั้งต้นเสมอ
  เหตุผลที่ยอมเปิด: ที่อยู่ที่ลูกค้าพิมพ์มาในแชทเป็นข้อความอิสระที่ parser พลาดได้บ่อย และทางเลือก
  เดิมคือร้านพิมพ์ที่อยู่ใหม่ด้วยมือ ซึ่งพลาดมากกว่าและทำให้พัสดุส่งผิด. ขอบเขตข้อยกเว้นถูกบังคับ
  ที่โค้ดจุดเดียว: `src/app/api/orders/parse-address/route.ts`
