...
| ~~Q16~~ | ✅ **ปิดแล้ว 2026-08-01 (user ตัดสิน):** ไม่เพิ่ม role ใหม่ ไม่ลดสิทธิ์ใคร — **เทสสิทธิ์ทุกจุดใช้ `ADMIN` แทน `STAFF`** และเปลี่ยนสิ่งที่พิสูจน์จาก "STAFF ถูกปฏิเสธ" เป็น **"ADMIN แก้ได้จริง"** · เคสที่คาดหวัง `FORBIDDEN_ROLE` (403) จาก role ให้ถือว่า **superseded — ห้ามรัน** เพราะไม่มีทางเกิด (รวม TC-SEC-01/02/06 ของฐานเดิม) · **403 ที่ต้องเทสแทนคือ `SHOP_LOCKED`** ซึ่ง implement จริงแล้วใน `forbidIfReadOnly()` เมื่อ 2026-08-01 (ร้าน Business ที่ `packageLockedAt` ไม่ null → เขียนไม่ได้ทุก endpoint แต่ยังอ่านได้) | TC-QAPI-05 และทุก TC ที่อ้าง 403 | ปิดแล้ว — ดู API.md §2.1 |
