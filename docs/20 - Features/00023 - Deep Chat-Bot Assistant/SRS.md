...
#### TFR-031: อัลกอริทึม `matchQna` — วิธีจับคู่ทางที่สองของกลุ่มคำ (เพิ่ม 2026-07-31)

- **Trace to:** Scope Baseline `00023-qna` S-03 · user decisions §3, §4 · [[DATABASE]] §3.9
- **ที่อยู่:** `src/lib/auto-reply-qna-match.ts` — export `matchQna(normalizedText, qnaSet, opts): QnaMatchResult`
- **ข้อบังคับ (เหมือน TFR-008/009 ทุกข้อ):** ฟังก์ชัน **บริสุทธิ์** — ห้าม import Prisma, ห้าม I/O, ห้าม `new Date()`, รับทุกอย่างเป็น argument · ผู้เรียกโหลด `qnaSet` มาให้ (TFR-032)
  เหตุผลเดียวกับ `auto-reply-match.service.ts`: `/api/shops/auto-reply/simulate` ต้องเรียก **ฟังก์ชันตัวเดียวกับเส้นทางตอบจริง** ไม่งั้น AC-020-05 ("ผลที่เห็นตรงกับที่จะเกิดจริง") เป็นเท็จทันที และจะเกิดอาการ "ทดสอบผ่านแต่ของจริงเงียบ"

**โหมดการจับคู่ (v1 มีโหมดเดียว):**

| โหมด | เงื่อนไข | สถานะ |
|---|---|---|
| `EXACT` | `normalizeMessage(ข้อความลูกค้า) === qna.normalizedQuestion` | **v1 — โหมดเดียวที่ใช้จริง** |
| `SIMILARITY` | trigram Dice ≥ threshold | 🛑 **ยังไม่ทำ** — user ตัดสิน 2026-07-31 "ตรงตัวก่อน ความคล้ายเปิดทีหลัง" |

`matchQna()` **ต้องรับพารามิเตอร์ `mode` ตั้งแต่ v1** แม้มีค่าเดียว — เพื่อให้การเปิดโหมดที่สองเป็นการเพิ่มสาขา ไม่ใช่การรื้อลายเซ็นฟังก์ชันที่มี caller อยู่แล้ว
คอลัมน์ `AutoReplyKeyword.qnaSimilarityEnabled` มีอยู่ใน DB แล้ว (**ยังไม่มีที่ไหนอ่าน** — [[DATABASE]] §3.7)

**เหตุผลที่ v1 เป็น `EXACT` ล้วนแล้วยังคุ้ม (ตัวเลขจริงจาก prod 2026-07-31):**
`AutoReplyLog` ที่ `skipReason='NO_KEYWORD_MATCH'` มี 401 แถว · 4 ข้อความที่ซ้ำมากที่สุดเป็น ice-breaker/quick-reply ของโฆษณา Meta ซึ่งเป็น **สตริงเดียวกันเป๊ะทุกครั้ง** (`สอบถามรายละเอียด` 68 · `สนใจทำไฟหน้า` 38 · `สอบถามโปรโมชั่น` 36 · `ราคาโช้คหลังเวฟเท่าไหร่` 28) รวม **170/401 = 42%** ⇒ การจับคู่ตรงตัวล้วนปิดคิวได้ 42% โดยมีโอกาสตอบผิด **เป็นศูนย์**

**ขั้นตอน:**
1. `normalizedText === ''` → คืน `null` ทันที (เหตุผลเดียวกับ TFR-008: สตริงว่างเป็น substring ของทุกสตริง)
2. กรอง `qnaSet` เหลือข้อที่ `isActive = true` **และ** กลุ่มเจ้าของไม่ใช่ `OFFLINE` (กลุ่ม OFFLINE ไม่ถูกโหลดเข้า ruleSet อยู่แล้ว — ข้อของมันจึงต้องเงียบตามกัน)
3. เทียบ `normalizedQuestion` แบบเท่ากันทุกอักษร
4. ตรงมากกว่า 1 ข้อ → ตัดสินด้วยเกณฑ์ 3 ชั้นของ [[DATABASE]] §3.9.1 (`keyword.priority` → `useCount` → `qna.id`) — **ห้ามพึ่งลำดับที่ DB คืนมา**
- **Postcondition:** ได้ผู้ชนะ 0 หรือ 1 ข้อเสมอ
- **Error / Edge cases:** `qnaSet` ว่าง → `null` (ไม่ใช่ error) · ข้อที่กลุ่มเจ้าของถูกลบไปแล้ว → ไม่มีทางเกิดเพราะ FK เป็น `Cascade`

#### TFR-032: ตำแหน่งของ QnA ในเส้นทางตัดสิน — "คำตรงตัวชนะก่อนเสมอ" (เพิ่ม 2026-07-31)

- **Trace to:** user decisions §3 ("คำตรงตัวชนะก่อนเสมอ") · Scope Baseline `00023-qna` D1, D2, D3
- **ที่อยู่:** `src/services/auto-reply.service.ts` (`processJob`) + `loadRuleSet`

**ลำดับ gate หลังเพิ่ม QnA (เปลี่ยนจากเดิม 2 จุด ที่เหลือคงเดิมทุกตัว):**

```
0 ข้อความฝั่งร้าน → 1 สวิตช์เธรด → 3-5 สแปม/handoff/pause → 6 เพดานตอบต่อเธรด
  → matchKeywords()
  → 🆕 ถ้าไม่มีกลุ่มคำใดตรง → matchQna()   ← จุดที่ 1
  → 6.5 สถานะกลุ่มที่ชนะ → 6.6 เวลาทำงาน → 7 cooldown
  → 🆕 ถ้าชนะด้วย QnA → ใช้ qna.answer ข้าม resolveRule ← จุดที่ 2
  → resolveRule() → 8-9 ไม่ตรง/ไม่มีกฎ
```

1. **QnA ทำงานเฉพาะเมื่อ `matchKeywords` ไม่มีผู้ชนะ** — ไม่ทำงานตอน `NO_RULE_MATCH`
   🛑 เหตุผลที่ไม่ให้ทำงานตอน `NO_RULE_MATCH` ด้วย: กลุ่มคำตรงแต่ไม่มีกฎ = **ร้านตั้งค่าค้างไว้ครึ่งทาง** ซึ่งร้านต้องเห็นว่าค้าง ถ้าปล่อยให้คลังมาตอบแทน อาการ "ตั้งกฎไว้แล้วไม่เคยถูกใช้" จะถูกกลบและไล่ไม่เจอ
2. **ผู้ชนะจาก QnA "ยืมกลุ่มคำเป็นเจ้าของ"** — เซ็ต `winner.keywordId = qna.keywordId` แล้วเดินผ่าน gate 6.5 / 6.6 / 7 **เหมือนกลุ่มคำปกติทุกประการ**
   🛑 นี่คือข้อที่ห้ามลัด: ถ้า QnA ไม่ผ่าน gate เหล่านี้ มันจะกลายเป็นช่องทางที่ข้ามสถานะ `OFFLINE`/`TEST`, ข้ามตารางเวลา และข้าม cooldown ได้ทั้งหมด — เป็นกับดัก "สวิตช์คนละที่" ตัวเดียวกับที่ถูกรื้อทิ้งเมื่อ 2026-07-30 (`AutoReplyConfig.isEnabled`) และเป็นเหตุผลที่ user เลือกให้คลังอยู่ในกลุ่มตั้งแต่แรก (decisions §3)
3. **คำตอบมาจาก `AutoReplyQna.answer` ตรง ๆ ไม่ผ่าน `resolveRule`** — คลังมีคำตอบของตัวเองอยู่แล้ว
   `AutoReplyLog.resolutionLevel = "QNA"`, `matchedVia = "QNA"`, `qnaId` = ข้อที่ชนะ
4. **`loadRuleSet()` โหลด `qnaSet` มาด้วยในคิวรีชุดเดียวและใช้ cache 60 วินาทีตัวเดียวกัน** — ไม่เพิ่มรอบ DB ต่อข้อความ
   🛑 cache ต้องถูก invalidate เมื่อคลังเปลี่ยน (TFR-034) ไม่ใช่แค่ตอนกลุ่มคำ/กฎเปลี่ยน — ไม่งั้นร้านกรอกคำตอบเสร็จแล้วทดสอบทันทีจะเจอ "ตั้งแล้วไม่ทำงาน" นาน 60 วินาที ซึ่งแยกไม่ออกจากบั๊กจริง (บทเรียนซ้ำรอย cooldown/ตารางเวลา 2026-07-31)
5. **นับการใช้งาน** — ตอบสำเร็จด้วย QnA → `useCount++`, `lastUsedAt = now` (อยู่ในเส้นทางเดียวกับที่ `autoReplyCount` ถูก increment) · ล้มเหลว → ไม่นับ
- **Postcondition:** ข้อความที่เข้าทั้งกลุ่มคำและคลัง ต้องตอบด้วยกลุ่มคำเสมอ (`matchedVia = "KEYWORD"`)

#### TFR-033: คิวคำถามที่ตอบไม่ได้ — เขียนตอนเกิดเหตุ (เพิ่ม 2026-07-31)

- **Trace to:** Scope Baseline `00023-qna` S-06, D4, D5 · [[DATABASE]] §3.10
- **ที่อยู่:** `src/services/auto-reply-unanswered.service.ts` — เรียกจาก `processJob` ตรงจุดที่ตัดสิน `NO_KEYWORD_MATCH`

1. เกิด `skipReason = 'NO_KEYWORD_MATCH'` → ผ่านตัวกรอง [[DATABASE]] §3.10.1 (F-1..F-4) ก่อน → ถ้าผ่าน จึง upsert `AutoReplyUnansweredQuestion` โดย `(shopId, normalizedQuestion)` แล้ว `hitCount++`, `lastSeenAt = now`, `rawSample` = ข้อความล่าสุด
2. 🛑 **ห้าม throw** — ห่อ `try/catch` แบบเดียวกับ `writeLog` (TD-013) · คิวพังต้องไม่ทำให้บอทพังหรือทำให้งานค้างสถานะ
3. 🛑 **ห้ามแตะ `handoffAt`** — ข้อนี้ย้ำเพราะเป็นจุดเดียวกับบั๊ก prod 2026-07-31 ที่ล็อกห้องถาวร 240 ห้อง (`NO_KEYWORD_MATCH` ต้องไม่ล็อกห้อง)
4. แถวที่ `status = 'ANSWERED'` หรือ `'DISMISSED'` แล้วถูกถามซ้ำ → **ยัง `hitCount++` และอัปเดต `lastSeenAt` ตามปกติ แต่ไม่เปลี่ยน `status` กลับ**
   เหตุผล: ถ้าตอบไปแล้วแต่ยังเข้ามาที่ `NO_KEYWORD_MATCH` อีก แปลว่าคำตอบที่กรอกไว้ยัง match ไม่ติด (พิมพ์ต่างกันนิดเดียว) — ตัวเลขนี้คือหลักฐานที่บอกว่าถึงเวลาต้องเปิดโหมดความคล้ายแล้ว ถ้ารีเซ็ต status กลับจะกลายเป็นคิวที่วนไม่จบและร้านจะเลิกเปิด
- **Error / Edge cases:** ข้อความเดียวกันมาพร้อมกัน 2 งาน → unique constraint บังคับให้เหลือแถวเดียว (upsert แข่งกันแล้วแพ้ = retry ครั้งเดียว ไม่ retry ซ้ำ)

#### TFR-034: CRUD / bulk / cache invalidation ของคลัง (เพิ่ม 2026-07-31)

- **Trace to:** Scope Baseline `00023-qna` S-05, S-09, S-10
- **ที่อยู่:** `src/services/auto-reply-qna.service.ts`

1. `shopId` derive จาก session เท่านั้น (TFR-005) · `keywordId` ที่รับเข้ามาต้องยืนยันว่าเป็นของร้านนั้นก่อนทุกครั้ง — **`where` ต้องมี `shopId` ไม่ใช่กรองใน JS** (memory `feedback_rsc_dal_authz`)
2. `normalizedQuestion` คำนวณที่ service ด้วย `normalizeMessage()` ทุกครั้งที่เขียน — **ห้ามรับจาก client** (หลักเดียวกับ `specificity` ใน TFR-004)
3. คำถามซ้ำในกลุ่มเดียวกัน → `409` (unique constraint เป็นคนบอก ไม่ใช่ `findFirst` ก่อนแล้วค่อยเขียน)
4. bulk 4 คำสั่ง: เปิด / ปิด / ย้ายกลุ่ม / ลบ — ทำงานกับ id ที่ **กรอง `shopId` แล้ว** เท่านั้น · ย้ายกลุ่มแล้วชนคำถามซ้ำในกลุ่มปลายทาง → รายงานเป็นรายข้อ **ไม่ล้มทั้งชุด**
5. 🛑 **ทุกเส้นทางที่เขียนคลังต้อง invalidate rule-set cache ของร้านนั้น** (TFR-032 ข้อ 4) — รวม bulk และการนำเข้าไฟล์

#### TFR-035: นำเข้า / ส่งออกคลังเป็น CSV (เพิ่ม 2026-07-31)

- **Trace to:** Scope Baseline `00023-qna` S-16, A3
- **รูปแบบ:** CSV UTF-8 **พร้อม BOM** (ไม่งั้น Excel ไทยเปิดแล้วเป็นภาษาต่างดาว) · คอลัมน์ `คำถาม, คำตอบ, เปิดใช้งาน`
  กลุ่มปลายทาง**ไม่อยู่ในไฟล์** — เลือกจาก UI เพราะหน้าจัดการเป็นรายกลุ่มอยู่แล้ว (A2)
1. 🛑 **ต้องแสดงตัวอย่างให้ยืนยันก่อนเขียนเสมอ** — บอกจำนวนที่จะเพิ่ม / ที่ซ้ำแล้วข้าม / ที่รูปแบบผิด · ข้อมูลผิดเป็นร้อยข้อแก้ทีหลังเจ็บกว่าการกดยืนยันเพิ่มหนึ่งครั้ง
2. แถวที่คำถามซ้ำกับที่มีอยู่ → **ข้าม ไม่ทับ** (ค่าเริ่มต้น) และรายงานจำนวน
3. `source = 'IMPORT'` ทุกแถวที่มาจากไฟล์
4. เพดานต่อไฟล์: **500 แถวข้อมูล** (ไม่นับหัวตาราง) · เกินให้ปฏิเสธพร้อมบอกจำนวนที่พบ (ไม่ตัดครึ่งแล้วนำเข้าเงียบ ๆ)
   - **แก้จาก 1,000 → 500 เมื่อ 2026-08-01 (user ตัดสิน)** เพื่อให้ตรงกับ `parseCsv()` ใน `src/lib/csv.ts`
     ซึ่ง cap ที่ 501 แถว (500 data + 1 header) อยู่แล้ว และเป็น asset กลางที่ต้อง reuse
     ตัวเลข 1,000 เดิมไม่เคยมีโค้ดรองรับ — ถ้าคงไว้ ไฟล์ 600 แถวจะถูก `parseCsv` ตัดเหลือ 500 เงียบ ๆ
     แล้วผ่านการตรวจ "ไม่เกิน 1,000" ไปได้ ซึ่งคือพฤติกรรม "ตัดเงียบ" ที่ข้อนี้ห้ามไว้เอง
   - mockup Modal 3 ที่เขียนว่า "นำเข้าเฉพาะ 500 แถวแรก" **ถือว่าล้าสมัย** — ต้องปฏิเสธ ไม่ใช่ตัด

#### TFR-036: ส่งรูปแนบพร้อมคำตอบอัตโนมัติ (เพิ่ม 2026-07-31)

- **Trace to:** Scope Baseline `00023-qna` S-21, A5, A6 · [[DATABASE]] §3.11 · user สั่ง "Auto Reply ต้องใส่รูปได้ และให้ AI ตอบพร้อมรูปด้วย"
- **ที่อยู่:** `src/services/auto-reply-send.service.ts` (`sendAutoReply`) — **จุดเดียวที่ระบบส่งข้อความออก** (grep gate เดิมยังต้องเป็นจริง)

1. `sendAutoReply()` รับ `imageFileIds: string[]` เพิ่ม (ค่าเริ่มต้น `[]` = พฤติกรรมเดิมทุกประการ)
2. **ลำดับส่ง: รูปทีละใบตามลำดับในอาร์เรย์ → แล้วข้อความปิดท้าย** (A6)
   เรียก `sendOutboundMessage()` 1 ครั้งต่อ 1 รูป (`imageFileId` เดี่ยว) แล้วอีก 1 ครั้งสำหรับข้อความ — **ห้าม** ส่ง `text` ไปพร้อม `imageFileId` ในการเรียกเดียว เพราะเส้นทางนั้นเป็นเส้นทางของ "คนกดส่ง" ที่กลืน error ของ caption (`channel-chat.service.ts:1063`) ซึ่งยอมรับไม่ได้สำหรับบอท (ข้อ 4)
3. **ความสำเร็จของชุด = ข้อความตัวอักษรส่งสำเร็จ** — ไม่ใช่ "ทุกชิ้นสำเร็จ"
   🛑 เหตุผล: ถ้านับว่าล้มเหลวเมื่อรูปใบใดใบหนึ่งพลาด ระบบ retry จะส่ง**รูปที่สำเร็จไปแล้วซ้ำ** ให้ลูกค้า ซึ่งถอนคืนไม่ได้และแย่กว่าการขาดรูปไป 1 ใบ · การกันตอบซ้ำของฟีเจอร์นี้อยู่ที่ระดับ "งาน" (`AutoReplyJob.chatMessageId @unique`) ไม่ใช่ระดับ "ชิ้นที่ส่ง" ⇒ retry ทั้งชุดจึงไม่ปลอดภัยเมื่อชุดมีหลายชิ้น
4. 🛑 **ทุกชิ้นที่ส่งไม่ผ่านต้องถูกบันทึก** ลง `AutoReplyLog.errorMessage` (รูปที่ N ล้ม / ข้อความล้ม) — ห้ามกลืน
   เหตุผล: ของเดิมกลืน error ของ caption ได้เพราะ**คนกดส่งเห็นกับตา**ว่าไม่ขึ้น · บอทตอบตอน "แทน admin ตอนหลับ" ⇒ ไม่มีใครเห็น อาการที่ได้คือ "ลูกค้าได้รูปเปล่า ๆ ไม่มีคำอธิบาย" ที่ไม่มีใครรู้จนลูกค้าบ่น
5. **`Conversation.autoReplyCount` เพิ่มขึ้น 1 ต่อ "ชุด" ไม่ใช่ต่อชิ้น** ([[DATABASE]] §3.11)
6. คำตอบที่มีแต่รูปไม่มีข้อความ = อนุญาต (`QuickMessage.body` ก็ว่างได้ถ้ามีรูป) — แต่ gate 8-9 เดิมที่เช็ค `replyText.trim()` ว่าง **ต้องแก้ให้ถือว่า "มีรูปก็นับว่ามีคำตอบ"** ไม่งั้นคำตอบที่เป็นรูปล้วนจะถูกตัดทิ้งเป็น `EMPTY_REPLY` แล้วล็อกห้องด้วย `handoffAt`
- **AI (นอกขอบเขต phase นี้):** คอลัมน์ `imageFileIds` คือสิ่งที่ทำให้ "AI ตอบพร้อมรูป" ทำได้ทีหลังโดยไม่ต้อง migrate ซ้ำ — AI เลือกจากรูปที่ร้านผูกไว้กับกลุ่ม/ข้อนั้นเท่านั้น **ห้ามให้ AI สร้างหรือเลือกรูปนอกรายการ**

#### TFR-037: สร้างชุดคำถาม-คำตอบจากห้องแชท (เพิ่ม 2026-07-31)

- **Trace to:** Scope Baseline `00023-qna` S-22 · user สั่ง "ต้องมีบันทึกจากหน้า chat ได้ด้วย … mini action ด้านล่าง … อย่างง่าย"
- **ที่อยู่:** `ChatThread.tsx` (UI) + `auto-reply-qna.service.ts` (เส้นทางเขียนเดิม ไม่สร้าง service ใหม่)

1. mini action อยู่ใต้ **บับเบิลของลูกค้า** — ข้อความนั้นคือ "คำถาม"
2. เปิด sheet ที่ **เติมมาให้แล้วทั้งคู่**: คำถาม = ข้อความลูกค้า · คำตอบ = **ข้อความฝั่งร้านถัดไปในเธรด**
   🛑 ข้อนี้คือสิ่งที่ทำให้คำว่า "อย่างง่าย" เป็นจริง — ถ้าเปิดมาแล้วช่องคำตอบว่าง ร้านต้องพิมพ์ซ้ำสิ่งที่เพิ่งพิมพ์ไป = งานเพิ่ม ไม่ใช่ทางลัด แล้วจะไม่มีใครใช้
   ข้อความถัดไปของร้านที่มี `autoReplyKind != null` (บอทตอบเอง) → **ไม่เติม** เพราะคำถามนี้ตอบได้อยู่แล้ว ไม่ใช่ของที่ต้องสอน
3. แก้ได้ทั้งคำถามและคำตอบก่อนบันทึก · เลือกกลุ่มปลายทาง + "สร้างกลุ่มใหม่"
4. `source = 'QUEUE'` · ต้อง invalidate rule-set cache (TFR-034 ข้อ 5)
5. ถ้าข้อความนั้นมีแถวใน `AutoReplyUnansweredQuestion` อยู่ → อัปเดตเป็น `status = 'ANSWERED'` + `qnaId` ในทรานแซกชันเดียวกัน ไม่งั้นคิวจะค้างข้อที่ตอบไปแล้ว
- **Error / Edge cases:** คำถามซ้ำในกลุ่มปลายทาง → `409` พร้อมลิงก์ไปข้อเดิม (ไม่ใช่ error เปล่า ๆ — ร้านต้องรู้ว่ามีอยู่แล้วที่ไหน)
...
| GET/POST | `/api/shops/auto-reply/keywords/{id}/qna` | รายการ/เพิ่มข้อในคลังของกลุ่มนั้น (ค้นหา + กรอง 4 แบบ) | อ่าน=`canAccessShop`, เขียน=OWNER/ADMIN | TFR-034 |
| PATCH/DELETE | `/api/shops/auto-reply/keywords/{id}/qna/{qnaId}` | แก้/ลบข้อในคลัง | OWNER/ADMIN | TFR-034 |
| POST | `/api/shops/auto-reply/keywords/{id}/qna/bulk` | เปิด/ปิด/ย้ายกลุ่ม/ลบหลายข้อ — รายงานผลรายข้อ ไม่ล้มทั้งชุด | OWNER/ADMIN | TFR-034 |
| POST | `/api/shops/auto-reply/keywords/{id}/qna/import` | นำเข้า CSV (พรีวิวก่อนยืนยันเสมอ) | OWNER/ADMIN | TFR-035 |
| GET | `/api/shops/auto-reply/keywords/{id}/qna/export` | ส่งออก CSV UTF-8 BOM | `canAccessShop` | TFR-035 |
| GET | `/api/shops/auto-reply/unanswered` | คิวคำถามที่ตอบไม่ได้ของร้าน (เรียงตามที่ถูกถามบ่อย) | `canAccessShop` | TFR-033 |
| POST | `/api/shops/auto-reply/unanswered/{id}/dismiss` | กดข้าม (ไม่ลบ — เก็บไว้เป็นหลักฐานว่าเคยตัดสินแล้ว) | OWNER/ADMIN | TFR-033 |
| POST | `/api/shops/auto-reply/unanswered/{id}/convert` | กรอกคำตอบ → สร้าง `AutoReplyQna` + ปิดคิวแถวนั้นในทรานแซกชันเดียว | OWNER/ADMIN | TFR-033, TFR-034 |
| POST | `/api/chat/conversations/{id}/qna-from-message` | mini action ในห้องแชท — สร้างชุดคำถาม-คำตอบจากบับเบิลที่เลือก | OWNER/ADMIN + เธรดของร้านนั้น | TFR-037 |
...
| `AutoReplyQna` | คลังคำถาม-คำตอบ **ผูกกับกลุ่มคำ** — วิธีจับคู่ทางที่สอง | ใหม่ 2026-07-31 | PostgreSQL |
| `AutoReplyUnansweredQuestion` | คิวคำถามที่ตอบไม่ได้ (เขียนตอนเกิดเหตุ กรอง PII ที่ขาเขียน) | ใหม่ 2026-07-31 | PostgreSQL |
...
| `AutoReplyLog` | +`matchedVia`, `qnaId` | เพิ่มคอลัมน์ 2026-07-31 | PostgreSQL |
| `AutoReplyRule` | +`imageFileIds` (รูปแนบ สูงสุด 5) | เพิ่มคอลัมน์ 2026-07-31 | PostgreSQL |
| `AutoReplyKeyword` | +`qnaSimilarityEnabled` (**ยังไม่มีที่ไหนอ่าน** — [[DATABASE]] §3.7) | เพิ่มคอลัมน์ 2026-07-31 | PostgreSQL |
