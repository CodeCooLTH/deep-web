---
title: "PRD — Deep Chat-Bot Assistant (ตอบแชทอัตโนมัติจาก Keyword + ChatBot)"
owner: shinobu22
...
> **เวอร์ชัน:** 1.1 (แก้ 2026-08-02 — sync กับโค้ดหลังฟีเจอร์ขึ้น prod และหลังตัด AI Enhance ออก)
> **วันที่จัดทำ:** 2026-07-29
> **สถานะ:** ขึ้น prod แล้ว · §3.9 superseded · §3.10 เขียนย้อนหลังจากโค้ด — ดู "สถานะ ณ 2026-08-02"
...
## 📌 สถานะ ณ 2026-08-02 — อ่านก่อนเชื่อเนื้อหาข้างล่าง

เอกสารฉบับนี้เขียน 2026-07-29 ตอนยังไม่ implement · ตั้งแต่นั้นฟีเจอร์ **ขึ้น prod แล้ว** และ
**ขอบเขตเปลี่ยนไปจากที่เขียนไว้** — หัวข้อที่ยังไม่ได้อัปเดตจะถูกกำกับไว้ในที่ของมันเอง

**ชื่อเรียก 3 ชั้น (ตกลงกับ user แล้ว ห้ามสลับ):** ฟีเจอร์/โฟลเดอร์ = **Deep Chat-Bot Assistant** ·
เมนู+หน้าจอ = **ผู้ช่วยอัตโนมัติ** · บอทในห้องแชท = **DeepBot** (ตอบจากกลุ่มคำ) / **DeepAI**
(ตอบจากคลังความรู้) · route `/settings/auto-reply` และ identifier ใน DB **คงเดิมทั้งหมด**

| ของที่เขียนไว้ในเอกสารนี้ | สถานะจริง |
|---|---|
| §3.1–3.8 ตอบจากกลุ่มคำ (keyword → คำตอบตามเพจ/โฆษณา/สินค้า) | **ขึ้น prod แล้ว** ตรงตามเอกสาร |
| §3.5 โหมดทดสอบ | **ขึ้น prod แล้ว** แต่ขยายเป็น **3 สถานะต่อกลุ่มคำ** `OFFLINE` / `TEST` / `LIVE` (เอกสารเขียนไว้เป็นสวิตช์เปิด-ปิด + test mode แยก) |
| §3.9 AI Enhance (เรียบเรียงคำตอบสำเร็จรูปด้วย AI) | 🛑 **ถูกถอดออกทั้งชุด** 2026-08-02 (`68c37cd3`) — implement ไปแล้วรอบหนึ่งแล้วตัดทิ้ง ดูหัวข้อ §3.9 |
| คลังคำถาม-คำตอบ **รายกลุ่มคำ** | 🛑 **ถูกถอดออก** (`68c37cd3`) แทนด้วย **คลังความรู้ระดับร้าน** (ดู §3.10) |
| คิวคำถามที่บอทตอบไม่ได้ (unanswered queue) | 🛑 **ถูกถอดออก** (`68c37cd3`) — ตาราง `AutoReplyUnansweredQuestion` ยังอยู่ใน DB แต่ไม่มีโค้ดเส้นทางไหนเขียน/อ่านแล้ว |
| — | ➕ **ของใหม่ที่ยังไม่มีในเอกสารนี้:** ChatBot (DeepAI) ตอบจากคลังความรู้ระดับร้าน + ตารางเวลาทำงาน + พนักงานตอบเอง=บอทหลบ · ดู §3.10 |

**เหตุผลที่ตัด AI Enhance ทิ้งแล้วทำ ChatBot แทน:** ทั้งสองอย่างใช้ AI เหมือนกัน แต่แก้คนละปัญหา —
AI Enhance ทำให้คำตอบ*ที่มีอยู่แล้ว*อ่านลื่นขึ้น (คุณค่าต่ำ ร้านเขียนเองก็ได้) ส่วน ChatBot ตอบ
*คำถามที่ไม่มีใครตั้งคำตอบไว้* ซึ่งคือ 317 แถวจริงใน DB ที่บอทเคยเงียบใส่ · งบ AI ก้อนเดียวกัน
ใช้กับอย่างหลังคุ้มกว่ามาก
...
### 3.9 AI Enhance — 🛑 ถูกถอดออกแล้ว (implement แล้วตัดทิ้ง 2026-08-02)

> 🛑 **หัวข้อนี้ไม่ตรงกับระบบจริงอีกต่อไป — เก็บไว้เพื่อประวัติการตัดสินใจเท่านั้น**
>
> AI Enhance ถูก implement จริงเมื่อ 2026-08-01 (`1859fb1c`, `af339579`, `782b1ec3`) แล้ว
> **ถอดออกทั้งชุดในวันถัดมา** (`68c37cd3`) พร้อมกับคลังคำถามรายกลุ่มคำและคิวคำถามที่ตอบไม่ได้
> ไฟล์ที่หายไป: `auto-reply/[id]/ai/AiEnhanceClient.tsx`, `auto-reply/[id]/ai/page.tsx`
>
> **สิ่งที่ยังใช้ได้จากหัวข้อนี้:** BR-AR-32 ถึง BR-AR-36 (ลำดับตรวจ Guardrails, fail-closed
> แยกปลายทาง, ร้านตั้ง Guardrails เอง, เพดานค่าใช้จ่ายต่อวัน, หักเครดิตตามต้นทุน token จริง)
> **ยังบังคับใช้อยู่ — แต่ย้ายไปเป็นกฎของ ChatBot แทน** (`auto-reply.service.ts:929` เพดาน/เครดิต,
> `:946` guardrails) กติกาเรื่องเงินและความปลอดภัยจึงไม่ได้หายไปกับ AI Enhance
>
> **สิ่งที่ใช้ไม่ได้แล้ว:** BR-AR-31 (งบเวลา 8 วินาที) — ChatBot ใช้งบเวลาของตัวเอง **20 วินาที**
> (`96af6b71` "งบเวลาของโหมดให้ AI ตอบเองแยกเป็น 20 วินาที") เพราะเป็นการ *สร้าง* คำตอบ
> ไม่ใช่ *เรียบเรียง* คำตอบที่มีอยู่แล้ว · และทุกข้อที่พูดถึง "เปิด AI Enhance รายกลุ่มคำ"
> เพราะ ChatBot ตั้งค่าที่ **ระดับร้าน** ไม่ใช่รายกลุ่มคำ
>
> ⚠️ **กับดักชื่อไฟล์:** `src/services/ai-enhance.service.ts` **ยังอยู่และยังทำงาน** แต่เปลี่ยน
> หน้าที่ไปแล้ว — ตอนนี้ export `answerFromKnowledge()` / `isChatbotWithinWindow()` ให้ ChatBot ใช้
> (`auto-reply.service.ts:17`) ชื่อไฟล์ไม่ตรงกับงานที่มันทำ **อย่าเห็นชื่อแล้วสรุปว่าเป็น dead code**

**ความต้องการ (ตามที่เคยตกลงไว้ — ไม่ได้ทำแล้ว):**
...
### 3.10 ChatBot (DeepAI) + คลังความรู้ระดับร้าน — ของที่ขึ้น prod จริง 2026-08-01/02

> เขียนย้อนหลังจากโค้ด 2026-08-02 (ไม่ใช่ doc-first — เป็นหนี้ที่รับไว้ตรง ๆ)
> แหล่งอ้างอิง: `AutoReplyConfig` (schema.prisma), `auto-reply.service.ts`, `ai-enhance.service.ts`

**ความต้องการ:**
เมื่อลูกค้าถามคำถามที่**ไม่มีกลุ่มคำไหนตรงเลย** — จุดที่เมื่อก่อนบอทเงียบใส่ — ให้ AI ลองตอบจาก
**คลังความรู้ที่ร้านเขียนไว้** ก่อนจะยอมเงียบ · ในฐาน dev มีคำถามแบบนี้ **317 แถวจริง**
(`AutoReplyLog.skipReason='NO_KEYWORD_MATCH'`) ซึ่งส่วนใหญ่ตอบได้ถ้ามีคำตอบเก็บไว้

**Business Rules:**

- **BR-AR-40 (ตำแหน่งในเส้นทางตัดสิน)** — ChatBot ทำงาน **เฉพาะในสาขา `NO_KEYWORD_MATCH` เท่านั้น**
  (`auto-reply.service.ts` ใน `else` ของ gate นั้น) · คำที่ร้านตั้งไว้เองจึงชนะเสมอ **โดยโครงสร้าง
  ไม่ใช่โดยลำดับ `if`** ที่ใครมาแก้ทีหลังแล้วสลับได้ — ข้อนี้เป็นข้อบังคับเชิงสถาปัตยกรรม ห้ามย้าย
- **BR-AR-41 (3 สถานะเหมือนกลุ่มคำ)** — `aiChatbotStatus` = `OFFLINE` / `TEST` / `LIVE`
  ใช้คำเดียวกับ `AutoReplyKeyword.status` โดยเจตนา (ร้านเรียนความหมายไปแล้วรอบหนึ่ง —
  ตั้งชื่อใหม่ให้ของที่ทำงานเหมือนกันคือบังคับให้เรียนซ้ำ) · `TEST` ตอบเฉพาะแชทใน `AiChatbotTestThread`
  - คอลัมน์เดิม `aiChatbotEnabled` **ยังอยู่แต่ไม่ใช่ตัวตัดสิน** — ตัวตัดสินจริงคือ `aiChatbotStatus`
  - migration ยกร้านที่เคยเปิดสวิตช์เดิมไว้ขึ้นเป็น `LIVE` ไม่ปล่อยเป็น `OFFLINE` (ปล่อย = ปิดของที่
    ร้านเปิดไว้เงียบ ๆ ซึ่งแย่กว่า)
- **BR-AR-42 (ขอบเขตคำตอบ)** — `aiChatbotShopOnly` = ตอบเฉพาะเรื่องของร้าน คำถามนอกเรื่องตอบด้วย
  `aiChatbotOutOfScopeText` แล้วจบ · แหล่งข้อมูลเปิด/ปิดได้ 3 ทาง: `aiChatbotUseShopData` (default
  เปิด) `aiChatbotUseChatHistory` (default เปิด) `aiChatbotUseWebSearch` (default **ปิด**)
- **BR-AR-43 (กันตอบรัวและกันบิลบาน)** — `aiChatbotCooldownSec` default 30 วิ · `aiChatbotMaxPerHour`
  default 10 ครั้ง/ชม. · เพดานเงินต่อวันและการหักเครดิตใช้กลไกเดียวกับที่ §3.9 วางไว้
  (`DAILY_CAP_OR_NO_CREDIT`)
- **BR-AR-44 (งบเวลา 20 วินาที)** — ต่างจาก AI Enhance ที่ให้ 8 วินาที เพราะ ChatBot **สร้าง**
  คำตอบใหม่ ไม่ได้เรียบเรียงของเดิม
- **BR-AR-45 (ช่วงเวลาทำงาน)** — `aiChatbotStartTime` / `aiChatbotEndTime` · null = ตลอดเวลา
  🛑 ช่วงข้ามคืน (18:00–09:00) **ยึดวันที่เริ่มเป็นเจ้าของช่วง** — ตั้งจันทร์ = คืนจันทร์ยาวถึงเช้าอังคาร
  เก็บเป็น **นาทีจากเที่ยงคืนเวลาไทย (Int)** ไม่ใช่ TIME/DateTime เพราะเซิร์ฟเวอร์รัน UTC แล้วจะ
  เพี้ยน 7 ชม. เงียบ ๆ · fail-open ทุกกรณีที่ตั้งค่าไม่ครบ
  - **กลุ่มสถานะ `TEST` ไม่ติดด่านเวลาและด่าน cooldown** — คนที่กำลังทดสอบตั้งใจยิงคำเดิมรัว ๆ
    และอาจทดสอบตอนบ่ายทั้งที่ตั้งเวลาไว้กลางคืน พอโดนปิดปากเงียบ ๆ อาการที่เห็นคือ
    "ตั้งแล้วไม่ทำงาน" ซึ่งแยกไม่ออกจากบั๊กจริง
- **BR-AR-46 (คลังความรู้อยู่ระดับร้าน ไม่ใช่รายกลุ่มคำ)** — ตาราง `AutoReplyQna` (ชื่อเดิม แต่
  `keywordId` ไม่ถูกใช้แล้ว) · จัดการที่ `/settings/chatbot/knowledge` ไม่ใช่ในหน้ากลุ่มคำ
- **BR-AR-47 (PII ห้ามเข้า AI)** — กรองเบอร์โทร/อีเมล/ที่อยู่ออกก่อนส่งเข้า Gemini ทุกครั้ง (`bf2a122b`)

**เหตุผล:**
ร้านส่วนใหญ่ตั้งกลุ่มคำได้ไม่กี่สิบคำ แต่ลูกค้าถามได้ไม่จำกัดรูปแบบ — ส่วนต่างนั้นคือที่ที่บอทเงียบ
และคือที่ที่ลูกค้าหายไป · การให้ AI ตอบตรงนั้นจึงเพิ่มความครอบคลุมโดยไม่แตะเส้นทางที่ร้านคุมเองอยู่แล้ว
