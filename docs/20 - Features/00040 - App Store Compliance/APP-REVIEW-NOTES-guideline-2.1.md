...
- iPhone 13 Pro Max — iOS <<เลข iOS>> (physical device, installed via TestFlight)
- iPad <<รุ่น iPad>> — iPadOS <<เลข iPadOS>> (physical device, installed via TestFlight)
...
| ข้อ | ต้องยืนยันว่า |
|---|---|
| 2 | **3 ค่าที่ยังว่าง** — `<<เลข iOS>>` (ตั้งค่า → ทั่วไป → เกี่ยวกับ → เวอร์ชันซอฟต์แวร์) · `<<รุ่น iPad>>` · `<<เลข iPadOS>>` — เครื่องหลัก **iPhone 13 Pro Max** ยืนยันแล้ว |
