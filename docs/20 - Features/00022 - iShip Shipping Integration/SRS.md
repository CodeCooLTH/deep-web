...
|---|---|---|---|
| ตำบล/แขวง | `dst_district` | **`dst_area`** | `subdistrict` |
| อำเภอ/เขต | `dst_amphure` | **`dst_district`** | `district` |

**`dst_district` มีความหมายกลับด้านกันระหว่างสองทิศทาง** — ขาออกคือตำบล ขาเข้าคืออำเภอ

**ยืนยันแล้ว 2026-08-07** ด้วย `GET /api/get_order` ของพัสดุจริงบน prod 2 ใบ:

| พัสดุ | `dst_district` | `dst_area` | ไฟล์ที่อยู่ของ iShip |
|---|---|---|---|
| TH066536981258 | กาญจนดิษฐ์ | ช้างซ้าย | สุราษฎร์ธานี · อ.กาญจนดิษฐ์ · ต.ช้างซ้าย · 84160 |
| TH681090UM471E | พนม | พังกาญจน์ | สุราษฎร์ธานี · อ.พนม · ต.พังกาญจน์ · 84250 |

กติกาที่ `parseReceiverLocality()` ใช้ตัดสิน (`src/lib/iship/unlinked.ts`): **ดูว่ามี `dst_area` ไหม**
— มี = รูปขาเข้า (`dst_area`=ตำบล, `dst_district`=อำเภอ) · ไม่มี = รูปขาออก (`dst_district`=ตำบล)
`dst_amphure` ชนะเสมอเมื่อมี

**เดิมจับคู่กลับหัว** (เดาว่าขาเข้าสมมาตรกับขาออก) โดยรับความเสี่ยงไว้ด้วยเหตุผลว่า FR-ISHIP-026
บังคับให้ร้านดูตารางเทียบก่อนกดผูก — **ด่านนั้นไม่ทำงานจริง**: ออเดอร์บน prod 23 ใบ (2026-08-01
ถึง 08-07) ถูกบันทึกตำบล/อำเภอสลับกันโดยไม่มีใครทัก แล้วไปโผล่เป็น `ADDRESS_INVALID` ตอนเปิด
พัสดุใบถัดไปของลูกค้าคนเดิม (คู่ที่สลับแล้วไม่มีอยู่ในไฟล์ที่อยู่ของ iShip) — คนละหน้าจอกับต้นเหตุ
backfill แล้วทั้ง 26 แถว (23 สลับ + 3 "อ.เมือง" ที่ไม่มีในไฟล์) เมื่อ 2026-08-07
...
## 19. แก้ 2026-08-06 (รอบสอง) — ลองใหม่ / นิยาม "มีพัสดุ" / เครดิตไม่พอ
...
## 20. แก้ 2026-08-06 (รอบสาม) — "สถานะปัจจุบัน" ของพัสดุมีเจ้าของเดียว
...
## 21. แก้ 2026-08-06 (รอบสี่) — `payment_success` หายจากตารางแมปทุกตัวพร้อมกัน
...
## 22. แก้ 2026-08-20 — พัสดุที่เดินทางนานกว่าหน้าต่าง sync ค้างสถานะตลอดกาล

### 22.1 ที่มา (user report, prod)

พัสดุ `TH068661575518` (SPX Express) — ลูกค้าไม่รับของ ขนส่งตีกลับ และ**ส่งคืนถึงร้าน
เรียบร้อยแล้วตั้งแต่ 12 ส.ค. 15:32** แต่หน้าจอของเราขึ้น:

| สิ่งที่แสดง | ค่า | ความจริง |
|---|---|---|
| ป้ายสถานะ | `พัสดุตีกลับ` (`return`) | `ส่งคืนสำเร็จ` (`return_success`) |
| แถบ 4 ขั้น | จุดที่ 3 "กำลังจัดส่ง" | จุดที่ 4 "ส่งคืนสำเร็จ" (แถบเทา) |
| อัปเดตล่าสุด | 10 ส.ค. 16:21 | 12 ส.ค. 15:32 |
| กองใน `/orders` | `SHIPPING` | `PROBLEM` (ร้านต้องตัดสินใจคืนเงิน/ส่งใหม่) |

**รายการการเดินทางในจอเดียวกัน ห่างจากป้ายลงมา 3 เซนติเมตร ขึ้นว่า "ส่งคืนสำเร็จ" อยู่แล้ว**
— หน้าจอค้านตัวเองมา 8 วันโดยไม่มีอะไรฟ้อง

### 22.2 สาเหตุ 2 ชั้นที่ไม่เกี่ยวกัน (แต่ปิดตากันเอง)

**ชั้นที่ 1 — `syncShipmentStatuses` มองไม่เห็นพัสดุใบนี้เลย**

`query_orders` รับช่วงวันที่ได้ไม่เกิน 7 วัน (code 1009) ระบบจึงขอ 6 วันย้อนหลังจากวันนี้เสมอ
ใบที่ไม่อยู่ในคำตอบเจอ `if (!row) continue;` แล้วถูกปล่อยผ่านทุกรอบ ⇒ **ค้างตลอดกาล**

🛑 การตีกลับคือเส้นทางที่กินเวลาเกิน 6 วันแทบทุกใบตามธรรมชาติของมัน (ส่งไป → ลูกค้าไม่รับ →
ขนส่งถือไว้ → ตีกลับ → ถึงต้นทาง) ⇒ **ใบที่ระบบต้องรีบบอกร้านที่สุด คือใบที่ระบบเงียบใส่นานที่สุด**

🛑 **ยิง cron ถี่ขึ้นแก้ข้อนี้ไม่ได้** — เป็นปัญหา *ขอบเขตข้อมูลที่ขอ* ไม่ใช่ *ความถี่ที่ถาม*
ถามทุก 5 นาทีก็ได้คำตอบชุดเดิมที่ไม่มีใบนี้อยู่ทุกรอบ (ข้อสังเกตนี้เกิดจาก user ตั้งคำถามเอง
ว่า "ถ้ากรองด้วย updated ก็น่าจะเจอเสมอป่ะ" — ซึ่งจะจริงถ้า iShip กรองด้วย `updated_at`
**แต่เราพิสูจน์ไม่ได้ว่ามันกรองด้วยฟิลด์ไหน**: เอกสาร iShip เป็น Postman documenter ที่
render ด้วย JS และ API ของ Postman ตอบ 503/404 ทุกทาง ⇒ ออกแบบให้ถูกต้องภายใต้ทั้งสองความหมาย)

**ชั้นที่ 2 — เปิดโมดัลแล้วฐานถูกอัปเดต แต่จอไม่รู้**

`getTraces()` ยิง `get_order` และ**เขียนสถานะใหม่ลงฐานจริง** (FR-ISHIP-075) แต่ endpoint
คืนเฉพาะ `ShipmentEvent[]` ส่วนหัวการ์ด/แถบขั้นอ่านจาก prop ที่ server render ไว้ตอนเปิดหน้า
⇒ ค่าถูกแล้วในฐาน แต่ร้านต้องรีโหลดเองถึงจะเห็น (ยืนยันโดย user: กด F5 แล้วค่าเปลี่ยนทันที)

### 22.3 FR-ISHIP-085 — ตามใบที่หลุดหน้าต่างด้วย `get_order` รายใบ

`syncShipmentStatuses` หลังลูปยกชุด:

1. `pickStaleParcelsForLookup(tracking, seen, now)` (`src/lib/iship/stale-lookup.ts`) เลือกใบที่
   **ไม่อยู่ในคำตอบของ `query_orders` รอบนี้** และขยับล่าสุดไม่เกิน `STALE_LOOKUP_MAX_AGE_DAYS` (45 วัน)
2. ยิง `get_order(trackingNo)` รายใบ → `parseParcelRow` → `carrierStatusCodeFromId(statusId)`
   → `applyCarrierStatus()` (จุดเขียนเดียวกับลูปยกชุด)
3. เก็บ `settlement_at` (BR-ISHIP-45) และต้นทุนค่าส่งจากคำตอบเดียวกัน — ใบที่เดินทางนานคือ
   ใบที่เรื่องเงินยังไม่จบบ่อยที่สุด ถ้าเก็บเฉพาะในลูปยกชุด ใบพวกนี้จะไม่มีวันถูกเก็บเลย
4. ใบเดียวล้ม = ข้ามใบนั้น ไม่ล้มทั้งรอบ

**เพดานต้นทุน (บังคับในโค้ด ไม่ใช่ข้อตกลง):** `STALE_LOOKUP_MAX_PER_ROUND = 8` คำขอ/รอบ/ร้าน
พร้อม **คิวหมุนเวียนตามช่วงเวลา** (`floor(now / 15นาที) % ceil(n/8)`) ⇒ ทุกใบได้คิวครบภายใน
`ceil(n/8)` รอบ โดยไม่ต้องเพิ่มคอลัมน์ "ถูกถามล่าสุดเมื่อไร" ในฐาน

🛑 **ห้ามถอดเพดานแล้ววนทุกใบ** — นั่นคือเหตุผลทั้งหมดที่ระบบเลือก `query_orders` แบบยกชุด
ตั้งแต่แรก (ร้านที่มีของเดินอยู่ 100 ใบ = 100 คำขอต่อรอบ)
🛑 **ห้ามเรียงเก่าก่อน** — ใบที่ตายสนิทจะยึดโควตาไว้ทุกรอบ ใบที่ยังเดินอยู่จะไม่ได้คิว
🛑 **ห้ามเอาแต่ N ใบแรกโดยไม่หมุนคิว** — ใบที่ N+1 เป็นต้นไปจะไม่ถูกถามเลยสักครั้ง = บั๊กเดิม
ย้ายที่อยู่ (จากใบที่หลุดหน้าต่างวันที่ กลายเป็นใบที่หลุดท้ายคิว)

### 22.4 FR-ISHIP-086 — endpoint การเดินทางต้องคืนสถานะล่าสุดกลับมาด้วย

`GET /api/seller/iship/shipments/{id}/traces` เปลี่ยนรูปคำตอบจาก `TraceEvent[]` เป็น:

```jsonc
{
  "events": [ /* รูปเดิมทุกช่อง */ ],
  "carrier": {
    "status": "CREATED",              // OrderShipment.status ของเรา
    "carrierStatus": "return_success",
    "carrierStatusText": "ส่งคืนสำเร็จ",
    "carrierStatusAt": "2026-08-12T08:32:00.000Z"
  }
}
```

- `carrier` **อ่านกลับจากฐานหลังบล็อกอัปเดตเสมอ** ไม่ใช่ประกอบจาก payload ที่เพิ่งได้ —
  `get_order` ล้มเหลวได้ (โค้ดกลืน error ไว้โดยเจตนา) กรณีนั้นค่าที่ถูกต้องคือค่าที่เก็บไว้
- หน้าจอ (`ShipmentStatusView`) ถือ `liveCarrier` แล้วให้ **ทุกจุดในไฟล์อ่านจาก
  `carrier = liveCarrier ?? shipment`** ตัวเดียว — ห้ามผสมทีละช่อง ไม่งั้นจะได้ป้าย
  "ส่งคืนสำเร็จ" คู่กับเวลาของสถานะก่อนหน้า
- ล้าง `liveCarrier` เมื่อ **เปลี่ยนใบ** (`shipment.id`) และเมื่อ **ลองใหม่สำเร็จ** — ค่าที่ค้าง
  จากใบเดิมจะไปทับสถานะของใบใหม่

### 22.5 เทสที่ผูกไว้ (`[blocker]` — แดงเมื่อไรห้าม merge)

`src/lib/iship/__tests__/stale-lookup.test.ts` (9) + `trace-status-contract.test.ts` (5)
พิสูจน์ด้วย mutation ครบ 6 แบบ: ถอดคิวหมุน · ถอด `?? createdAt` · ถอดเพดานอายุ ·
คืน `if (!row) continue` แบบเดิม · หน้าจอกลับไปอ่าน prop ตรง ๆ · route กลับไปคืน array เปล่า

### 22.6 FR-ISHIP-087 — cron ทุก 15 นาที (user เคาะ 2026-08-20)

`GET /api/cron/iship-status-sync` (`vercel.json` → `*/5 * * * *`, แพลน Pro รองรับ sub-daily)

🛑 **ตารางเวลา 5 นาที ไม่ได้แปลว่ายิง iShip ถี่ขึ้น** — throttle 15 นาทีใน `syncShipmentStatuses`
ยังเป็นตัวคุมความถี่จริงเพียงตัวเดียว. เหตุผลที่ไม่ตั้ง `*/15`: cron 15 นาที + throttle 15 นาที
ชนกันพอดีจนเกิด drift — ร้านที่มีคนเปิดแชทก่อน cron 14 นาที จะถูกข้ามทั้งรอบแล้วต้องรออีก 15 นาที
⇒ เพดานความเก่าของสถานะกลายเป็น ~30 นาที. ที่ 5 นาทีเหลือ ~20 นาที และรอบที่ถูกข้ามจบใน ~300ms
(วัดจาก prod จริง 2026-08-20: 357ms ตอนทุกร้านถูก throttle)

เดิม `syncShipmentStatuses` มีทางเข้าเดียวคือ **เกาะจังหวะที่มีคนเปิดกล่องข้อความ**
(`inbox/page.tsx` + `GET /api/chat/conversations`) ⇒ ร้านที่ไม่เปิดแชทเลย สถานะไม่เคยอัปเดต

- เรียงร้านด้วย `statusSyncedAt` **นับ NULL ก่อน** (ร้านที่เพิ่งเชื่อมต้องไม่ไปอยู่ท้ายแถว) ·
  เพดาน 40 ร้าน/รอบ ⇒ ร้านที่เกินได้คิวรอบถัดไปเสมอ ไม่มีร้านไหนอดถาวร
- **ไม่ใช้ `force: true`** — ให้ throttle 15 นาทีของ service เป็นเจ้าของความถี่ที่เดียว
  ร้านที่เพิ่ง sync จากการเปิดแชทจะถูกข้าม (คืน 0) ซึ่งถูกแล้ว ไม่ใช่การพลาด
- ร้านเดียวล้ม ≠ รอบล้ม (นับ `failed` แล้วไปต่อ)
- log สรุปรอบเสมอ: `[iship-cron] เสร็จรอบ { shops, shopsChanged, rowsChanged, failed }`

🛑 **`[iship-sync] ตามใบที่หลุดหน้าต่าง query_orders { shopId, picked, changed }`** — log แยก
ที่ยิงเฉพาะรอบที่คิวของ §22.3 ทำงานจริง **ห้ามถอด**: ตัวเลข `changed` รวมของ `syncShipmentStatuses`
ปนสาเหตุอื่นหมด (สถานะจากคำตอบยกชุด · เงิน COD · ต้นทุนค่าส่ง) แยกไม่ออกว่าเส้นทาง "ตามใบที่
หลุดหน้าต่าง" เจอของและแก้ได้จริงไหม — ถ้าวันหนึ่งมันพังหรือเลือกใบไม่เจอ มันจะเงียบสนิท
**แบบเดียวกับบั๊กเดิมเป๊ะ ๆ** (สถานะค้างโดยไม่มี error) ซึ่งคือสิ่งที่ทั้งรอบนี้พยายามเลิกทำ

🛑 **cron ไม่ได้แก้ปัญหาใน §22.3** — ยิงถี่แค่ไหนก็ได้ชุดข้อมูลเดิมที่ไม่มีใบที่หลุดหน้าต่าง
ถ้ามีคนถอด `pickStaleParcelsForLookup` ออกเพราะคิดว่า "มี cron แล้วนี่" สถานะจะกลับไปค้างทันที

### 22.7 FR-ISHIP-088 — ทางปิดงานของออเดอร์ที่ของตีกลับมาถึงร้านแล้ว

เดิม `Order.status` ค้างเป็น `SHIPPED` ตลอดไป — **กลไกปิดงานมีอยู่แล้ว (ยกเลิก) และอยู่ใน
`⋯` ตามกฎตำแหน่งปุ่มอยู่แล้ว** สิ่งที่ขาดคือ 2 อย่างที่ทำให้ร้านไม่ใช้มัน:

1. **ไม่มีเหตุผลที่ตรงความจริงให้เลือก** — ชุดของ `ONLINE_SALES` มีแต่ "ลูกค้าไม่โอนเงิน /
   ลูกค้าขอยกเลิก / สินค้ามีปัญหา / ตกลงกันได้" ไม่มีข้อไหนจริงเลย
   → เพิ่ม **`PARCEL_RETURNED` "ลูกค้าไม่รับของ พัสดุตีกลับ"** + migration
   `20260820120000_order_cancel_reason_parcel_returned` (additive ตามแบบของ 20260812140000
   — อ่านนิยามเดิมจากฐานมาต่อท้าย ไม่ hardcode) เทส `[blocker]`
   `cancel-reason-db-constraint.test.ts` บังคับความสัมพันธ์นี้อยู่แล้ว
2. **ร้านกลัวเสียอัตราความสำเร็จ** → กล่องบนหน้ารายละเอียดบอกตรง ๆ ว่าไม่ถูกนับเป็นความผิดร้าน
   ซึ่ง **ยืนยันกับโค้ดแล้วว่าจริง** (`shop.service.ts:381` หักใบที่ `carrierStatus` เป็นตีกลับ
   ออกจากตัวหารเสมอ — BR-OSM-04) ไม่ใช่คำปลอบใจ

**เงื่อนไขการแสดง** = `shouldPromptCloseReturnedOrder({ status, parcelReturned })`
(`lib/order-stage.ts` — ฟังก์ชันบริสุทธิ์ ไม่ใช่เทอร์นารีกลาง JSX ตาม
`docs/conventions/ui-boolean-needs-a-testable-home.md`) · `parcelReturned` คำนวณที่ server
จาก `isReturnedCarrierStatus` **ตัวเดียวกับที่ `isRateExcludedCancellation` ใช้** ⇒ ข้อความ
บนจอกับตัวเลขที่ระบบคิดมาจากเกณฑ์เดียวกันเสมอ

🛑 **ห้าม derive จาก `shippingStage`** — กอง `PROBLEM` รวม `issue`/`cannot_pickup` ซึ่งของ
ยังอยู่กับขนส่ง = คนละสถานการณ์กับ "ของกลับมาถึงร้านแล้ว"
🛑 **ปุ่มเป็นปุ่มขอบ ไม่ใช่ปุ่มทึบ** — ยกเลิกเป็น action ที่ถอนคืนไม่ได้
(`docs/conventions/seller-action-placement.md`) กล่องนี้ทำให้มัน *หาเจอ* ไม่ใช่ *เร่งให้กด*
🛑 **`CONFIRMED` ไม่ชวนปิด** — ผู้ซื้อยืนยันรับของแล้ว (หรือเงิน COD เข้าจริง) การชวนให้
ยกเลิกตรงนั้นคือการชวนให้ทำลายหลักฐานที่แข็งแรงกว่า

### 22.8 สิ่งที่ยังไม่ทำในรอบนี้

- ยังไม่ผ่าน `safepay-ux` (HR8) และยังไม่ได้รัน `/impeccable critique` — session ตั้งค่า
  ห้ามเรียก subagent (หนี้ชุดเดียวกับ 4 รอบก่อนหน้า)
- browser QA ทั้งหมด (E11-7..E11-10 + กล่องปิดงาน)

---

## 23. แยกกอง "ตีกลับ" ออกจาก "พัสดุมีปัญหา" (ส่วนขยาย 2026-08-24)

**อาการที่ user เจอบน prod:** ออเดอร์ที่ iShip รายงานว่า **"ส่งคืนสำเร็จ"** (`return_success`)
ไปแล้ว ยังค้างอยู่ในไทล์/ชิป **"พัสดุมีปัญหา"** ทั้งบนหน้าแรกและ `/orders` — และในแถวรายการแชท
ก็ขึ้นชิป "พัสดุมีปัญหา" ซ้อนกับชิปพฤติกรรม "ตีกลับ 1 รายการ" ที่พูดถึงพัสดุใบเดียวกัน

**ทำไมถึงผิด:** คำว่า "พัสดุมีปัญหา" อ่านว่า *ยังไม่รู้ว่าเกิดอะไรขึ้น ต้องไปตามขนส่ง* แต่
`return_success` แปลว่า **เรื่องกับขนส่งจบแล้ว ของอยู่ในมือร้าน** เหลือแต่การตัดสินใจเชิงธุรกิจ
(คืนเงิน / ส่งใหม่ / ปิดงานตาม §22.7) — คนละงาน คนละความเร่งด่วน จึงไม่ควรอยู่กองเดียวกัน

### 23.1 ชุดสถานะ (`src/lib/iship/status.ts`)

| ชุด | ค่า | ความหมาย |
|-----|-----|-----------|
| `PROBLEM_CARRIER_STATUSES` | `issue` `cannot_pickup` `is_expired` `cod_refund` | ของไม่เดินหน้า และยังไม่รู้ว่าจะจบยังไง |
| `RETURNED_CARRIER_STATUSES` | `return` `return_success` | ของเดินทางกลับ / กลับถึงร้านแล้ว |

- `PROBLEM_STAGE_CARRIER_STATUSES` + `isProblemStageCarrierStatus()` **ถูกลบ** — มันมีอยู่เพื่อ
  พ่วง `return_success` เข้ากอง PROBLEM ซึ่งเป็นสิ่งที่รอบนี้เลิกทำ ผู้เรียกทั้ง 3 จุด
  (`order-stage.ts` · `chat.service.ts` · `order-stage.service.ts`) ใช้ `PROBLEM_CARRIER_STATUSES` แล้ว
- 🛑 **สองชุดต้องไม่ทับกันเลย** — มีเทส `[blocker]` บังคับ
  (`src/services/__tests__/chat-problem-filter.test.ts`) เพราะค่าที่อยู่ทั้งสองชุดจะถูกนับสองไทล์
  แล้วผลรวมบนหน้าจอเกินจำนวนใบจริงโดยไม่มีอะไรฟ้อง

### 23.2 `ShippingStageKey` เพิ่มกอง `RETURNED`

`deriveShippingStage()` เช็ค `isReturnedCarrierStatus()` **เหนือสาขา terminal** — จำเป็น เพราะ
`return_success` เป็น terminal ตัวหนึ่ง ถ้าหลุดลงไปข้างล่างจะกลายเป็น `DONE` (หรือ
`AWAITING_COD` ในใบ COD = ชวนร้านไปตามเก็บเงินจากของที่ไม่เคยส่งถึง) ⇒ ของที่กองอยู่ที่ร้าน
หายจากทุกไทล์ ซึ่งคือบั๊กที่ `PROBLEM_STAGE_CARRIER_STATUSES` เคยถูกสร้างมาอุด

| จุดที่แสดง | ผลลัพธ์ |
|-----------|---------|
| ไทล์ Command Center | ไทล์ที่ 6 "ตีกลับ" (`grid-cols-4` → แถวสองมี 2 ช่องพอดี) icon `solar:undo-left-round-bold-duotone` tone warning |
| ชิป/ดรอปดาวน์ `/orders?stage=` | `RETURNED` ต่อท้าย `PROBLEM` · badge warning |
| ป้ายบนแถว/หัวรายละเอียด (`resolveOrderStatusBadge`) | "ตีกลับ" · `bg-warning/15 text-warning-ink` · icon `arrow-back-up` |
| แถบพัสดุ 4 จุด (ร้าน + ผู้ซื้อ) | ปักจุดรถเหมือน `PROBLEM` แต่จุดปัจจุบันเป็น **warning ไม่ใช่ danger** · ผู้ซื้อเห็นกล่อง "พัสดุถูกตีกลับไปยังร้านค้า" |
| แถวรายการแชท (`deriveOrderStage`) | **ไม่ขึ้นชิปเลย (คืน `null`)** |

**ทำไมแถวแชทไม่ขึ้นชิป (user สั่ง):** แถวนั้นมีชิปพฤติกรรม "ตีกลับ N รายการ"
(`customer-behavior.ts` → `behaviorBadges`) อยู่แล้ว ซึ่งนับทุกใบของลูกค้าคนนั้นและไม่หมดอายุ
ชิปที่สองไม่ได้เพิ่มข้อมูล มันแค่กินพื้นที่ rail 320px และบอกความเร่งด่วนผิด
🛑 ต้องคืน `null` ตรง ๆ ห้ามปล่อยให้ตกไปสาขาล่าง — `labelPrintedAt` ไม่เคยถูกล้าง ⇒ ใบที่
ตีกลับมาแล้วจะขึ้น "พิมพ์เอกสารแล้ว" ซึ่งผิดยิ่งกว่าป้ายเดิม

### 23.3 ยังห้าม derive `shouldPromptCloseReturnedOrder` จาก `shippingStage` (ย้ำ §22.7)

เหตุผลเปลี่ยนไปแล้วแต่ข้อห้ามยังอยู่: เดิมห้ามเพราะกอง `PROBLEM` รวม `issue`/`cannot_pickup`
ที่ของยังอยู่กับขนส่ง — ตอนนี้กอง `RETURNED` รวม **`return` (กำลังเดินทางกลับ ยังไม่ถึงร้าน)**
กับ `return_success` ไว้ด้วยกัน ซึ่งยังคนละสถานการณ์กับ "ของกลับมาถึงร้านแล้ว" อยู่ดี
ตัวตัดสินยังต้องเป็น `isReturnedCarrierStatus` ที่ระดับ carrierStatus เหมือนเดิม

### 23.4 ตัวเลขจริงบน prod ก่อนแก้ (2026-08-24)

ใบที่ยังไม่ยกเลิก + พัสดุ `CREATED`/ไม่ใช่ dry-run ใบล่าสุด: `return_success` 11 · `issue` 2 ·
`return` 1 = **13 ใบ ตรงกับชิป "พัสดุมีปัญหา 13" ที่ user ส่งภาพมาเป๊ะ** ⇒ หลังแก้เป็น
**พัสดุมีปัญหา 2 · ตีกลับ 12**

### 23.5 สิ่งที่ยังไม่ทำในรอบนี้

- **ไม่ได้เพิ่มตัวกรอง "ตีกลับ" ในกล่องแชท** — `conversationIdsByShipmentState()` ยังมีแต่
  `problem` (ซึ่งตอนนี้แคบลงตามชุดใหม่ = ตรงกับไทล์ `/orders` เหมือนเดิม) ถ้าจะเพิ่มต้องเพิ่ม
  ทั้ง `ShipmentFilter`, ชิปในกล่องแชท และ `/api/chat/problem-count`
- ยังไม่ผ่าน `safepay-ux` (HR8) และยังไม่ได้รัน `/impeccable critique` — session ตั้งค่าห้าม
  เรียก subagent (หนี้ชุดเดียวกับ 5 รอบก่อนหน้า)
- browser QA — ยังไม่ได้เปิดหน้าจริง

---

## 24. แก้ 2026-09-05 — ยอดเก็บปลายทางของพัสดุ ให้ "คำสั่งซื้อ" ตัดสินคนเดียว

### 24.1 ที่มา (user report, prod)

> "สร้างคำสั่งซื้อใน Chat เป็นการชำระเงินโอนเงิน แต่พอไปสร้างพัสดุ iShip กลับมาการชำระเงิน
> เป็นเก็บปลายทางหมดเลย ทั้ง ๆ ที่ตอนสร้าง iShip ก็ไม่ได้กรอกยอดเงินเก็บเงินปลายทาง"

ลำดับเหตุการณ์จริง (ออเดอร์ `DP2569091C7BA99F` ฿590 · 4 ก.ย. 2569 19:48 UTC):

1. ฟอร์มสร้างพัสดุส่ง `override.codAmount` **เฉพาะตอนช่องมีตัวเลข** — ช่องว่างคือ "ไม่ส่งคีย์เลย"
2. `createShipment()` จึงตกไปใช้ค่าตั้งต้น **ระดับร้าน**:
   `override?.codAmount ?? (account.defaultCodEnabled ? Number(order.totalAmount) : 0)`
   ร้านที่รายงานเปิด `defaultCodEnabled = true` ไว้ ⇒ พัสดุถูกเปิดเป็น COD เท่ายอดบิลทุกใบ
3. `syncOrderPaymentToParcel()` (FR-ISHIP-073) เห็นพัสดุเป็น COD แต่ออเดอร์เป็น `TRANSFER`
   จึงเขียน `Order.paymentMethod = "COD"` ทับ **ตามกติกาที่ถูกต้องของมันเอง** ("พัสดุชนะ")

🛑 **จุดที่ผิดคือยอดบนพัสดุ ไม่ใช่ตัวซิงก์** — FR-ISHIP-073 ทำงานถูกทุกบรรทัด มันแค่เชื่อ
หลักฐานที่ระบบเราปลอมขึ้นมาเอง

**นิยามเดียวกันเคยอยู่ 2 ที่แล้วไม่ตรงกัน (Hard Rule 16):** `ShipmentContext.codSuggested`
(ยอดที่ฟอร์ม prefill) ตัดสินจากวิธีชำระเงินของออเดอร์มาตลอด และเขียนคอมเมนต์เตือนไว้เองว่า
"ร้านอาจเปิด COD ไว้ แต่ใบนี้ลูกค้าโอนมาแล้ว — ถ้าเติมยอดให้จะกลายเป็นเก็บเงินซ้ำ" แต่ฝั่งที่
ยิงออกไปจริงไม่ได้ทำตามนั้น · `tsc`/build/เทสเดิม/grep ผ่านหมด เพราะทั้งสองนิยาม "ถูก" ในตัวเอง

**ผลกระทบบนฐาน prod (นับ 2026-09-05):** เหตุการณ์ `PAYMENT_METHOD_SYNCED` ที่ `paymentFrom`
ไม่ใช่ COD รวม **12 ครั้ง / 9 คำสั่งซื้อ** — 4 ใบขนส่งเก็บเงินไปจริง (มี `codSettledAt`) ·
ที่เหลือร้านยกเลิกพัสดุแล้วแก้วิธีชำระกลับเอง · เหลือติดค้าง 2 ใบซึ่งเป็นออเดอร์ที่ `CANCELLED`
ไปแล้วทั้งคู่ ⇒ **ไม่มีการแก้ข้อมูลย้อนหลัง** (ดู §24.5)

### 24.2 BR-ISHIP-89 — ยอด COD ของพัสดุมาจากคำสั่งซื้อใบนั้นเท่านั้น

| ผู้ตัดสิน | ยอดที่ได้ |
|---|---|
| ผู้ขายกรอกเอง (`override.codAmount` ส่งมา รวมค่า `0`) | ใช้ค่านั้นตรง ๆ |
| ไม่ได้กรอก + คำสั่งซื้อเป็น COD (`isCODPayment`) | `Order.totalAmount` |
| ไม่ได้กรอก + คำสั่งซื้อไม่ใช่ COD **หรือไม่เคยระบุวิธีชำระ** | `0` |

SSOT: **`resolveDefaultCodAmount()` ใน `src/lib/iship/payment-sync.ts`** — วางไว้ไฟล์เดียวกับ
`resolvePaymentSync()` โดยเจตนา (สองฟังก์ชันนี้พูดเรื่องเดียวกันคนละทิศ ต้องอ่านคู่กันเสมอ)

- ผู้เรียก **2 จุดเท่านั้น**: `codSuggested` ใน `getShipmentContext()` (เลขที่ร้านเห็น) และ
  fallback ใน `createShipment()` (เลขที่ยิงออกไปจริง) — ต้องเป็นฟังก์ชันเดียวกันตลอดไป
- ใช้ `isCODPayment()` (SSOT ของทั้งระบบ) ไม่ใช่ `=== "COD"` — ร้านพิมพ์วิธีชำระเองได้
- "ไม่รู้วิธีชำระ" (`null`) = `0` ไม่ใช่เก็บเงิน — เดาผิดทางนี้แค่ไม่ได้เงินตอนส่ง
  เดาผิดอีกทางคือไปเก็บเงินลูกค้าที่จ่ายมาแล้ว

### 24.3 BR-ISHIP-90 — ค่าตั้งต้นระดับร้านห้ามตัดสินเงินของใบเดียว

`ShopShippingAccount.defaultCodEnabled` **ถูกถอดออกจากระบบทั้งชุด** (user เคาะ 2026-09-05):
ไม่มีในหน้าตั้งค่าการจัดส่ง · ไม่มีใน `IShipSettingsSchema` · ไม่มีใน `SettingsInput`/`getSettings`
· ไม่มีใน `ParcelDefaults.codEnabled` ที่ส่งเข้าฟอร์ม

**คอลัมน์ยังอยู่ในฐานข้อมูลแต่ไม่มีใครอ่าน/เขียน** — ไม่มี migration ในรอบนี้ (ทิ้งไว้ปลอดภัย
กว่าลบ; ถ้าจะลบต้องเป็นรอบของตัวเองพร้อมแจ้งล่วงหน้า) BR-ISHIP-27 (ค่าเริ่มต้น COD = ปิด)
ถูกแทนที่ด้วยข้อนี้ — ไม่มี "ค่าเริ่มต้น COD ระดับร้าน" ให้ตั้งอีกแล้ว

### 24.4 BR-ISHIP-91 — คีย์ที่ไม่ส่ง ≠ "ไม่มีอะไรจะบอก"

ฟอร์มสร้างพัสดุต้องส่ง `codAmount` **ทุกครั้ง** ช่องว่าง = ส่ง `0` (เดิมเป็น conditional spread
ที่ละคีย์ทิ้ง) เพื่อให้ "ช่องว่าง" แปลว่า **ไม่เก็บเงินปลายทาง** อย่างเดียวเสมอ ไม่ใช่คำเชิญให้
เซิร์ฟเวอร์เดาแทน — ช่องอื่น (น้ำหนัก/ขนาด/ขนส่ง) ยังละคีย์ได้ตามเดิม เพราะค่าตั้งต้นของช่อง
เหล่านั้นไม่ใช่เรื่องเงินของลูกค้า

**UI ที่เปลี่ยนตามไปด้วย** (ผ่าน `safepay-ux` — mode Operate):
- placeholder ช่องเก็บเงินปลายทาง: `0` (เดิมสลับข้อความตาม `defaults.codEnabled`)
- คำกำกับใต้ช่อง: `ยอดนี้อ้างอิงวิธีชำระเงินของคำสั่งซื้อ — เว้นว่างหรือ 0 = ไม่เก็บเงินปลายทาง`
- "บริการเสริม" ในหน้าตั้งค่า: แถบ `bg-info/15` แทนที่ checkbox ที่ถอดออก
  `ยอดเก็บเงินปลายทางกำหนดจากวิธีชำระเงินของแต่ละคำสั่งซื้อตอนสร้างพัสดุ ไม่ต้องตั้งเป็นค่าเริ่มต้นที่นี่`

### 24.5 ข้อมูลเดิมบน prod — ไม่แก้ย้อนหลัง (ตัดสิน 2026-09-05)

4 ใบที่ถูกซิงก์เป็น COD **ขนส่งเก็บเงินไปจริงแล้ว** (`codSettledAt` มีค่า, `carrierStatus =
payment_success`) ⇒ `paymentMethod = "COD"` เป็นความจริงของใบนั้นไปแล้ว การเขียนกลับเป็น
`TRANSFER`/`CASH` จะทำให้เงินที่รับมาทาง COD ไม่มีที่ยืน · อีก 2 ใบที่ยังติดค้างเป็น COD อยู่
เป็นออเดอร์ที่ `CANCELLED` แล้วทั้งคู่ (ไม่กระทบยอดขาย/กองรอเงิน) ⇒ **ไม่รันสคริปต์แก้ข้อมูล**

### 24.6 เทสที่ผูกไว้ (`[blocker]` — แดงเมื่อไรห้าม merge)

`src/lib/iship/__tests__/payment-sync.test.ts` · `src/lib/iship/__tests__/cod-amount-single-source.test.ts`

| เทส | จับอะไร |
|---|---|
| `resolveDefaultCodAmount` 6 เคส | TRANSFER/CASH/null → 0 · COD → ยอดบิล · คำไทยที่ร้านพิมพ์เอง → ยอดบิล · ยอดบิลใช้ไม่ได้ → 0 ไม่ใช่ NaN |
| สแกน `iship.service.ts` | ห้ามมีคำว่า `defaultCodEnabled` อีก · ต้องเรียก `resolveDefaultCodAmount` ครบ 2 จุด · ห้ามคำนวณ COD จาก `totalAmount` เอง |
| สแกน `ShipmentCreateForm.tsx` | ต้องส่ง `codAmount` เสมอ · ห้ามกลับไปใช้ conditional spread |

พิสูจน์ด้วย mutation 4 แบบ แดงครบทุกแบบ (คืนบั๊กเดิมที่ service · ถอดด่าน `isCODPayment` ·
คืน conditional spread ที่ฟอร์ม · ให้ `codSuggested` เขียนเงื่อนไขเอง)

### 24.7 สิ่งที่ยังไม่ทำในรอบนี้

- **ยังไม่มีอะไรบอกร้านว่า "ใบนี้จะไม่เก็บเงินปลายทางนะ" ตอนสร้างพัสดุจากโหมด AUTO/ในแชท** —
  `runAfterOrderCreate()` อ่านเฉพาะ `status`/`trackingNo` จึงกลืน `paymentNotice` ชนิด
  `warning` (`WARN_NO_COD`) ทิ้งทั้งก้อน: ออเดอร์ COD ที่พัสดุเปิดแบบไม่เก็บเงินยังเงียบอยู่
- ไม่ได้แตะ `retryShipment()` — ใบที่ถูกเปิดเป็น COD ไปแล้วและกด "ลองใหม่" ยังใช้ยอดเดิมของ
  แถวนั้น (ถูกตามเจตนา "ลองใหม่ = ใบเดิม") ร้านแก้ได้ด้วยการกรอกยอดใหม่ในฟอร์ม
- browser QA — ยังไม่ได้เปิดหน้าจริง
