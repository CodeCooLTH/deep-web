...
| BR-ISHIP-27 | ~~COD default ปิด~~ ยกเลิก 2026-09-05 | — (แทนที่ด้วย BR-ISHIP-89/90/91) |
| BR-ISHIP-89 | ยอด COD มาจากวิธีชำระเงินของคำสั่งซื้อ | E13-1..E13-5 |
| BR-ISHIP-90 | ค่าตั้งต้นระดับร้านห้ามตัดสินเงินรายใบ | E13-6, E13-7 |
| BR-ISHIP-91 | ช่องว่าง = ไม่เก็บเงินปลายทาง | E13-8, E13-9 |
...
| กลุ่ม | เคส |
|---|---|
| **คู่ตำบล/อำเภอ ขาเข้า (blocker)** | `dst_area`→ตำบล และ `dst_district`→อำเภอ (กลับด้านกับขาออก — ยืนยันกับ prod 2026-08-07) · `dst_amphure` ชนะ `dst_district` ในช่องอำเภอ · payload จริง TH066536981258 ต้องได้ ต.ช้างซ้าย อ.กาญจนดิษฐ์ · **ไม่มี `dst_area` = รูปขาออก → ถอยไปใช้ `dst_district`=ตำบล** |
...
| M-2 | ~~**ยิง `query_orders` กับพัสดุที่ตำบล ≠ อำเภอ**~~ **ทำแล้ว 2026-08-07 — พบว่าสลับจริง** (`get_order` TH066536981258/TH681090UM471E) | แก้ mapping + backfill แล้ว · เคสนี้กลายเป็น unit test ถาวรใน `unlinked.test.ts` แทนการตรวจด้วยตา |
...
| E8-6 | หน้าจอเมื่อเครดิตไม่พอ | shipment FAILED + `lastErrorCode=INSUFFICIENT_BALANCE` | ปุ่มทึบใบเดียวของจอ = "เข้าระบบ iShip เพื่อเติมเงิน" **เต็มความกว้าง สูง ≥44px** (แท็บใหม่ → `app.iship.cloud/`) · ทั้ง "ลองใหม่" และ "แก้ที่อยู่หรือขนส่ง" เป็น ghost **ห้ามมีปุ่มทึบใบที่สอง** · คำอธิบายท้ายจอ = "กดลองใหม่ได้หลังเติมเงินแล้ว" | ⬜ browser |
| E8-7 | error ที่แก้ด้วยการกดซ้ำไม่ได้ | `lastErrorCode=ADDRESS_INVALID` | ปุ่มทึบ = "แก้ที่อยู่หรือขนส่ง" (ไม่ใช่ "ลองใหม่") · **มีบล็อกที่อยู่ที่ส่งไปให้ขนส่งตรวจแสดงอยู่บนจอ** · คำอธิบายท้ายจอ = "กดลองใหม่ตอนนี้จะได้ผลเดิม…" | ⬜ browser |
| E8-9 | error ที่กดซ้ำแล้วมีโอกาสผ่าน | `lastErrorCode=UPSTREAM_ERROR` | ปุ่มทึบ = "ลองใหม่" · ระหว่างกดป้ายเปลี่ยนเป็น "กำลังลองใหม่…" · ปุ่มทั้งแถบ disabled พร้อมกัน | ⬜ browser |
| E8-10 | ตำแหน่งปุ่มนิ่งข้ามชนิด error | สลับ `lastErrorCode` ระหว่าง `UPSTREAM_ERROR` ↔ `ADDRESS_INVALID` | ปุ่มหลักอยู่ตำแหน่งเดิมเสมอ (บนสุด/ซ้ายสุด) เปลี่ยนแค่ว่าใบไหนได้สีทึบ — ห้ามสลับข้าง | ⬜ browser |
| E8-11 | ทางถอยเมื่อเปิดพัสดุ iShip ไม่ได้ | ออเดอร์มีใบ FAILED ใบเดียว | โมดัลแจ้งเลขพัสดุยังมีตัวเลือก **"ส่งเอง"** ให้กรอกเลขที่ไปส่งเองได้ (`hasIshipShipment` นับเฉพาะ `CREATED && !isDryRun`) | ⬜ browser |
...
### E11 — พัสดุที่เดินทางนานกว่าหน้าต่าง sync (แก้ 2026-08-20 · SRS §22)

อัตโนมัติแล้ว: `src/lib/iship/__tests__/stale-lookup.test.ts` (9) + `trace-status-contract.test.ts` (5)
พิสูจน์ด้วย mutation ครบ 6 แบบ — ดู SRS §22.5

| # | เคส | ข้อมูลตั้งต้น | คาดหวัง | สถานะ |
|---|-----|---------------|---------|-------|
| E11-1 | ใบตีกลับที่หลุดหน้าต่าง 6 วัน | ใบไม่อยู่ในคำตอบ `query_orders` · ขยับล่าสุด 8 วันก่อน | ถูกเลือกมายิง `get_order` รายใบ **ไม่ใช่ถูกข้ามเงียบ** | ✅ unit |
| E11-2 | เพดานคำขอต่อรอบ | ใบค้าง 50 ใบ | ยิง `get_order` ไม่เกิน 8 ครั้ง/รอบ/ร้าน | ✅ unit |
| E11-3 | ความครบของคิว | ใบค้าง 20 ใบ · 3 รอบติดกัน | ทุกใบถูกถามอย่างน้อย 1 ครั้ง (ห้ามมีใบที่ไม่ถูกถามเลย) | ✅ unit |
| E11-4 | ใบที่ตายสนิท | ไม่ขยับ 60 วัน | หลุดออกจากคิว (ห้ามยึดโควตาถาวร) | ✅ unit |
| E11-5 | ใบที่ขนส่งยังไม่สแกน | `carrierStatusAt = null` · เพิ่งสร้างเมื่อวาน | นับอายุจาก `createdAt` → ยังอยู่ในคิว | ✅ unit |
| E11-6 | endpoint การเดินทาง | เปิดโมดัลพัสดุ | คำตอบมี `carrier` คู่กับ `events` · ค่าอ่านกลับจากฐานหลังเขียน | ✅ unit |
| E11-7 | หัวการ์ดอัปเดตเอง | เปิดโมดัลใบที่สถานะในฐานเก่ากว่าความจริง | ป้าย/แถบ 4 ขั้น/บรรทัด "อัปเดต" เปลี่ยนทันทีหลังไทม์ไลน์โหลดเสร็จ **โดยไม่ต้องรีโหลด** | ⬜ browser |
| E11-8 | ไม่ขัดกันในจอเดียว | ใบ `return_success` | ป้ายหัวการ์ด = "ส่งคืนสำเร็จ" · แถบเทาถึงจุดที่ 4 ป้าย "ส่งคืนสำเร็จ" · ตรงกับรายการเดินทางบรรทัดบนสุด | ⬜ browser |
| E11-9 | เปลี่ยนใบในคอมโพเนนต์เดิม | สลับดูพัสดุอีกใบ | สถานะที่ค้างจากใบก่อนหน้าถูกล้าง (ห้ามโชว์ของใบเก่าบนใบใหม่) | ⬜ browser |
| E11-10 | `get_order` ล้มกลางคิว | ใบที่ 3 จาก 8 ตอบ error | ใบที่เหลือยังถูกอัปเดตครบ · รอบ sync ไม่ล้มทั้งรอบ | ⬜ |

### E12 — cron + ทางปิดงานของที่ตีกลับ (2026-08-20 · SRS §22.6-22.7)

อัตโนมัติแล้ว: `src/lib/order-stage.test.ts` (E12-3..E12-5, mutation 2 แบบ) +
`src/lib/__tests__/cancel-reason-db-constraint.test.ts` (E12-2)

| # | เคส | ข้อมูลตั้งต้น | คาดหวัง | สถานะ |
|---|-----|---------------|---------|-------|
| E12-1 | cron ยิงเข้ามาโดยไม่มี secret | ไม่มี/ผิด `Authorization` | `401` · ไม่แตะฐานเลย | ⬜ |
| E12-2 | เหตุผลใหม่ตรงกับ CHECK ของฐาน | `PARCEL_RETURNED` | ปรากฏใน migration ของ `Order_cancel_reason` | ✅ unit |
| E12-3 | กล่องปิดงานขึ้นตอนของตีกลับ | `SHIPPED` + `return_success` | `shouldPromptCloseReturnedOrder` = true | ✅ unit |
| E12-4 | ใบที่ปิดไปแล้ว | `CONFIRMED`/`CANCELLED` + ตีกลับ | false (ห้ามชวนทำลายหลักฐานที่แข็งแรงกว่า) | ✅ unit |
| E12-5 | ไม่มีพัสดุตีกลับ | ทุกสถานะ | false | ✅ unit |
| E12-6 | ยกเลิกด้วยเหตุผลใหม่บนฐานจริง | ร้าน ONLINE_SALES · ใบที่ของตีกลับ | บันทึกผ่าน (ไม่ติด 23514) · ใบนั้นถูกหักออกจากตัวหารอัตราความสำเร็จ | ⬜ browser |
| E12-7 | ร้านคิวงาน/ที่พักเลือกเหตุผลนี้ไม่ได้ | vertical ≠ ONLINE_SALES | `isValidCancelReason` = false → API ปฏิเสธ | ⬜ |
| E12-8 | cron รอบที่ร้านเพิ่ง sync ไป | เปิดกล่องแชทเมื่อ 2 นาทีก่อน | ข้ามร้านนั้น (คืน 0) ไม่ยิง iShip ซ้ำ | ⬜ |

---

## E13 — ยอดเก็บปลายทางให้คำสั่งซื้อตัดสิน (แก้ 2026-09-05 · SRS §24)

| # | เคส | ข้อมูลนำเข้า | คาดหวัง | สถานะ |
|---|-----|-------------|---------|-------|
| E13-1 | ออเดอร์โอนเงิน ไม่กรอกยอด COD | `paymentMethod=TRANSFER` ฿590 · ช่องว่าง | พัสดุ `codAmount = 0` · ออเดอร์ยังเป็น `TRANSFER` · ไม่มี event `PAYMENT_METHOD_SYNCED` | ✅ unit |
| E13-2 | ออเดอร์เงินสด ไม่กรอกยอด COD | `paymentMethod=CASH` | `codAmount = 0` | ✅ unit |
| E13-3 | ออเดอร์ไม่เคยระบุวิธีชำระ | `paymentMethod=null` | `codAmount = 0` (ไม่รู้ ≠ ให้ไปเก็บเงิน) | ✅ unit |
| E13-4 | ออเดอร์ COD ไม่กรอกยอด | `paymentMethod=COD` ฿360 | `codAmount = 360` | ✅ unit |
| E13-5 | วิธีชำระที่ร้านพิมพ์เอง | `"เก็บเงินปลายทาง"` ฿220 | `codAmount = 220` (ผ่าน `isCODPayment`) | ✅ unit |
| E13-6 | ร้านที่เคยเปิด `defaultCodEnabled` ไว้ | คอลัมน์ยังเป็น `true` ในฐาน · ออเดอร์ TRANSFER | `codAmount = 0` — ค่าคอลัมน์ไม่มีผลกับอะไรทั้งสิ้น | ✅ unit (source scan) |
| E13-7 | หน้าตั้งค่าการจัดส่ง | เปิด `/settings` → การจัดส่ง | ไม่มี checkbox "เก็บเงินปลายทาง (COD) เป็นค่าเริ่มต้น" · มีแถบข้อมูลอธิบายแทน | ⬜ browser |
| E13-8 | เว้นช่อง COD ในฟอร์ม | ออเดอร์ COD ที่ร้านลบตัวเลข prefill ทิ้ง | ส่ง `codAmount: 0` → พัสดุไม่เก็บเงิน + เตือน `WARN_NO_COD` | ⬜ browser |
| E13-9 | กรอกยอด COD สวนกับออเดอร์ | ออเดอร์ TRANSFER · ร้านพิมพ์ ฿500 | พัสดุ COD ฿500 · ออเดอร์ถูกซิงก์เป็น COD + แจ้งร้าน (พฤติกรรมเดิม FR-ISHIP-073 ยังอยู่) | ⬜ browser |
