...
| ความเสี่ยง | สถานะ |
|---|---|
| ~~`dst_area` (อำเภอ ขาเข้า) พิสูจน์จากเอกสารไม่ได้~~ **เกิดขึ้นจริงแล้ว 2026-08-07** — ขาเข้า `dst_district`=อำเภอ `dst_area`=ตำบล (กลับด้านกับขาออก) ของเดิมจับคู่กลับหัว | ยิง `get_order` จริงจนได้ข้อสรุป → แก้ `parseReceiverLocality()` + backfill 26 ออเดอร์ (SRS §16.3) · **บทเรียน: "ให้ร้านดูตารางเทียบ" ไม่ใช่ด่านกันข้อมูลเพี้ยน — ผ่านไป 23 ใบไม่มีใครทัก** |
