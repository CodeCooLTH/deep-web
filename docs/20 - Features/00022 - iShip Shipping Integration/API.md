...
**รูปคำตอบ (แก้ 2026-08-20 — เดิมเป็น `TraceEvent[]` เปล่า ๆ):**

```jsonc
{
  "events": [{ "status","statusText","statusDesc","location","occurredAt" }],
  "carrier": { "status","carrierStatus","carrierStatusText","carrierStatusAt" }
}
```

🛑 **`carrier` ต้องมาด้วยเสมอ** — คำขอนี้ยิง `get_order` และ *เขียนสถานะใหม่ลงฐานระหว่างทาง*
(FR-ISHIP-075) ถ้าไม่ส่งค่าใหม่กลับไป หน้าจอที่เป็นคนสั่งให้เกิดการอัปเดตจะเป็นที่สุดท้ายที่รู้
— บั๊กจริงบน prod 2026-08-20: หัวการ์ดขึ้น "กำลังจัดส่ง" ขณะที่รายการเดินทางใต้มันขึ้น
"ส่งคืนสำเร็จ" ติดกัน 8 วัน (SRS §22.4) · ค่าอ่านกลับจากฐาน **หลัง** เขียน ไม่ใช่จาก payload
...
> **ข้อยกเว้นเดียว (เพิ่ม 2026-08-20, SRS §22.3):** ใบที่ **ไม่อยู่ในคำตอบของ `query_orders` เลย**
> (หลุดหน้าต่าง 6 วัน) ยิง `get_order` รายใบได้ เพราะไม่มีทางอื่นให้รู้สถานะเลย — ไม่ใช่การเลือก
> ระหว่างสองวิธี. ต้องอยู่ใต้เพดาน `STALE_LOOKUP_MAX_PER_ROUND = 8` คำขอ/รอบ/ร้าน + คิวหมุนเวียน
> เสมอ (`src/lib/iship/stale-lookup.ts`) เจตนาเดิมของกฎข้อนี้ยังอยู่ครบ: **ห้ามวนทุกใบ**
...
🛑 **`settlement_at` = วันนัดโอน ไม่ใช่หลักฐานว่าโอนแล้ว** — ตัวอย่างข้างบนมี `status: 12`
กำกับอยู่ด้วยจึงเชื่อได้ แต่แถวที่ `status: 3` (จัดส่งแล้ว) ก็มี `settlement_at` เหมือนกัน
โดยเป็นค่า `delivered_at + 24 ชม.` ที่ขนส่งประกาศไว้ล่วงหน้า (พบ 20 แถวจาก 115 บนบัญชีจริง
2026-08-06 — 11 แถวเป็นวันพรุ่งนี้) **ตัวตัดสินคือ `status === 12` เสมอ** ดู SRS §18.1.1
