...
- กฎ "ครั้งเดียวต่อคนต่อโพสต์" ใช้กับ **การทักแชทอัตโนมัติ** เท่านั้น (BR-CR-A2b) — ไม่ใช้กับคน
  และตั้งแต่ 2026-08-15 ไม่ใช้กับการตอบใต้คอมเมนต์อีกต่อไป (BR-CR-A2a)
...
| ~~**BR-CR-A2**~~ | ~~หนึ่งคน ได้รับการตอบอัตโนมัติ **ครั้งเดียวต่อโพสต์**~~ — แทนที่ด้วย A2a/A2b เมื่อ 2026-08-15 |
| **BR-CR-A2a** | **ตอบใต้คอมเมนต์ = ทุกคอมเมนต์ระดับบนของลูกค้า ไม่มีเพดานต่อคน** — ลูกค้าคนเดิมถามใหม่ใต้โพสต์เดิมต้องได้คำตอบใหม่เสมอ (Facebook ไม่จำกัดการตอบสาธารณะ เพดานเดิมเป็นกฎที่เราตั้งเอง) |
| **BR-CR-A2b** | **ทักแชทอัตโนมัติ = ครั้งเดียวต่อคนต่อโพสต์** (เหมือนเดิม) — DM ซ้ำคือสแปมจริง และ Facebook ให้ทักได้ครั้งเดียวต่อคอมเมนต์อยู่แล้ว บังคับที่ชั้น DB ด้วย partial unique index บน `privateAttemptedAt` ซึ่งจองสิทธิ์ **ก่อน** ยิง Graph |
...
| **BR-CR-A7** | (2026-08-15) `{ชื่อ}` ในข้อความตั้งค่า = ชื่อ Facebook ของผู้คอมเมนต์ ใช้ได้ทั้ง 2 ช่องทาง + ช่องพิมพ์ของปุ่มทักแชทแบบแมนนวล — SSOT อยู่ที่ `src/lib/comment-reply-template.ts` ที่เดียว · ไม่มีชื่อ = ตัดคำนี้ทิ้งพร้อมเก็บกวาดช่องว่าง (ห้ามยุบบรรทัดว่างระหว่างย่อหน้า) · ข้อความที่เหลือว่างหลังแทน = ตกที่ `DISABLED` ไม่ส่งอะไรออกไป |
