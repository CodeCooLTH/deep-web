...
- **คำอธิบายเชิงเทคนิค:** สถานะปัจจุบัน "ตอบแล้ว" คำนวณที่ **client** (`CommentsClient.tsx:510`
  ...
  🛑 **แก้ 2026-08-09 — "ยังไม่ตอบ" ไม่ได้ดูแค่คำตอบสาธารณะอีกต่อไป:** คอมเมนต์ที่ไม่มีคำตอบใต้โพสต์
  แต่ **ทักแชทส่วนตัวสำเร็จแล้ว** (`CommentReplyLog.privateReplyStatus='SENT'`) ต้องไม่นับเป็น
  "ยังไม่ตอบ" — ให้ตกเข้าสถานะที่มีอยู่แล้วตามว่าใครเป็นคนทัก: `trigger='AUTO'` → บอทตอบแล้ว ·
  `trigger='MANUAL'` → คนตอบแล้ว (**ไม่เพิ่มค่าที่ 4 ในเอนัม** — มติ user 2026-08-09 "ไม่ต้องเพิ่มเยอะ")
  คำตอบสาธารณะยังชนะเสมอ: คอมเมนต์ที่มีทั้งคำตอบใต้โพสต์และทักแชทแล้ว อ่านจากคำตอบสาธารณะ
  เหตุผล: เดิมคอมเมนต์ที่บอททักแชทไปแล้ว ลูกค้าเข้าห้องแล้ว กำลังคุยกันอยู่ในกล่องข้อความ **ค้างใน
  คิว "ยังไม่ตอบ" ตลอดไป** เพราะไม่มีใครไปตอบใต้คอมเมนต์สาธารณะด้วย คิวจึงไม่มีวันลดลงเองแม้งานจบแล้ว
  🛑 ต้องเช็ค `privateReplyStatus = 'SENT'` เป๊ะ — `SKIPPED`/`FAILED` คือ "มีแถวแต่ไม่ได้ทัก"
  ถ้าเช็คแค่ว่ามีแถว คอมเมนต์ที่ทักไม่สำเร็จจะหายจากคิวทั้งที่ยังไม่มีใครคุยกับลูกค้าเลย
  (implement: `deriveCommentState()` + CASE ใน `countCommentPostStatesByShop()` ต้องเรียงเงื่อนไข
  ตรงกันบรรทัดต่อบรรทัด — เทส `[blocker]` ที่ `comment-unanswered-count.test.ts` ล็อกไว้)
...
> 🛑 **แก้ 2026-08-15 — หน่วยเปลี่ยนจาก "โพสต์" เป็น "คอมเมนต์"** ข้อความด้านล่างที่ว่า badge นับ
> *จำนวนโพสต์* **ไม่จริงอีกต่อไป** ตั้งแต่คอลัมน์ซ้ายของ `/inbox/comments` เปลี่ยนเป็น
> 1 แถว = 1 คอมเมนต์ (ผู้ใช้เคาะ) — badge และตัวนับ 4 กลุ่มบนแท็บผ่าน
> `countCommentStatesByShop()` แล้ว ไม่ใช่ `countCommentPostStatesByShop()`
> เหตุผลของกฎเดิม ("ต้องเป็นหน่วยเดียวกับ badge แท็บข้อความที่นับเธรด") ถูกแทนด้วยกฎที่แข็งกว่า:
> **หน่วยของตัวนับต้องเท่ากับหน่วยของแถวที่มันลอยอยู่ข้างบน** ไม่งั้นได้ badge "7" เหนือรายการ
> 12 แถว · รายละเอียด + ด่าน `[blocker]` → `EXTENSIONS-2026-08-15.md`
