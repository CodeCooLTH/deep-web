...
## 11. ส่วนขยาย 2026-08-13 — เส้นทางคลิป C

### 11.1 ที่มา

รอบแรกแปลตามเส้นทางคลิป A/B (เข้าสู่ระบบ → ช่องทางการขาย → เลือกเพจ → กล่องข้อความ → ห้องแชท)
แล้วบันทึกไว้ว่าที่เหลือ "อยู่นอกเส้นทางคลิป จงใจข้าม" — แต่ **คลิป C ของ `pages_read_engagement`
เดินคนละเส้น** (`APP-REVIEW.md` §6.1) และเส้นนั้นยังเป็นภาษาไทยทั้งหมด

คลิป C ต้องโชว์ 2 อย่างที่คำอธิบาย permission §2.4 อ้างไว้:
1. **แบนเนอร์ที่มาของบทสนทนา** ในห้องแชท (โพสต์/โฆษณาต้นทางที่เราอ่านมาจาก Graph)
2. **รีลของเพจที่ร้านเลือกไปแสดงบนหน้าร้าน** — จังหวะที่ `GET /{page-id}/video_reels` ทำงาน

### 11.2 D-I18N-7 — รอบนี้แปลเฉพาะฝั่งผู้ขาย (ผู้ใช้ตัดสิน 2026-08-13)

| Surface | สถานะ | เหตุผล |
|---|---|---|
| แบนเนอร์ที่มาของแชท (`ChatThread` contextItems + `ThreadContextBar`) | ✅ แปลแล้ว | อยู่ใต้ `(paces)` ต่อ `useT()` ได้ตรง |
| หน้า `/public-profile` (หัวข้อ/ลิงก์หน้าร้าน/สวิตช์เผยแพร่/ตัวเลือกคลิป) | ✅ แปลแล้ว | เหมือนกัน + `getT()` ฝั่ง server |
| หน้าร้านสาธารณะ `/u/[username]` · `/b/[slug]` | ❌ **ยังเป็นไทย** | อยู่ `(marketing)` นอก `LocaleProvider` และ cookie `deep_locale` ผูกกับ host `seller.` เท่านั้น (`locale-cookie.ts` จงใจไม่ตั้ง `domain=` เพราะฝั่งผู้ซื้อยังไม่แปล) ⇒ ต้องหาทางส่ง locale ข้ามโดเมนก่อน ซึ่งกระทบผู้ซื้อทั้งฝั่ง |

ผลกับคลิป: ฉากที่เปิดหน้าร้านสาธารณะให้เห็นรีลจะยังเป็นไทย — กริดรีลเป็นรูปภาพล้วน
ไทยที่เหลือคือชื่อแท็บกับข้อความรีวิว ใช้ caption อังกฤษกำกับในคลิปแทน

### 11.3 กับดักที่เจอเพิ่ม (นอกเหนือจาก §"ค่าคงที่ระดับ module" ของรอบแรก)

**ค่า default ที่เป็นภาษาไทยฝังอยู่ใน component/helper ที่ใช้ร่วมกัน** — ผู้เรียกที่ไม่ส่ง prop
จะได้ไทยกลับมาโดยไม่มีอะไรฟ้อง และมันโผล่ *ข้าง ๆ* ข้อความที่แปลแล้ว:
- `CopyLinkButton` — `successMessage` default `'คัดลอกลิงก์แล้ว'` ⇒ ปุ่มอังกฤษ toast ไทย
- `pacesConfirm` (`paces-swal.ts`) — `cancelButtonText` default `'ยกเลิก'` ⇒ โมดัลอังกฤษทั้งใบ
  แต่ปุ่มยกเลิกเป็นไทยกลางจอ

⇒ **เวลาแปลหน้าไหน ต้องไล่ prop ที่ไม่ได้ส่งด้วย ไม่ใช่ไล่แค่สตริงที่เห็นในไฟล์นั้น**
`tsc` ช่วยไม่ได้เลยเพราะ prop เป็น optional และค่า default ถูกต้องตามชนิดทุกประการ

**`export const metadata` อ่าน locale ไม่ได้** — ค่าคงที่คำนวณตอน build ต้องเปลี่ยนเป็น
`generateMetadata()` (ท่าเดียวกับ `seller/auth/sign-in/page.tsx`) ไม่งั้น title ของแท็บเบราว์เซอร์
ค้างเป็นไทยตลอดฉากที่ reviewer เปิดหน้านั้นค้างไว้
