...
- **Error / Edge cases:**
  ...
  - **สถานะจริงของงานที่ 3 (อัปเดต 2026-09-06 — ปิด OQ-12/OQ-13 แล้ว):** บันทึกผลได้ **ครบทั้ง 6 ข้อ** — `scam_db` (พบในฐาน→ไม่ผ่าน · ค้นไม่สำเร็จ/เจ้าของไม่มีเบอร์→ไม่บันทึก **ห้าม fallback เป็นผ่าน**) · `phone_identity` (L1 ขึ้นไป→ผ่าน · ระดับ 0→ยังไม่มีข้อมูล ไม่ใช่ไม่ผ่าน) · `complaints` (ไม่มีเรื่องค้าง→ผ่าน) · **`account_age` (≥ 30 วัน→ผ่าน)** · **`chat_response_speed` (≥ 80% และต้องผ่านเกณฑ์ตัวอย่างขั้นต่ำของ `resolveChatResponse()` ก่อน)** · **`duplicate_listing` (เทียบ sha256 ของรูปข้ามร้าน ตัดสินรายหลัง — ดู SDS TD-020)** · 🛑 ทะเบียนยังต้องครบ 6 ข้อเสมอและข้อที่ตัดสินไม่ได้ต้องรายงานเหตุผลผ่าน `skipped` ห้ามหายจากลูปเงียบ ๆ
