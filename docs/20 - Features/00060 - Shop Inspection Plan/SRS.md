...
- **คำอธิบายเชิงเทคนิค:** `subscribeInspectionPlan({ shopId, userId, step, termsAccepted })` ทำงานใน `prisma.$transaction` เดียวตามลำดับนี้:
  ...
  5. **INSERT แถว `InspectionTermsAcceptance`** (`shopId`, `acceptedAt=now`, `step`, `priceSnapshotBaht`, `termsVersion`) — **นี่คือแหล่งความจริงของการรับทราบเงื่อนไข**
  6. `deductCredit(shopId, amount, refId, description, reason, tx)` — ส่ง `tx` เข้าไปเสมอ เพื่อให้การหักเงินอยู่ในทรานแซกชันเดียวกับการจองโควตา

  > **หมายเหตุลำดับ (แก้ 2026-08-30 ตอน implement):** เดิม SRS เขียนหักเงินไว้ก่อนความยินยอม ส่วน [[SDS]] §5.9 เขียนว่าความยินยอมต้องมาก่อน — **เอกสารสองฉบับขัดกันเอง** ภายในทรานแซกชันเดียวผลลัพธ์เท่ากันทั้งสองทาง (ล้มข้อใดข้อหนึ่งก็ rollback ทั้งก้อน) จึงยึดตาม SDS เพราะเป็นเอกสารที่ออกแบบชั้นบริการ และ `priceSnapshotBaht` ที่บันทึกต้องเป็น**จำนวนเดียวกับที่ส่งเข้า `deductCredit`** เสมอ (มีเทส `[blocker]` เทียบสองค่านี้ตรง ๆ — ถ้าหลุดจากกัน หลักฐานที่ใช้ตอนพิพาทจะระบุราคาคนละตัวกับที่หักจริง)
...
- **Error / Edge cases:**
  ...
  - **สถานะจริงของงานที่ 3 ณ 2026-09-05 (ตอน implement):** บันทึกผลได้ **3 จาก 6 ข้อ** — `scam_db` (พบในฐาน→ไม่ผ่าน · ค้นไม่สำเร็จ/เจ้าของไม่มีเบอร์→ไม่บันทึก **ห้าม fallback เป็นผ่าน**) · `phone_identity` (L1 ขึ้นไป→ผ่าน · ระดับ 0→ยังไม่มีข้อมูล ไม่ใช่ไม่ผ่าน) · `complaints` (ไม่มีเรื่องค้าง→ผ่าน · มีเรื่องค้าง→ไม่ผ่าน โดยนับจาก `Order.disputeOpenedAt`/`disputeResolvedAt`) · อีก 3 ข้อ (`account_age`, `chat_response_speed`, `duplicate_listing`) ประกาศตัวในทะเบียน `src/lib/inspection/auto-checks.ts` ว่ายังตัดสินไม่ได้พร้อมเหตุผล (`CRITERIA_NOT_DECIDED` / `NO_DETECTOR`) และถูกนับใน `skipped` ของ response ⇒ อ่านจาก log ได้ว่าข้อไหนไม่เคยถูกตรวจเพราะอะไร — ดู SDS OQ-12/OQ-13 · 🛑 **ข้อที่ยังตัดสินไม่ได้ต้องอยู่ในทะเบียนเสมอ ห้ามหายไปจากลูปเงียบ ๆ** เพราะฝั่งผู้ซื้อเห็น "ยังไม่มีข้อมูล" เหมือนกันทั้งกรณี *ตรวจแล้วไม่มีข้อมูล* และกรณี *ไม่มีใครเขียนโค้ด*
...
- **Error / Edge cases:**
  ...
  - **สถานะ ณ 2026-08-30 (implement):** สูตรร่างอยู่ที่ `upgradeChargeBaht()` ใน `src/lib/inspection/pricing.ts` **ที่เดียว** = "ส่วนต่างค่าเดือน + ค่าแรกเข้าของขั้นใหม่ที่ยังไม่เคยจ่าย" (แต่ละส่วนตัดที่ 0 แยกกัน) มติออกมาต่างจากนี้ให้แก้ฟังก์ชันนี้ฟังก์ชันเดียว ห้ามไปแก้ที่ผู้เรียก
  - 🛑 **รอบบิลไม่ถูกรีเซ็ตทั้งการขึ้นและลงขั้น** (`nextRenewalAt`/`currentPeriodStart` เดิมคงอยู่) — รีเซ็ตเมื่อไหร่ การอัปเกรดกลางรอบจะกลายเป็นการเก็บค่าเดือนสองครั้งในเดือนเดียว มีเทส `[blocker]` ยืนยันว่า `update` ไม่มีสองคีย์นี้
  - **การลดขั้นไม่ใช้โควตา ไม่คิดเงิน และไม่ต้องรับทราบเงื่อนไขใหม่** เพราะไม่มีการชำระเงินเกิดขึ้น
  - 🛑 **ทั้งขึ้นและลงต้องเรียก `recomputeExpiryForPlanStep()`** — อายุผลตรวจของ `video_tour`/`operating_evidence` ผูกกับขั้นของแผน (ขั้น 4 ตรวจซ้ำถี่กว่า) ไม่คิดใหม่ = ป้าย "ผ่าน" อ้างอิงความถี่ของขั้นใหม่ทั้งที่วันหมดอายุยังเป็นของขั้นเก่า **คือการขายความถี่ที่ไม่ได้ส่งมอบ** · ตัวนี้อยู่ใน `inspection-result.service.ts` เพราะไฟล์นั้นเป็นผู้เขียน `InspectionResult` เพียงรายเดียว และมันแตะเฉพาะ `expiresAt` (ชุดคอลัมน์เดียวกับที่ CONFIRM แตะได้)
...
- **A-T4:** ราคาค่าตรวจต่อขั้นถูกอ่านจากค่าคงที่ตัวเดียวใน `src/lib/inspection/pricing.ts` — **ยังไม่มีมติราคา** ห้ามกระจายตัวเลขไปหลายไฟล์ก่อนเคาะ (PRD A-4)
  - **บังคับแล้วตั้งแต่ 2026-08-30 ไม่ใช่แค่เขียนไว้:** `assertInspectionPricingDecided()` โยน `PRICING_NOT_DECIDED` เมื่อ `INSPECTION_PRICING_IS_DRAFT === true` **และ** `NODE_ENV === 'production'` ⇒ ต่อหน้าจอเสร็จแล้ว deploy ก็ยังเก็บเงินด้วยราคาร่างไม่ได้ (dev/เทสยังเดินเส้นทางเก็บเงินได้ครบเพื่อพิสูจน์ว่ามันถูก) — ประโยค "เปิดขายจริงไม่ได้จนกว่าจะมีมติ" ใน PRD เป็น **กฎที่เขียนไว้** ซึ่งบังคับอะไรไม่ได้ด้วยตัวมันเอง และบั๊กจะปรากฏเป็น "เก็บเงินลูกค้าผิดจำนวน" ไม่ใช่ error (`docs/conventions/rule-must-be-enforced-not-described.md`)
  - **D-16 ย้ายฐานต้นทุนไปเป็น "ต่อที่พักหนึ่งหลัง" แล้ว** ⇒ ราคาเหมาต่อร้านที่เป็นร่างอยู่ตอนนี้ **ขาดทุนกับลูกค้ารายใหญ่ที่สุดอย่างเป็นระบบ** โครงสร้างที่ถูกคือ "ฐานต่อร้าน + ต่อหลัง" ซึ่งยังไม่มีมติ ⇒ ตัวคูณจำนวนหลัง**ยังไม่มีในโค้ดโดยเจตนา** (ใส่ค่ามั่วไว้ก่อนจะกลายเป็นตัวเลขที่ถูกอ้างต่อว่าตัดสินแล้ว)
- **A-T5:** เวอร์ชันข้อความเงื่อนไข (`INSPECTION_TERMS_VERSION`) ต้องชี้ไฟล์ที่อ่านย้อนหลังได้จริงเสมอ — **เวอร์ชันที่ย้อนไปอ่านข้อความไม่ได้ = ไม่มีเวอร์ชัน** มีเทส `[blocker]` ยืนยันว่าไฟล์นั้นมีอยู่บนดิสก์ · ถ้อยคำเปลี่ยน = **สร้างไฟล์เวอร์ชันใหม่** ห้ามแก้ไฟล์เดิมในที่เดิมหลังเริ่มใช้จริง (ร่างปัจจุบัน: `TERMS-v1-2026-08-29.md` — ยังไม่ผ่านการตรวจทางกฎหมาย)
