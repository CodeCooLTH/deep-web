...
### 10.2a โครงสร้างต้นทุนที่ราคาต้องครอบ (ร่าง — รอตัวเลขจริงจากทีม)

> ส่วนนี้ไม่ใช่ราคา แต่คือ **สิ่งที่ราคาต้องครอบให้ได้** เขียนไว้เพื่อให้การเคาะราคาใน §10.2 มีฐานคิด ไม่ใช่ตัวเลขที่เดาเอา

**ต้นทุนของแต่ละขั้นไม่ได้โตเท่ากัน และ 3 ใน 4 ขั้นโตตามจำนวนที่พัก ไม่ใช่ตามจำนวนร้าน**

| ขั้น | หน่วยที่ทำให้ต้นทุนโต | งานที่เกิดขึ้นจริงต่อปี |
|---|---|---|
| 1 ตรวจต่อเนื่องอัตโนมัติ | **คงที่ต่อร้าน** | ไม่มีคนทำ (cron) — ต้นทุนส่วนเพิ่มเกือบศูนย์ |
| 2 ตรวจเอกสาร | **ต่อที่พักแต่ละหลัง** | ตรวจเอกสารสิทธิ์ปล่อยเช่า + ใบอนุญาต 1 ครั้ง/หลัง/ปี |
| 3 ตรวจเห็นของจริง | **ต่อที่พักแต่ละหลัง** | วิดีโอคอลนำชม 2 ครั้ง/หลัง/ปี + ตรวจหลักฐานการเปิดจริง 4 ครั้ง/หลัง/ปี |
| 4 ตรวจถึงที่ | **ต่อที่พักแต่ละหลัง** | ผู้ตรวจลงพื้นที่ 1 ครั้ง/หลัง/ปี + ทวนข้อของขั้น 3 อีก 4 ครั้ง/หลัง/ปี |

🛑 **ราคาแบบเหมาต่อร้านจะขาดทุนกับลูกค้ารายใหญ่ที่สุดเสมอ** — ร้านที่มีที่พัก 5 หลังใช้แรงตรวจประมาณ 5 เท่าของร้านหลังเดียว แต่จ่ายเท่ากัน ⇒ ยิ่งขายได้ลูกค้าใหญ่ยิ่งเจ็บ และเป็นความเจ็บที่ไม่มีใครเห็นจนกว่าจะปิดงบ · เกิดจากมติ D-16 (ขอบเขตผลตรวจผูกตามสิ่งที่ข้อนั้นตรวจ) ซึ่งถูกในเชิงความหมาย แต่ย้ายฐานต้นทุนจาก "ต่อร้าน" เป็น "ต่อหลัง" ไปด้วย — **§10.2 ที่ตั้งราคาแบบเหมาต่อร้านจึงยังไม่สอดคล้องกับ D-16**

**โครงสร้างราคาที่สอดคล้องกับต้นทุน (เสนอ):** ค่าฐานต่อร้าน + ค่าต่อที่พักที่ขอให้ตรวจ — ร้านเลือกได้ว่าจะให้ตรวจกี่หลัง (สอดคล้องกับ FR-INS-029 ที่ผลตรวจแยกรายหลังอยู่แล้ว และทำให้ร้านหลังเดียวไม่ต้องอุดหนุนร้าน 10 หลัง)

**ตัวแปรที่ยังไม่มีค่าจริง — ราคาคำนวณไม่ได้จนกว่าจะมี**

| # | ตัวแปร | ใช้กำหนดอะไร |
|---|---|---|
| V-1 | ค่าแรงทีมตรวจภายในต่อชั่วโมง | ราคาขั้น 2 และ 3 |
| V-2 | ค่าจ้างผู้ตรวจท้องถิ่นต่อการลงพื้นที่ 1 ครั้ง (รวมเดินทาง) | ราคาขั้น 4 และค่าแรกเข้า |
| V-3 | ชั่วโมงที่ทีมกันให้งานนี้ได้ต่อเดือน | โควตารับสมัครต่อเดือน (AC-INS-09-1) |
| V-4 | ส่วนต่างที่ถือว่าคุ้ม | ตัวคูณจากต้นทุนเป็นราคา |

**ตัวเลขใน §10.2 เป็นค่าตั้งต้นเพื่อการอภิปรายเท่านั้น** ยังไม่ผ่านการคำนวณจาก V-1..V-4 และยังไม่สะท้อนโครงสร้างต่อหลัง
...
🛑 **"รอตรวจซ้ำ" เกิดจาก "ผ่าน" ที่เกินอายุหรือถูกทำให้เป็นโมฆะเท่านั้น ไม่เกิดจาก "ไม่ผ่าน"** — ข้อที่ไม่ผ่านแล้วร้านแก้ไขและขอตรวจใหม่ ยังคงสถานะ "ไม่ผ่าน" (ไม่มีป้าย) จนกว่าจะมีผลตรวจใหม่ที่ผ่านจริง ระหว่างนั้นฝั่งร้านเห็นว่ามีรอบตรวจรออยู่ ส่วนฝั่งสาธารณะไม่ต่างกันเพราะทั้งสองกรณีไม่มีป้ายอยู่แล้ว
...
flowchart LR
    ...
    D -- ร้านแก้ไขแล้วขอตรวจใหม่ (ผลยังเป็นไม่ผ่านจนกว่าจะตรวจใหม่แล้วผ่าน) --> B
