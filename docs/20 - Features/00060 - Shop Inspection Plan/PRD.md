...
### 3.10 ขอบเขตของผลตรวจ — ผูกกับร้าน หรือผูกกับที่พักรายหลัง
...
**Business Rules:**
- **ขั้นที่ 1 ถึงขั้นที่ 3 ผูกกับร้าน** — ข้อตรวจของขั้นเหล่านี้ตรวจตัวร้านและตัวเจ้าของ (ฐานมิจฉาชีพ ตัวตน เอกสารสิทธิ์ บัญชีรับเงิน) ซึ่งเป็นคุณสมบัติของร้านทั้งร้าน ไม่ใช่ของที่พักหลังใดหลังหนึ่ง
- **ขั้นที่ 4 ผูกกับที่พักรายหลัง** — ผลตรวจ "ตรวจถึงที่" ปรากฏเฉพาะบนที่พักหลังที่ผู้ตรวจไปยืนตรวจจริงเท่านั้น ที่พักหลังอื่นของร้านเดียวกันที่ยังไม่เคยถูกตรวจ ต้องแสดงสถานะ "ยังไม่มีข้อมูล" ไม่ใช่สืบทอดผลตรวจของหลังที่ตรวจแล้ว และไม่ใช่ "ไม่ผ่าน"
- ร้านที่ต้องการให้ที่พักหลายหลังมีผลตรวจขั้นที่ 4 ต้องขอตรวจแยกรายหลัง

**เหตุผล:**
- ขั้นที่ 4 เป็นการยืนยัน **สถานที่** ไม่ใช่การยืนยันร้าน การให้ผลตรวจของหลังหนึ่งครอบไปถึงหลังอื่นคือการออกคำรับรองให้สถานที่ที่ไม่มีใครเคยเห็น ซึ่งขัดกับหลักการข้อแรกของทั้งฟีเจอร์ (ป้ายมาจากผลตรวจ ไม่ได้มาจากการจ่ายเงิน) โดยตรง
...
- ข้อตรวจ "วิดีโอคอลนำชมสด" ของขั้นที่ 3 ตรวจ **สถานที่** เช่นเดียวกับขั้นที่ 4 แต่ตามมติปัจจุบันถูกจัดให้ผูกกับร้าน ⇒ ร้านที่นำชมหลังเดียวจะได้ผลตรวจ "เห็นของจริง" ครอบทุกหลัง ต้องตัดสินว่าจะคงตามนี้ หรือเปลี่ยนหลักการผูกจาก "ตามขั้น" เป็น "ตามสิ่งที่ข้อนั้นตรวจ" (ข้อที่ตรวจตัวร้าน → ผูกร้าน · ข้อที่ตรวจสถานที่ → ผูกรายหลัง)
...
| **ผลตรวจขั้นที่ 4 ครอบเฉพาะที่พักหลังที่ไปตรวจ** | ร้านที่มีที่พักหลายหลังต้องขอตรวจแยกรายหลัง หลังที่ยังไม่ตรวจแสดง "ยังไม่มีข้อมูล" (ดู §3.10) |
...
| D-16 | ขั้นที่ 1-3 ผูกกับร้าน · ขั้นที่ 4 ผูกกับที่พักรายหลัง (หลังที่ยังไม่ตรวจ = "ยังไม่มีข้อมูล") | ล็อก — ดู §3.10 (มีประเด็นค้างเรื่องขั้นที่ 3) |
