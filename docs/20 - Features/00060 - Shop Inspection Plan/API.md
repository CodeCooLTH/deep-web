...
คอลัมน์ `method` เป็นส่วนหนึ่งของคีย์ที่ cron ใช้จัดกลุ่มรอบ (`(shopId, roomId, step, method)` — §3.2 ช) ⇒ ถ้าติดป้ายเป็น `VIDEO_CALL` มันจะถูกจับกลุ่มกับ `video_tour` แล้ว **ลากให้ทั้งกลุ่มต้องโทรวิดีโอทุก 90 วันตามอายุที่สั้นกว่า** ทั้งที่ `video_tour` เองต้องการแค่ 180 วัน = งานผู้ตรวจเพิ่มเท่าตัวโดยไม่ได้ข้อเท็จจริงอะไรเพิ่มเลย
