...
| **`inspection-result.service.ts`** | ผลตรวจ + หลักฐาน + การอ่านฝั่งสาธารณะ: **`recordCheckOutcome()` = ผู้เขียนคนเดียวของตาราง** (ตัดสินเองว่า UPDATE หรือ INSERT — TD-002) · `recordResults` (ผู้ตรวจบันทึกทั้งรอบ, เรียก `recordCheckOutcome` ต่อข้อ) · `invalidateResults` · `getInspectionForPublicProfile` · `getInspectionForOwner` | `prisma`, `lib/inspection/*`, `scam-report.service`, `chat-metrics`/`Shop.chat*` fields, `verification.service` |
| **`inspection-auto-check.service.ts`** | ข้อตรวจอัตโนมัติของขั้นที่ 1: `collectAutoCheckFacts` · `runAutomaticStep1Checks` — 🛑 **แยกออกจาก `inspection-result.service.ts` ตอน implement (2026-09-05)** เพราะมันต้อง import แหล่งข้อมูลภายนอก 3 ตัว (ฐานมิจฉาชีพ · การยืนยันตัวตน · ข้อร้องเรียน) ซึ่งจะลาก dependency เข้าไฟล์ที่ทั้งระบบ import เพื่อ *เขียนผลตรวจ* โดยไม่จำเป็น · กติกา "ผู้เขียนคนเดียว" ยังอยู่ครบเพราะไฟล์นี้เขียนผ่าน `recordCheckOutcome()` เท่านั้น (เทส [blocker] สแกนซอร์สบังคับอยู่) | `prisma`, `lib/inspection/auto-checks.ts`, `scam-report.service`, `verification.service`, `Shop.chat*`, `Order.dispute*` |
...
| **T8 (ที่ส่งมอบจริง 2026-09-05)** | `src/app/api/cron/inspection-lifecycle/route.ts` (5 งานตามลำดับ + `maxDuration=60` + รายการที่ 11 ใน `vercel.json` `"0 16 * * *"`) · `lib/inspection/auto-checks.ts` (ทะเบียนข้อตรวจอัตโนมัติ 6 ข้อ) · `inspection-auto-check.service.ts` | T4, T5, T6 | 🛑 **งานที่ 3 บันทึกผลได้จริง 3 จาก 6 ข้อ** (`scam_db` · `phone_identity` · `complaints`) — อีก 3 ข้อประกาศตัวในทะเบียนว่ายังตัดสินไม่ได้ (`account_age`/`chat_response_speed` = **ยังไม่มีมติเรื่องเกณฑ์** ดู OQ-12 · `duplicate_listing` = **ยังไม่มีตัวตรวจจับ** ดู OQ-13) ห้ามคืน `PASS` ให้ข้อที่ยังไม่มีเกณฑ์/ไม่มีตัวตรวจเด็ดขาด เพราะป้ายพวกนี้เป็นคำรับรองต่อผู้ซื้อ · ข้อที่ยังตัดสินไม่ได้ **ต้องอยู่ในทะเบียนเสมอ ห้ามหายจากลูป** (ข้อที่หายจากลูปคือข้อที่ไม่มีใครรู้ว่ามันไม่เคยถูกตรวจ — ผู้ซื้อเห็น "ยังไม่มีข้อมูล" เหมือนกันทั้งกรณีตรวจแล้วไม่มีข้อมูลและกรณีไม่มีใครเขียนโค้ด) |
...
### TD-020 — ตัวตรวจจับการประกาศที่พักซ้ำข้ามบัญชี (ปิด OQ-13)

ตาราง `RoomImageFingerprint` (migration `20260906120000_room_image_fingerprint`, additive ล้วน)
เก็บ sha256 ของรูปประกาศแต่ละไฟล์ · เขียนโดย `hashPendingRoomImages()` ใน
`src/services/room-image-fingerprint.service.ts` ซึ่ง cron เรียกเป็น **งานแรกของรอบ**

| ข้อตัดสิน | เหตุผลที่เลือกทางนี้ |
|---|---|
| **แฮชรูปของทุกร้านที่มีที่พัก ไม่ใช่เฉพาะร้านที่ซื้อแผน** | ตัวตรวจนี้ทำงานด้วยการเทียบข้ามร้าน ถ้าคลังเทียบมีแต่ร้านที่จ่ายเงิน (กลุ่มที่สุจริตอยู่แล้วโดยนิยาม) ผลจะเป็น "ไม่พบว่าซ้ำ" ทุกครั้งตลอดกาล = ตัวตรวจที่รันทุกคืนโดยไม่มีวันเจออะไร ซึ่ง **แย่กว่าไม่มี** เพราะมันออกคำรับรองด้วย |
| **ผู้ที่ประกาศก่อนคือผู้ที่ผ่าน** (`foreign.firstListedAt < own.firstListedAt` เท่านั้น) | ถ้านับแค่ "มีร้านอื่นถือรูปเดียวกัน" เหยื่อที่ถูกมิจฉาชีพก็อปรูปไปจะได้ `FAIL` พร้อมกับคนก็อป · ใช้ `<=` ก็ผิด เพราะเวลาเท่ากันเป๊ะแปลว่าบอกไม่ได้ว่าใครก่อน และการเดาผิดข้างนี้คือการกล่าวหาร้านที่ไม่ได้ทำ · `firstListedAt` = `Room.createdAt` (เวลาที่ประกาศขึ้นระบบจริง) **ไม่ใช่เวลาที่เราแฮช** ซึ่งขึ้นกับลำดับการทำงานของ cron ล้วน ๆ |
| **หลักฐานฝั่งบวกสรุปได้ทันที · คำกล่าวฝั่งลบต้องครอบคลุมเต็มร้อย** | เจอรูปที่ก็อปมา 1 ใบ = `FAIL` แม้แฮชยังไม่ครบทุกใบ · แต่ "ไม่พบว่าก็อป" ต้องแฮชครบทุกใบก่อน ไม่งั้นเป็น "ยังไม่มีข้อมูล" — สลับลำดับสองข้อนี้แล้วคนก็อปจะรอดได้ด้วยการอัปไฟล์เสียไว้ 1 ใบให้แฮชไม่ผ่าน |
| **ตัดสินทีละหลัง** (`AutoCheckDef` เป็น union `SHOP`/`ROOM` ที่รับข้อเท็จจริงคนละชนิด) | ถ้าทุกข้อรับ `AutoCheckFacts` เหมือนกัน ข้อรายหลังจะถูกตัดสินครั้งเดียวแล้ว fan-out ผลเดียวกันไปทุกหลัง = ออกคำรับรองให้หลังที่ไม่เคยถูกตรวจ (ผิด FR-INS-029) และ `tsc` มองไม่เห็นเลยเพราะพารามิเตอร์ชนิดเดียวกัน |
| bounded ต่อรอบ (60 ไฟล์ / 120MB / ไฟล์ใหญ่เกิน 12MB ข้าม / ลองซ้ำได้ไม่เกิน 3 ครั้ง) | cron มีเพดาน `maxDuration=60` · แถวที่ `sha256=null` แปลว่า "ลองแล้วไม่สำเร็จ" ไม่ใช่ "ยังไม่เคยลอง" · ค่า `remaining` ใน response สูงค้างแปลว่าเพดานต่อรอบต่ำเกินจริง |

**ข้อจำกัดที่รู้ตัว:** เหยื่อที่ลบห้องเดิมแล้วสร้างใหม่จะได้ `Room.createdAt` ใหม่ที่ช้ากว่าคนก็อป
⇒ กลายเป็นฝ่ายที่ถูกตัดสินว่าก็อป · ยังไม่มีทางแก้ในรอบนี้ (ต้องมีตัวตัดสินจากหลักฐานอื่น เช่น
วันที่ในไฟล์ EXIF หรือการยืนยันด้วยคน)
...
| **OQ-12** ✅ **ปิดแล้ว 2026-09-06** | เกณฑ์ผ่านของ `account_age` และ `chat_response_speed` | **มติ user: อายุบัญชี ≥ 30 วัน · อัตราตอบแชท ≥ 80%** (ค่าคงที่ `ACCOUNT_AGE_MIN_DAYS` / `CHAT_RESPONSE_MIN_RATE_PERCENT` ใน `src/lib/inspection/auto-checks.ts` — วางติดกับตัวประเมินเพื่อให้เห็นพร้อมกัน) · เลือก 80 เพราะเป็น **ช่องว่างจริงในข้อมูล prod** (ค่าที่มีอยู่เกาะกันสองกลุ่ม 56/67 กับ 99/100/100/100) ไม่ใช่เลขกลมที่ฟังดูดี · 🛑 **ค่าที่ป้อนเข้าเกณฑ์ต้องผ่าน `resolveChatResponse()` เสมอ** ซึ่งเป็นที่ที่ `CHAT_RESPONSE_MIN_SAMPLE = 3` ถูกบังคับ — อ่าน `Shop.chatResponseRate` ดิบแปลว่าป้ายบอก "ตอบแชทผ่าน" จากบทสนทนาเดียว ขณะที่หน้าร้านสาธารณะของร้านเดียวกันเลือกจะไม่พูดอะไรเลย (HR16) มีเทส `[blocker]` สแกนซอร์สกันไว้ที่ `src/services/__tests__/inspection-chat-response-gate.test.ts` |
| **OQ-13** ✅ **ปิดแล้ว 2026-09-06** | `duplicate_listing` จะตรวจด้วยอะไร | **มติ user: เทียบรูปด้วย sha256 ข้ามร้าน** (ดู TD-020) — ต่างกันแม้บิตเดียว = คนละรูป (นิยามเดียวกับ `MediaAsset.hash` ของ 00051) ไม่ใช่ perceptual hash ⇒ คนก็อปที่ crop/ใส่ลายน้ำใหม่ยังรอด ซึ่งยอมรับได้ในรอบแรกเพราะ false positive มีราคาสูงกว่ามาก (กล่าวหาร้านที่ไม่ได้ทำ) |
