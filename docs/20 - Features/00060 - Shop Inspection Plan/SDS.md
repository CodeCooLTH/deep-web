...
| **OQ-12** | เกณฑ์ผ่านของ `account_age` (อายุบัญชีกี่วัน) และ `chat_response_speed` (ตอบแชทกี่ % / ภายในกี่นาที) | ข้อมูลสองตัวนี้ **มีครบอยู่แล้ว** (`Shop.createdAt` และ `Shop.chatResponseRate` ที่ cron `chat-response-metrics` เขียนให้) ติดแค่ไม่มีมติว่าเส้นอยู่ตรงไหน — ตราบใดที่ยังไม่เคาะ ข้อทั้งสองจะขึ้น "ยังไม่มีข้อมูล" ตลอดไป ⇒ **ร้านที่จ่ายค่าขั้นที่ 1 ได้ป้ายจริง 3 จาก 6 ข้อ** · ห้ามให้ผู้เขียนโค้ดตั้งเส้นเอง เพราะป้ายนี้คือคำรับรองต่อผู้ซื้อ ไม่ใช่ตัวเลขภายใน |
| **OQ-13** | `duplicate_listing` ("ที่พักนี้ไม่ถูกประกาศซ้ำโดยบัญชีอื่นบน Deep") จะตรวจด้วยอะไร | **ยังไม่มีกลไกตรวจจับเลยแม้แต่นิดเดียว** และเป็นข้อเดียวของขั้นที่ 1 ที่ผูกรายหลัง · ตัวเลือกที่มองไว้: เทียบภาพข้ามร้าน (ต้อง hash รูป) · เทียบชื่อ+พิกัด · เทียบข้อความประกาศ — ทั้งสามอย่างเป็นงานคนละขนาดกัน 🛑 ระหว่างที่ยังไม่มี **ห้ามคืน PASS เด็ดขาด** ("ไม่มีตัวตรวจ" กับ "ตรวจแล้วไม่พบว่าซ้ำ" คนละความหมาย และข้อนี้คือข้อที่ผู้ซื้อใช้ดูว่าที่พักถูกมิจฉาชีพ เอาไปประกาศซ้ำหรือเปล่า — PASS ปลอมตรงนี้คือคำรับรองที่อันตรายที่สุดในชุด) |
