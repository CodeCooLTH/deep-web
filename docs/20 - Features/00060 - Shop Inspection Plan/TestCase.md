...
**กลุ่ม K — เกณฑ์ข้อตรวจอัตโนมัติที่เพิ่งเคาะ 2026-09-06 (ปิด OQ-12/OQ-13)**

ไฟล์: `src/lib/inspection/auto-checks.test.ts` · `src/lib/inspection/fingerprint-match.test.ts` ·
`src/services/__tests__/inspection-chat-response-gate.test.ts` ·
`src/services/__tests__/inspection-auto-check.service.test.ts`
**สถานะ: รันแล้วเขียวครบ และพิสูจน์ด้วย mutation ครบทุกแถว (2026-09-06)**

| TC | เคส | Fixture ต้องมี | mutation ที่ต้องทำให้แดง | Expected | Trace |
|---|---|---|---|---|---|
| TC-K1 | อายุบัญชีเท่าเกณฑ์พอดี (30 วัน) | `accountAgeDays: 30` **และ** `29` (สองข้างของเส้น — ค่า 400 กับ 2 จะเขียวต่อไปแม้เส้นขยับ) | `>=` → `>` | 30 = ผ่าน · 29 = ไม่ผ่าน | OQ-12 |
| TC-K2 | อัตราตอบแชทเท่าเกณฑ์พอดี (80%) | `80` และ `79` + ค่าจริงบน prod `56/67/99/100` | เปลี่ยนค่าคงที่ 80 → 70 | 80/99/100 = ผ่าน · 56/67/79 = ไม่ผ่าน | OQ-12 |
| TC-K3 | ตัวอย่างบทสนทนาไม่พอ | `chatResponseSampleSize: 1`, `chatResponseRate: 100` | อ่าน `shop.chatResponseRate` ดิบแทน `resolveChatResponse()` | ไม่บันทึกข้อนี้เลย (ไม่ใช่ "ผ่าน" และไม่ใช่ "ไม่ผ่าน") | OQ-12 · HR16 |
| TC-K4 | เหยื่อที่ถูกก็อปรูป (เราประกาศก่อน) | ของเรา `firstListedAt` ม.ค. · ของร้านอื่น มิ.ย. | ตัดเงื่อนไขเวลาออก (นับทุกร้านที่ถือรูปเดียวกัน) | **ผ่าน** — ห้ามถูกนับว่าก็อป | OQ-13 · TD-020 |
| TC-K5 | เวลาประกาศเท่ากันเป๊ะ | `firstListedAt` เท่ากันทั้งสองฝั่ง | `<` → `<=` | ผ่าน (บอกไม่ได้ว่าใครก่อน ⇒ ห้ามกล่าวหา) | OQ-13 · TD-020 |
| TC-K6 | เจอรูปที่ก็อปมาแต่แฮชยังไม่ครบ | `hashedImageCount: 1`, `totalImageCount: 4`, `copied: 1` | สลับลำดับกฎ (เช็ค "แฮชครบไหม" ก่อน "เจอของก็อปไหม") | **ไม่ผ่าน** — หลักฐานฝั่งบวกสรุปได้ทันที | OQ-13 · TD-020 |
| TC-K7 | ไม่เจอของก็อปแต่แฮชไม่ครบ | `hashedImageCount: 2`, `totalImageCount: 10`, `copied: 0` | ยอมให้ผ่านทั้งที่แฮชไม่ครบ | ยังไม่มีข้อมูล — ห้ามผ่าน | OQ-13 · TD-020 |
| TC-K8 | ห้องที่ยังไม่มีรูปเลย | `totalImageCount: 0` | ลบกฎนี้ออก | ยังไม่มีข้อมูล (ไม่ใช่ "สะอาด") | OQ-13 |
| TC-K9 | ค้นลายนิ้วมือล้มเหลว | `lookupFailed: true` | คืน PASS เมื่อค้นล้ม | ยังไม่มีข้อมูล | OQ-13 |
| TC-K10 | ร้านมี 2 หลัง หลังหนึ่งก็อป อีกหลังสะอาด | `room-clean` / `room-copied` | เขียน `roomId: null` แทน id ของหลังนั้น | ผลต่างกันรายหลัง (`PASS`/`FAIL`) และ `roomId` ตรงกับหลังจริง | FR-INS-029 |
| TC-K11 | ลำดับงานใน cron | mock ทั้ง `hashPendingRoomImages` และ `runAutomaticStep1Checks` | ย้ายงานแฮชรูปไปหลังข้อตรวจอัตโนมัติ | แฮชรูปต้องมาก่อนเสมอ | TD-020 |
| TC-K12 | `kind` ในทะเบียนตรงกับ `scope` ใน SSOT | ทุกคีย์ใน `STEP1_AUTO_CHECK_KEYS` | เปลี่ยน `kind` ของข้อใดข้อหนึ่ง | ตรงกันทุกข้อ | FR-INS-029 |
