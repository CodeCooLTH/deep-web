...
### D-2a — ปิดรูของ D-2 (แก้ 2026-08-31, user เคาะ)

🛑 **D-2 เขียนว่า "ไม่ปลดล็อก 00050 ให้ ONLINE_SALES" แต่ไม่เคยมีใครกันทิศกลับ** — การ์ด/ปุ่ม
"ได้รับเงินแล้ว" ของฟีเจอร์นี้ **ไหลเข้าร้าน `SERVICE_QUEUE`** มาตลอด เพราะเงื่อนไข render มีแค่
`canSellerConfirmPayment(paymentMethod) && status !== 'CANCELLED'` ซึ่งตัดสินจาก *วิธีชำระ*
ไม่ใช่ *ประเภทร้าน* (ด่านของ 00050 กั้นด้วย `vertical` ครบ 4 จุด — ของฟีเจอร์นี้ไม่มีสักจุด)

**วัดจอจริง 2026-08-31** บิลบริการ PENDING ฿900 มัดจำ ฿900:

| อาการ | รายละเอียด |
|---|---|
| ปุ่มซ้ำ | **"รับเงินแล้ว"** (บัญชี `OrderPayment`) + **"ได้รับเงินแล้ว"** (ธง `paymentConfirmedAt`) ในหน้าเดียว |
| การ์ดซ้ำ | ชื่อ **"การชำระเงิน" 2 ใบ** (`PaymentReceivedCard` + `BillingDetails`) |
| ไม่ sync กัน | `recordPayment()` ไม่แตะธง · `setPaymentConfirmed()` ไม่สร้างแถวบัญชี |
| ป้ายโกหก | บิลบริการบัญชี **0 บาท** ขึ้นเขียว "ชำระแล้ว" เพราะ `status === 'CONFIRMED'` |

**มติ:** เจ้าของคำถาม *"ได้เงินหรือยัง"* แยกตามประเภทร้าน

| ประเภทร้าน | เจ้าของ | เหตุผล |
|---|---|---|
| `SERVICE_QUEUE` | **บัญชี `OrderPayment`** (00050) | เก็บ 2 ครั้ง (มัดจำ + ส่วนที่เหลือ) · ธงใบเดียวบอกจำนวนเงินไม่ได้ · หน้า `/o/{token}` ของลูกค้าแสดง "รับแล้ว/ค้าง" จากบัญชีอยู่แล้ว |
| `ONLINE_SALES` | **ธง `paymentConfirmedAt`** (ฟีเจอร์นี้) | จ่ายก้อนเดียว ไม่มีมัดจำ (ตาม §5) |
| `LODGING` | **ธง `paymentConfirmedAt`** | ยังไม่มีร้านจริง (0 ร้าน 0 ออเดอร์ · ตรวจ 2026-08-31) และธงคือกลไกเงินเดียวที่มันมี — **ห้ามตัดทิ้งพร้อม SERVICE_QUEUE** |
| COD (ทุก vertical) | `codReceivedAt` | มีมาก่อน + CHECK `Order_payment_confirm_exclusive_check` กันชนกับธง |

**สิ่งที่บังคับในโค้ด** (ไม่ใช่แค่ประกาศ — `docs/conventions/rule-must-be-enforced-not-described.md`):

1. `OrderDetailClient.tsx` — `!isServiceQueue &&` หน้าเงื่อนไข render `<PaymentReceivedCard>`
   🛑 **เป็น deny-list เฉพาะ `SERVICE_QUEUE` ห้ามเปลี่ยนเป็น allow-list** — vertical ใหม่ในอนาคต
   ต้องได้พฤติกรรมเดิมไว้ก่อน ไม่ใช่ถูกซ่อนเงียบ ๆ
2. `getPaymentBadge()` รับพารามิเตอร์ที่ 5 `money?` — **บัญชีเงินชนะ `Order.status` เสมอเมื่อมีบัญชี**
   ไม่ส่ง = ตรรกะเดิมทุกบรรทัด (ต้องเป็นพารามิเตอร์ของฟังก์ชันเดิม ไม่ใช่ฟังก์ชันที่สอง — HR16)
3. ชุดเงินที่ป้อน **ป้าย** คือ `serviceMoney` (กั้นด้วย vertical อย่างเดียว) ไม่ใช่ `orderMoney`
   (กั้นเพิ่มด้วย `hasMoneyStory()` สำหรับ *บล็อกเงิน*) — ใบที่ยังไม่มีเรื่องเงินให้เล่าคือใบที่ป้ายโกหกพอดี
4. `BillingDetails` เปลี่ยนชื่อการ์ดเป็น **"รายละเอียดการชำระเงิน"** — ใบของฟีเจอร์นี้ยังชื่อ
