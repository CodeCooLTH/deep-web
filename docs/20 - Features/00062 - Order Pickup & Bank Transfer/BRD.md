...
> **สถานะ:** Requirements approved — user เคาะมติ D-1..D-5 ครบแล้ว 2026-08-28 (ดู PRD §4.3 · เหตุผลเต็ม §7.2 ของเอกสารนี้)
> **เจ้าของเอกสาร:** BA
...
4. สร้างความเข้าใจร่วมกันระหว่างทีมธุรกิจและทีมพัฒนา ก่อนเข้าสู่ขั้นตอน SRS/SDS — มติ D-1..D-5 (ดู §7.2) user เคาะครบแล้ว 2026-08-28
...
**Acceptance Criteria:**
- [ ] ออเดอร์ที่ `handedOverAt` ผ่านมาแล้วเกิน grace period (**48 ชั่วโมง — user เคาะแล้ว**) และไม่มีข้อพิพาทค้าง (`disputeOpenedAt` ไม่ว่าง + `disputeResolvedAt` ว่าง) → ระบบตั้งสถานะเป็น `CONFIRMED` อัตโนมัติ
...
**Acceptance Criteria:**
- [ ] ออเดอร์ที่ `paymentMethod ∈ {TRANSFER, PROMPTPAY, CASH}` มีปุ่ม "ได้รับเงินแล้ว" (ขอบเขต user เคาะแล้ว) (mirror ปุ่ม "ได้รับเงินปลายทางแล้ว" ที่มีอยู่แล้วสำหรับ COD)
...
#### FR-BANK-05: QR พร้อมเพย์ที่ฝังยอดเงินของออเดอร์ (มติ D-5)

**User Story:**
> ในฐานะผู้ซื้อ ฉันต้องการสแกน QR แล้วจ่ายได้เลยโดยไม่ต้องพิมพ์เลขบัญชีและยอดเงินเอง เพราะสองจุดนี้คือจุดที่โอนผิดบ่อยที่สุด

**Acceptance Criteria:**
- [ ] ออเดอร์ที่ `paymentMethod ∈ {TRANSFER, PROMPTPAY}` และร้านตั้งหมายเลขพร้อมเพย์ไว้ → หน้า `/o/{token}` แสดง QR พร้อมเพย์ที่**ฝังยอดเงินของออเดอร์ใบนั้น**
- [ ] QR ต้องเป็น payload มาตรฐาน EMVCo ของพร้อมเพย์ (แอปธนาคารไทยสแกนได้ทุกเจ้า) — ยอดเงินอยู่ใน tag ยอดเงิน ไม่ใช่ QR ยอดเปิด
- [ ] ยอดใน QR ต้องตรงกับยอดที่แสดงบนหน้าจอเสมอ — ออเดอร์ถูกแก้ยอดทีหลัง QR ต้องเปลี่ยนตาม (คำนวณจากยอดปัจจุบัน ไม่ใช่ค่าที่เก็บค้างไว้)
- [ ] ร้านไม่ได้ตั้งหมายเลขพร้อมเพย์ → **ไม่แสดง QR เลย** (ไม่แสดง QR ว่าง/QR ที่สแกนไม่ได้) แต่ยังแสดงเลขบัญชีธนาคารตามปกติ
- [ ] QR แสดงต่อผู้ซื้อที่ยังไม่ล็อกอินได้ (guest view) เช่นเดียวกับเลขบัญชี
...
🛑 **ทุกข้อในหัวข้อนี้ user เคาะตามที่เสนอแล้วทั้งหมด 2026-08-28** (ดูตารางมติที่ PRD §4.3) — ข้อความด้านล่างคือเหตุผลที่ใช้ประกอบการตัดสิน เก็บไว้เพื่อให้คนอ่านทีหลังรู้ว่า *ทำไม* ไม่ใช่ข้อเสนอที่ยังเปิดอยู่
