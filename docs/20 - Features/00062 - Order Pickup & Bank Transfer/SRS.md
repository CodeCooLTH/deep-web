...
- **คำอธิบายเชิงเทคนิค:**
  **(A) ฝั่งตั้งค่า** — `PATCH /api/shops/payout` → `shop.service.ts::updateShopPayout(shopId, userId, data)` (sequence เต็มที่ `API.md` §6.1): reauth 2 ทาง (`PASSWORD` ผ่าน `verifyPassword()` จาก `src/lib/password.ts`, หรือ `OTP` ผ่านกลไก OTP เดิม) → ถ้าไม่มีทั้ง `passwordHash` และ `phone` → `throw new PayoutReauthUnavailableError()` (409) → reauth ผ่าน → best-effort `hashIdentifier('BANK_ACCOUNT', accountNo)` (`src/lib/scam-identifier.ts:34`) เทียบกับ `ScamReportIdentifier(type='BANK_ACCOUNT')` (ไม่บล็อก แค่แจ้งเตือนทีมงาน — FR-BANK-04 Should) → `shop.update({ payout*, payoutUpdatedAt: now })`
  🛑 **บันทึกครั้งแรก (ยังไม่เคยมีบัญชี) ไม่ต้อง reauth** — BR-BANK-02 พูดถึง "เปลี่ยน" ไม่ใช่ "ตั้งครั้งแรก" (UX-Design-Spec A6 ยืนยันตรงนี้) — service ต้องเช็ค `shop.payoutUpdatedAt === null` (ยังไม่เคยตั้ง) เพื่อข้าม reauth guard เฉพาะกรณีนี้เท่านั้น

    🛑 **แก้ 2026-08-29:** ฉบับร่างใช้ `payoutBankCode === null && payoutAccountNo === null` เป็นตัวตัดสิน "ตั้งครั้งแรก" ซึ่งเป็นช่องโหว่: ร้านที่ตั้ง**เฉพาะพร้อมเพย์** (ไม่กรอกบัญชีธนาคาร) จะเข้าเงื่อนไขนั้นตลอดไป ⇒ เปลี่ยนหมายเลขพร้อมเพย์กี่ครั้งก็ไม่ต้องยืนยันตัวตนเลย ทั้งที่เป็นปลายทางที่เงินลูกค้าจะไหลไป — ตัวตัดสินที่ถูกคือ `payoutUpdatedAt` (มีค่า = เคยบันทึกมาแล้วครั้งหนึ่ง ไม่ว่าจะกรอกช่องไหน) ซึ่งตรงกับคอมเมนต์ใน `prisma/schema.prisma` และกับโค้ดที่ implement จริง
