...
### 4.3 ข้อเท็จจริงของแพลตฟอร์ม — **ยืนยันแล้วทั้งหมด 2026-08-11**

ที่มา: ซอร์สเอกสารทางการ `github.com/line/line-developers-docs-source`
(`docs/en/reference/messaging-api/index.html.md`) — หน้าเว็บ reference เป็น SPA ที่ดึงตรงไม่ได้
จึงอ่านจากซอร์สแทน ตัวเลขทุกตัวด้านล่างคัดจากที่นั่น ไม่ใช่จากความจำ

| หัวข้อ | ค่า |
|--------|-----|
| ชนิดไฟล์ภาพ | JPEG หรือ PNG |
| ขนาดไฟล์ | **≤1MB** |
| ความกว้าง | 800–2500px |
| ความสูง | ≥250px |
| อัตราส่วน (กว้าง/สูง) | **≥1.45** |
| ขนาดมาตรฐานที่เอกสารใช้เป็นตัวอย่าง | 2500×1686 |
| พื้นที่กดได้ต่อเมนู | **สูงสุด 20 ช่อง** |
| `chatBarText` (คำบนแถบเปิด/ปิดเมนู) | 🛑 **สูงสุด 14 ตัวอักษร** |
| `name` (ชื่อเมนูสำหรับร้านดูเอง) | ≤300 ตัวอักษร |
| จำนวนเมนูต่อ OA | ≤1000 |
| เพดานเรียก `POST /v2/bot/richmenu` | **100 ครั้ง/ชั่วโมง** (ไม่นับที่ทำผ่าน OA Manager) |

**ปลายทาง** (สังเกตว่าอัปโหลดรูปอยู่คนละโฮสต์):
1. `POST https://api.line.me/v2/bot/richmenu` → ได้ `richMenuId`
2. `POST https://api-data.line.me/v2/bot/richmenu/{id}/content` (`Content-Type: image/jpeg`)
3. `POST https://api.line.me/v2/bot/user/all/richmenu/{id}` — ตั้งเป็นเมนูเริ่มต้น
4. ยกเลิก: `DELETE /v2/bot/user/all/richmenu` · ลบเมนู: `DELETE /v2/bot/richmenu/{id}`

**ชนิด action ที่ใช้ได้:** `postback` `message` `uri` `datetimepicker` `camera` `cameraRoll`
`location` `richmenuswitch` `clipboard`

🛑 **ผลที่ตามมาซึ่งกำหนดรูปแบบการ "แก้ไข" เมนู** — อัปโหลดรูปลงเมนูเดิมซ้ำไม่ได้
(`"An image has already been uploaded to the richmenu"`) และไม่มี endpoint สำหรับแก้เมนู ⇒
**เมนูเป็นของที่แก้ไม่ได้ (immutable)** การ "แก้ไข" ทุกครั้งคือ *สร้างใบใหม่ → ตั้ง default ใหม่ →
ลบใบเก่า* ⇒ ระบบต้องจำ id ของใบเก่าไว้เพื่อลบ ไม่งั้นเมนูขยะจะกองสะสมจนชนเพดาน 1000 ใบ
และเพดาน 100 ครั้ง/ชั่วโมงคือเหตุผลที่ **ห้ามยิงสร้างใหม่ทุกครั้งที่ร้านพิมพ์แก้คำ** ต้องสร้างตอนกดบันทึกเท่านั้น

🛑 **`chatBarText` 14 ตัวอักษรคือข้อจำกัดที่ภาษาไทยชนง่ายมาก** — "แตะเพื่อเปิดเมนู" = 16 ตัว
ยาวเกินแล้ว ต้องนับความยาวจริงและกันไว้ที่ฟอร์ม ไม่ใช่ปล่อยให้ LINE ปฏิเสธทีหลัง

**สภาพปัจจุบัน:** OA ทดสอบ `@502sjent` มี **0 เมนู** (`GET /v2/bot/richmenu/list` คืน `{"richmenus":[]}`)

---

## 4.4 มติที่ user เคาะแล้ว (2026-08-11)

| # | มติ | ผล |
|---|-----|-----|
| **D-RM-1** | ชุดปุ่มสำเร็จรูป **แยกตาม `Shop.vertical`** ตามที่เสนอใน BRD FR-RM-01 | ONLINE_SALES / SERVICE_QUEUE / LODGING ได้คนละชุด ใช้คลังคำเดิม (`ORDER_VOCAB`) |
| ~~**D-RM-2**~~ | ~~รอบแรก **ใช้เทมเพลตอย่างเดียว** ร้านอัปโหลดภาพเองไม่ได้~~ | 🛑 **ถูกกลับมติแล้วโดย D-RM-2b (2026-08-11) — อ่านแถวถัดไป** เก็บข้อความเดิมไว้เพื่อให้ตามรอยได้ว่าเคยตัดสินอะไรไว้และเปลี่ยนเพราะอะไร |
| **D-RM-2b** | **ร้านอัปโหลดภาพเมนูของตัวเองได้** และ **เลือกเลย์เอาต์เองได้** (6 แบบตามเทมเพลตมาตรฐานของ LINE) | user เคาะ 2026-08-11 หลังเห็นผลจริงว่าเมนูที่ระบบวาดให้ (พื้นขาว+ตัวหนังสือ) เรียบเกินกว่าที่ร้านคาดหวัง และส่งภาพเมนูของ LINE STICKERS มาเป็น ref · **เฉพาะโหมด "ใช้ภาพของร้านเอง" เท่านั้นที่เลือกเลย์เอาต์ได้** — โหมด "ให้ระบบสร้างให้" ยังล็อกตาม `vertical` เหมือนเดิม เพราะปุ่มที่เพิ่มมาต้องมีปลายทางจริงตาม BR-RM-03 และตอนนี้เรามีปลายทางแค่ 5 แบบ · 🛑 **เลย์เอาต์ต้องเป็นคีย์ที่เลือก ไม่ใช่เดาจากจำนวนช่อง** เพราะ `top-1-bottom-2` กับ `row-3` มี 3 ช่องเท่ากันทั้งคู่ เดาผิดแล้วลูกค้ากดโดนช่องผิดโดยไม่มีอะไรฟ้อง · ไม่ต้อง migration (ใช้ `templateKey` เก็บคีย์) |
| **D-RM-2c** | **ร้านปรับสัดส่วนช่องเองได้** — แต่ละแถวสูงไม่เท่ากันได้ และแต่ละช่องในแถวกว้างไม่เท่ากันได้ (เลย์เอาต์สำเร็จ 6 แบบกลายเป็น "ค่าตั้งต้นที่ปรับต่อได้") | user เคาะ 2026-08-14 · 🛑 **เลือกทางนี้แทน "ลากวาดช่องอิสระ" โดยตั้งใจ** เพราะยังเป็นการปูกระเบื้อง ⇒ รูโหว่และช่องซ้อนกันยังเป็นไปไม่ได้โดยโครงสร้าง และเทส "ผลรวมพื้นที่ = พื้นที่ภาพเป๊ะ" ยังใช้เป็นตาข่ายได้ · การวาดอิสระถูกทดลองแล้วและ **ปฏิเสธ**: LINE ไม่ฟ้องทั้งรูโหว่และช่องซ้อน (เงียบทั้งคู่) และช่องที่ซ้อนกัน LINE จะเลือกช่องแรกในลิสต์เสมอ ⇒ ช่องที่วางทีหลังกดไม่โดนตลอดกาลโดยร้านไม่มีทางรู้ (หลักฐาน: `docs/superpowers/specs/2026-08-14-00045-rich-menu-freeform-areas-prototype.html`) · ไม่ต้อง migration — เลย์เอาต์ที่ตรงเทมเพลตยังเขียนเป็นสตริงเดิมเป๊ะ |
| **D-RM-3** | ปุ่ม "เช็คสถานะพัสดุ" **ให้ระบบตอบเองทันทีด้วย reply** | 🛑 ผูกกฎเหล็ก: ตอบได้เฉพาะใน reply token เท่านั้น **ห้าม fallback ไป push เด็ดขาด** (BR-LINE-18) — ตอบไม่ทัน = เงียบ + ขึ้นในกล่องแชทให้คนตอบต่อ |
