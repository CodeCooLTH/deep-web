...
#### FR-RM-09: 🛑 ระบบตอบสถานะพัสดุเองทันที (มติ D-RM-3)

**User Story:**
> ในฐานะลูกค้า ฉันต้องการกดปุ่มเดียวแล้วรู้เลยว่าของฉันถึงไหน โดยไม่ต้องรอแอดมินว่าง

**Acceptance Criteria:**
- [ ] ลูกค้าแตะปุ่ม → ระบบตอบสถานะพัสดุของออเดอร์ล่าสุดของลูกค้ารายนั้นในร้านนั้น
- [ ] 🛑 **ตอบด้วย reply token เท่านั้น** — ถ้าโทเคนหมดอายุ/ถูกใช้ไปแล้ว **ห้ามส่งด้วย push เด็ดขาด**
      (BR-LINE-18: ระบบห้ามใช้เงินร้านโดยร้านไม่ได้สั่ง) กรณีนั้นให้เงียบ และให้ข้อความของลูกค้า
      ขึ้นในกล่องแชทตามปกติเพื่อให้คนตอบต่อ
- [ ] ไม่พบออเดอร์ของลูกค้ารายนั้น → ตอบข้อความที่บอกทางออกจริง ไม่ใช่ "ไม่พบข้อมูล" เฉย ๆ
- [ ] ลูกค้ามีหลายออเดอร์ที่ยังไม่จบ → ต้องนิยามให้ชัดว่าตอบใบไหน (ล่าสุดที่ยังไม่ปิด) และบอกว่าเป็นใบไหน
- [ ] ข้อความที่ระบบตอบต้องถูกบันทึกในเธรดเหมือนข้อความขาออกอื่น และ **ติดป้ายว่าระบบเป็นผู้ส่ง**
      ไม่ใช่แสดงเหมือนพนักงานพิมพ์เอง
- [ ] สถานะที่ตอบมาจาก `deriveShippingStage()` ตัวเดียวกับที่หน้า `/orders` ใช้ (HR16)

**Business Flow:**
1. ลูกค้าแตะปุ่ม → เกิด postback event พร้อม reply token
2. ระบบหาลูกค้าจากเธรด → หาออเดอร์ล่าสุดที่ยังไม่ปิดของร้านนั้น
3. มีออเดอร์ → ประกอบข้อความสถานะ → ตอบด้วย reply token
4. โทเคนใช้ไม่ได้แล้ว → **หยุด** ไม่ส่งอะไรเลย (ข้อความของลูกค้ายังขึ้นในกล่องแชท)
...
### 7.2 ข้อจำกัดทางเทคนิค (ยืนยันกับเอกสารทางการแล้ว — ดู PRD §4.3)
- 🛑 **มองไม่เห็นและแก้ไขเมนูที่สร้างจาก OA Manager ไม่ได้** และทิศกลับก็จริง — ข้อจำกัดถาวรของ LINE
- 🛑 **เมนูแก้ไขไม่ได้ (immutable)** — อัปโหลดรูปซ้ำลงใบเดิมไม่ได้ และไม่มี endpoint แก้เมนู
  ⇒ ทุกการ "แก้ไข" คือสร้างใบใหม่ → ตั้ง default ใหม่ → ลบใบเก่า (ต้องจำ id เก่าไว้ ไม่งั้นขยะกองถึงเพดาน 1000 ใบ)
- 🛑 **`chatBarText` ยาวได้ 14 ตัวอักษร** — ภาษาไทยชนง่ายมาก ต้องกันที่ฟอร์ม ไม่ใช่ให้ LINE ปฏิเสธทีหลัง
- สร้างเมนูได้ **100 ครั้ง/ชั่วโมง** ⇒ ห้ามยิงสร้างใหม่ระหว่างที่ร้านพิมพ์แก้คำ ต้องสร้างตอนกดบันทึกเท่านั้น
- ภาพ: JPEG/PNG ≤1MB · กว้าง 800–2500 · สูง ≥250 · สัดส่วน ≥1.45 · พื้นที่กดได้ ≤20 ช่อง
- Rich Menu ต้องมีภาพเสมอ ไม่มีโหมดข้อความล้วน
- ส่งไฟล์ภาพต้องผ่าน direct upload เท่านั้น (เพดาน body ของ function)
- อัปโหลดรูปอยู่คนละโฮสต์กับ API อื่น (`api-data.line.me` ไม่ใช่ `api.line.me`)
...
✅ **3 ประเด็นที่เคยค้าง — user เคาะแล้ว 2026-08-11** (ดู PRD §4.4): D-RM-1 แยกชุดปุ่มตาม `vertical` ·
~~D-RM-2 รอบแรกใช้เทมเพลตอย่างเดียว~~ (**กลับมติแล้วโดย D-RM-2b** — ร้านอัปโหลดภาพเองและเลือกเลย์เอาต์ได้ ดู PRD §4.4) · D-RM-3 ปุ่มเช็คสถานะพัสดุให้ระบบตอบเอง
