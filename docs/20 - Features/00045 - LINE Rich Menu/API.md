...
🛑 `templateKey` รับได้ **3 รูป**: คีย์เทมเพลตของระบบ · `custom:<layoutKey>` (ร้านอัปโหลดภาพเอง —
D-RM-2b) · `custom:g:<สเปก>` (ร้านปรับสัดส่วนช่องเอง — D-RM-2c, เช่น `custom:g:2*1|1*2-1`)
· จำนวนสมาชิกใน `buttons` ต้องเท่าจำนวนช่องของเลย์เอาต์นั้น ไม่งั้น `activate` จะตกด้วย
`RICH_MENU_BUTTON_COUNT_MISMATCH` (ตรวจตอนประกอบ payload ไม่ใช่ตอนบันทึกร่าง — ร้านบันทึกร่างที่ยัง
ไม่ครบไว้ก่อนได้)

🛑 **สเปกเลย์เอาต์ที่อ่านไม่ออกหรือเกินเพดาน ต้องถอยไป AUTO เงียบ ๆ ไม่ใช่ 400** — เป็นเจตนา fail-closed
เดียวกับ `custom:<คีย์ที่ไม่รู้จัก>` ที่มีอยู่แล้ว: แถวที่ข้อมูลเพี้ยน (แก้มือ/ของเก่า) ต้องยังเปิดเมนูได้
เพดานที่บังคับด้วย `isValidLayout()`: น้ำหนักจำนวนเต็ม 1–12 · แถว ≤10 · ช่องต่อแถว ≤10 · รวมช่อง ≤20
