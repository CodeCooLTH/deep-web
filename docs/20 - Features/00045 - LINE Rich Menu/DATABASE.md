...
| `templateKey` | String | ไม่ | — | คีย์เทมเพลต **หรือ** โหมด+เลย์เอาต์ที่ร้านเลือก · 🛑 `custom:<layoutKey>` = ร้านอัปโหลดภาพเอง (D-RM-2b) · 🛑 `custom:g:<สเปก>` = ร้านอัปโหลดภาพเอง **และปรับสัดส่วนช่องเอง** (D-RM-2c, 2026-08-14) · ค่าอื่น = เทมเพลตของระบบ (ผูกกับ `Shop.vertical`) — **ไม่มีคอลัมน์แยกโดยตั้งใจ** เพราะคอลัมน์นี้ตอบคำถาม "เมนูนี้ประกอบจากอะไร" อยู่แล้ว เพิ่มคอลัมน์ = ข้อมูลสองที่ที่ต้องคอยให้ตรงกัน (TD-RM-8) · อ่านด้วย `parseTemplateKey()` ซึ่ง fail-closed ถอยไป AUTO เมื่ออ่านไม่ออก |

**รูปแบบ `custom:g:<สเปก>` (D-RM-2c)** — แถวคั่นด้วย `|` แต่ละแถวเขียน `<น้ำหนักความสูง>*<น้ำหนักช่อง คั่นด้วย ->`
เช่น `custom:g:2*1|1*2-1` = แถวบนสูง 2 ส่วนมีช่องเดียว / แถวล่างสูง 1 ส่วน แบ่ง 2:1

🛑 **ไม่ต้อง migration และแถวเดิมบน prod ไม่ขยับ** — เลย์เอาต์ที่ "ตรงกับเทมเพลตสำเร็จพอดี" ยังถูกเขียนเป็น
`custom:grid-2x2` แบบเดิมเป๊ะ ๆ (`encodeCustomTemplateKey()` เช็ค `matchPresetKey()` ก่อนเสมอ)
ถ้าไม่ทำแบบนี้จะเกิด **สตริงสองแบบที่หมายถึงของเดียวกัน** ซึ่งเป็นคลาสปัญหาที่ TD-RM-8 ตั้งใจเลี่ยงมาแต่แรก
