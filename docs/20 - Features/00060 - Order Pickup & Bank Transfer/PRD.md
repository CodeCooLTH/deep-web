...
> **สถานะ:** Draft — รอมติ D-1..D-5 (ดู §4.3) ก่อนเข้า SRS/SDS
...
### 4.3 มติที่ต้องเคาะ (Decisions Required — D-1 ถึง D-5)

หัวข้อนี้คือประเด็นที่ต้องได้รับการยืนยันจาก Controller/ผู้บริหารก่อนเข้าสู่ขั้น SRS/SDS — ดูรายละเอียดเหตุผลแบบเต็มใน BRD §7.2

1. **D-1 — ใครปิดงานนัดรับ**: เสนอ "ร้านกดมอบของแล้ว → auto-confirm หลัง grace period (เสนอ 48 ชม.) เว้นแต่มีข้อพิพาท" (mirror feature 00039) แทนการให้ร้านกด CONFIRMED ตรง ๆ ทันที
2. **D-2 — เก็บ "ได้รับเงินแล้ว" ที่ไหน**: เสนอฟิลด์ใหม่บน `Order` (`paymentConfirmedAt`/`paymentConfirmedByUserId`) mirror `codReceivedAt` แทนการปลดล็อก `OrderPayment` ledger (feature 00050) ให้ `ONLINE_SALES`
3. **D-3 — ใช้ `fulfillmentMode = 'PICKUP'` ค่าเดิมหรือค่าใหม่**: เสนอใช้ค่าเดิม (ที่ปัจจุบันเขียนโดย booking เท่านั้น) เพราะโค้ดที่อ่านค่านี้ (action-set) ปฏิบัติกับมันถูกต้องอยู่แล้ว
4. **D-4 — ความสัมพันธ์กับ `salesChannel = STOREFRONT`**: เสนอแยกเป็นคนละแกน — เพิ่มตัวเลือก "นัดรับ" อิสระจากช่องทางขาย ไม่ผูกกับ STOREFRONT
5. **D-5 — บัญชีรับเงินของร้าน**: เสนอเก็บเป็นฟิลด์เดี่ยวบน `Shop` (ไม่ใช่ตารางใหม่) + snapshot ลงออเดอร์ตอนสร้าง + แสดงผลใน guest view
...
- Grace period ของการปิดงานนัดรับอัตโนมัติ เสนอเริ่มต้นที่ **48 ชั่วโมง** (สั้นกว่า 7 วันของพัสดุ เพราะเป็นการรับมอบต่อหน้า ไม่ต้องรอขนส่ง) — ตัวเลขจริงต้องได้รับการยืนยันจาก user (ส่วนหนึ่งของมติ D-1)
- ปุ่ม "ได้รับเงินแล้ว" ครอบคลุมทั้ง `TRANSFER`, `PROMPTPAY`, และ `CASH` (ไม่ใช่แค่โอนเงินตามคำขอเดิม) เพราะเป็นกลุ่ม "ร้านต้องยืนยันเอง ไม่มีบุคคลที่สามยืนยันแทน" เดียวกันทั้งหมด และ `CASH` ไม่มีกลไกยืนยันใด ๆ อยู่เลยในปัจจุบัน (ต่างจาก `COD` ที่มี `codReceivedAt`) — ถ้าไม่เห็นด้วยให้ตัดเหลือเฉพาะ `TRANSFER`/`PROMPTPAY` ตามคำขอเดิม
...
สำหรับ technical specification ดู SRS ของโมดูลนี้ (ยังไม่จัดทำ — รอมติ §4.3)
