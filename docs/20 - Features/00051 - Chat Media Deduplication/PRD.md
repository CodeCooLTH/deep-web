...
ฟีเจอร์นี้แก้ที่ **ต้นตอเดียว** คือใส่ content-addressed deduplication (hash เนื้อไฟล์ → ตรวจ hit ก่อนเขียน) ที่ choke point `saveMirroredBuffer` ขอบเขตต่อร้าน (per-shop) ตามข้อมูลจริงที่ไม่มีการซ้ำข้ามร้าน เสริมด้วยชั้น cache สำหรับรูปโฆษณาที่ทราบ ad ID ล่วงหน้า (ข้ามการดาวน์โหลดซ้ำได้ทั้งหมด) และงาน backfill แยกต่างหาก (dry-run ก่อน, resumable, ลบไฟล์เป็นขั้นสุดท้าย) เพื่อทวงพื้นที่ที่ซ้ำอยู่แล้ว 4.3 GB คืน
...
- สมมติว่าไม่มีจุดอื่นในระบบ (นอกเหนือ `saveMirroredBuffer`) ที่เขียนไฟล์ลง storage โดยไม่ผ่าน choke point นี้สำหรับสื่อที่ mirror มาจากภายนอก — อิงจาก comment ในโค้ดที่ระบุชัดว่าเป็นจุดเดียว แต่ยังไม่ได้ทำการ audit ทั้ง repo อย่างละเอียดในชั้น PRD (ควรยืนยันซ้ำในชั้น SRS)
