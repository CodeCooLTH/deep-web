...
| กฎ | คำอธิบาย |
|----|----------|
| **นิยาม "ขายแล้ว"** | `OrderItem` ของ `Order.status != 'CANCELLED'` — เหมือน `getBestSellerProducts()` ไม่เหมือนหน้ากำไร/ขาดทุน · ณ วันนี้ครอบ **PENDING + CONFIRMED + SHIPPED + RETURNED** |
| **ออเดอร์ที่ถูกคืนของ (`RETURNED`) นับเป็นยอดขาย** | ตั้งใจ — เกณฑ์ผูกกับ "ไม่ยกเลิก" ไม่ใช่รายการสถานะ และต้องเท่ากับ `getBestSellerProducts()` เป๊ะ · **ยังไม่เคยมีใครตัดสินว่าควรกันออกไหม** (สถานะนี้เพิ่งมีจาก feature 00056 หลัง `getBestSellerProducts` ถูกเขียน) — ดู §5 |
...
| **ตัดสินว่าออเดอร์ที่ถูกคืนของควรนับเป็นยอดขายไหม** | ค้างเป็นคำถามเปิด — ตอนนี้ **นับ** ตามเกณฑ์ที่ยึด `getBestSellerProducts()` ถ้าจะกันออกต้องแก้ทั้งสองที่พร้อมกัน ไม่งั้นตัวเลขสองหน้าจะขัดกันเอง (HR16) |
...
|-----------|---------|-------------|
| ปริมาณ `OrderItem` ที่ต้องดึงมา bucket ใน JS โตตามจำนวนออเดอร์ | หน้าช้าลงเมื่อร้านโตขึ้น | เส้นทาง query ใช้ index ที่มีอยู่แล้วครบ (`Order_shopId_status_createdAt_idx` → `OrderItem_orderId_idx`) · เพดาน `MAX_ITEM_ROWS` 20,000 แถว + ธง `truncated` · เกณฑ์ย้ายไปคำนวณฝั่ง DB ~1,500 ออเดอร์/เดือน (prod วันนี้ ร้านใหญ่สุด ~421) — 🛑 **ยังวัดเวลาจริงที่ขนาด prod ไม่ได้** ฐาน dev มีร้าน 8,888 ออเดอร์แต่ไม่มี `OrderItem` เลยสักแถว |
| ~~`OrderItem` ไม่มี index บน `productId`~~ | **ไม่กระทบรายงานนี้** (แก้ 2026-08-29) | รายงานจัดกลุ่มตาม `productId` ใน JS ไม่ใช่ SQL ⇒ index นี้ไม่ถูกใช้ไม่ว่าจะมีหรือไม่ · เป็นความเสี่ยงของ *วันที่ย้ายไป `GROUP BY`* เท่านั้น |
| payload อนุกรมรายวันของทุกสินค้าส่งลง client | ขนาดหน้าโตเมื่อร้านมีสินค้าเยอะ | **วัดแล้ว 2026-08-29:** ร้าน 300 สินค้า/ขายได้จริง 25 ตัว = **122 KB** (แบบเต็มจะเป็น 156 KB ⇒ การย่อประหยัด 22% ไม่ใช่ "หลายเท่า" อย่างที่ประมาณไว้ตอนออกแบบ และ **แย่กว่า 23%** ถ้าสินค้าขายเกือบทุกวัน) — ตัวเลขรวม 120–160 KB ทั้งสองแบบ ต่ำกว่าหน้า `/sales` ที่ส่ง 500–800 KB อยู่แล้ว ⇒ **ไม่ใช่ความเสี่ยงจริง** |
