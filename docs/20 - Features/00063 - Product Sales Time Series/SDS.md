...
## ส่วนขยาย 2026-08-30 — รื้อหน้าจอมือถือ

ที่มา: user เปิดของจริงบน iPhone แล้วชี้ 3 ข้อ — **"ข้อความด้านบนเยอะไป · วันที่กดยาก ·
padding เยอะไป"** แล้วสั่งให้ทำม็อกอัพเทียบกัน 4 รอบจนเลือกได้ (โครง A · หัวการ์ดแบบ 4 ·
แถวแบบ ต้น/กลาง/ปลายเดือน) · ม็อกอัพทั้งหมดอยู่ที่ `docs/superpowers/specs/2026-08-30-*`

### สาเหตุที่วัดได้ (ไม่ใช่ความรู้สึก)

ก่อนแก้ ผู้ใช้ต้องเลื่อนผ่าน **7 ชั้น ≈ 805px** ก่อนเห็นสินค้าชิ้นแรก บนจอที่มองเห็นจริง ~660px
⇒ หน้าที่ชื่อ "ยอดขายรายสินค้า" ไม่แสดงสินค้าเลยในจอแรก

🛑 **ตัวการไม่ใช่ค่า padding** — ไล่ทุกจุดแล้วไม่มีที่ไหนใช้ค่านอกบันไดของธีม (`.card-header`
20/15px · `.card-body` 20px คือโทเคนที่การ์ดทุกใบในหลังบ้านใช้เหมือนกัน) ตัวที่กินพื้นที่จริงคือ
**จำนวนแถวที่บังคับความสูง 44px ซ้อนกัน** (ช่องติ๊ก 2 ตัว = 92px สำหรับตัวกรอง 2 อัน)

### สิ่งที่เปลี่ยน

| # | เปลี่ยน | เหตุผลที่ยึด |
|---|---|---|
| 1 | ซ่อน `<h4>` ชื่อหน้าต่ำกว่า `md` (prop ใหม่ `hideTitleBelowMd` ของ `PageBreadcrumb`, default `false`) | `SellerMobileHeader` พิมพ์คำเดียวกันเป๊ะห่างกัน 40px · **ผูกเทส `[blocker]`** ใน `getSellerPageTitle.test.ts` เพราะข้อสมมตินี้อยู่คนละไฟล์ — วันที่มีคนถอด route ออกจากเมนู หัวจะกลายเป็น `Deep ผู้ขาย` เงียบ ๆ แล้วหน้าจะไม่มีชื่อเหลือเลย |
| 2 | แถบเครื่องมือ **sticky** (เดือน + หน่วย) ใต้หัวแอป | user เลือกโครง A · `top-[calc(4.25rem+env(safe-area-inset-top))]` — ตัวเลขตายตัวใช้ไม่ได้เพราะ `env()` เป็นค่า runtime ที่ต่างกันตามเครื่อง · `z-20` ไม่ใช่ `z-10` เพราะ **`.btn` ของธีมตั้ง `z-index:10` ให้ทุกปุ่มอยู่แล้ว** (เทส `[blocker]` `paces-sticky-z-index.test.ts` จับได้จริงตอนเขียน) |
| 3 | ป้ายเดือน **กดได้** → `MonthPickerSheet` (12 ช่อง เดินปี) | ปุ่ม `‹ ›` ผ่านเกณฑ์นิ้ว 44px อยู่แล้ว — ตัวที่กดไม่ได้คือป้ายกลาง ⇒ ย้อนไป ก.พ. ต้องกด `‹` **หกครั้ง** · 🛑 **ไม่ใช้ `<select>`** — ทำแล้ว user ปฏิเสธ ("input เลือกเดือนมันแปลกๆ") · ขอบมี **2 ด้าน** ทั้ง `maxSelectableMonth` และ `MIN_MONTH_ISO` · ทุกเดือนเป็น `<Link>` ไม่ใช่ `router.push` (รักษาหลักการเดิมของ `MonthSwitcher`) |
| 4 | ช่องติ๊ก 2 ตัว → **ดรอปดาวน์เดียวมุมขวาของหัวการ์ด** (มุมมอง + เรียงลำดับ) เฉพาะ `<lg` | user ทักว่า "มันแปลกๆ" — ต้นเหตุคือ **สองสวิตช์ทำงานคนละทิศแต่แต่งตัวเหมือนกัน**: ตัวหนึ่ง *กรองให้แคบลง* (35→3) อีกตัว *เพิ่มของเข้ามา* (35→37) · **เดสก์ท็อปคงช่องติ๊กเดิม** เพราะตารางมีการเรียงในหัวคอลัมน์อยู่แล้ว เมนู "เรียงตาม" จะเป็นตัวควบคุมซ้ำ |
| 5 | **มือถือเรียงลำดับได้เป็นครั้งแรก** (`sortProductRows`, 5 แบบ) | เดิมล็อกที่ "ยอดมากไปน้อย" มาตั้งแต่วันแรก ⇒ คำถาม "ตัวไหนเงียบที่สุด" ตอบไม่ได้บนอุปกรณ์ที่หน้านี้ออกแบบมาเพื่อมันโดยเฉพาะ |
| 6 | สวิตช์ "รวมสินค้าที่ยังไม่ขายเดือนนี้" ย้ายไป **ท้ายรายการ** | มันคือ "เพิ่มของเข้ามา" ผลของมันอยู่ตรงนั้นพอดี ⇒ แยกร่างจากตัวที่กรอง อ่านสลับกันไม่ได้อีก |
| 7 | **ตัด `DayStrip` ออกจากแถวมือถือ** → `MonthThirdsBar` | user สั่ง "ตัด chart ยาวๆ ในแต่ละ card ออก" · วัดจริง: แถบเดิมได้ช่องละ **~5.4px ที่ 360px** (~4.3px ที่ 320px) — คอมเมนต์เดิมที่เขียนว่า ~7px **สูงไป 30%** · แถบ 31 ช่องยังอยู่ที่ตารางเดสก์ท็อปและในชีต |
| 8 | บรรทัด "ข้อมูล ณ …" ย้ายไป **ท้ายหน้า** | เป็นข้อควรรู้ของตัวเลขที่ดูจบแล้ว ไม่ใช่สิ่งที่ต้องรู้ก่อนดู · **ย้ายที่ ไม่ได้ตัดทิ้ง** |
| 9 | `SALES_BASIS_NOTE` ย้ายมา **ใต้กราฟ แบบเห็นตลอด** | 🛑 ม็อกอัพเคยยุบไว้หลังปุ่ม `i` — **ผิด HR16** เพราะคอมเมนต์ของค่านั้นเขียนกำกับตัวเองว่า "ต้องโผล่ใต้กราฟเสมอ" (ระบบมีนิยามยอดขายหลายชุด ตัวเลขไม่ตรงกับหน้าอื่นโดยไม่มีคำอธิบาย = ร้านเลิกเชื่อทั้งหน้า) ⇒ **ถอดปุ่ม `i` ทิ้ง** |
| 10 | การ์ด **เต็มขอบจอบนมือถือ** (`-mx-4 md:mx-0`) | user ทัก — และเป็นแบบแผนที่มีอยู่แล้ว (`OrdersList` `ProductsListing` `CommandCenter`) หน้านี้เป็นหน้าเดียวที่ไม่ได้ทำ 🛑 **ไม่ยกคลาส `*-fullbleed` มาด้วย** เพราะ marker นั้นพ่วง `margin-top:0` + ซ่อน footer ซึ่งสองหน้านั้นต้องการเพราะ **แทนที่หัวแอปด้วยหัวของตัวเอง** (`SellerMobileHeader` คืน `null` ให้พวกเขา) — หน้านี้ยังใช้หัวแอปปกติ |
| 11 | แก้บั๊ก `loading.tsx` ที่ค้างจาก 2026-08-29 | ตอนเอากราฟขึ้นมือถือ ลืมถอด `hidden md:block` ของโครงกราฟ ⇒ มือถือเห็นโครงตารางก่อน แล้วการ์ดกราฟ ~300px โผล่มาดันทุกอย่างลง = จอกระตุก ซึ่งเป็นสิ่งที่ skeleton มีไว้กันโดยเฉพาะ (จับได้โดย `/impeccable audit`) |

### TD-010: การแบ่ง ต้น/กลาง/ปลายเดือน ใช้ 1–10 · 11–20 · 21–สิ้นเดือน

- **ทางเลือก:** หารสามส่วนเท่ากัน (31÷3 = 10.33) vs แบ่งตามที่คนไทยพูด
- **เลือก:** แบ่งตามภาษา เพราะตัวเลขนี้มีไว้ให้ร้านเทียบกับสิ่งที่เขารู้อยู่แล้ว — "ต้นเดือน"
  ในหัวคนไทยคือช่วงเงินเดือนออก = วันที่ 1–10 ไม่ใช่ 1–10.33
- **ผลข้างเคียงที่ยอมรับ:** ช่วงท้ายยาว 8–11 วันแล้วแต่เดือน ⇒ **ห้ามเทียบผลรวมข้ามช่วงตรง ๆ
  โดยไม่ดูจำนวนวัน** — `monthThirds()` จึงคืน `totalDays`/`elapsedDays` มาด้วยทุกช่วง

### TD-011: ช่วงที่ยังมาไม่ครบต้องแยกจาก "ขายไม่ได้"

🛑 วันที่ 30 ส.ค. ช่วง "ปลายเดือน" มีข้อมูลจริง 10 จาก 11 วัน — ถ้าวาดแท่งเทียบกับช่วงอื่นตรง ๆ
ผู้ขายอ่านว่า "ปลายเดือนขายตก" ทั้งที่ **ยังไม่เกิด** (`partial-data-must-be-labeled-or-filled.md`)

- `MonthThird.complete` / `.notStarted` แยกสองสถานะนี้ออกจาก `sum === 0`
- แท่งของช่วงที่ยังไม่ครบได้ **ขอบประ** · ช่วงที่ยังไม่ถึงแสดงป้าย `—` ไม่ใช่ `0`
- ประโยคเตือนโผล่ **ครั้งเดียวเหนือรายการ** ไม่ใช่ทุกแถว (เป็นคุณสมบัติของเดือน ไม่ใช่ของสินค้า)
- เทส `[blocker]` 7 ข้อ + mutation ("`complete` เป็น true เสมอ" → แดง)

### TD-012: สีของแถบ 3 ท่อน — ความสูงแบกความหมาย ไม่ใช่ความเข้ม

ม็อกอัพร่างแรกใช้ `opacity` ไล่ระดับแยกช่วง — **ผิดซ้ำรอย `DayStrip`** ซึ่งเคยทำแบบนั้นแล้ว
วัดได้ **1.42:1** ตกเกณฑ์กราฟิก 3:1 และต้องแก้ไปเมื่อ 2026-08-29 ⇒ ที่นี่ทั้งสามท่อนเป็น
`bg-primary` **ทึบสีเดียวกันหมด** แยกด้วยช่องว่างและความสูง (WCAG 1.4.1 — ไม่พึ่งสีอย่างเดียว)

### หนี้ที่ยังเปิด

- 🛑 **ยังไม่เคยเปิดหน้าจริงสักครั้ง** (เวิร์กทรีไม่มี `.env`) — ทุกตัวเลขมาจากการอ่านโค้ดและคำนวณ
- `/impeccable audit` รอบ 2026-08-30 ยังเหลือ P1 ที่ไม่ได้แก้ในรอบนี้: เรียงคอลัมน์ตารางด้วย
  คีย์บอร์ดไม่ได้ (`DataTable` ใช้ร่วมทั้งโปรเจกต์ ⇒ ต้องเป็นคอมมิตของตัวเอง) · สีแท่งกราฟ
  ตกเกณฑ์ 4/6 ตัวบนพื้นขาว (**โหมดมืดผ่านหมด** · เป็นโทนที่ user เคาะเอง + โทเคนใช้ร่วมกับ
  กราฟหน้าอื่น ⇒ ต้องถามก่อนแก้) · รูปสินค้าโหลดขนาดเต็มมาแสดงที่ 44px ทั้งที่ `variantUrlOf`
  มีอยู่ · รายการมือถือไม่มีเพดานแถว
- `TestCase.md` ยังไม่มีเคสของแถบ 3 ท่อน / ชีตเลือกเดือน / การเรียงลำดับบนมือถือ
