...
|-----------|--------|----------------|
| **ทุกคนที่เข้าหน้า `/customers` ได้ (`OWNER`/`ADMIN`)** | ตัดสินใจว่าลูกค้า 2 แถวเป็นคนเดียวกันจริงหรือไม่ | เพิ่มเบอร์ + รวมลูกค้าได้เต็มสิทธิ์ (มติ OD-8 รอบสอง 2026-08-10) |
| ~~พนักงานร้าน (STAFF)~~ | — | 🛑 **กลุ่มนี้ไม่มีอยู่จริงในระบบ** — `ShopMember.role` มีแค่ `OWNER`/`ADMIN` (`prisma/schema.prisma:1062`) คนที่ถูกเชิญเข้าร้านได้ `ADMIN` ไปเลย แถวนี้เคยถูกเขียนไว้จากข้อสันนิษฐานที่ผิด และเป็นเหตุให้มติ OD-8 รอบแรกกลายเป็น no-op |
...
- ไม่มีการจำกัดสิทธิ์เพิ่มจากที่หน้า `/customers` อนุญาตอยู่แล้ว (มติ OD-8 รอบสอง — Role Permission เป็นฟีเจอร์แยกที่ user จะทำตามมา)
...
- ไม่มีชั้นสิทธิ์มาช่วยกรอง (มติ OD-8 รอบสอง) — ด่านกันรวมผิดคนอยู่ที่หน้าจอยืนยัน + `CustomerMergeLog` เท่านั้น
...
| **BR-CM-41** | 🛑 **แก้มติ 2026-08-10 (รอบสอง):** ไม่มีเงื่อนไขสิทธิ์ใหม่สำหรับการรวมลูกค้า — ใครที่เข้าหน้า `/customers` ได้ (ผ่าน `requireActiveShop`) กดรวมได้ทั้งหมด. มติรอบแรก ("เฉพาะ OWNER/ADMIN") ตั้งอยู่บนความเข้าใจว่ามี role `STAFF` ซึ่ง **ไม่มีอยู่จริงในระบบ** จึงเป็นเงื่อนไขที่เป็นจริงเสมอ = ไม่ได้กันใครเลย. user: "ตอนนี้ให้ทุกคนทำได้ก่อนครับ ผมจะทำ Role Permission ตามมาทีหลัง" |
