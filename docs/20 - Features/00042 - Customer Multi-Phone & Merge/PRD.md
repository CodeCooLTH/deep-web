...
Open Decisions ทั้ง 8 ข้อ (§4.3) **ถูกเคาะแล้ว 2026-08-10** — user เลือกตัวเลือก (a) ทั้งหมดตามที่ PM เสนอ รวมถึงข้อที่กระทบผู้ใช้ชัดที่สุดคือการรวม**ย้อนกลับไม่ได้** (OD-2)

🛑 **OD-8 ถูกแก้มติ 2026-08-10 (รอบสอง)** หลังพบว่าข้อสันนิษฐานที่ใช้ตัดสินใจรอบแรกไม่ตรงกับโค้ด: ระบบ**ไม่มี role `STAFF` อยู่จริง** — `ShopMember.role` มีแค่ `OWNER` กับ `ADMIN` (`prisma/schema.prisma:1062`, `src/lib/shop-context.ts:76`) คนที่ถูกเชิญเข้าร้านได้ `ADMIN` ไปเลย ⇒ มติเดิม "เฉพาะ OWNER/ADMIN" จึงไม่ได้จำกัดใครเลยสักคน. user เคาะใหม่: **ให้ทุกคนที่เข้าหน้า `/customers` ได้กดรวมได้ไปก่อน แล้วค่อยทำ Role Permission เป็นฟีเจอร์แยกตามมาทีหลัง**
...
### 2.2 สมาชิกร้านที่ถูกเชิญ (ShopMember role=ADMIN)

> 🛑 แก้ 2026-08-10: หัวข้อนี้เดิมเขียนว่า "พนักงานร้าน (Shop Staff)" ซึ่งเป็นกลุ่มที่**ไม่มีอยู่จริงในระบบ** — ไม่มี role `STAFF` ที่ไหนเลย คนที่ถูกเชิญเข้าร้านได้ `ADMIN` เท่ากับเจ้าของในเชิงสิทธิ์ทุกประการ (ต่างกันแค่ `OWNER` มีแถวเดียวเสมอ). ข้อความด้านล่างปรับให้ตรงกับความจริงแล้ว
...
**จุดปวด (Pain Points):**
- กด "รวมลูกค้า" ได้เท่าเจ้าของร้านทุกประการ (มติ OD-8 รอบสอง) — ความเสี่ยง "รวมผิดคนแล้วย้อนไม่ได้" จึงต้องพึ่งด่านยืนยันบนหน้าจอ + `CustomerMergeLog` ล้วน ๆ ไม่มีชั้นสิทธิ์มาช่วยกรองจนกว่าจะมีฟีเจอร์ Role Permission
...
| **OD-8** | สิทธิ์ที่กดรวมได้ | (a) เฉพาะ `OWNER` (b) ทุกคนที่เข้าหน้า `/customers` ได้ (`OWNER`+`ADMIN`) | มติ **(b)** — เคาะรอบสอง 2026-08-10 หลังพบว่า **ไม่มี role `STAFF` ในระบบ** ตัวเลือกที่ตั้งไว้รอบแรกจึงเทียบของที่ไม่มีอยู่จริง (มติเดิม "OWNER/ADMIN" = ไม่จำกัดใครเลย). user: "ตอนนี้ให้ทุกคนทำได้ก่อนครับ ผมจะทำ Role Permission ตามมาทีหลัง" ⇒ ด่านกันรวมผิดคนอยู่ที่ **หน้าจอยืนยัน + audit log** เท่านั้นในเฟสนี้ |
...
- สิทธิ์การเข้าถึงหน้า `/customers` วันนี้ไม่มี role gate พิเศษ (แค่ `requireActiveShop`) — มติ OD-8 รอบสองคือ **ไม่เพิ่ม gate ใหม่** จึงไม่มีงานส่วนนี้ในเฟสนี้ (Role Permission เป็นฟีเจอร์แยกที่ user จะทำตามมา)
