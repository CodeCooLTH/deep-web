...
**Acceptance Criteria**
- AC-04-1 ทุกแถวในรายการมีตัวบอกร้าน และอ่านชื่อร้านเต็มได้
- AC-04-2 หน้าเธรดบอกชื่อร้านที่กำลังตอบในนามนั้น โดยเห็นได้ก่อนเริ่มพิมพ์
- AC-04-3 **ทุกร้านแยกออกจากกันได้ด้วยชื่อเสมอ ไม่ต้องพึ่งโลโก้**

  > แก้ถ้อยคำ 2026-08-08 (เดิม: "ร้านที่ไม่มีโลโก้ต้องยังแยกออกได้ด้วยตัวอักษรย่อ") — ข้อเดิมตั้งอยู่บนสมมติฐานว่าตัวบอกร้านเป็น **รูป** จึงต้องมีทางถอยเมื่อไม่มีรูป. บนข้อมูลจริงพบว่าร้านตั้งโลโก้ร้าน = โลโก้เพจ เป็นเรื่องปกติ ทำให้ badge รูปซ้ำกับ badge ช่องทางที่มีอยู่แล้ว (user report prod: "ใช้ยาก เพราะจริง ๆ profile + icon เพจ อยู่แล้ว") จึงเปลี่ยนเป็น **ป้ายข้อความชื่อร้าน** ซึ่งไม่มีวันซ้ำโดยไม่ตั้งใจและอ่านได้เต็มกว่าตัวย่อ — ข้อกำหนดจึงยกระดับขึ้น ไม่ใช่ลดลง
...
- AC-06-2 กดสร้างจากหน้ารายการ (ไม่มีเธรดเปิด) ในโหมดรวมที่มีหลายร้าน → ต้องเลือกร้านก่อน โดยร้านที่กำลังใช้งานเป็นค่าตั้งต้น
  - 🛑 **สถานะ: เลื่อนออกไปโดยตั้งใจ — ยังไม่มีของให้ทดสอบ** (ตรวจ 2026-08-12). `SRS.md` §ข้อจำกัดที่รู้ตัว ข้อ 4 เขียนไว้ว่า *"ไม่มีปุ่มสร้างรายการจากหน้ารายการในรอบนี้ — โมดัลร่างผูกกับ `conversationId` ตามสถาปัตยกรรมเดิม ... เลื่อนไปรอบถัดไป"* แต่ AC ข้อนี้ไม่เคยถูกทำเครื่องหมายว่าเลื่อน ⇒ คนที่อ่าน BRD อย่างเดียวจะนับว่าผ่านแล้ว. ปัจจุบัน `useDraftOrders().openDraft` ต้องมี `conversationId` เสมอ จึงไม่มีทางเข้าที่ไม่มีเธรด และไม่มีตัวเลือกร้านให้เลือก
...
- AC-06-6 บันทึกรายการที่ร้านของร่างไม่ตรงกับร้านของเธรด → **ปฏิเสธ** ไม่ใช่บันทึกเข้าร้านใดร้านหนึ่ง
  - 🛑 **สถานะ: ไม่เคยถูก implement จนถึง 2026-08-11** — ข้อนี้เขียนไว้ครบทั้งที่นี่ `PRD.md` และคอมเมนต์ใน `DraftOrderProvider.tsx:48` แต่ฟอร์มรับ prop `shopId` มาแล้วโยนทิ้ง (`shopId: _shopId`) และ `POST /api/orders` ไม่มีที่ให้ใส่ ⇒ ออเดอร์ที่คีย์จากเธรดของอีกร้าน **ลงร้านที่ active แทน** (มีนัด → 404 · เลือกจากแคตตาล็อก → 400 · **พิมพ์รายการเอง → 201 เข้าร้านผิดถาวรเงียบ ๆ**) ปิดแล้วที่ `fb65e249` ด้วย `requireShopForRequest()` + `409 DRAFT_SHOP_MISMATCH` พร้อมเทส `[blocker]` — ดู `API.md` §2.1 และบทเรียน `docs/conventions/rule-must-be-enforced-not-described.md`
...
- AC-09-2 จำนวนคำสั่งที่ระบบต้องทำเพื่อแสดงรายการในโหมดร้านเดียวไม่เพิ่มขึ้น
  - ⚠️ **สถานะ: ไม่ผ่านตามตัวอักษร ผ่านตามเจตนา** (ตรวจ 2026-08-12). `resolveChatScope()` เพิ่ม `prisma.user.findUnique({ select: { chatScopeMode } })` **อีก 1 query ต่อคำขอ ทุกโหมด** — จำนวนคำสั่งจึงเพิ่มขึ้นจริง. คอมเมนต์ใน `chat-scope.ts:70` ตอบคนละคำถาม (เรื่องเวลา ไม่ใช่จำนวน) โดยบอกว่ายิงขนานกับ `resolveActiveShopContext` จึงไม่ยืดเวลารอ และแลกมาเพื่อไม่ให้ค่าค้างใน JWT จนผู้ใช้สลับโหมดแล้วไม่เห็นผล. 🛑 ถ้าเจตนาจริงคือ "ไม่ช้าลง" ให้แก้ถ้อยคำ AC ให้ตรง — ปล่อยไว้แบบนี้จะมีคนมาอ่านแล้วนับว่าผ่านทั้งที่วัดแล้วไม่ผ่าน
