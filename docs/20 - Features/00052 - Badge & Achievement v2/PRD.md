...
**ข้อมูลพื้นฐาน:**
- ใช้หน้าแอดมินจัดการแคตตาล็อกเหรียญ · **เหรียญใบใหม่ขึ้นฐานจริงผ่าน migration เท่านั้น ไม่ใช่การรันสคริปต์ seed ด้วยมืออีกต่อไป** (ดูข้อจำกัดด้านล่าง)
...
| การนำเหรียญขึ้นฐาน | การ deploy ไม่ seed ให้ · 🛑 **และสคริปต์ seed ก็ไปไม่ถึง prod แล้ว** — `npm run seed:badges` ผูกกับ `.env.local` ซึ่งชี้ `localhost:5434` ตั้งแต่แยกฐาน dev/prod (2026-08) ⇒ เหรียญใบใหม่ต้องขึ้นด้วย SQL ใน migration |
...
| **เหรียญใบใหม่ขึ้น prod ได้ทางเดียวคือ migration** | การ deploy ไม่ seed ให้ และสคริปต์ seed ชี้ฐาน dev ⇒ ถ้าไม่ใส่เป็น SQL ใน migration เหรียญจะอยู่แต่ในโค้ด **โดยสคริปต์รายงานว่าสำเร็จ** เพราะมันทำงานสำเร็จจริงกับอีกฐานหนึ่ง |
