...
🛑 **ฟิลด์ใน response ไม่ได้เป็นคอลัมน์ทั้งหมด — อย่าเดาจากชื่อ** (ตรวจกับ `DATABASE.md` §3.4 แล้ว 2026-08-21)

| ฟิลด์ใน response | ที่มา |
|---|---|
| `runId` | คอลัมน์ `BadgeMetricsRun.id` (คนละชื่อโดยตั้งใจ — `id` ลอย ๆ ใน JSON ตอบไม่ได้ว่า id ของอะไร) |
| `processed` · `ok` · `failed` · `awarded` · `stoppedBy` | คอลัมน์ชื่อเดียวกันเป๊ะ |
| **`remaining`** | 🛑 **คำนวณสด ไม่ใช่คอลัมน์** — ได้จาก "จำนวนร้าน active ทั้งหมด − `processed`" ณ เวลาที่ปิดรอบ ⇒ **ห้ามเพิ่มเป็นคอลัมน์ภายหลัง** เพราะมันจะกลายเป็นค่าที่นิ่งอยู่กับที่ ขณะที่จำนวนร้านเปลี่ยนได้ทุกวัน (ค่าที่บันทึกไว้จะกลายเป็นเท็จโดยไม่มีอะไรฟ้อง) · ถ้าอยากรู้ย้อนหลังว่ารอบนั้นเหลือกี่ร้าน ให้คำนวณจาก `processed` เทียบกับจำนวนร้าน ณ ตอนนั้น |
| `startedAt` · `finishedAt` · `failedShopIds` · `lastError` | **ไม่อยู่ใน response ของ cron** — เป็นคอลัมน์สำหรับการสืบสวนย้อนหลังเท่านั้น (`finishedAt IS NULL` = รอบถูกตัดกลางคัน ซึ่ง response ไม่มีทางรายงานได้อยู่แล้วเพราะโค้ดไม่ได้ทำงานถึงบรรทัดที่ตอบ) |
