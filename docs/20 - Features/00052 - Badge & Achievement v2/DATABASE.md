...
related: ["[[00052 - Badge & Achievement v2/PRD]]", "[[00052 - Badge & Achievement v2/BRD]]", "[[00052 - Badge & Achievement v2/API]]", "[[CONTEXT]]", "[[00008 - Business Account & Packages/DATABASE]]", "[[migration-check-constraint-additive]]", "[[Feature-Docs-Ownership]]"]
---
...
> **สถานะ:** Draft (ผลคิวรี pre-flight บน prod ยืนยันแล้ว 2026-08-21 · มติ OQ ครบทั้ง 7 ข้อ · v1.2 เพิ่มตาราง `BadgeMetricsRun` ตามข้อเสนอของ `API.md`)
...
สิ่งที่เปลี่ยนมี 4 ก้อน โดยมี **ตารางใหม่ 1 ตารางในเฟส P2** (เพิ่มใน v1.2 — เหตุผลใน §3.4):
...
| **`BadgeMetricsRun` — บันทึกผลของงานรายวันแต่ละรอบ (ตารางใหม่)** | P2 | ตารางหลักฐานที่ตอบว่า *"รอบนั้นเกิดอะไรขึ้น"* ซึ่ง `Shop.badgeMetricsUpdatedAt` ตอบไม่ได้ (FR-BDG-20 · BRD §6.3) |

- **เอกสารออกแบบต้นทาง:** `SDS.md` · `SRS.md` · `API.md` ของโมดูลนี้เขียนเสร็จแล้วในวันเดียวกัน (2026-08-21) ⇒ เอกสารฉบับนี้ trace กลับทั้ง `BRD.md` (FR-BDG-01..27) และ component ใน SDS · **ที่ใดขัดกัน: SRS ชนะเรื่องสูตร/เกณฑ์ · SDS ชนะเรื่องการแบ่งไฟล์และลำดับ build · DATABASE ชนะเรื่อง DDL/SQL และแผน migration**
...
erDiagram
    ...
    Shop {
        ...
        datetime badgeMetricsUpdatedAt "P2 — เวลาที่งานรายวันเขียนล่าสุด (ตอบว่าร้านไหนตกรอบ)"
    }
    Badge {
        string id PK "uuid"
        string nameEN "unique — คีย์ upsert"
        string family "P1 — ตระกูล (NULL = ไม่มีตระกูล)"
        int tier "P1 — ขั้นภายในตระกูล เริ่มที่ 1 (มีช่องว่างได้ ดู §3.1)"
    ...
    BadgeMetricsRun {
        string id PK "uuid — ตารางหลักฐาน ไม่มี FK ไปหาใครเลยโดยตั้งใจ"
        datetime startedAt "เวลาที่รอบเริ่ม (index DESC)"
        datetime finishedAt "NULL = รอบถูกตัดกลางคัน — ข้อมูลที่มีค่าที่สุดของตารางนี้"
        int processed "จำนวนร้านที่หยิบมาทำในรอบนี้"
        int ok "สำเร็จกี่ร้าน"
        int failed "ล้มกี่ร้าน"
        int awarded "มอบเหรียญไปกี่ใบในรอบนี้"
        string stoppedBy "COMPLETED / LIMIT / DEADLINE (CHECK)"
        string_array failedShopIds "id ร้านที่ล้ม — เก็บเป็น array ไม่ใช่ relation (ต้องรอดแม้ร้านถูกลบ)"
        string lastError "ข้อความ error ตัวสุดท้ายของรอบ (null ได้)"
    }
```
...
> **`BadgeMetricsRun` ลอยอยู่เดี่ยว ๆ ใน ERD โดยตั้งใจ ไม่ใช่เพราะวาดตกความสัมพันธ์** — มันไม่มี FK ไปหาตารางไหนเลย เหตุผลเต็มอยู่ใน §3.4
...
#### 3.1.1 🛑 ค่า `tier` ของ 31 ใบเดิม **มีช่องว่างโดยตั้งใจ — ห้าม "แก้" ให้เรียงติดกัน**

หลัง P1 จะมีตระกูลที่เลข `tier` ของเหรียญเดิม **กระโดดข้ามเลข** เพราะขั้นที่หายไปคือขั้นที่ **เหรียญใหม่ของ P2 จะมาเติม** (BRD §2.4.1 "เหรียญใหม่ที่ต้องเพิ่มในรอบนี้ 14 ใบ") ⇒ ถ้าใครเห็นแล้วคิดว่าพิมพ์ตกแล้วบีบให้เป็น 1,2,3 ติดกัน **ขั้นจะชนกันทันทีตอน P2 seed เหรียญใหม่เข้ามา** ซึ่งจะไปโผล่เป็น V-5 แดง (ขั้นซ้ำในตระกูล) หรือแย่กว่านั้นคือ **rollup บนโปรไฟล์เลือก "ขั้นสูงสุด" ผิดใบ** โดยไม่มีอะไรฟ้อง (FR-BDG-25)

| ตระกูล | ขั้นที่ 31 ใบเดิมถืออยู่ | ขั้นที่ว่างไว้ให้ P2 | ปลายทางเมื่อจบ P2 |
|---|---|---|---|
| `SHOP_TENURE` | 1 (`3 Months Strong` 90 วัน) · **3** (`Veteran` 365 วัน) | **2** (180 วัน) · **4** (730 วัน) | 4 ขั้นเรียง 90/180/365/730 |
| `ORDER_VOLUME` | 1–5 (`First Sale`/`Getting Started`/`Rising Seller`/`Trusted Seller 50`/`Century Club`) | **6** (250 ใบ) · **7** (500 ใบ) | 7 ขั้น |
| `NO_SELLER_CANCEL` | 1 (`Zero Complaint`) · 2 (`Spotless 100`) | **3** (ตัวอย่าง 300 ใบ) | 3 ขั้น |
| `SHIP_SPEED` | 1 (`Speed Demon` ≤24 ชม.) · 2 (`Same-Day Hero` ≤12 ชม.) | **3** (≤6 ชม.) | 3 ขั้น |
| `REVIEW_REPLY` | — (ไม่มีเหรียญเดิมเลย) | **1** (90%/5 ใบ) · **2** (100%/20 ใบ) | 2 ขั้น (ใหม่ทั้งตระกูล) |
| `TRACKING_COVERAGE` | — (ไม่มีเหรียญเดิมเลย) | **1** (95%/20 ใบ) · **2** (100%/100 ใบ) | 2 ขั้น (ใหม่ทั้งตระกูล) |
| `SALES_VOLUME` | — (ไม่มีเหรียญเดิมเลย) | **1–4** (฿50k/฿250k/฿1M/฿5M) | 4 ขั้น (ใหม่ทั้งตระกูล) |

**กติกาที่ต้องยึด:** `tier` คือ **ตำแหน่งความยากในตระกูลที่ถูกประกาศไว้ล่วงหน้าทั้งชุด** ไม่ใช่ลำดับการมาถึงของเหรียญ ⇒ **เลขของเหรียญที่ปล่อยไปแล้วห้ามขยับตลอดกาล** (BR-BDG-09 ห้ามแก้เกณฑ์หลังปล่อย — การขยับ `tier` คือการแก้ความหมายของเหรียญที่ร้านถืออยู่โดยที่ร้านไม่ได้ทำอะไรผิด) · เหรียญใหม่ต้องเสียบเข้าเลขที่ว่างไว้ให้เท่านั้น · **ห้ามใช้ "ความต่อเนื่องของเลข" เป็นเกณฑ์ตรวจความถูกต้องในเทสหรือในคิวรี** — V-5 ตรวจแค่ว่า *ไม่ซ้ำ* ไม่ได้ตรวจว่า *ไม่มีช่องว่าง* และนั่นถูกแล้ว
...
| Veteran | `SHOP_TENURE` | **3** (ขั้น 2 = 180 วัน มาใน P2 — §3.1.1) | **EVIDENCE** | SHOP | `{}` |
...
| `badgeMetricsUpdatedAt` | เวลาที่งานรายวันเขียนค่าชุดนี้ล่าสุด | NULL = ยังไม่เคยรัน · **คอลัมน์นี้ตอบได้แค่ "ร้านไหนตกรอบ"** ⇒ KPI "0 ร้านที่ค่าเก่ากว่า 48 ชม." อ่านจากคอลัมน์นี้ตัวเดียว วัดด้วยวิธีเดียวกับ `chatMetricsUpdatedAt` · **ส่วนคำถามว่า "รอบนั้นเกิดอะไรขึ้น" ตอบด้วย `BadgeMetricsRun` (§3.4) ไม่ใช่ด้วยคอลัมน์นี้** |
...
### 3.4 `BadgeMetricsRun` (PostgreSQL — ตารางใหม่ใน P2, บันทึกผลของงานรายวันแต่ละรอบ)

เพิ่มใน v1.2 ตามข้อเสนอของ `API.md` ซึ่ง Controller รับแล้ว

**เหตุผลที่ต้องมีตารางนี้ — เอกสารฉบับนี้เขียนข้อกำหนดไว้เองแล้วไม่มีที่ให้เขียน:** §5.3 (Query impact, แถวงานรายวัน) ระบุว่างานรายวัน *"ต้องรายงานผลรวมของรอบลงฐานข้อมูล ไม่ใช่ลง log ที่อ่านย้อนหลังไม่ได้"* ซึ่งมาจาก PRD §6.2 โดยตรง แต่ v1.1 ไม่มีตารางไหนรองรับข้อกำหนดนั้นเลย — `Shop.badgeMetricsUpdatedAt` ตอบได้แค่ *"ร้านไหนตกรอบ"* **ไม่ได้ตอบ *"รอบนั้นเกิดอะไรขึ้น"*** (รันกี่ร้าน ล้มกี่ร้าน ล้มเพราะอะไร จบเองหรือถูกตัด) ⇒ ข้อกำหนดที่เขียนไว้แต่ไม่มีที่ให้บังคับใช้ คือกฎที่ "เขียนไว้" ไม่ใช่กฎที่ "บังคับได้" (`docs/conventions/rule-must-be-enforced-not-described.md`)

**precedent ในรีโปคือ `ChatHandoverEvent` ซึ่งถูกสร้างขึ้นด้วยเหตุผลเดียวกันเป๊ะเมื่อ 2026-08-08** — ตอนนั้นบันทึกไว้ว่า **Vercel plan นี้ query runtime log ย้อนหลังไม่ได้** (`/v1/deployments/{id}/runtime-logs` คืน 404 · `/v2/.../events` คืนแต่ build log) ⇒ `console.log` ที่อ่านย้อนหลังไม่ได้ **มีค่าเท่ากับไม่มี** และตารางหลักฐานคือทางเดียวที่คำตอบอยู่รอดจนถึงวันที่มีคนถาม ซึ่งมักเป็นหลายวันหลังเหตุการณ์

| Column | Type | Null | Default | Key |
|--------|------|------|---------|-----|
| `id` | `text` | `NO` | `uuid()` | `PK` |
| `startedAt` | `timestamp(3)` | `NO` | `now()` | `IDX (DESC)` |
| `finishedAt` | `timestamp(3)` | `YES` | `NULL` | `-` |
| `processed` | `integer` | `NO` | `0` | `-` |
| `ok` | `integer` | `NO` | `0` | `-` |
| `failed` | `integer` | `NO` | `0` | `-` |
| `awarded` | `integer` | `NO` | `0` | `-` |
| `stoppedBy` | `text` | `NO` | `'COMPLETED'` | `CHECK` |
| `failedShopIds` | `text[]` | `NO` | `'{}'` | `-` |
| `lastError` | `text` | `YES` | `NULL` | `-` |

**นิยามและเหตุผล:**

| คอลัมน์ | นิยาม | หมายเหตุ |
|---------|------|----------|
| `startedAt` | เวลาที่รอบเริ่มทำงาน — เขียนแถวทันทีที่รอบเริ่ม **ไม่ใช่ตอนจบ** | ถ้าเขียนตอนจบ รอบที่ตายกลางทางจะไม่มีแถวเลย ซึ่งคือรอบที่เราต้องการหลักฐานมากที่สุด · เป็นคอลัมน์ที่มี index (§4.2) |
| `finishedAt` | เวลาที่รอบจบ — **`NULL` = รอบนี้ไม่เคยจบ** | 🛑 **นี่คือข้อมูลที่มีค่าที่สุดของตารางนี้** — แถวที่ `finishedAt IS NULL` และ `startedAt` เก่ากว่าหนึ่งชั่วโมง แปลว่า **รอบนั้นถูกตัดกลางคันโดยแพลตฟอร์ม** (function timeout / instance ถูกรีไซเคิล / deploy แทรก) ซึ่งเป็นความล้มเหลวชนิดที่ **ไม่มีทางเขียน `lastError` ได้เลย เพราะโค้ดไม่ได้ทำงานถึงบรรทัดนั้น** ⇒ วิธีเดียวที่จะรู้คือดูว่าแถวถูกเปิดไว้แล้วไม่เคยถูกปิด · อาการของมันจากภายนอกคือ "ทุกอย่างดูปกติ" เพราะร้านที่ทำไปแล้วมีค่าใหม่ ร้านที่ยังไม่ถึงมีค่าเก่า และไม่มี error อยู่ที่ไหนเลย |
| `processed` | จำนวนร้านที่รอบนี้หยิบมาทำ | คู่กับ `ok`/`failed` ต้องบวกกันได้ (`ok + failed = processed` เมื่อรอบจบปกติ) — ถ้าไม่เท่า แปลว่ารอบถูกตัดกลาง |
| `ok` · `failed` | สำเร็จ/ล้ม กี่ร้าน | ตอบคำถามที่ PRD §6.2 บังคับให้ตอบได้: *"สำเร็จกี่ร้าน ล้มกี่ร้าน"* |
| `awarded` | จำนวนเหรียญที่ถูกมอบจริงในรอบนี้ | 🛑 **ตัวเลขนี้ต้องเป็น 0 ในรอบปกติ และไม่เป็น 0 เฉพาะวันที่มีร้านข้ามเกณฑ์จริง** ⇒ เป็นเครื่องมือตรวจ FR-BDG-20 AC "รันซ้ำในวันเดียวกันแล้วผลต้องเท่าเดิม ไม่มอบเหรียญซ้ำ" ได้โดยไม่ต้องไล่นับแถว `UserBadge` — รันสองรอบติดกันแล้วรอบที่สอง `awarded > 0` = การกันมอบซ้ำพัง |
| `stoppedBy` | เหตุที่รอบหยุด: `COMPLETED` ทำครบทุกร้าน · `LIMIT` ชนเพดานจำนวนร้านต่อรอบ · `DEADLINE` ชนเพดานเวลาที่ตั้งไว้เอง (หยุดเองก่อนถูกแพลตฟอร์มตัด) | 🛑 **`DEADLINE` ต่างจาก `finishedAt IS NULL` โดยสิ้นเชิง** — `DEADLINE` คือ *เราหยุดเอง* (รอบจบเรียบร้อย มีแถวปิดครบ รู้ว่าเหลือร้านที่ยังไม่ได้ทำ) ส่วน `finishedAt IS NULL` คือ *เราถูกหยุด* (ไม่มีใครได้เขียนอะไรอีกเลย) การรวมสองอย่างนี้เป็นค่าเดียวจะทำให้แยกไม่ออกว่าระบบทำงานตามที่ออกแบบ หรือแพลตฟอร์มกำลังฆ่างานเราอยู่ทุกคืน |
| `failedShopIds` | รายการ id ของร้านที่ล้มในรอบนี้ | **`text[]` ไม่ใช่ relation** (เหตุผลด้านล่าง) · ตอบว่า "ล้มร้านไหน" ได้โดยไม่ต้องมีตารางลูก · ถ้ารายการยาวผิดปกติในหลายรอบติดกัน = ร้านนั้นมีข้อมูลที่ทำให้ตัวคำนวณพังซ้ำ ๆ ไม่ใช่ความบังเอิญ |
| `lastError` | ข้อความ error ตัวสุดท้ายที่เกิดในรอบ | `NULL` = ไม่มี error · **เก็บตัวสุดท้ายพอ ไม่เก็บทุกตัว** เพราะเป้าหมายคือ "มีเบาะแสให้เริ่มสืบสวน" ไม่ใช่ระบบรวบรวม log — ถ้าต้องการทุกตัวคือสัญญาณว่าควรมีตารางลูก ซึ่งรอบนี้ยังไม่จำเป็น |

🛑 **ตารางนี้ไม่มี FK ไปหาตารางไหนเลย และนั่นคือการตัดสินใจ ไม่ใช่การลืม** — เป็น **ตารางหลักฐาน** ที่ต้องรอดแม้ร้านที่มันพูดถึงจะหายไปแล้ว (แบบเดียวกับ `ChatHandoverEvent` ที่ไม่ผูก FK ด้วยเหตุผลเดียวกัน) ⇒ `failedShopIds` เป็น `String[]` ไม่ใช่ `ShopBadgeMetricsRunFailure[]` เพราะ:
1. FK + `ON DELETE CASCADE` จะ **ลบหลักฐานทิ้งพร้อมร้าน** ซึ่งย้อนแย้งกับเหตุผลทั้งหมดที่ตารางนี้มีอยู่ — เคสที่ต้องสืบสวนมากที่สุดคือ "ร้านที่มีปัญหาจนถูกลบ" พอดี
2. FK + `ON DELETE RESTRICT` จะ **บล็อกการลบร้าน** เพราะมีบันทึกงานรายวันอ้างถึง ซึ่งเป็นผลข้างเคียงที่ไม่มีใครคาดคิดและจะไปโผล่เป็นบั๊กในฟีเจอร์ลบบัญชี
3. FK + `SET NULL` จะได้แถวที่บอกว่า "มีร้านล้ม 1 ร้าน แต่ไม่รู้ร้านไหน" ซึ่งไร้ประโยชน์
⇒ **id ที่ชี้ไปหาแถวที่หายไปแล้ว ยังมีค่ามากกว่าไม่มี id เลย** (ตามไปดูใน `_00052_p1_userbadge_backup` หรือใน log ของฟีเจอร์อื่นได้)

**ตารางนี้เป็นข้อยกเว้นของหลาย ๆ กฎในเอกสารนี้ — ต้องรู้ว่ายกเว้นตรงไหนบ้าง:**
- **เป็นตารางเดียวในฟีเจอร์นี้ที่โตทุกวันแบบไม่มีเพดาน** ⇒ เป็นตารางเดียวที่ควรมี index ตั้งแต่วันแรก (§4.2)
- **เป็นตารางเดียวในฟีเจอร์นี้ที่ "ลบข้อมูลได้"** ⇒ มีนโยบาย retention จริง ต่างจาก `UserBadge` ที่ห้ามลบทุกกรณี (§6)
- **ไม่มีข้อมูลของผู้ใช้ปลายทางเลย** — ไม่มีชื่อ ไม่มีเบอร์ ไม่มียอดเงิน มีแต่ id ร้านและตัวเลขนับ

### 3.4.1 นิยามที่ต้องบังคับฝั่งโค้ด (เขียนไว้กันตีความผิด)

- **1 รอบของงานรายวัน = 1 แถว** ห้ามเขียนแถวต่อร้าน (นั่นคือหน้าที่ของ `Shop.badgeMetricsUpdatedAt`) และห้ามเขียนแถวเดียวแล้วอัปเดตทับข้ามวัน
- **เขียนแถวตอนเริ่ม → อัปเดตตอนจบ** (ไม่ใช่เขียนตอนจบครั้งเดียว) — เป็นสิ่งเดียวที่ทำให้ `finishedAt IS NULL` มีความหมาย
- **การเขียนแถวนี้ต้องล้มไม่ได้เงียบ ๆ** — ถ้าเขียนบันทึกไม่ได้ ให้ถือว่ารอบนั้นล้ม ไม่ใช่ทำงานต่อไปโดยไม่มีบันทึก (บันทึกที่หายเป็นบางรอบ อันตรายกว่าไม่มีบันทึกเลย เพราะเราจะสรุปจากรอบที่เหลือว่าทุกอย่างปกติ)
- **ห้ามใช้ตารางนี้เป็นตัวกันรันซ้ำ** — การกันมอบเหรียญซ้ำอยู่ที่ partial unique index ของ `UserBadge` เท่านั้น (FR-BDG-20 AC ข้อสุดท้าย) ตารางนี้เป็นบันทึก ไม่ใช่ล็อก
...
| `UserBadge` | `("shopId","badgeId") WHERE "shopId" IS NOT NULL` | `partial UNIQUE` (`UserBadge_shopId_badgeId_key`) | กันมอบซ้ำของ **เหรียญร้าน** · หลัง P1 ตัวนี้กลายเป็นด่านกันซ้ำของร้านส่วนตัวด้วย (เดิมร้านส่วนตัวพึ่งตัวบน) · **เป็นตัวกันมอบซ้ำตัวจริงของงานรายวัน ไม่ใช่ `BadgeMetricsRun`** — **unmanaged SQL ห้าม db pull** |
...
**มี 1 ตัว — บน `BadgeMetricsRun` ซึ่งเป็นตารางเดียวในฟีเจอร์นี้ที่โตไม่มีเพดาน**

| Table | Columns | Type | Rationale (query pattern ที่รองรับ) |
|-------|---------|------|--------------------------------------|
| `BadgeMetricsRun` | `("startedAt" DESC)` | `BTREE` (`BadgeMetricsRun_startedAt_idx`) | คิวรีที่จะใช้จริงมีสองแบบและทั้งคู่ใช้ index นี้: **"รอบล่าสุด N รอบ"** (`ORDER BY "startedAt" DESC LIMIT n` — หน้าตรวจสุขภาพ/แอดมิน) และ **"รอบที่ยังไม่จบ"** (`WHERE "finishedAt" IS NULL ORDER BY "startedAt" DESC` — ตัวจับรอบที่ถูกตัดกลางคัน) · **สร้างตั้งแต่วันแรกไม่ใช่รอให้ช้าก่อน** เพราะต่างจากตารางอื่นในฟีเจอร์นี้ที่มีเพดานตามธรรมชาติ (แคตตาล็อก 45 แถว · ร้าน 14 แถว) ตารางนี้ได้ 1 แถวต่อรอบต่อวันตลอดไป และคิวรี "รอบล่าสุด" คือคิวรีที่จะถูกยิงบ่อยที่สุดของทั้งฟีเจอร์ |

**ตัวที่พิจารณาแล้วไม่สร้าง พร้อมเหตุผลที่ตรวจสอบได้:**
...
| `BadgeMetricsRun(finishedAt)` เดี่ยว ๆ / partial `WHERE "finishedAt" IS NULL` | คิวรี "รอบที่ยังไม่จบ" มักพ่วง `ORDER BY "startedAt" DESC` อยู่แล้ว และจำนวนแถวที่ค้างมีน้อยมากโดยธรรมชาติ ⇒ index บน `startedAt` เพียงพอ · ถ้าวันหนึ่งรอบค้างเยอะจนคิวรีช้า **นั่นคือปัญหาที่ต้องแก้ที่ต้นเหตุ ไม่ใช่ที่ index** |
...
| `BadgeMetricsRun` | `BadgeMetricsRun_stoppedBy_check` | `stoppedBy IN ('COMPLETED','LIMIT','DEADLINE')` | ค่าสามค่านี้ตอบคำถามคนละคำถามกัน (จบเอง / ชนเพดานจำนวน / ชนเพดานเวลา) — ค่าที่สี่ที่หลุดเข้ามาจะทำให้การอ่านประวัติสรุปผิดว่าระบบทำงานปกติ · ตารางว่างตอน migrate ⇒ VALIDATE ผ่านทันที |

🛑 **ทุก CHECK ข้างบนต้องเขียนตาม `docs/conventions/migration-check-constraint-additive.md`** — คือ **อ่านนิยามเดิมจาก `pg_constraint` มาต่อท้าย ห้าม hardcode รายชื่อแล้ว DROP/ADD ตรง ๆ** scope ด้วย `conrelid` · idempotent · `NOT VALID` แล้วค่อย `VALIDATE` แยกคำสั่ง เหตุผลไม่ใช่ทฤษฎี: 2026-08-06 migration สอง branch ที่ timestamp ชนกันลบค่าของกันเองเงียบ ๆ โดยที่ `migrate deploy` รายงานว่าสำเร็จทุกไฟล์ แล้วไปโผล่เป็น insert ล้มบนฐานจริง · **`BadgeMetricsRun_stoppedBy_check` เป็นตารางใหม่จึงเข้ากิ่ง "ยังไม่มี constraint" ของ DO block แต่ต้องเขียนด้วยโครงเดียวกัน** เพราะวันที่มีคนเพิ่มค่าที่สี่ (เช่น `ABORTED_BY_DEPLOY`) เขาจะมาแก้ไฟล์นี้เป็นแม่แบบ

> ตรวจแล้ว ณ 2026-08-21: ไม่มี migration ของ branch ไหนใช้ timestamp `20260821*` หรือ `20260822*` (`git log --all --name-only --pretty=format: -- 'prisma/migrations/*'`) — ต้องตรวจซ้ำอีกครั้งก่อนตั้งชื่อโฟลเดอร์จริง
...
| 4 | `20260822090000_shop_badge_metrics` — เพิ่ม 9 คอลัมน์บน `Shop` + `Shop_badge_metrics_range_check` **และสร้างตาราง `BadgeMetricsRun` + `BadgeMetricsRun_stoppedBy_check` + index `BadgeMetricsRun_startedAt_idx` ในไฟล์เดียวกัน** | P2 | ไม่มี dependency กับ P1 ในเชิงเทคนิค แต่ **BR-BDG2-35 ห้ามเริ่มก่อน P1 จบและพิสูจน์ผลแล้ว** · 🛑 **ตารางบันทึกรอบต้องขึ้นพร้อมคอลัมน์ที่งานรายวันเขียน ไม่ใช่ไฟล์แยกทีหลัง** — ถ้าแยก จะมีช่วงที่งานรายวันทำงานได้แล้วแต่ไม่มีที่บันทึก ซึ่งเป็นช่วงที่ความล้มเหลวรอบแรก ๆ (ที่มีโอกาสเกิดสูงที่สุด) จะหายไปโดยไม่มีใครเห็น |
| 5 | `20260822091000_badge_v2_catalog_rows` — เพิ่มเหรียญใหม่ 14 ใบเข้าแคตตาล็อกแบบ idempotent (`INSERT ... ON CONFLICT ("nameEN") DO UPDATE`) | P2 | ต้องหลังลำดับ 1 (คอลัมน์ต้องมีก่อน) · **ดู §5.5 — นี่คือทางเดียวที่เหรียญใหม่ขึ้น prod ได้ สคริปต์ seed ไปไม่ถึงแล้ว** · **ค่า `tier` ของ 14 ใบต้องเสียบเข้าเลขที่ §3.1.1 ว่างไว้ให้เท่านั้น** |
...
**ทั้งสามร้านต่ำกว่าเพดาน 10 ⇒ คะแนนส่วนเหรียญลดลงจริงร้านละ 1 แต้ม** — ผลนี้ถูกยอมรับเป็นมติแล้ว (OQ-3) รายละเอียดและสิ่งที่ต้องทำตามมาอยู่ใน §5.3.1
...
| 4 (`_shop_badge_metrics`) | `DROP CONSTRAINT` + `DROP COLUMN` 9 คอลัมน์ · `DROP INDEX "BadgeMetricsRun_startedAt_idx"` · `DROP TABLE "BadgeMetricsRun"` | 9 คอลัมน์บน `Shop` ปลอดภัย — ทุกค่ามาจากงานรายวัน คำนวณใหม่ได้ทั้งหมดจากข้อมูลต้นทางที่ยังอยู่ครบ (คุณสมบัติของการเลือกเก็บเป็น denormalized cache ไม่ใช่ SSOT) · 🛑 **แต่ `BadgeMetricsRun` คำนวณใหม่ไม่ได้ — มันคือบันทึกเหตุการณ์ที่ผ่านไปแล้ว** การ drop = ทิ้งหลักฐานทั้งหมดว่างานรายวันเคยทำอะไรบ้าง ⇒ ถ้าต้องย้อนลำดับ 4 **ให้ dump ตารางนี้เก็บไว้ก่อนเสมอ** และการ drop ต้องเป็นคำสั่งชัดจาก Controller ไม่ใช่ผลพลอยได้ของการย้อน migration |
...
2. **`Shop.trustScore` และ `User.trustScore` ย้อนไม่ได้ด้วยสคริปต์นี้** — ทั้งคู่เป็น monotonic (`Math.max`) ⇒ ค่าที่เขียนไปแล้วไม่ลดลงเอง และ **แถวใน `TrustScoreHistory` เป็นบันทึกประวัติ ห้ามลบ** ⇒ ร่องรอยของ P1 จะอยู่ในประวัติถาวร (รวมถึงส่วนต่าง −1 แต้มของ 3 ร้านตาม §5.3.1 ซึ่งเป็นผลที่ตั้งใจ) วิธีรับมือกับส่วนที่ *ไม่* ตั้งใจคือ **ป้องกันด้วย V-3 ก่อน push ไม่ใช่ย้อนทีหลัง**
...
**Downtime / lock:** ไม่มี — `ADD COLUMN` ที่มี default เป็นค่าคงที่บน Postgres 11+ เป็น metadata-only ไม่เขียนทุกแถวใหม่ · `CREATE TABLE` ของ `BadgeMetricsRun` ไม่แตะตารางที่มีอยู่เลย · ตารางใหญ่ที่สุดที่ถูกแตะคือ `Shop` (14 แถว) และ `UserBadge` (63 แถว) · `NOT VALID` + `VALIDATE` แยกคำสั่งตาม precedent `Shop_pinSlots_min1`/`Shop_vertical_check` เพื่อไม่ถือ ACCESS EXCLUSIVE ระหว่างตรวจ
...
| งานรายวัน (P2) | ไม่มี | สแกนออเดอร์/รีวิว 90 วันต่อร้าน วันละครั้ง — 14 ร้านวันนี้ · **เขียน 1 แถวลง `BadgeMetricsRun` ตอนเริ่ม + อัปเดต 1 ครั้งตอนจบ = 2 คิวรีต่อรอบ ไม่ใช่ต่อร้าน** ⇒ ค่าใช้จ่ายคงที่ไม่ว่าจะมีกี่ร้าน · **นี่คือที่ที่ข้อกำหนด "ต้องรายงานผลรวมของรอบลงฐานข้อมูล ไม่ใช่ลง log ที่อ่านย้อนหลังไม่ได้" (PRD §6.2) ถูกบังคับใช้จริง** |
| ตรวจสุขภาพงานรายวัน (P2) | ไม่มี | `ORDER BY "startedAt" DESC LIMIT n` และ `WHERE "finishedAt" IS NULL` → `BadgeMetricsRun_startedAt_idx` |
...
- **`BadgeMetricsRun`:** ตารางใหม่ที่ไม่มีใครอ่านนอกจากงานรายวันและหน้าตรวจสุขภาพ ⇒ ไม่มีผลกับโค้ดเดิมเลย
...
-- 🛑 คิวรีนี้ตรวจ "ไม่ซ้ำ" เท่านั้น ไม่ได้ตรวจ "ไม่มีช่องว่าง" — และนั่นถูกแล้ว (§3.1.1)
--    หลัง P1 ตระกูล SHOP_TENURE จะมีขั้น 1 และ 3 โดยไม่มี 2 ซึ่งเป็นสภาพที่ตั้งใจ
```
...
WHERE indexname IN ('UserBadge_userId_badgeId_personal_key','UserBadge_shopId_badgeId_key',
                    'UserBadge_shopId_idx','Shop_userId_personal_key',
                    'BadgeMetricsRun_startedAt_idx');   -- ต้องได้ครบ 5 แถวหลัง P2 (4 แถวหลัง P1)
SELECT conname, pg_get_constraintdef(oid) FROM pg_constraint
WHERE conname IN ('Shop_vertical_check','Badge_surface_check','Badge_ownerScope_check',
                  'Badge_verticals_check','BadgeMetricsRun_stoppedBy_check');
...
```sql
-- V-10 · (P2) งานรายวันบันทึกรอบจริง และไม่มีรอบที่ค้างเปิดไว้
SELECT count(*) FILTER (WHERE "finishedAt" IS NULL
                          AND "startedAt" < now() - interval '1 hour') AS stuck_runs,
       count(*) FILTER (WHERE "stoppedBy" = 'COMPLETED')               AS completed_runs,
       max("startedAt")                                               AS last_run_at
FROM "BadgeMetricsRun";
-- stuck_runs ต้องเป็น 0 · last_run_at ต้องไม่เก่ากว่า 48 ชม. (เกณฑ์เดียวกับ KPI ความสด)
-- 🛑 stuck_runs > 0 คือสัญญาณว่ารอบถูกตัดกลางคันโดยแพลตฟอร์ม ซึ่งเป็นความล้มเหลวชนิดที่
--    ไม่มีทางเขียน lastError ได้เลย — ถ้าไม่ดูคอลัมน์นี้จะไม่มีใครรู้ว่ามันเกิดขึ้น
```

```sql
-- V-11 · (P2) รันงานรายวันซ้ำในวันเดียวกันต้องไม่มอบเหรียญซ้ำ (FR-BDG-20 AC ข้อสุดท้าย)
SELECT "id", "startedAt", "processed", "ok", "failed", "awarded", "stoppedBy"
FROM "BadgeMetricsRun" ORDER BY "startedAt" DESC LIMIT 2;
-- รอบที่สองของวันเดียวกันต้องได้ awarded = 0 — ถ้าไม่ใช่ แปลว่าการกันมอบซ้ำพัง
-- (ตัวกันจริงคือ partial unique index ของ UserBadge ไม่ใช่ตารางนี้ — ตารางนี้แค่ทำให้เห็น)
```
...
🛑 **`npm run seed:badges` ไปไม่ถึง prod อีกต่อไป (มติ OQ-7, ตรวจแล้ว 2026-08-21)**
...
**สถานะเอกสาร ณ 2026-08-21:**
- ✅ **`PRD.md` และ `BRD.md` แก้แล้ว** — Controller แก้เอง 3 จุดใน PRD (§4.2 ข้อจำกัด "การนำเหรียญขึ้นฐานต้องทำด้วยมือ" · §6.2 ความเสี่ยงทางเทคนิคข้อแรก · §9.1 "สคริปต์นำเหรียญขึ้นฐาน") และจุดที่เกี่ยวข้องใน BRD ให้ชี้มาที่ **migration** แทนการรัน `seed:badges` ⇒ เอกสารสองฉบับนี้ไม่พาคนไปทางที่ตันอีกต่อไป
- ⚠️ **ยังค้าง: บันทึกความจำของโปรเจกต์** ที่ยังเขียนว่าต้องรัน `npm run seed:badges` เอง — เป็นคำแนะนำที่ **ตกยุคตั้งแต่วันที่แยกฐาน dev/prod (2026-08)** และเป็นที่ที่ agent รอบถัดไปจะอ่านก่อนเปิดเอกสารฟีเจอร์ ⇒ ต้องถูกแก้ให้ตรงกับ §5.5 นี้ ไม่งั้นความรู้ที่แก้แล้วในเอกสารจะถูกกลบด้วยความจำที่ยังผิด · **อาการถ้าไม่แก้:** มีคนรัน seed แล้วเห็นรายงาน `created`/`updated` สำเร็จสวยงาม (เพราะมันทำงานสำเร็จจริง — แค่ทำกับฐานอีกฐานหนึ่ง) แล้วสรุปว่าเหรียญขึ้น prod แล้ว ⇒ **ไม่มีอะไรขึ้น prod เลย และไม่มี error ให้ใครเห็น** ซึ่งเป็นเหตุการณ์ที่เคยเกิดแล้วกับเหรียญประมูลชุดหลัง
...
- **Data Retention:**
  ...
  - 9 คอลัมน์ใหม่บน `Shop` เป็น **denormalized cache ที่ถูกเขียนทับทุกวัน ไม่มีประวัติย้อนหลัง** ⇒ ผลโดยตรงคือ **ทำเกณฑ์ "ทำได้ต่อเนื่อง N เดือน" ไม่ได้ในรอบนี้** (BR-BDG-14/BR-BDG2-28) ข้อจำกัดนี้เกิดจากการเลือกไม่สร้างตารางสถิติรายเดือน ต้องเขียนไว้ให้ชัดตั้งแต่ตอนนี้ ไม่ใช่ไปค้นพบตอนออกแบบเกณฑ์ 🛑 **`BadgeMetricsRun` ไม่ใช่ตารางสถิติรายเดือนและห้ามถูกใช้แทน** — มันบันทึก *ผลของรอบงาน* (รันกี่ร้าน ล้มกี่ร้าน) ไม่ได้บันทึก *ค่าของแต่ละร้าน* ⇒ ไม่มีทางย้อนดูได้ว่าเมื่อ 3 เดือนก่อนร้านหนึ่งมีอัตราส่งไวเท่าไร
  - **`BadgeMetricsRun` เป็นตารางเดียวในฟีเจอร์นี้ที่ "ลบได้"** — เสนอ retention **90 วัน** แล้วลบรอบที่เก่ากว่านั้น 🛑 **ตัวลบต้อง scope ด้วย `WHERE "startedAt" < now() - interval '90 days'` เสมอ ห้ามมี `DELETE FROM "BadgeMetricsRun"` เปล่า ๆ ปรากฏที่ไหนในรีโปเด็ดขาด** (Hard Rule 13 — และตารางนี้เป็นเป้าที่ล่อตาที่สุดเพราะ "มันแค่ log") · **รอบนี้ยังไม่ต้องสร้างตัวลบ** — 14 ร้าน = 1 รอบ/วัน = **365 แถว/ปี** ซึ่งเล็กจนการเขียนตัวลบตอนนี้เป็นการเพิ่มโค้ดที่ลบข้อมูลได้เข้ามาโดยไม่ได้แก้ปัญหาอะไร ⇒ **ข้อนี้เป็นข้อกำหนดของอนาคต ไม่ใช่งานที่ค้าง** ทบทวนเมื่อแถวเกิน ~10,000 หรือเมื่อความถี่ของรอบเพิ่มขึ้น
...
- **PII / ข้อมูลอ่อนไหว:**
  - ไม่มีคอลัมน์ใหม่ใดเป็น PII — `Badge.*` เป็นข้อมูลแคตตาล็อกสาธารณะ · 9 คอลัมน์บน `Shop` เป็นสถิติระดับร้าน · `BadgeMetricsRun` มีแต่ id ร้านและตัวเลขนับ ไม่มีชื่อ ไม่มีเบอร์ ไม่มียอดเงิน
  - 🛑 **แต่หลายคอลัมน์บน `Shop` เป็นข้อมูลที่ร้านไม่ได้ตั้งใจเปิดเผย** — จำนวนใบที่ร้านยกเลิกเอง · จำนวนออเดอร์ใน 90 วัน (`orderSample90d` = ปริมาณการขาย) ⇒ **ห้ามส่งทั้งแถว `Shop` เข้า flight payload ของหน้าสาธารณะ ต้อง `select` เฉพาะฟิลด์ที่ใช้จริง** (บทเรียน PII RSC leak 2026-06-06 และ `feedback_rsc_pii_neutralize_at_source`) · ตระกูล `SALES_VOLUME` ห้ามขึ้นหน้าสาธารณะทุกกรณี (BR-BDG-11) ซึ่งบังคับที่ `surface = 'GOAL'` และต้องมีเทส snapshot กันใครตั้งกลับเป็น `EVIDENCE`
  - 🛑 **`BadgeMetricsRun.lastError` เป็นคอลัมน์ที่ข้อความจากที่อื่นไหลเข้ามาได้** — ถ้า error ของ Postgres/Prisma มีค่าจากแถวข้อมูลติดมาด้วย (เช่นข้อความ constraint violation ที่มีค่าคอลัมน์) มันจะถูกเก็บลงตารางนี้ ⇒ **ห้ามเปิดคอลัมน์นี้สู่หน้าจอสาธารณะหรือ API ที่ไม่ใช่แอดมิน** และผู้เขียนงานรายวันควรตัดความยาวก่อนเก็บ
  - ห้ามใช้ `SELECT *` กับ `Shop` ในตัวอย่างคิวรีหรือในโค้ดฝั่งสาธารณะ (ตารางนี้มีทั้งที่อยู่ พิกัด และสถานะแพ็กเกจ)
- **Performance:**
  - ไม่มี hot row — งานรายวันเขียนแถวละครั้งต่อวัน · `BadgeMetricsRun` เขียน 2 ครั้งต่อรอบ (เปิด/ปิด) ไม่ใช่ต่อร้าน · ไม่มี lock contention ที่คาดได้บนสเกลนี้
  - `UserBadge` ถูกเขียนแบบ "insert ไปก่อนแล้วดักตอนชน" ผ่าน partial unique ⇒ **Postgres จะเขียน ERROR ลง log ทุกครั้งที่ชน แม้แอปจะดักด้วย try/catch แล้ว** (`docs/conventions/insert-then-catch-logs-every-error.md`) — เห็น `UserBadge_shopId_badgeId_key` ไหลใน log **ไม่ใช่สัญญาณว่าพัง** ให้ประเมินที่ *ต้นทุน* (dead tuple ที่ autovacuum ต้องตามเก็บ) ไม่ใช่ที่ *ความถูกต้อง* · **ห้ามถอดตัวดักเดิมออกไม่ว่าจะเพิ่ม pre-check หรือไม่** ความถูกต้องต้องอยู่ที่ index เสมอ · 🛑 **งานรายวันจะเป็นตัวผลิต ERROR ชุดนี้รายวัน** (ประเมินเหรียญเดิมซ้ำทุกวัน = ชนทุกวัน) ⇒ จำนวนบรรทัดใน log = ปริมาณงานที่สำเร็จ ไม่ใช่ที่ล้ม — `BadgeMetricsRun.awarded` คือตัวเลขที่ควรอ่านแทนการนับบรรทัด log
...
- **ทะเบียน unmanaged SQL ของฟีเจอร์นี้ (ห้าม `prisma db pull` / `migrate dev` ตลอดไป):** `UserBadge_userId_badgeId_personal_key` · `UserBadge_shopId_badgeId_key` · `Shop_userId_personal_key` · `Shop_vertical_check` · `Badge_surface_check` · `Badge_ownerScope_check` · `Badge_verticals_check` · `Shop_badge_metrics_range_check` · **`BadgeMetricsRun_stoppedBy_check`** · ตาราง `_00052_p1_userbadge_backup`
- **ความเสี่ยงที่ต้องจดไว้แม้ยังไม่เกิด:** `UserBadge.shopId` เป็น `ON DELETE CASCADE` ⇒ หลัง P1 เหรียญของร้านส่วนตัวผูกชะตากับแถว `Shop` แล้ว **ใครเพิ่ม physical DELETE ของ `Shop` ในอนาคตจะลบประวัติเหรียญทิ้งเงียบ ๆ และละเมิด BR-BDG-05** วันนี้กันไว้ด้วย soft delete + `purgedAt` tombstone + `Order.shopId` เป็น RESTRICT ซึ่งเป็น **สถานะปัจจุบัน ไม่ใช่ข้อรับประกัน** · 🛑 **`BadgeMetricsRun` จงใจไม่มี FK จึงไม่อยู่ใต้ความเสี่ยงนี้ — ห้ามมีใคร "ปรับปรุง" ด้วยการเพิ่ม FK ให้ `failedShopIds` ภายหลัง** (เหตุผลครบทั้ง 3 ข้ออยู่ใน §3.4)

---

## 7. Traceability

> `SDS.md` เขียนเสร็จแล้ว (2026-08-21) — component ที่รับผิดชอบแต่ละคอลัมน์อยู่ใน SDS §3 (รหัส `C-1xx` สำหรับ P1 · `C-2xx` สำหรับ P2) · ตารางนี้ยังชี้ไปที่ FR/BR เป็นหลักเพราะเป็นระดับที่ตรวจสอบได้ด้วยคิวรี ส่วนการชี้ component อยู่ใน SDS §7 ซึ่งเป็นเจ้าของเรื่องนั้นโดยตรง

| Table / Column | BRD FR / PRD BR ต้นทาง | สถานะ |
|--------------------|--------------------------|-------|
| `Badge.family` · `Badge.tier` | FR-BDG-01 · BR-BDG2-06/07/08 · D-BDG-2 | Draft |
| `Badge.tier` — ช่องว่างของเลขขั้น | §3.1.1 · BR-BDG-09 (ห้ามแก้เกณฑ์หลังปล่อย) · BRD §2.4.1 (เหรียญใหม่ 14 ใบ) | Draft |
...
| **`BadgeMetricsRun` (ทั้งตาราง)** | **FR-BDG-20** (งานเบื้องหลังรายวัน — ข้อกำหนด "รันซ้ำแล้วผลเท่าเดิม" และ "ต้องตรวจย้อนหลังได้จากฐานข้อมูล") · **BRD §6.3 ความน่าเชื่อถือ** · PRD §6.2 (ตัวประเมินรายวันเป็นตัวเดียวที่ทำงานตอนไม่มีอะไรเกิดขึ้น — ถ้ามันตาย อาการคือ "ทุกอย่างดูปกติ") | Draft |
| `BadgeMetricsRun.finishedAt` (NULL = ถูกตัดกลางคัน) | PRD §6.2 · precedent `ChatHandoverEvent` (2026-08-08 — Vercel query runtime log ย้อนหลังไม่ได้) | Draft |
| `BadgeMetricsRun.awarded` | FR-BDG-20 AC "รันซ้ำในวันเดียวกันแล้วผลต้องเท่าเดิม ไม่มอบเหรียญซ้ำ" · FR-BDG-08 (ไม่แจ้งเตือนซ้ำ) | Draft |
| `BadgeMetricsRun_startedAt_idx` | §4.2 (คิวรี "รอบล่าสุด N รอบ" + "รอบที่ยังไม่จบ") | Draft |
| CHECK constraint ทั้ง 5 ตัว | BR-BDG-20/21 · `migration-check-constraint-additive.md` | Draft |
...
สาระที่สั้นที่สุด: **`UserBadge` ไม่ถูกแก้โครงสร้าง · P1 เพิ่ม 5 คอลัมน์บน `Badge` แล้วย้ายเจ้าของเหรียญ 63 แถวด้วย migration ที่ตรวจตัวเองในทรานแซกชันเดียวกัน · P2 เพิ่ม 9 คอลัมน์ nullable บน `Shop` ตามแบบ `chatResponse*` ที่ทำงานอยู่แล้ว พร้อมตารางใหม่ 1 ตาราง (`BadgeMetricsRun`) ที่ทำให้ความล้มเหลวของงานรายวันมองเห็นได้ · P3/P4 ไม่แตะฐานข้อมูลเลย** ความเสี่ยงที่แท้จริงของงานนี้ไม่ได้อยู่ที่ DDL (ซึ่งเล็กและย้อนกลับได้) แต่อยู่ที่ **โค้ดที่เชื่อมาตลอดว่า `shopId = NULL` แปลว่าเหรียญของร้านส่วนตัว** — ความเชื่อนั้นจะเป็นเท็จตั้งแต่วินาทีที่ migration ลำดับ 2 รันจบ
...
| OQ-7 | ช่องทางนำเหรียญใหม่ขึ้น prod | **migration เท่านั้น** — `seed:badges` ชี้ localhost แล้ว · PRD/BRD แก้แล้ว เหลือบันทึกความจำของโปรเจกต์ | §5.5 |
...
- แก้ **บันทึกความจำของโปรเจกต์** ที่ยังเขียนว่าต้องรัน `npm run seed:badges` เอง ให้ชี้มาที่ migration ตาม §5.5 (PRD/BRD แก้ไปแล้ว เหลือที่นี่ที่เดียว — และเป็นที่ที่ agent รอบถัดไปอ่านก่อนเปิดเอกสารฟีเจอร์)
- ตรวจ timestamp ชนของโฟลเดอร์ migration ซ้ำอีกครั้งในวันที่สร้างไฟล์จริง (`git log --all` ครอบทุก branch ไม่ใช่ `ls`)
- ยืนยันกับ `API.md` ว่าชื่อคอลัมน์ของ `BadgeMetricsRun` ที่ใช้ในสัญญาของ endpoint ตรวจสุขภาพ ตรงกับ §3.4 ทุกตัว — ตารางนี้เกิดจากข้อเสนอของเอกสารนั้น การหลุดชื่อกันคือหนี้ตั้งแต่วันแรก
