...
- [ ] มีสคริปต์เทียบคะแนนเหรียญของ **ทุกร้านบนระบบจริง** ก่อน/หลัง migration แล้วต้องเท่ากันทุกร้าน (ผลต่างที่ยอมรับได้ = 0)
...
- [ ] `ONLINE_SALES` เห็น 7 ตระกูล · `SERVICE_QUEUE` และ `LODGING` เห็น 5 ตระกูล (ชุดกลางอย่างเดียว)
- [ ] ค่าประเภทร้านที่ระบบไม่รู้จัก ต้องตกไปที่ **ชุดกลาง 5 ตระกูล** ไม่ใช่ตกไปที่ชุดของประเภทใดประเภทหนึ่ง และไม่ใช่เห็นทั้งหมด
...
**บังคับที่:** โครงเดียวกับ `VERTICAL_VISIBLE_SLUGS` ใน `src/lib/seller-menu.ts` (allow-list + fallback ที่ประกาศชัด) · เทส `[blocker]` ส่งค่าประเภทร้านมั่ว แล้วต้องได้ชุดกลาง 5 ตระกูล — เทสแดงทันทีถ้าเปลี่ยนเป็น deny-list
...
- ✅ ร้าน `SERVICE_QUEUE` และ `LODGING` เห็น 5 ตระกูล · `ONLINE_SALES` เห็น 7 ตระกูล · ค่าประเภทร้านที่ไม่รู้จักได้ 5 ตระกูล
...
**ขั้นตอน:**
1. แคตตาล็อกตามประเภทร้านให้ร้านนี้เห็น **5 ตระกูล** — ตระกูล "ส่งไว" และ "ตามพัสดุได้ทุกใบ" ไม่ปรากฏเลย
...
**ผลลัพธ์:**
- ร้านบริการมีเหรียญให้ไล่เก็บ 5 ตระกูล น้อยกว่าร้านขายของ 2 ตระกูล — **ยอมรับโดยตั้งใจ ห้ามเติมเหรียญให้เท่ากัน**
...
**ขั้นตอน:**
1. ทดสอบด้วยร้านจำลองประเภท `LODGING` ⇒ ต้องเห็นชุดกลาง 5 ตระกูล ไม่มีตระกูลของร้านขายของหลุดเข้ามา
2. ทดสอบด้วยค่าประเภทร้านที่ระบบไม่รู้จัก ⇒ ต้องได้ชุดกลาง 5 ตระกูลเช่นกัน
...
- ค่าที่ไม่ได้อัปเดตมานานเกินกำหนด ต้องถือว่า "ยังไม่รู้" ไม่ใช่ใช้ค่าค้างต่อไปเรื่อย ๆ (เกณฑ์จำนวนวันให้ SRS เคาะ)
...
- **ยอมรับว่าร้านบริการมีเหรียญน้อยกว่าร้านขายของ** — 5 ตระกูล เทียบกับ 7
- 🛑 **มี 2 จุดที่ต้องให้เจ้าของงานเคาะก่อนเริ่มลงมือ:** (1) การตีความเรื่องตัวนับ Trust Score ใน FR-BDG-05 (2) ปลายทางของเหรียญ "ขวัญใจชุมชน" ใน FR-BDG-17
