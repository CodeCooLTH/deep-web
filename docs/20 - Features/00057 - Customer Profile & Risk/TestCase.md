...
🛑 **ข้อควรทราบก่อนใช้เอกสารนี้:** เคสกลุ่ม `cod_refund` (TC-CPR-U07..U10, D08) ขึ้นอยู่กับมติที่ยังไม่ปิด — การตรวจข้อมูล prod 2026-08-24 พบว่า `carrierStatus = 'cod_refund'` **ไม่เคยเกิดขึ้นเลยสักครั้ง** (0/427 พัสดุ · 0/1,026 เหตุการณ์ · 0/17 payload ดิบ) ⇒ เคสเหล่านี้ทดสอบได้เฉพาะด้วย fixture ที่แต่งขึ้นเท่านั้น และป้ายที่สร้างจะยังไม่มีวันปรากฏบนหน้าจอจริง ดูรายละเอียดที่ท้ายเอกสาร §6
...
| TC-CPR-A05 | "ซื้อซ้ำแล้ว" = `totalOrders >= 2` | X มี 2 ออเดอร์ (นับทุกสถานะ), Y มี 1 | เลือก "ซื้อซ้ำแล้ว" | เหลือแค่ X | FR-003 |
| TC-CPR-A06 | "ยังซื้อครั้งแรก" = `totalOrders === 1` | เหมือน A05 | เลือก "ซื้อครั้งเดียว" | เหลือแค่ Y | FR-003 |
| TC-CPR-A07 | มีตัวกรองแค่ 2 ตัว ไม่มีตัวกรองช่วงเวลา | — | เปิด dropdown ทั้งหมดในหน้า | เห็นแค่ 2 dropdown | FR-003, BR-CUSTP-13 |
