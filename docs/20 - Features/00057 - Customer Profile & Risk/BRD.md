...
#### FR-003: ชิปกรอง 4 ตัว เลือกได้ทีละอัน

> **แก้ 2026-08-25 (รอบ UI)** — เดิมข้อนี้เป็นดรอปดาวน์ 2 ตัว ("ซื้อซ้ำแล้ว / ยังซื้อครั้งแรก")
> user บอกว่าหน้าเดิมบนมือถือ **"ดูยาก"** — ดรอปดาวน์ 2 ตัว + ช่องจำนวนแถวกินพื้นที่เกือบ 100px
> ก่อนถึงลูกค้าคนแรก และกดยากด้วยนิ้ว ⇒ เปลี่ยนเป็นชิปแนวนอนแถวเดียว
> **"ยังซื้อครั้งแรก" ถูกตัดทิ้ง** — บนข้อมูลจริงมันคือลูกค้าเกือบทั้งหมด (381 จาก 397 คน)
> ชิปที่เลือกแล้วแทบไม่กรองอะไรออกเลย

**User Story:**
> ในฐานะเจ้าของร้าน ฉันต้องการกรองลูกค้าด้วยนิ้วเดียวจากแถวชิปที่เห็นตัวเลือกครบพร้อมกัน

**Acceptance Criteria:**
- [ ] ชิป 4 ตัว เลือกได้ทีละอัน ผูกกับ `?f=` ค่าเดียว (`all` = ไม่ใส่พารามิเตอร์)
  | ค่า | ป้ายบนหน้าจอ | เกณฑ์ | ขอบเขต |
  |---|---|---|---|
  | `all` | ทั้งหมด | — | — |
  | `warn` | ต้องเฝ้าระวัง `{n}` | `hasBehaviorWarning(customerBadges(...))` | ร้านนี้ |
  | `returned` | เคยตีกลับกับร้านนี้ | `shopReputation.returned > 0` | ร้านนี้ |
  | `repeat` | ซื้อซ้ำ | `totalOrders >= 2` | ร้านนี้ |
- [ ] `parseCustomerFilter()` **fail-closed** — ค่าที่ไม่รู้จัก (URL ที่พิมพ์เอง / bookmark เก่าที่ยังเป็น `?repeat=first`) ตกเป็น `all` ไม่ใช่ throw ไม่ใช่จอว่าง
- [ ] 🛑 **ตัวเลขบนชิป "ต้องเฝ้าระวัง" ต้องเป็นค่าเดียวกับการ์ดสถิติใบที่ 3** — คำนวณจากธงชุดเดียวกัน ห้ามคำนวณแยกสองที่ (ไม่งั้นการ์ดบอก 12 แล้วกดกรองได้ 9 โดยไม่มีอะไรฟ้อง)
- [ ] `warn` กับ `returned` **ไม่ใช่อันเดียวกัน** — `warn` รวมการยกเลิกด้วย ⇒ `returned` ⊂ `warn` เสมอ
- [ ] ชิปสูงอย่างน้อย 44px (`min-h-11`) และเลื่อนแนวนอนได้โดย**ไม่ทำให้ทั้งหน้าเลื่อนข้าง**
- [ ] ห้ามมีชิปที่อิงช่วงเวลา (เหตุผลเดิมยังใช้ได้ — §4.3 ของ PRD)

#### FR-015: การ์ดสถิติความน่าเชื่อถือระดับร้าน 4 ใบ

> **เพิ่ม 2026-08-25** — user สั่ง *"อยากให้เน้นสถิติความน่าเชื่อถือ เช่น อัตราปฏิเสธรับของ อัตราคืนของ"*

**Acceptance Criteria:**
- [ ] 4 ใบ: อัตรารับของสำเร็จ (ร้านนี้) · พัสดุตีกลับ (ร้านนี้) · ลูกค้าต้องเฝ้าระวัง · ลูกค้าทั้งหมด
- [ ] **ขอบเขต = ร้านนี้** (KPI ของร้าน) และ **ไม่ผูกกับตัวกรอง/คำค้นหา** — ตัวเลขภาพรวมต้องคงที่ไม่ว่าผู้ขายกำลังพิมพ์ค้นหาอะไรอยู่
- [ ] ตัวหารของอัตรา = **จำนวนใบที่เปิดพัสดุจริง** ไม่ใช่จำนวนออเดอร์หรือจำนวนลูกค้า (ใบที่รับหน้าร้าน/ดิจิทัล/บริการ ไม่มีทางตีกลับได้)
- [ ] ฐาน < 3 ใบ → อัตราแสดง `—` ไม่ใช่ `0%` หรือ `100%`
- [ ] **ไม่มี badge % เปลี่ยนแปลง** แบบธีม (ไม่มีข้อมูลย้อนหลังมาเทียบ) และ**ไม่ใช้ `CountUp`**
- [ ] ร้านที่ยังไม่มีลูกค้าเลย → **ไม่ render แถวการ์ด** (4 ใบที่อ่านว่า 0 ทั้งหมดซ้ำกับข้อความว่างในตาราง)

#### FR-016: แถบสัดส่วนความน่าเชื่อถือรายลูกค้า

**Acceptance Criteria:**
- [ ] แถบ 3 ช่วง: รับของ (`bg-success`) · ตีกลับ (`bg-warning`) · ยังไม่จบ (`bg-default-200`) — **ห้ามมีสีแดง**
- [ ] 🛑 **ขอบเขต = ทั้งระบบ (ข้ามร้าน)** และต้องมีป้าย `(ทั้งระบบ)` กำกับเสมอ เพราะการ์ดสถิติกับชิปในหน้าเดียวกันเป็นระดับร้าน — ภาพเดียวกันสองความหมายคือรอยที่ระบบนี้เคยเจ็บ (HR16)
- [ ] อัตราแสดงเฉพาะฐาน ≥ 3 ใบ · ต่ำกว่านั้นแสดง `ยังบอกอัตราไม่ได้` แต่ **แถบกับจำนวนดิบยังแสดงเสมอ**
- [ ] 3 สถานะที่ต้องพูดคนละประโยค: ไม่ผูก `Customer` กลาง (`ยังไม่มีประวัติข้ามร้าน`) · ผูกแล้วแต่เปิดพัสดุ 0 ใบ (`ยังไม่เคยเปิดพัสดุ`) · มีข้อมูลแต่ฐานไม่พอบอกอัตรา
- [ ] `role="img"` + `aria-label` ที่บอกตัวเลขครบ — **ห้าม `role="progressbar"`** (สื่อว่า "คืบหน้าไป 100%" ซึ่งไม่ตรงความหมาย และ 3 ช่วงจะถูกอ่านปนกัน)
- [ ] ยอดซื้อสะสมรายคน **ยังต้องเด่นเท่าเดิม** (user ย้ำ 2026-08-25) — คอลัมน์ขวาของทุกแถว ตัวหนา ไม่ใช่ข้อความเทาเล็ก ๆ
...
- ✅ ชิปกรอง 4 ตัวเลือกทีละอัน ไม่มีตัวกรองช่วงเวลา · ตัวเลขบนชิปตรงกับการ์ดสถิติเสมอ
...
- **BR-CUSTP-13** *(แก้ 2026-08-25)* ตัวกรองเป็น **ชิป 4 ตัวเลือกทีละอัน** (`all`/`warn`/`returned`/`repeat`) ผูกกับ `?f=` ค่าเดียว · **ห้ามมีตัวกรองช่วงเวลา** (เหตุผลเดิม §4.3) · ตัวเลขบนชิป `warn` ต้องมาจากธงชุดเดียวกับการ์ดสถิติ
