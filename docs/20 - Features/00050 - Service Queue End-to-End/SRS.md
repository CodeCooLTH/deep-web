...
### TFR-SQ-04 · ปุ่มเรื่องเงิน (แชท **และหน้าออเดอร์**)

`chatOrderActions({ orderStatus, appointmentStatus, hasAppointment, money }) → ChatOrderAction[]`

🛑 **ชื่อฟังก์ชันยังขึ้นต้นด้วย `chat` แต่ไม่ใช่ของแชทอย่างเดียวแล้วตั้งแต่ 2026-08-31** —
หน้ารายละเอียดออเดอร์ (`OrderDetailClient.tsx`) เรียกตัวเดียวกันนี้ แล้วหยิบเฉพาะ
`RECORD_PAYMENT` มาเป็นปุ่มหลัก

**ทำไมต้องมีที่หน้าออเดอร์ด้วย:** ปุ่มเดิมมีที่แชทที่เดียว และแชทหาบิลด้วย `Order.customerId`
ซึ่ง server derive จาก **เบอร์โทร** ตอน `createOrder` ⇒ บิลที่เปิดจากปฏิทิน/ตารางงาน
**โดยไม่กรอกเบอร์** ไม่ผูก `Customer` ⇒ ไม่โผล่ในเธรดไหนเลย ⇒ **บันทึกรับเงินไม่ได้ตลอดกาล**
และไม่มีหน้าจอให้แก้ `conversationId` ทีหลัง (schema เขียนห้ามไว้เอง — backfill = เดาจากเบอร์)

**หน้าออเดอร์หยิบเฉพาะ `RECORD_PAYMENT`** — อีก 3 ตัวไม่เอา และไม่ใช่เพราะลืม:

| ปุ่ม | ทำไมไม่เอาที่หน้าออเดอร์ |
|---|---|
| `REQUEST_DEPOSIT` · `MARK_SERVED` | `AppointmentCard` บนหน้านั้นมีปุ่มเดียวกันอยู่แล้ว (ส่งการ์ดสรุปนัด / เข้ารับบริการ-ไม่มาตามนัด) ใส่อีกชุด = ปุ่มคำเดียวกัน 2 ใบในจอเดียว |
| `START_WALK_IN` | **หัวหน้าสั่งให้คงไว้ในแชทเท่านั้น** (2026-08-31) — มีเทส `[blocker]` กันไม่ให้หลุดไป |

🛑 **ชุดเงินที่ป้อนปุ่มต้องไม่ผ่าน `hasMoneyStory()`** — บิลที่ยังไม่ตั้งมัดจำและยังไม่เคยรับเงิน
คือบิลที่ *ต้องการปุ่มมากที่สุด* ถ้าผูกปุ่มไว้กับด่านของการแสดงผล ปุ่มจะหายในกรณีที่มันมีประโยชน์
ที่สุดพอดี (`page.tsx` จึงแยก `serviceMoney` = กั้นด้วย vertical อย่างเดียว ออกจาก `orderMoney`
= กั้นเพิ่มด้วย `hasMoneyStory` สำหรับ *บล็อกเงิน*)
...
### TFR-SQ-04a · "ได้เงินหรือยัง" มีเจ้าของคนเดียวต่อประเภทร้าน (2026-08-31)

| ประเภทร้าน | เจ้าของคำตอบ |
|---|---|
| `SERVICE_QUEUE` | **บัญชี `OrderPayment`** (ฟีเจอร์นี้) |
| `ONLINE_SALES` · `LODGING` | ธง `paymentConfirmedAt` (feature 00062) |
| COD ทุก vertical | `codReceivedAt` |

🛑 ก่อนหน้านี้ **ทั้งสองระบบโผล่พร้อมกันบนบิลบริการ** — ปุ่ม "รับเงินแล้ว" (บัญชี) กับ
"ได้รับเงินแล้ว" (ธง) ในหน้าเดียว และไม่เขียนถึงกันเลย (`recordPayment()` ไม่แตะธง ·
`setPaymentConfirmed()` ไม่สร้างแถวบัญชี) ⇒ กดคนละปุ่มได้ผลคนละอย่างโดยไม่มีอะไรฟ้อง
แก้โดยกั้นการ์ดของ 00062 ไม่ให้เข้า `SERVICE_QUEUE` (ดู `00062/PRD.md` §D-2a)

**ป้ายก็ต้องเดินตามเจ้าของเดียวกัน** — `getPaymentBadge()` รับพารามิเตอร์ที่ 5 `money?`
และ **บัญชีเงินชนะ `Order.status` เสมอเมื่อมีบัญชี**: `status === 'CONFIRMED'` แปลว่า
*ผู้ซื้อยืนยันว่าได้รับบริการแล้ว* ไม่ได้แปลว่าได้เงิน — ร้านบริการปิดงานก่อนเก็บเงินครบเป็นเรื่องปกติ
(วัดจอจริง 2026-08-31: บิล ฿2,500 บัญชี **0 บาท** ขึ้นเขียว "ชำระแล้ว")

ป้ายที่เป็นไปได้เมื่อมีบัญชี: `ชำระเงินแล้ว` (ครบ · เขียว) · `รับบางส่วน` (ได้มัดจำแล้ว) ·
`รอชำระ` (ยังไม่ได้สักบาท) — `PAYMENT_STATE_LABEL` เป็น SSOT ของสามคำนี้

**ครอบ 3 จอ ไม่ใช่แค่จอร้าน** (แก้ 2026-08-31):

| จอ | ผู้ชม | ที่มาของป้าย |
|---|---|---|
| `/orders/{token}` (Paces) | ร้าน | `getPaymentBadge(…, serviceMoney)` |
| `/o/{token}` → `OrderDetailMobile` | ผู้ซื้อที่ล็อกอิน | `getPaymentBadge(…, order.serviceMoney)` |
| `/o/{token}` → `GuestOrderView` | ผู้ถือลิงก์ | `getPaymentBadge(…, order.money)` |

🛑 **จอผู้ซื้อทั้งสองแบบต้องกินตัวเลขชุดเดียวกัน** — `page.tsx` คำนวณ `serviceMoney` **ครั้งเดียว
เหนือจุดแยกสาขา** `if (!session || !viewerUserId)` แล้วส่งให้ทั้งสองทาง · แก้ข้างเดียวไม่ได้
เพราะบิลใบเดียวกันจะขึ้นป้ายคนละอย่างก่อน/หลังล็อกอิน
(เหตุผลเดียวกับ `verifyScope` ในไฟล์นั้นที่เคยเขียนแยกกันแล้วให้ผลไม่ตรงกัน)

จอ guest แสดง **"จ่ายมาแล้ว" + "ยอดค้างชำระ"** ด้วย (user เคาะ 2026-08-31 · ทางเลือก ก) —
รายละเอียด allow-list อยู่ที่ `00041/SRS.md` §ส่วนขยาย 2026-08-31
