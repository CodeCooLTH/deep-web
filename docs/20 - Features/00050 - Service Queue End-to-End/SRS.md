...
#### ความซื่อสัตย์ของตัวเลข (แก้ 2026-08-23 หลัง user เจอบน prod)

`unrecordedValues[]` = ยอดขายของใบที่ **ไม่มีแถว `OrderPayment` เลยสักรายการ** ต่อ bucket

🛑 ใบแบบนั้นคือ **"ไม่รู้"** ไม่ใช่ **"ยังไม่จ่าย"** — ร้านอาจเก็บเงินสดแล้วไม่ได้กดบันทึก
(เคสจริง: ใบวันที่ 1 ส.ค. ของร้าน BT Premium สถานะ `CONFIRMED` งานปิดแล้ว แต่ไม่มีแถว
`OrderPayment` ⇒ ตารางเคยขึ้น "ค้างรับ ฿300" **สีส้ม** ซึ่งอ่านว่าลูกค้ายังไม่จ่าย)

กติกาบนจอ (`partial-data-must-be-labeled-or-filled.md` — เติมข้อมูลให้เต็มไม่ได้ ⇒ ต้อง **บอก**):
- ตัวเลขค้างรับยัง **แสดงอยู่** (ซ่อน = ปิดข้อมูล) แต่ใช้ **สีเตือนเฉพาะแถวที่มีบันทึกรับเงินแล้ว
  บางส่วน** (`received > 0`) ซึ่งเป็นแถวที่ *รู้แน่* ว่าเก็บไม่ครบ
- มีหมายเหตุใต้ตาราง 2 ประโยค (ขึ้นเฉพาะร้านบริการ): **มัดจำนับรวมอยู่ใน "รับจริง" แล้ว**
  (กันผู้ขายบวกสองคอลัมน์เองแล้วได้เกินยอดขาย — ชีตนี้ออกแบบให้ไล่บวกลบตามด้วยนิ้วได้) และ
  **ยอดค้างรับเป็นเพดานบนเมื่อยังมีใบที่ไม่ได้บันทึก** (ประโยคหลังขึ้นเฉพาะเมื่อมีใบแบบนั้นจริง)

#### ทั้งหน้าใช้ "แกนเงิน" แกนเดียว (แก้ 2026-08-23 · user เคาะหลังรีวิวบทวิเคราะห์)

ที่มา: user ถามว่าทำไม **"ยืนยันแล้ว 20,290"** บนหัว กับ **"รับจริง 20,890"** ในตาราง ไม่ตรงกัน
— สองเลขนั้นต่างกัน **2 ชั้น** (ทั้งเดือน vs วันเดียว · แกน "ลูกค้ายืนยัน" vs แกนเงิน) และ
**ไม่มีวันเท่ากัน** · ต้นเหตุคือจอเดียวมีสองแกนพูดคนละภาษา ไม่ใช่ตัวเลขผิด

| ส่วน | เดิม (แกนของร้านขายของ) | ใหม่ (แกนเงิน) |
|---|---|---|
| ชื่อหน้า | ยอดขายและกำไร | **ยอดขายและการเก็บเงิน** |
| ตัวเลขใหญ่ | กำไร/ขาดทุน | **เงินที่รับจริง** |
| แถบสมการ | `รอยืนยัน + ยืนยันแล้ว − ต้นทุน` | **`รับจริง + ค้างรับ = ยอดขาย`** |
| แท่งในกราฟ | ยืนยันแล้ว / รอยืนยัน | **รับจริง / ค้างรับ** |
| ตาราง | 8 คอลัมน์ | **5 คอลัมน์** `วันที่ · งาน · ยอดขาย · รับจริง · ค้างรับ` |

🛑 **"กำไร" ไม่ได้ให้ข้อมูลเพิ่มเลยกับร้านบริการ** — ไม่มีต้นทุนสินค้าและถูกล็อก `NO_SHIPPING`
⇒ กำไร = ยอดขาย **เป๊ะทุกบาท** (ร้านตัวอย่าง ส.ค. 2569: 22,393 = 22,393) มันแค่เรียกยอดขาย
ด้วยชื่อที่ผิด · ส่วน "เงินที่รับจริง" ต่างจากยอดขายจริง (20,890 vs 22,393)

🛑 คอลัมน์ที่ถอดออก: **มัดจำ** (subset ที่บวกไม่ได้ ⇒ ผู้ขายบวกรวมแล้วเกินยอดขาย แล้วสรุปว่า
ระบบคำนวณผิด) และ **ยืนยันแล้ว/ยังไม่ยืนยัน** (คนละแกน — ย้ายไปแถวสถานะงาน)

#### แถวสถานะงาน — `jobStatusCounts`

`งานทั้งหมด · เสร็จแล้ว · นัดไว้ · ไม่มาตามนัด · ยกเลิก` ใต้ตัวเลขใหญ่ (เฉพาะร้านบริการ)

🛑 **เป็นที่เดียวในหน้านี้ที่นับใบที่ยกเลิก** — ทุกตัวเลขอื่นตัด `CANCELLED` ทิ้งตั้งแต่ query
⇒ ร้านตัวอย่างยกเลิก **2 จาก 10 งาน** ในเดือน ส.ค. แล้วไม่มีทางรู้จากหน้านี้เลย
การซ่อนงานที่ยกเลิกคือการรายงานเดือนที่ดูดีกว่าความจริง

4 กลุ่ม **ไม่ทับกันและบวกกันได้ `total` พอดี** — ลำดับการตัดสินสำคัญ:
`ยกเลิก (ชนะทุกอย่าง) → เสร็จแล้ว → ไม่มาตามนัด → ที่เหลือ`
⇒ ใบที่ `CANCELLED` + `appointmentStatus='COMPLETED'` นับเป็นยกเลิกอย่างเดียว (มีจริงบนฐาน)

**perf:** เป็น `groupBy` ที่อยู่ใน `Promise.all` **ก้อนเดิมกับ query หลัก** ⇒ ขนาน ไม่เพิ่ม
wall time · และรอบนี้ถอด `depositValues`/`receivedConfirmedValues` ที่ไม่มีผู้อ่านแล้วออกจาก
payload (เลิกจ่ายทั้ง join `kind` และขนาด flight payload ทุกครั้งที่เปิดหน้า)

#### คำที่ใช้เรียก "จำนวนใบ"

ร้านบริการเรียกว่า **"งาน"** ผ่าน `countNounOf()` **นิยามเดียว** — คำนี้โผล่ 3 ที่
(หัวคอลัมน์ในตาราง · ป้ายใต้กราฟ · tooltip) เขียนแยกกันเมื่อไหร่จะมีที่ที่ยังเรียก
"คำสั่งซื้อ" อยู่แล้วผู้อ่านนึกว่าเป็นคนละตัวเลข (HR16) · ตรงกับที่ระบบใช้อยู่แล้วใน FAB
(`createLabelShort: 'งานใหม่'`)

#### ป้ายสถานะเงินบนหน้ารายละเอียด

🛑 `getPaymentBadge` (`src/lib/order-display.ts`) **ไม่รู้จักตาราง `OrderPayment` เลย** — มันอ่าน
`Order.status` + `paymentMethod` เท่านั้น ⇒ ใบที่ `PENDING` + วิธีชำระนอก enum (เช่น `CASH`)
ตกไป fallback `"ยังไม่ยืนยันการชำระ"` เสมอ

ร้าน `SERVICE_QUEUE` จึงเคยขึ้นป้ายขัดกันสองใบติดกันบนใบเดียว: `"ชำระเงินแล้ว"` (จากเงินจริง)
คู่กับ `"ยังไม่ยืนยันการชำระ"` (จาก status) — user เจอบน prod 2026-08-23 (`DP256908CEB304D4`)

**แก้ที่การ render ไม่ใช่แก้ `getPaymentBadge`** — ฟังก์ชันนั้นถูกใช้โดย vertical อื่นที่ยังไม่มี
ตาราง `OrderPayment` เลย · `OrderSummary` ซ่อนป้ายเดิมเมื่อมี `serviceBadge` (HR16 — คำถาม
เดียวกันต้องมีคำตอบเดียวบนจอ)
