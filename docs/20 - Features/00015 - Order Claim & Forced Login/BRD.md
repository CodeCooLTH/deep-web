...
## 11. ภาคผนวก — Phase 2 (2026-07-25): ความต้องการเพิ่มเติม

> ต่อท้าย ไม่แก้ §1–§10 ของ v1.0 · อ้างอิงข้อตัดสิน D1–D7 ใน [[PRD]] §11.2

### FR-OCL-20 ยืนยันเบอร์เมื่อบัญชียังพิสูจน์ความเป็นเจ้าของไม่ได้

แทนสถานะทางตันเดิมที่มีเพียงปุ่มออกจากระบบ

**AC-20-01** — **กำหนดให้** ผู้ซื้อล็อกอินแล้วแต่บัญชีไม่มีเบอร์ หรือมีเบอร์คนละตัวกับที่ร้านบันทึกไว้ **เมื่อ** เปิดลิงก์คำสั่งซื้อ **แล้ว** ต้องเห็นหน้ายืนยันเบอร์ที่กรอกเบอร์เองได้ ไม่ใช่หน้าที่ไปต่อไม่ได้

**AC-20-02** — **กำหนดให้** อยู่ที่หน้ายืนยันเบอร์ **เมื่อ** หน้าถูกแสดง **แล้ว** ต้องไม่เปิดเผยเบอร์ที่ร้านบันทึกไว้แม้แต่หลักเดียว ไม่ว่าจะเต็มหรือปิดบางส่วน เพราะผู้ใช้ยังไม่ได้พิสูจน์ว่าเกี่ยวข้องกับคำสั่งซื้อนี้

**AC-20-03** — **กำหนดให้** กรอกเบอร์และรับรหัสยืนยันสำเร็จ **เมื่อ** เบอร์ตรงกับที่ร้านบันทึกไว้และยังไม่มีบัญชีอื่นถือเบอร์นั้น **แล้ว** ระบบผูกเบอร์เข้ากับบัญชีปัจจุบันและให้เข้าถึงคำสั่งซื้อได้ทันที

**AC-20-04** — **กำหนดให้** กรอกเบอร์และรับรหัสยืนยันสำเร็จ **เมื่อ** เบอร์ไม่ตรงกับที่ร้านบันทึกไว้ **แล้ว** ต้องแจ้งว่าไม่ตรงพร้อมทางเลือกที่ทำได้จริง คือลองเบอร์อื่นหรือติดต่อร้าน

**AC-20-05** — **กำหนดให้** บัญชีปัจจุบันผูกเบอร์อื่นไว้แล้ว **เมื่อ** ยืนยันเบอร์ของคำสั่งซื้อสำเร็จ **แล้ว** ต้องไม่เขียนทับเบอร์เดิม และแจ้งว่าเบอร์เปลี่ยนไม่ได้พร้อมบอกทางออก (ข้อตัดสิน D5)

**AC-20-06** — **กำหนดให้** ผู้ใช้กำลังจะผูกเบอร์กับบัญชี **เมื่อ** ยังไม่ยืนยันว่าตรวจสอบเบอร์แล้ว **แล้ว** ปุ่มขอรหัสต้องกดไม่ได้ คำเตือนต้องเด่นพอให้หยุดอ่าน ไม่ใช่ข้อความช่วยเหลือตัวจาง

### FR-OCL-21 เชื่อมบัญชีเมื่อเบอร์เป็นของบัญชีที่มีอยู่แล้ว

**AC-21-01** — **กำหนดให้** ยืนยันเบอร์สำเร็จ **เมื่อ** เบอร์นั้นเป็นของบัญชีอื่นที่มีอยู่แล้ว **แล้ว** ต้องเสนอเชื่อมบัญชีในปุ่มเดียว พร้อมอธิบายว่าเชื่อมแล้วได้อะไร ไม่ใช่เพียงแจ้งว่ามีบัญชีอยู่แล้ว

**AC-21-02** — **กำหนดให้** กดเชื่อมบัญชี **เมื่อ** ทำสำเร็จ **แล้ว** ช่องทางล็อกอินที่ใช้อยู่ต้องย้ายไปผูกกับบัญชีเดิม และสถานะการเข้าสู่ระบบต้องกลายเป็นบัญชีเดิม โดยผู้ใช้ไม่ต้องกรอกอะไรเพิ่ม

**AC-21-03** — **กำหนดให้** บัญชีที่ล็อกอินอยู่มีคำสั่งซื้อ ร้าน รีวิว หรือเบอร์อยู่แล้ว **เมื่อ** กดเชื่อมบัญชี **แล้ว** ต้องปฏิเสธและส่งต่อให้แอดมิน ห้ามรวมข้อมูลอัตโนมัติ (ข้อตัดสิน D4)

**AC-21-04** — **กำหนดให้** บัญชีเดิมเชื่อมช่องทางประเภทเดียวกันไว้อยู่แล้ว **เมื่อ** กดเชื่อมบัญชี **แล้ว** ต้องปฏิเสธ ห้ามเขียนทับการเชื่อมเดิม เพราะแปลว่าเป็นคนละบัญชีของช่องทางนั้น

**AC-21-05** — **กำหนดให้** มีการออกสิทธิ์เชื่อมบัญชี **เมื่อ** สิทธิ์นั้นถูกนำไปใช้โดยผู้ใช้อื่น กับคำสั่งซื้อใบอื่น หรือเกินเวลาที่กำหนด **แล้ว** ต้องถูกปฏิเสธทุกกรณี

### FR-OCL-22 ความน่าเชื่อถือของร้านบนหน้าลิงก์

**AC-22-01** — **กำหนดให้** เปิดลิงก์คำสั่งซื้อโดยยังไม่ล็อกอิน **เมื่อ** หน้าถูกแสดง **แล้ว** ต้องเห็นภาพและตัวตนของร้าน พร้อมหลักฐานความน่าเชื่อถือ ก่อนถูกขอให้ล็อกอิน

**AC-22-02** — **กำหนดให้** ร้านยังไม่มีคำสั่งซื้อสำเร็จ **เมื่อ** หน้าถูกแสดง **แล้ว** ต้องซ่อนแถบสถิติทั้งแถบ ห้ามแสดงเลขศูนย์ และห้ามแสดงตัวเลขที่ไม่ได้มาจากข้อมูลจริง

**AC-22-03** — **กำหนดให้** ร้านไม่มีรีวิวที่มีข้อความ **เมื่อ** หน้าถูกแสดง **แล้ว** ต้องซ่อนส่วนรีวิว ห้ามแสดงข้อความชมที่ระบบสร้างขึ้นเอง

**AC-22-04** — **กำหนดให้** ยังไม่ล็อกอิน **เมื่อ** หน้าถูกแสดง **แล้ว** ต้องไม่ปรากฏรายการสินค้าในคำสั่งซื้อ (ข้อตัดสิน D7)

**AC-22-05** — **กำหนดให้** ร้านยังไม่ได้อัปโหลดภาพหน้าปก **เมื่อ** หน้าถูกแสดง **แล้ว** ต้องใช้โลโก้ร้านเป็นพื้นหลังแทน ไม่ปล่อยพื้นที่ว่าง

### FR-OCL-23 ภาพหน้าปกร้าน

**AC-23-01** — **กำหนดให้** อยู่ที่หน้าตั้งค่าร้าน **เมื่อ** อัปโหลดภาพหน้าปกและบันทึก **แล้ว** ภาพต้องปรากฏบนหน้าลิงก์คำสั่งซื้อของร้านนั้น

**AC-23-02** — **กำหนดให้** เป็นสมาชิกของร้าน **เมื่อ** ส่งคำขอแก้ไขข้อมูลร้าน **แล้ว** ต้องแก้ได้เฉพาะช่องที่อนุญาตเท่านั้น ช่องอื่นในตารางต้องไม่ถูกเขียนทับแม้ส่งมาในคำขอ

### 12. กฎทางธุรกิจเพิ่มเติม

| รหัส | กฎ |
|---|---|
| BR-OCL-20 | เบอร์โทรคือแหล่งความจริงเดียวของตัวตน ช่องทางล็อกอินอื่นพิสูจน์ความเป็นเจ้าของคำสั่งซื้อแทนไม่ได้ |
| BR-OCL-21 | คำสั่งซื้อที่ไม่มีเบอร์ผูกไว้ ไม่มีใครยึดเป็นเจ้าของได้ทั้งสิ้น |
| BR-OCL-22 | ห้ามรวมข้อมูลข้ามบัญชีโดยอัตโนมัติ |
| BR-OCL-23 | สถิติและรีวิวที่แสดงต้องมาจากข้อมูลจริงเท่านั้น ไม่มีก็ซ่อน |
