...
- 🛑 Postgres คืน SQLSTATE **`23P01 exclusion_violation`** ซึ่ง Prisma **ไม่ได้ map เป็น `P2002`** — service ต้องดักด้วยการตรวจ `err.code === 'P2010'` หรือ meta ที่มี `23P01` แล้วโยน `RoomUnavailableError`
...
- Supabase เปิด extension `btree_gist` ได้ — **ต้องยืนยันก่อนเริ่ม P2** ถ้าเปิดไม่ได้ต้องเปลี่ยนไปใช้ล็อกระดับแถว (`SELECT ... FOR UPDATE` บน `Room`) ซึ่งด้อยกว่าเพราะพึ่งพาวินัยของโค้ด
...
| **`btree_gist` ใช้ไม่ได้บน Supabase** | กลไกกันจองทับหลักใช้ไม่ได้ | ยืนยันก่อนเริ่ม P2; แผนสำรอง = ล็อกแถว `Room` ในธุรกรรม |
| **`23P01` ไม่ถูกดัก** | ผู้ใช้เห็น 500 แทนข้อความว่าห้องถูกจองแล้ว | ทดสอบเคสนี้เป็น acceptance ไม่ใช่ optional |
