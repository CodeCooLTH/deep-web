...
|------|------|----------|-------------------|
| `VALIDATION_ERROR` | 400 | ข้อมูลไม่ถูกต้อง | ข้อมูลไม่ถูกต้อง กรุณาตรวจสอบอีกครั้ง |
| `INVALID_DATE_RANGE` | 400 | วันเช็คเอาท์ไม่หลังวันเข้าพัก | วันเช็คเอาท์ต้องอยู่หลังวันเข้าพักอย่างน้อย 1 คืน |
| `DEPOSIT_EXCEEDS_TOTAL` | 400 | มัดจำเกินยอดรวม | ยอดมัดจำต้องไม่เกินยอดรวมของการจอง |
| `ROOM_INACTIVE` | 400 | ห้องถูกปิดการใช้งาน | ห้องนี้ถูกปิดการใช้งานอยู่ ไม่สามารถสร้างการจองใหม่ได้ |
| `NOT_A_BOOKING` | 400 | ออเดอร์นี้ไม่ใช่การจอง | รายการนี้ไม่ใช่การจอง |
| `CANCEL_REASON_REQUIRED` | 400 | ไม่ได้เลือกเหตุผล | กรุณาเลือกเหตุผลในการยกเลิก |
| `INVALID_CANCEL_REASON` | 400 | เหตุผลไม่อยู่ในรายการ | เหตุผลในการยกเลิกไม่ถูกต้อง |
| `FORBIDDEN` | 403 | ไม่มีสิทธิ์ | คุณไม่มีสิทธิ์ดำเนินการนี้ |
| `NOT_LODGING_SHOP` | 403 | ร้านไม่ใช่ประเภทบ้านพัก | ธุรกิจนี้ไม่ใช่ประเภทบ้านพักตากอากาศ |
| `BOOKING_CONFIRM_VIA_SHOP` | 403 | ผู้จองพยายามยืนยันเอง | การจองต้องได้รับการยืนยันจากเจ้าของที่พัก |
| `ROOM_NOT_FOUND` | 404 | ไม่พบห้อง | ไม่พบห้องพักนี้ |
| `NOT_FOUND` | 404 | ไม่พบรายการ | ไม่พบรายการที่ต้องการ |
| **`ROOM_UNAVAILABLE`** | **409** | ช่วงวันถูกจองแล้ว | ห้องนี้ถูกจองในช่วงวันที่เลือกแล้ว กรุณาเลือกวันอื่น |
| `DEPOSIT_LOCKED` | 409 | แก้มัดจำหลังแนบสลิป | ผู้จองชำระตามยอดเดิมแล้ว หากต้องการเปลี่ยนเงื่อนไขต้องยกเลิกแล้วสร้างใหม่ |
| `SLIP_REQUIRED` | 409 | ยืนยันโดยยังไม่มีสลิป | ยังไม่มีสลิปการโอน กรุณาตรวจสอบก่อนยืนยัน |
| `BOOKING_NOT_EDITABLE` | 409 | สถานะไม่อนุญาตให้แก้ | การจองนี้ไม่อยู่ในสถานะที่แก้ไขได้ |
| `BOOKING_CANCELLED` | 409 | ดำเนินการกับการจองที่ยกเลิกแล้ว | การจองนี้ถูกยกเลิกไปแล้ว |
| `INVALID_TRANSITION` | 409 | เปลี่ยนสถานะไม่ถูกต้อง | ไม่สามารถเปลี่ยนสถานะของรายการนี้ได้ |
