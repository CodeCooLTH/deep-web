...
- [ ] เมื่อเลือกทรัพยากรที่มีระยะเวลามาตรฐาน ชิป "ใช้เวลา" ของค่านั้นถูกเลือกไว้ให้ และเปลี่ยนได้ (แก้ 2026-08-09 — ผู้ขายเลือก *ระยะเวลา* ไม่ใช่กรอก *เวลาสิ้นสุด*; ระบบคำนวณเวลาสิ้นสุดให้)
...
4. เลือกวันและเวลาเริ่ม → เลือกชิป "ใช้เวลา" (ค่ามาตรฐานของคิวงานถูกเลือกไว้ให้) → ระบบคำนวณเวลาสิ้นสุด
...
#### FR-RSV-12: เก็บมัดจำบางส่วนตอนล็อกวัน แล้วเก็บส่วนที่เหลือหน้างาน

> **หมายเหตุการเปลี่ยนขอบเขต (2026-07-31):** เดิม §7.1 และ PRD §4.2 ระบุว่า "ไม่มีระบบมัดจำเฉพาะทาง" เป็นข้อจำกัดของ phase 1 — เจ้าของผลิตภัณฑ์กลับการตัดสินใจนี้ เพราะรูปแบบ "มัดจำ 30% ล็อกวัน แล้วจ่ายส่วนที่เหลือหน้าร้าน" เป็นวิธีทำงานปกติของร้านบริการ ไม่ใช่ความสามารถเสริม ข้อจำกัดเดิมถูกแก้ให้ตรงกันแล้วทั้งสองที่

**User Story:**
> ในฐานะเจ้าของร้านบริการ ฉันต้องการเก็บมัดจำบางส่วนตอนที่ลูกค้าล็อกวัน เพื่อลดความเสียหายเมื่อลูกค้าไม่มา และให้ลูกค้ารู้ล่วงหน้าว่าต้องจ่ายอีกเท่าไรตอนมาถึงหน้าร้าน

**Acceptance Criteria:**
- [ ] ทรัพยากรแต่ละหน่วยตั้ง **มัดจำเริ่มต้น** ได้ 2 แบบ: จำนวนเงินคงที่ (บาท) หรือสัดส่วนของยอดรวม (เปอร์เซ็นต์)
- [ ] มัดจำเริ่มต้นเป็นศูนย์ได้ = ทรัพยากรนั้นไม่เก็บมัดจำ
- [ ] เปอร์เซ็นต์ต้องอยู่ระหว่าง 0 ถึง 100; จำนวนเงินต้องไม่ติดลบ
- [ ] ตอนสร้างออเดอร์พร้อมนัด ระบบเติมยอดมัดจำให้อัตโนมัติจากค่าเริ่มต้นของทรัพยากรที่เลือก **และผู้ใช้แก้เป็นยอดอื่นได้เสมอ**
- [ ] ยอดมัดจำที่บันทึกต้องไม่เกินยอดรวมของออเดอร์ — ถ้าคำนวณแล้วเกิน ให้เท่ากับยอดรวม
- [ ] ยอดมัดจำของออเดอร์เป็นค่าที่บันทึกไว้ ณ ตอนสร้าง **ไม่อ้างอิงค่าเริ่มต้นของทรัพยากรแบบสด** — แก้ค่าเริ่มต้นของทรัพยากรภายหลังต้องไม่ทำให้ยอดของออเดอร์เก่าขยับ
- [ ] ลูกค้าเห็นบนหน้าออเดอร์ว่า **มัดจำเท่าไร** และ **เหลือจ่ายหน้างานอีกเท่าไร**
- [ ] 🛑 **มัดจำไม่ใช่เงื่อนไขของการล็อกวัน** — นัดถูกกันคิวทันทีที่บันทึก ไม่ว่าจะจ่ายมัดจำแล้วหรือยัง
- [ ] 🛑 ระบบ **ไม่ติดตามสถานะการจ่าย** ในเฟสนี้ — แสดงยอดอย่างเดียว ไม่มีการแนบสลิป ไม่มีปุ่มยืนยันรับเงิน
- [ ] ออเดอร์ที่ไม่มีนัด และออเดอร์ที่มัดจำเป็นศูนย์ ต้องไม่แสดงส่วนมัดจำเลย

**Business Flow:**
1. เจ้าของร้านตั้งที่ทรัพยากร "คลาสเช้า" ว่าเก็บมัดจำ 30 เปอร์เซ็นต์
2. แอดมินร้านสร้างออเดอร์ยอดรวม 1,000 บาท เลือกทรัพยากร "คลาสเช้า" และวันนัด
3. ระบบเติมยอดมัดจำให้อัตโนมัติ 300 บาท
4. แอดมินปรับเป็น 500 บาทได้ถ้าตกลงกับลูกค้าไว้แบบนั้น
5. บันทึก → นัดถูกล็อกทันที ยอดมัดจำถูกเก็บไว้กับออเดอร์
6. ลูกค้าเปิดลิงก์ เห็น "มัดจำ 500 บาท · จ่ายหน้างานอีก 500 บาท"

**Example:**
```
ทรัพยากร "คลาสเช้า" ตั้งมัดจำ 30%
→ ออเดอร์ 1,000 บาท   มัดจำอัตโนมัติ 300 บาท   เหลือจ่ายหน้างาน 700 บาท
→ ออเดอร์ 1,000 บาท แอดมินแก้เป็น 500   เหลือจ่ายหน้างาน 500 บาท

ทรัพยากร "หมอนวด A" ตั้งมัดจำ 200 บาท (คงที่)
→ ออเดอร์ 500 บาท     มัดจำ 200 บาท          เหลือจ่ายหน้างาน 300 บาท
→ ออเดอร์ 150 บาท     มัดจำ 150 บาท          เหลือจ่ายหน้างาน 0 บาท   (ตัดไม่ให้เกินยอดรวม)

ทรัพยากร "ครูส่วนตัว" ตั้งมัดจำ 0
→ ออเดอร์ 800 บาท     ไม่แสดงส่วนมัดจำเลย
```

### 2.7 หน่วยเวลาของการนัด

#### FR-RSV-13: ร้านเลือกได้ว่าจะจองเป็นวัน หรือระบุช่วงเวลา

> **ที่มา (2026-07-31):** ลูกค้ากลุ่มแรกคือ **ร้านรับตกแต่งไฟหน้ารถยนต์** — ลูกค้าขับรถเข้ามาแล้ว "จองวันเข้ามาใช้" ไม่ได้นัดเป็นชั่วโมง การบังคับกรอกเวลาเริ่ม-สิ้นสุดจึงเป็นภาระที่ไม่ได้ให้ประโยชน์กับร้านกลุ่มนี้เลย
>
> แต่ร้านเดียวกันก็อาจต้องระบุเวลาในบางงาน (เจ้าของยกตัวอย่าง: วันเดียวกัน คันแรกจอง 09:00–12:00 คันที่สอง 13:00–14:00 คันที่สาม 14:00–15:00) จึงต้องเลือกได้ ไม่ใช่บังคับทางใดทางหนึ่ง
>
> ข้อนี้ปิดคำถามที่ค้างไว้ใน BR-RSV-42 (การนัดแบบ "ทั้งวัน")

**User Story:**
> ในฐานะเจ้าของร้าน ฉันต้องการเลือกได้ว่าร้านฉันรับนัดเป็น "รายวัน" หรือ "ระบุช่วงเวลา" เพื่อไม่ต้องกรอกเวลาที่ไม่มีความหมายกับธุรกิจของฉัน

**Acceptance Criteria:**
- [ ] มีเมนูตั้งค่าระดับร้าน ให้เลือกหน่วยเวลาของการนัดได้ 2 แบบ: **รายวัน** หรือ **ระบุช่วงเวลา**
- [ ] ค่าเริ่มต้นคือ **รายวัน** — เป็นแบบที่กรอกน้อยที่สุด ร้านที่ต้องการละเอียดกว่าค่อยเปลี่ยนเอง
- [ ] โหมด **รายวัน**: ฟอร์มสร้างออเดอร์แสดงเฉพาะช่องวันที่ ไม่มีช่องเวลาเริ่ม/เวลาสิ้นสุด
- [ ] โหมด **ระบุช่วงเวลา**: ปฏิทินมีขั้น "เลือกเวลา" = ชิปเวลาเริ่ม + ชิประยะเวลา (โหมด "กำหนดเอง" ยังกรอกเวลาสิ้นสุดเองได้)
- [ ] เปลี่ยนโหมดได้ตลอดเวลา และ **ไม่กระทบนัดที่บันทึกไว้แล้ว** — นัดเก่ายังแสดงตามที่บันทึกไว้จริง
- [ ] ปฏิทินคิวและหน้าออเดอร์ฝั่งลูกค้าแสดงให้เหมาะกับข้อมูลที่มี: นัดรายวันแสดงแค่วันที่ ไม่โชว์ช่วงเวลาที่ไม่มีความหมาย
- [ ] การกันจองเกินความจุทำงานเหมือนเดิมทั้งสองโหมด — นัดรายวันของวันเดียวกันบนคิวงานเดียวกันนับเข้าความจุตามปกติ

**Business Flow:**
1. เจ้าของร้านตกแต่งไฟเข้าเมนูตั้งค่า เลือก "รับนัดเป็นรายวัน"
2. คีย์ออเดอร์ → เห็นแค่ช่องวันที่ ไม่มีช่องเวลา
3. เลือก 5 ส.ค. → บันทึก → คิวงานนั้นถูกกันไว้ทั้งวัน
4. ภายหลังร้านรับงานถี่ขึ้น เปลี่ยนเป็น "ระบุช่วงเวลา"
5. คีย์ออเดอร์ใหม่ → เห็นช่องเวลา → จองได้ 09:00–12:00, 13:00–14:00, 14:00–15:00 ในวันเดียวกัน
6. นัดที่บันทึกไว้ตอนโหมดรายวันยังแสดงเป็น "ทั้งวัน" เหมือนเดิม ไม่ถูกแปลงค่า

**Example:**
```
ร้านตกแต่งไฟหน้ารถ — โหมดรายวัน, คิวงาน "ช่องบริการ 1" รับพร้อมกัน 2 คิว
→ 5 ส.ค. คันที่ 1   สำเร็จ (จองแล้ว 1 จาก 2)
→ 5 ส.ค. คันที่ 2   สำเร็จ (จองแล้ว 2 จาก 2)
→ 5 ส.ค. คันที่ 3   ปฏิเสธ — เต็มแล้ว 2 จาก 2

ร้านเดียวกัน เปลี่ยนเป็นโหมดระบุช่วงเวลา
→ 6 ส.ค. 09:00-12:00   สำเร็จ
→ 6 ส.ค. 13:00-14:00   สำเร็จ (ไม่ทับกับช่วงแรก)
→ 6 ส.ค. 14:00-15:00   สำเร็จ (ต่อกันพอดี ไม่ถือว่าทับ)
```
...
**เมื่อระบบทำงานถูกต้อง:**
- ✅ ตั้งมัดจำเริ่มต้นที่ทรัพยากรได้ทั้งแบบจำนวนเงินและแบบเปอร์เซ็นต์ และตั้งเป็นศูนย์เพื่อไม่เก็บมัดจำได้
- ✅ ยอดมัดจำถูกเติมให้อัตโนมัติตอนเลือกทรัพยากร และร้านแก้เป็นยอดอื่นได้ก่อนบันทึก
- ✅ ยอดมัดจำไม่เกินยอดรวมของออเดอร์ในทุกกรณี
- ✅ แก้ค่ามัดจำเริ่มต้นของทรัพยากรแล้ว ยอดของออเดอร์ที่สร้างไปแล้วไม่ขยับตาม
- ✅ ลูกค้าเห็นชัดว่ามัดจำเท่าไรและเหลือจ่ายหน้างานอีกเท่าไร
- ✅ นัดถูกกันคิวทันทีที่บันทึก ไม่ว่าจะจ่ายมัดจำแล้วหรือยัง — ไม่มีสถานะรอชำระที่กั้นการกันคิว
- ✅ ออเดอร์ที่มัดจำเป็นศูนย์ และออเดอร์ที่ไม่มีนัด ไม่แสดงส่วนมัดจำเลย
...
flowchart TD
    ...
    F --> G[เลือกวันและเวลาเริ่ม แล้วเลือกระยะเวลา ระบบคำนวณเวลาสิ้นสุด]
    G --> H[กดบันทึก]
...
3. เลือกทรัพยากร "หมอนวด A" วันเสาร์ 14:00 → ชิป "1 ชม. 30 นาที" ถูกเลือกไว้ให้ → ระบบคำนวณเวลาสิ้นสุด 15:30
...
- มัดจำเก็บได้ (FR-RSV-12) แต่เป็น **ยอดที่บันทึกไว้เท่านั้น** — ระบบไม่ติดตามสถานะการจ่าย ไม่มีการแนบสลิป และไม่มีการกระทบยอดคงเหลือใด ๆ การรับเงินจริงเกิดนอกระบบ
- มัดจำไม่กั้นการล็อกวัน — นัดถูกกันคิวทันทีที่บันทึก ไม่ว่าลูกค้าจะจ่ายมัดจำแล้วหรือยัง (ต่างจาก feature 00017 ที่การจองห้องพักรอสลิปก่อนยืนยัน)
...
- **BR-RSV-42** การนัดแบบ "ทั้งวัน" เก็บเป็นช่วงเวลาที่ครอบทั้งวัน ไม่ใช่โครงสร้างข้อมูลแยก — ต่างกันที่การแสดงผลเท่านั้น ✅ **user ยืนยันแล้ว 2026-07-31** (ดู FR-RSV-13)

### 8.11 หน่วยเวลาของการนัด (FR-RSV-13)

- **BR-RSV-53** หน่วยเวลาของการนัดเป็นค่าตั้งค่า **ระดับร้าน** มี 2 แบบ: **รายวัน** หรือ **ระบุช่วงเวลา** — ค่าเริ่มต้นคือรายวัน (กรอกน้อยที่สุด)
- **BR-RSV-54** 🛑 ทั้งสองโหมดเก็บข้อมูลด้วย **โครงสร้างเดียวกัน** (`serviceStart`/`serviceEnd`) — โหมดรายวันเก็บเป็นช่วงที่ครอบทั้งวันตามเวลาไทย ไม่มีตาราง/คอลัมน์แยก และ **ไม่มี flag บอกว่าแถวนี้เป็นรายวัน** เพราะนัดที่กินทั้งวันพอดีก็คือทั้งวันอยู่แล้ว
- **BR-RSV-55** โหมดเป็นแค่ค่าที่บอกว่า **"ตอนนี้ฟอร์มควรถามอะไร"** — เปลี่ยนโหมดต้องไม่แตะนัดที่บันทึกไว้แล้วแม้แต่แถวเดียว นัดเก่าแสดงตามช่วงเวลาที่บันทึกไว้จริงเสมอ
- **BR-RSV-56** การกันจองเกินความจุใช้กลไกเดียวกันทั้งสองโหมด — นัดรายวันของวันเดียวกันบนคิวงานเดียวกันทับกันเต็ม ๆ จึงนับเข้าความจุตามปกติโดยไม่ต้องมีกฎพิเศษ
- **BR-RSV-57** การแสดงผลตัดสินจาก **ข้อมูลจริงของแถวนั้น** ไม่ใช่จากโหมดปัจจุบันของร้าน — ช่วงเวลาที่ครอบทั้งวันพอดีแสดงเป็น "ทั้งวัน" ไม่งั้นแสดงช่วงเวลา ทำแบบนี้ร้านที่สลับโหมดไป-มาจะไม่เห็นนัดเก่าเพี้ยน

### 8.10 มัดจำ (FR-RSV-12)

- **BR-RSV-43** ทรัพยากรแต่ละหน่วยเก็บ **มัดจำเริ่มต้น** ได้ 2 รูปแบบ: จำนวนเงินคงที่ หรือสัดส่วนของยอดรวมเป็นเปอร์เซ็นต์ — รูปแบบและค่าเป็นของทรัพยากร ไม่ใช่ของร้าน
- **BR-RSV-44** ค่าเริ่มต้นเป็นศูนย์ได้ และหมายความว่า "ทรัพยากรนี้ไม่เก็บมัดจำ" — ออเดอร์ที่ได้ยอดมัดจำเป็นศูนย์ต้องไม่แสดงส่วนมัดจำในทุก surface
- **BR-RSV-45** เปอร์เซ็นต์ต้องอยู่ในช่วง 0–100 และจำนวนเงินต้องไม่ติดลบ — ค่านอกช่วงต้องถูกปฏิเสธที่ทั้งฝั่งหน้าจอและฝั่งหลังบ้าน
- **BR-RSV-46** 🛑 ยอดมัดจำของออเดอร์เป็น **ค่าที่บันทึกไว้ ณ ตอนสร้าง** ห้ามให้ออเดอร์อ้างอิงค่าเริ่มต้นของทรัพยากรแบบสด — แก้ค่าเริ่มต้นของทรัพยากรภายหลังต้องไม่ทำให้ยอดของออเดอร์เก่าขยับ (บทเรียนเดียวกับ `Room.depositMode` ของ feature 00017 และเหตุผลเดียวกับที่ `durationMinutes` เป็นแค่ค่าตั้งต้น ตาม BR-RSV-09)
- **BR-RSV-47** ยอดมัดจำต้องไม่เกินยอดรวมของออเดอร์ — ถ้าคำนวณจากค่าเริ่มต้นแล้วเกิน ให้เท่ากับยอดรวม
- **BR-RSV-48** ยอดมัดจำที่เติมอัตโนมัติเป็นเพียงค่าตั้งต้น ผู้ใช้ฝั่งร้านแก้เป็นยอดอื่นได้เสมอก่อนบันทึก
- **BR-RSV-49** 🛑 มัดจำ **ไม่ใช่เงื่อนไขของการล็อกวัน** — นัดถูกกันคิวทันทีที่บันทึกสำเร็จ ไม่ว่าจะจ่ายมัดจำแล้วหรือยัง ห้ามเพิ่มสถานะรอชำระที่กั้นการกันคิว (ต่างจาก flow การจองห้องพักของ feature 00017 โดยเจตนา)
- **BR-RSV-50** 🛑 ระบบ **ไม่ติดตามสถานะการจ่ายมัดจำ** ในเฟสนี้ — ไม่มีการแนบสลิป ไม่มีปุ่มยืนยันรับเงิน ไม่มีสถานะจ่ายแล้ว/ยังไม่จ่าย การรับเงินจริงเกิดนอกระบบทั้งหมด
- **BR-RSV-51** ยอดที่ลูกค้าเห็นว่า "จ่ายหน้างาน" คำนวณเป็น ยอดรวม ลบ ยอดมัดจำ เสมอ — เป็นค่าที่คำนวณตอนแสดงผล ไม่เก็บเป็นฟิลด์แยก เพื่อไม่ให้มีสองแหล่งที่ขัดกันได้
- **BR-RSV-52** มัดจำใช้ได้เฉพาะออเดอร์ที่มีนัด — ออเดอร์ที่ไม่มีนัดต้องไม่มีทั้งช่องกรอกและส่วนแสดงผล เพื่อไม่ให้กระทบเส้นทางออเดอร์เดิม (สอดคล้อง BR-RSV-36 ที่ห้ามฟีเจอร์นี้เปลี่ยนขั้นตอนออเดอร์เดิม)
...
| **คิวงาน** | หน่วยที่ถูกจองเวลา เช่น ช่องบริการหนึ่งช่อง ช่างหนึ่งคน เตียงหนึ่งเตียง ห้องหนึ่งห้อง คลาสหนึ่งคลาส<br>🛑 **คำที่ผู้ใช้เห็นคือ "คิวงาน"** (user ตัดสิน 2026-07-31 — คำเดิม "ทรัพยากร" อ่านแล้วไม่เข้าใจ) ส่วนชื่อใน schema/โค้ดยังเป็น `ServiceResource`/`resourceId` ตามเดิม ไม่ต้องเปลี่ยน |
| **ความจุ** | จำนวนนัดที่คิวงานหนึ่งหน่วยรับได้พร้อมกันในช่วงเวลาที่ทับกัน — ร้านตั้งเอง ค่าเริ่มต้น 1 |
| **หน่วยเวลาของการนัด** | ค่าตั้งค่าระดับร้านว่ารับนัดเป็น **รายวัน** หรือ **ระบุช่วงเวลา** (FR-RSV-13) — เปลี่ยนได้ตลอด ไม่กระทบนัดเก่า |
...
| **การกันคิว** | การที่นัดหนึ่งนับเข้าความจุของคิวงานในช่วงเวลานั้น ทำให้ที่ว่างลดลงหนึ่งที่ |
