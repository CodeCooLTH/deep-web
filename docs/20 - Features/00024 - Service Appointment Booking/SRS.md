...
- ต้อง mask/ตัดข้อมูลลูกค้าที่ไม่จำเป็น **ที่ server boundary** ไม่ใช่ตอนแสดงผล
- **อีเมลไม่ส่งออกทุกกรณี** — ยังไม่มีจอไหนขอ

**แก้ 2026-08-11 — เบอร์โทรส่งได้แล้ว แต่เฉพาะคำขอระดับ "วัน"**

ข้อกำหนดเดิมเขียนว่า *"ปฏิทินต้องการแค่ชื่อย่อลูกค้า + เลขออเดอร์ ไม่ต้องการเบอร์"* — นั่นคือ
**ความต้องการ ณ ตอนนั้น ไม่ใช่กฎความปลอดภัย** และเลิกเป็นจริงเมื่อ user สั่ง (2026-08-11) ให้การ์ด
คิวงานรายวันแสดง **ช่องทางที่มา · รูปโปรไฟล์ลูกค้า · เบอร์โทร** ให้ผู้ขายโทรหาลูกค้าได้จากจอนั้น

กติกาที่ใช้แทน — **แยกตามขนาดของคำขอ ไม่ใช่ตามหน้าจอ**:

| คำขอ | คืนอะไร | เหตุผล |
|---|---|---|
| `GET …/appointments?from&to` (ระดับเดือน — ปฏิทิน) | **ไม่มีเบอร์ ไม่มีรูป** เหมือนเดิมทุกประการ | ปฏิทินต้องการแค่ "วันไหนมีกี่นัด" |
| `GET …/appointments/day?date` (ระดับวัน — การ์ด) | `buyerContact` · `customerAvatarUrl` · `source{channel,pageName,pageAvatarUrl}` · `conversationId` | ผู้ขายต้องโทร/ทักแชทได้ทันที |

- 🛑 endpoint รายวัน **รับ `date` (หนึ่งวัน) ไม่รับช่วงเวลาอิสระ** — เพดานถูกบังคับด้วย *รูปร่างของ
  input* ไม่ใช่ด้วยความตั้งใจของผู้เรียก (เพดานที่บังคับไม่ได้ = เพดานที่ไม่มีอยู่จริง)
- 🛑 **ห้ามยุบสอง endpoint กลับเป็นตัวเดียว** — ปฏิทินยิงครอบทั้งเดือน การรวมกันแปลว่าเบอร์ลูกค้า
  ทั้งเดือน (ร้านที่ยุ่ง 100–200 เบอร์) นอนอยู่ใน flight payload เพื่อแสดงผลวันเดียว
- `customerAvatarUrl` เป็น `null` ได้เสมอและเป็นค่าปกติ — Meta บล็อกรูปโปรไฟล์ Messenger ทั้งหมดอยู่
  (ต้อง Advanced Access ของ `Business Asset User Profile Access`) ฝั่ง UI ต้องให้ "ตัวย่อ" เป็นของหลัก
