...
#### 🛑 advisory lock — เงื่อนไขที่ทำให้กลไกข้างบนทำงานได้จริง (แก้ 2026-07-31 หลัง TC-A05 ไม่ผ่าน)

คำอธิบายข้างบนจริงเฉพาะเมื่อ "ถูกปฏิเสธ" **เกิดขึ้นทันที** ซึ่งเป็นจริงเฉพาะกับแถวที่ **commit ไปแล้ว**

EXCLUDE บน GiST สร้าง **predicate lock** — ถ้า transaction อื่นเขียนที่นั่งเดียวกันไว้แต่ยังไม่ commit ตัวที่มาทีหลังจะ **ถูกบล็อกให้รอ ไม่ใช่ได้ error ทันที** จึงติดค้างที่นั่งนั้น ไปลองที่นั่งถัดไปไม่ได้ TC-A05 วัดจริงเมื่อ 2026-07-31: ยิง 12 request พร้อมกันบนความจุ 8 → **สำเร็จ 0 ราย** ด้วย deadlock (40P01) + transaction timeout

จึงต้องเรียงคิวการจัดสรร **ต่อทรัพยากรหนึ่งหน่วย** ก่อนเข้าลูป:

```sql
SELECT pg_advisory_xact_lock(24, hashtext($resourceId));
```

- `24` = namespace ของฟีเจอร์นี้ (ตรวจแล้วยังไม่มีที่ไหนในโปรเจกต์ใช้ advisory lock — ตัวนี้เป็นตัวแรก จะเพิ่มที่อื่นต้องจองเลขใหม่)
- ขอบเขต = ต่อ `resourceId` ไม่ใช่ทั้งร้าน → ทรัพยากรคนละตัวจองพร้อมกันได้ปกติ
- `xact` = ปลดเองตอน transaction จบ ไม่มีทางลืมปลด
- มี lock เดียว → ไม่มีทางเกิด deadlock
- 🛑 **ไม่ได้แทนที่ EXCLUDE** — constraint ยังเป็นตัวตัดสินสุดท้ายเหมือนเดิม advisory lock แค่ทำให้ลูปเห็น state ที่ commit แล้วอย่างที่กลไกต้องการ

ผลหลังแก้: ความจุ 8 ยิง 12 → 8 สำเร็จ / 4 ได้ 409 เสถียร 4 รอบติด; ทดสอบหนักขึ้น 40 ยิงบนคิว 8 และ 60 ยิงบนคิว 20 ผ่านทั้งคู่ ไม่มี timeout กลับมา (ดู TestCase.md §5.1)
