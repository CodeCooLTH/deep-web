...
- หน้าต่างคงที่ **08:00–20:00 ทุก 1 ชั่วโมง = 12 ปุ่ม** กริด 4 คอลัมน์ — user เคาะให้ใช้ค่านี้ไปก่อน 🛑 **ไม่มีคอลัมน์ "เวลาทำการ" ใน `Shop`/`ServiceResource` ใน DB จริง** ไม่มี migration ไปกับงานนี้ — ระยะห่างคงที่ 1 ชม. ไม่ผูกกับ `resourceDurationMinutes` (บริการ 25 นาทีจะได้ ~28 ปุ่มซึ่งล้นทุกความกว้าง)
- ช่องเวลา `<input type="time">` เดิมยังอยู่ครบใต้ปุ่มพวกนี้ — เป็น **ทางลัด ไม่ใช่ทางเดียว** ร้านที่เปิดนอกช่วง 08:00–20:00 หรือเวลาไม่ลงล็อกยังกรอกเองได้
- กด 1 ปุ่มได้ทั้งเวลาเริ่ม+สิ้นสุด (auto-fill จาก `resourceDurationMinutes` ผ่าน `applyStart()` ตัวเดียวกับ ทางลัด "ต่อจากคิวก่อนหน้า")
- 🛑 **ปุ่มที่ชนคิวเดิมไม่ `disable`** — ติดจุดเตือน (`s.busy`, สัญลักษณ์เดียวกับ legend "มีคิวแล้ว") เท่านั้น ยึด BR-RSV-18 (การตรวจฝั่ง client เป็นแค่ UX ไม่ใช่กลไกความถูกต้อง — ข้อมูลอาจ stale ระหว่างชีตเปิดค้าง) การ disable จาก client data จะบล็อกช่วงที่จริง ๆ ยังจองได้
- เวลาที่ผ่านไปแล้วของวันนี้ (`s.past`) มัวลง (`opacity-50`) แต่ยังกดได้ (FR-RSV-03/BR-RSV-15 อนุญาตนัดย้อนหลัง)
