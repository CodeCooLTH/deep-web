...
- 🛑 **`s.busy` = "เต็ม" ไม่ใช่ "มีคิวแล้ว" — คนละความหมายกับจุดในปฏิทิน** (แก้ 2026-08-09)
  จุดในช่องวันของปฏิทินคือ `used > 0` (มีคิวบ้าง) ส่วนชิปคือ `overlap >= capacity` (เต็ม)
  เดิมทั้งสองใช้จุด `bg-warning` 6px เหมือนกันเป๊ะ และ `aria-label` ของชิปยังพูดว่า "มีคิวแล้ว"
  ทั้งที่เงื่อนไขคือเต็ม → ผู้ใช้เห็น/ได้ยินสัญญาณเดียวกันในสองสถานะที่ต่างกัน
  ปฏิทินสื่อ "เต็ม" ด้วย **กากบาทแทนเลขวัน** (เปลี่ยนรูปร่าง) ชิปจึงต้องพูดภาษาเดียวกัน:
  พื้น `bg-warning/15` + ตัวหนังสือ `text-warning-ink` (6.20–6.56:1 — ตัวแบกความหมายคือ
  **สีตัวหนังสือ** ซึ่งวัดด้วยเกณฑ์ข้อความ 4.5:1 ไม่ใช่จุด 6px ที่ได้ 1.55:1) + ไอคอน
  `alert-triangle-filled` เป็นตัวบอกด้วย **รูปร่าง** ตาม WCAG 1.4.1 (ห้ามสื่อด้วยสีอย่างเดียว)
  ยังกดได้ตามเดิม (BR-RSV-18) — สิ่งที่เปลี่ยนคือผู้ขายเห็นก่อนว่ากำลังจะจองทับ
...
4. **`overflow-x-hidden` ที่กล่องเลื่อน** — `overflow-y-auto` ทำให้แกน x กลายเป็น `auto` ตามสเปก CSS
   โดยอัตโนมัติ ลูกที่ล้นแนวนอนแม้พิกเซลเดียวจะทำให้ทั้งชีตเลื่อนข้างได้ (ดู "บทเรียน" ท้ายหัวข้อ)

**เดินวัน — 2 ทางเข้าเรียกฟังก์ชันเดียวกัน** (`‹ ›` · จิ้มวันในปฏิทิน)
· เปลี่ยนวันในเดือนเดียวกัน **ไม่ยิง API** ของปฏิทิน · ข้ามขอบเดือนต้อง `gotoDate` ให้ปฏิทินข้างหลัง
ตามด้วย ไม่งั้นปิดชีตแล้วเจอเดือนเก่า · **ลูกศรห้ามกระโดดข้ามวันว่าง** (เดินไม่เท่ากันทุกครั้ง =
ผู้ใช้เลิกเชื่อว่ามันคือ "วันถัดไป")

🛑 **ไม่มีการปัดซ้ายขวาเปลี่ยนวัน** (ถอดออก 2026-08-12 — user สั่ง "เอา slide ข้าง ออกก็ได้นะ")
ที่ถอดแล้วไม่เสียอะไร: ลูกศรทำงานได้ทุกกรณีอยู่แล้ว **รวมคีย์บอร์ด/screen reader ที่ปัดไม่ได้เลย**
และ iOS/WebView ยึด "ปัดขวาจากขอบซ้าย" ไว้เป็นปุ่มย้อนกลับอยู่แล้ว การปัดจึงไม่เคยเป็นทางที่เชื่อถือได้
ตั้งแต่แรก · ที่ได้กลับมา: ไม่มี `transform` บนกล่องเลื่อน = ไม่มีทางที่เนื้อหาจะค้างเลื่อนออกนอกจอ
และไม่ต้องแย่งแกนกับการเลื่อนอ่านรายการ

**บทเรียน 2026-08-12 (ขึ้น prod แล้วผู้ใช้เจอทันที):** ชื่อเพจยาว `Messenger · BT Premium Auto Xenon
คลอง4 ธัญบุรี` **ไม่ถูกตัด** ทั้งที่ใส่ `truncate` ไว้แล้ว เพราะกล่อง `flex-wrap` ไม่มี `min-w-0`
และลูกไม่มี `max-w-full` ⇒ ตัวหนังสือดันกล่องกว้างเกินจอ ⇒ `overflow-y-auto` เปิดแกน x ให้เอง ⇒
การ์ดถูกดันหลุดขอบซ้ายทั้งจอ · รายละเอียดเต็ม + วิธีกันสองชั้น: `docs/conventions/flex-header-truncation.md` §5
