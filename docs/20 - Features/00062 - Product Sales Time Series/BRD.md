...
- การติ๊กเลือก/ยกเลิกเส้นบนกราฟ (เฉพาะ **≥1024px** — ดูหมายเหตุเส้นแบ่งจอด้านล่าง)

**ออกจากระบบ (Output):**
- กราฟเส้น Top 5 สินค้าขายดีของเดือน (**≥768px**)
- ตารางสินค้าทุกตัว + ช่องติ๊กเลือกเส้น (**≥1024px**)
- รายการสินค้าแบบการ์ด + แถบสรุปรายวัน (**<1024px**)

🛑 **เส้นแบ่งจอมี 3 ช่วง ไม่ใช่จุดเดียว** (แก้ 2026-08-29 ให้ตรงกับโค้ด):
| ความกว้าง | กราฟรวม | รายการ | ติ๊กเลือกเส้นได้ |
|---|---|---|---|
| `<768` | ไม่มี (5 เส้น × 31 จุดบน 360px อ่านไม่ออก) | การ์ด | ไม่ได้ |
| `768–1023` | **มี** (อ่านออกสบายที่ความกว้างนี้) | การ์ด | **ไม่ได้** — กราฟล็อก Top 5 ตายตัว มีคำกำกับบอกไว้ |
| `≥1024` | มี | ตาราง 6 คอลัมน์ | ได้ |

ตารางถูกกั้นที่ `lg` (1024) ไม่ใช่ `md` เพราะ 6 คอลัมน์ต้องการความกว้างระดับนั้น — แพตเทิร์นเดียวกับ
`ProductsListing.tsx` · ชีตเต็มจอรายสินค้าจึงใช้ได้ถึง `<1024px` ไม่ใช่ `<768px`
...
**Acceptance Criteria:**
- [ ] นับ `OrderItem` ของ `Order.status != 'CANCELLED'` — ณ วันนี้คือ **PENDING + CONFIRMED + SHIPPED + RETURNED**
- [ ] 🛑 เกณฑ์ผูกกับ *สิ่งที่ไม่นับ* ไม่ใช่รายการที่นับ (`Order.status` เป็น `String` ไม่มี CHECK constraint ⇒ เพิ่มค่าใหม่ได้โดยไม่มีอะไรฟ้อง)
- [ ] 🛑 **ออเดอร์ที่ถูกคืนของครบ (`RETURNED`) ถูกนับเป็นยอดขายด้วย** — ตั้งใจ เพราะต้องเท่ากับ `getBestSellerProducts()` เป๊ะ การกันออกจะสร้างนิยาม "ยอดขาย" ชุดที่ 4 · **ยังไม่เคยมีใครตัดสินว่าควรกันออกไหม** (สถานะนี้เพิ่งมีจาก feature 00056 หลัง `getBestSellerProducts` ถูกเขียน)
- [ ] มีข้อความอธิบายนิยามนี้แสดงอยู่บนหน้าจอเสมอ พร้อมระบุว่าต่างจากหน้า "ภาพรวมกำไร/ขาดทุน"

**Example:**
```
ร้าน A มี 30 ออเดอร์ในเดือน: PENDING 5 · SHIPPED 19 · CONFIRMED 3 · RETURNED 1 · CANCELLED 2
ยอด "ขายแล้ว" ของรายงานนี้      = จากออเดอร์ 28 ใบ (ทุกสถานะยกเว้น CANCELLED — รวม RETURNED)
...
### 2.5 การแสดงผลบนจอกว้าง (กราฟ ≥768px · ตาราง ≥1024px)
...
- 🛑 **แก้ 2026-08-29 หลังตรวจกับ schema จริง:** เดิมบันทึกว่า "`OrderItem` ไม่มี index บน `productId`" เป็นความเสี่ยงของรายงานนี้ — *ข้อเท็จจริงถูก แต่ผูกผิดที่* query กรองผ่านความสัมพันธ์กับ `Order` (มี `Order_shopId_status_createdAt_idx`) แล้วเข้าถึง `OrderItem` ผ่าน `OrderItem_orderId_idx` และ **จัดกลุ่มตาม `productId` ใน JS ไม่ใช่ SQL** ⇒ index บน `productId` ไม่ถูกใช้ไม่ว่าจะมีหรือไม่ · จะเป็นความเสี่ยงจริงในวันที่มีคนย้ายไป `GROUP BY "productId"` เท่านั้น — วันนั้นต้องสร้าง index ในคอมมิตเดียวกัน
- ปริมาณ `OrderItem` ที่ดึงมา bucket ใน JS โตตามจำนวนออเดอร์ — กันด้วยเพดาน `MAX_ITEM_ROWS` 20,000 แถว + ธง `truncated` ที่บอกผู้ใช้ · เกณฑ์ย้ายไปคำนวณฝั่ง DB ~1,500 ออเดอร์/เดือน
