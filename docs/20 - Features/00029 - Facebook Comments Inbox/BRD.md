...
## 8. ข้อตัดสินก่อนเขียน SRS/SDS

> ทั้ง 4 ข้อ **user ยืนยันครบแล้ว 2026-08-03** — Q-2 และ Q-4 ยืนยันตามที่ทีมเสนอ

| # | ประเด็น | ข้อตัดสิน (user 2026-08-03) |
|---|---|---|
| Q-1 | ชื่อคนคอมเมนต์ | **เก็บชื่อไว้ด้วย** — ถ้า webhook ไม่ให้ `from.name` มา ให้ยิง Graph เติมทีหลังแบบ best-effort (ไม่บล็อกการบันทึกคอมเมนต์) และ cache ไว้ต่อผู้ใช้ ไม่ยิงซ้ำทุกคอมเมนต์ |
| Q-2 | ย้อนหลังแค่ไหน | **ตอนเชื่อมครั้งแรก: โพสต์ย้อนหลัง 30 วัน เฉพาะที่มีคอมเมนต์ (สูงสุด 25 โพสต์)** · คอมเมนต์ในโพสต์: โหลดเมื่อ "กดเปิดโพสต์" ครั้งละ 30 อัน เลื่อนโหลดเพิ่ม · ตั้งแต่เชื่อมแล้วอาศัย webhook เป็นหลัก ไม่ poll |
| Q-3 | ช่องค้นหา | **มี** — ค้นได้ทั้งข้อความคอมเมนต์และชื่อผู้คอมเมนต์ ขอบเขต = คอมเมนต์ของร้านตัวเองเท่านั้น |
| Q-4 | ตัวนับ "ยังไม่ตอบ" | **นับเฉพาะคอมเมนต์ของลูกค้าที่ยังไม่มีคำตอบของเพจอยู่ข้างใต้** — คอมเมนต์ที่เพจเขียนเอง (เช่น ปักหมุดราคา) ไม่ถูกนับเป็น "ยังไม่ตอบ" เพราะเพจไม่ต้องตอบตัวเอง |

### 8.1 เหตุผลของ Q-2 (ทำไม 30 วัน / 25 โพสต์)

- โฆษณาที่ยังวิ่งอยู่มักอายุไม่เกิน 2-4 สัปดาห์ — คอมเมนต์ที่ "ยังตอบทัน" แทบทั้งหมดอยู่ในช่วงนี้
- โพสต์ 1 โพสต์ที่มีคอมเมนต์ 220 อัน = อย่างน้อย 3 หน้า Graph (100/หน้า) ถ้าดึงเต็มทุกโพสต์ตอนเชื่อม = สิบ ๆ คำขอในคำสั่งเดียว เสี่ยงชน rate limit ตั้งแต่วันแรก จึงดึง "หัวโพสต์" ก่อน แล้วค่อยดึงคอมเมนต์ตอนเปิดจริง
- ผู้ขายไม่ได้ไล่ตอบคอมเมนต์ของโพสต์เมื่อ 3 เดือนก่อน — ถ้าอยากได้จริงค่อยเพิ่มปุ่ม "โหลดโพสต์เก่ากว่านี้" ทีหลัง (ถูกกว่าการดึงมาก่อนโดยไม่มีใครดู)
- ไม่มีการลบคอมเมนต์เก่าอัตโนมัติใน v1 — สิ่งที่เข้าระบบแล้วอยู่ต่อ (ฐานเดียวกับ prod ลบทิ้งมีความเสี่ยงมากกว่าเก็บไว้)
