...
> **กลุ่ม X–AD เพิ่ม 2026-08-11** — ครอบของที่สร้าง *หลัง* เอกสารนี้ถูกเขียน (Batch D/E) และของที่
> `/impeccable critique` จับได้หลัง ship ไปแล้ว. ทุกเคสในกลุ่มนี้ผ่าน `tsc`/`build`/detector/
> theme-guard มาหมดตอนที่ยังพัง เพราะเป็นโค้ดที่ถูกต้องตามชนิดทุกตัวอักษร — **สิ่งที่ผิดคือความหมาย
> ซึ่งมีแต่คนเปิดหน้าจอดูเท่านั้นที่จับได้**

### กลุ่ม X — Browser QA: timeline พัสดุ 4 จุดบนหน้าผู้ซื้อ 🛑 เคยพังทุกสถานะยกเว้น "กำลังส่ง"

> วิธีหาออเดอร์: ฝั่งร้าน `/orders?stage=<ค่า>` — ชิปกรองใช้ค่าเดียวกับที่ระบบคำนวณ
> **กลุ่ม N ที่มีอยู่เดิมตรวจแค่ "ป้ายสถานะ" (badge) ไม่ได้ตรวจจุดบน timeline** — นั่นคือเหตุผลที่บั๊กนี้หลุด

| # | หาออเดอร์จาก | Expected บน `/o/{token}` |
|---|--------------|---------------------------|
| X01 | `?stage=AWAITING_PICKUP` | ไฮไลต์จุดที่ 1 "สร้างพัสดุ" |
| X02 | `?stage=SHIPPING` | ไฮไลต์จุดที่ 3 "กำลังจัดส่ง" · จุด 1–2 เขียว |
| **X03** | ออเดอร์ที่ขนส่งส่งถึงแล้ว (CONFIRMED / จบเส้นทาง) | 🛑 **ทั้ง 4 จุดเขียว ไม่มีจุดไหนเป็นสีน้ำเงิน** — เดิมไฮไลต์ "สร้างพัสดุ" (จุดแรก) และจุด "จัดส่งสำเร็จ" ไม่มีทางติดได้เลย |
| **X04** | `?stage=PROBLEM` | 🛑 **แถบแดง "พัสดุมีปัญหา" ต้องขึ้นเหนือ timeline** — เดิม**ไม่เคยขึ้นเลยสักครั้งตั้งแต่วันแรก** (เทียบ `'PROBLEM'` กับ `'PARCEL_PROBLEM'` ซึ่งไม่มีวันตรงกัน) · จุดที่ 3 เป็นสีแดง |
| X05 | ออเดอร์ที่ยังไม่เปิดพัสดุ (`AWAITING_PARCEL`) | **ไม่วาด timeline เลย** ไม่ใช่แถบเทา 4 จุดลอย ๆ |
| X06 | ออเดอร์บริการ/ดิจิทัลที่จบแล้ว (ไม่เคยมีพัสดุ) | ไม่วาด timeline — ห้ามวาดแถบเขียวครบให้พัสดุที่ไม่มีอยู่จริง |
| X07 | เทียบ X02/X03/X04 กับ `MiniShipmentTimeline` ฝั่งร้านของใบเดียวกัน | **จุดที่ไฮไลต์ตรงกันทั้งสองจอ** (ตอนนี้อ่าน `SHIPMENT_STAGE_DOT_INDEX` ตัวเดียวกัน) |
| X08 | ป้ายใต้จุด (screen reader / zoom 200%) | อ่านออก — ไม่ใช่ 10.5px สีจาง · มี `role="img"` พร้อมชื่อ "สถานะพัสดุ: …" |

---

### กลุ่ม Y — Browser QA: ด่านก่อนยืนยันรับของ 🛑 การกระทำที่ย้อนไม่ได้

> ⚠️ **กดยืนยันจริงแล้วย้อนไม่ได้** — ใช้ออเดอร์ทดสอบเท่านั้น ห้ามใช้ของลูกค้าจริง

| # | Precondition | Expected |
|---|--------------|----------|
| **Y01** | order PENDING + วิธีชำระ = โอนเข้าบัญชี | 🛑 ปุ่มเขียนว่า **"ยืนยันรับสินค้า"** — ห้ามมีคำว่า "ยืนยันการชำระเงิน" ปรากฏที่ไหนเลย (เดิมป้ายบอกคนละเรื่องกับสิ่งที่ปุ่มทำ = รูปร่างของสแกมที่ product นี้มีไว้กัน) |
| Y02 | order PENDING + COD | ปุ่มเขียนว่า "ยืนยันรับสินค้า" เหมือนกัน (ไม่แตกตามวิธีจ่ายเงินอีกแล้ว) |
| Y03 | order ที่ไม่ต้องจัดส่ง (บริการ/ดิจิทัล) | "ยืนยันว่าได้รับแล้ว" |
| **Y04** | กดปุ่มหลัก | 🛑 **ขึ้นกล่องถามก่อน ไม่ใช่ยืนยันทันที** · ในกล่องต้องมีข้อความว่า "ยืนยันแล้วจะแจ้งปัญหากับคำสั่งซื้อนี้ไม่ได้อีก" |
| Y05 | ในกล่อง กด "ยังไม่ยืนยัน" | ปิดกล่อง · **สถานะออเดอร์ไม่เปลี่ยน** · ปุ่มแจ้งปัญหายังอยู่ |
| Y06 | ในกล่อง กด "ได้รับแล้ว" | ยืนยันสำเร็จ · toast บอกว่า **บันทึกลงประวัติของร้านแล้ว** (ไม่ใช่แค่ "ขอบคุณ") · ปุ่มแจ้งปัญหาหายไป |
| Y07 | ใต้ปุ่มหลัก | **ไม่มีข้อความที่ขึ้นต้นว่า "แตะเพื่อ…"** (บรรยายท่าทางแทนผลลัพธ์ + ซ้ำกับป้ายปุ่ม) |
| Y08 | เทียบตำแหน่ง "ยืนยัน" กับ "ยกเลิกคำสั่งซื้อ" | ทั้งคู่มีกล่องยืนยัน — ไม่มีอันไหนกดทีเดียวจบ |

---

### กลุ่ม Z — Browser QA: หลักฐานร้านบนหน้า guest (ต้องเปิดใน **incognito**)

> 🛑 หน้านี้จะไม่โผล่เลยถ้าเบราว์เซอร์ยังมี session ค้างอยู่ — ทดสอบในหน้าต่างส่วนตัวเท่านั้น

| # | Precondition | Expected |
|---|--------------|----------|
| Z01 | ร้าน L1 (ยืนยันเบอร์อย่างเดียว) | ป้าย **"ยืนยันเบอร์แล้ว" โทนเทา** — ไม่ใช่เขียว และไม่ใช่คำว่า "ยืนยันแล้ว" ลอย ๆ |
| Z02 | ร้าน L2 | "ยืนยันเอกสารแล้ว" เขียว |
| Z03 | ร้าน L3 | "จดทะเบียนธุรกิจแล้ว" ทอง |
| Z04 | ร้านที่ยังไม่ยืนยันอะไรเลย (L0) | **ไม่มีป้ายเลย** — ห้ามมีป้ายที่เขียนว่า "ยังไม่ยืนยัน" (ไม่ประจานร้านใหม่) |
| **Z05** | ร้านที่มีออเดอร์สำเร็จแล้ว | 🛑 **แบนเนอร์ไล่สีตาม tier** — ถ้าเห็น**ทุกร้าน**เป็นเทาหมด แปลว่าส่ง `completionRate` เข้า `ProfileBanner` ตรง ๆ ซึ่งเป็นค่าที่ถูก hardcode เป็น `null` เสมอ |
| **Z06** | ร้านที่ยังไม่มีออเดอร์สำเร็จเลย | 🛑 **แบนเนอร์เทา + ป้าย "ร้านใหม่"** ไม่ใช่ไล่สีที่หน้าตาเหมือนรางวัล |
| Z07 | ร้านที่มีสถิติ | เห็นยอด "ออเดอร์สำเร็จ" + คะแนน "จาก N รีวิว" |
| Z08 | ร้านที่ยังไม่มีสถิติ | **ซ่อนทั้งแถบ** ไม่ใช่แสดงเลข 0 ตัวโต |
| Z09 | ร้านที่เชื่อมเพจ/IG ไว้ | เห็นรูป+ชื่อเพจ และคำว่า Facebook Page / Instagram |
| Z10 | ร้านที่มีรีวิวที่เขียนข้อความ | เห็นรีวิวจริง 1 อัน + ป้าย "ซื้อจริง" — **ไม่มีชื่อผู้รีวิวและไม่มีวันที่** (ประกอบกันชี้ตัวคนซื้อได้ในร้านที่ออเดอร์ยังน้อย) |
| Z11 | ใต้หัวข้อ "ข้อมูลผู้รับ" | มีบรรทัดอธิบายว่าทำไมเห็นแค่บางส่วน — จุดไข่ปลาที่ไม่มีคำอธิบายอ่านได้ว่า "เว็บนี้ปิดบังอะไรอยู่" |
| **Z12** | กดปุ่มเข้าสู่ระบบ → ดูหน้าถัดไป | 🛑 **ป้ายยืนยันเป็นคำเดียวกันและสีเดียวกัน** กับที่เพิ่งเห็น (สองจอนี้ห่างกันไม่กี่วินาที ผู้ใช้สังเกตได้) |

---

### กลุ่ม AA — Browser QA: ตอบกลับรีวิวฝั่งผู้ขาย (`/seller/reviews`)

| # | Precondition | Expected |
|---|--------------|----------|
| AA01 | ร้านมีรีวิวที่ยังไม่ตอบ | มุมขวาบนเป็น **จำนวนรีวิวที่ยังไม่ได้ตอบ** ไม่ใช่ placeholder "กราฟแนวโน้มรีวิว" |
| AA02 | ตอบครบทุกรีวิวแล้ว | วงกลมเขียว + "ตอบครบทุกรีวิวแล้วในตอนนี้" · **ไม่มีปุ่ม "ตอบตอนนี้"** (ไม่มีปุ่มที่กดแล้วไม่มีอะไรให้ทำ) |
| AA03 | กด "ตอบตอนนี้" | สลับไปแท็บ "ยังไม่ตอบ" + เลื่อนไปที่รายการ · ถ้าเปิด reduce-motion ของ OS ต้องไม่มีอนิเมชันเลื่อน |
| **AA04** | กดเปิดฟอร์มตอบ | 🛑 **มีบรรทัดบอกว่า "ลูกค้าจะเห็นคำตอบนี้ในหน้าคำสั่งซื้อของเขา"** — เดิมระบบยอมรับว่าเป็นสาธารณะแค่ตอน *กดลบ* คือบอกตอนกำลังจะเอาคืน แต่ไม่บอกตอนกำลังจะพูด |
| **AA05** | พิมพ์คำตอบค้างในรีวิว A → กดตอบรีวิว B → กลับมา A | 🛑 **ข้อความที่พิมพ์ต้องยังอยู่** — เดิมหายเงียบ ๆ ไม่มี error แยกไม่ออกจาก "แอปกินข้อความไป" |
| AA06 | พิมพ์เกิน 1,000 ตัวอักษร | ตัวนับเปลี่ยนสี + ข้อความเตือน + ปุ่มบันทึกกดไม่ได้ · **ข้อความที่วางมาต้องไม่ถูกตัดหางทิ้ง** |
| AA07 | บันทึกสำเร็จ | ตัวเลข "ยังไม่ได้ตอบ" ลดลง 1 · badge บนแท็บลดตาม · การ์ดเปลี่ยนเป็นสถานะ "ตอบแล้ว" โดยไม่ต้องรีโหลด |
| AA08 | กดลบคำตอบ | มีกล่องยืนยันที่บอกว่า "คำตอบจะหายจากหน้าที่ลูกค้าเห็นทันที" · ลบแล้วกลับเป็น "ยังไม่ตอบ" |
| AA09 | แท็บ "ยังไม่ตอบ" ที่ว่างเปล่า (ร้านมีรีวิวแต่ตอบครบ) | ข้อความ "ตอบครบทุกรีวิวแล้ว" — **ไม่ใช่ "ยังไม่มีรีวิว"** ซึ่งขัดกับตัวนับบนหัวการ์ดที่เพิ่งบอกว่ามี |
| AA10 | ค้นหาคำที่อยู่ใน **เนื้อรีวิว** (ไม่ใช่ชื่อสินค้า/ชื่อคน) | เจอ — ช่องเขียนว่า "ค้นหารีวิว" ต้องค้นรีวิวได้จริง |
| AA11 | มือถือ: ปุ่มบันทึก/ยกเลิกในฟอร์ม | สูงพอกดด้วยนิ้ว (≥44px) ไม่ใช่ปุ่มเล็กแบบเดสก์ท็อป |
| AA12 | รีวิวที่ลูกค้าแนบรูป | เห็นรูปย่อ · กดแล้วเปิดเต็มจอ · เลื่อนซ้าย-ขวาได้ · Esc/กดพื้นหลังปิดได้ · **ลากนิ้วบนมือถือแล้วหน้าข้างหลังต้องไม่เลื่อนตาม** |

---

### กลุ่ม AB — Browser QA: อัปโหลดสลิป (เพดานจริง)

| # | เคส | Expected |
|---|-----|----------|
| **AB01** | แนบรูปจากมือถือขนาด **6–8MB** | 🛑 **ต้องผ่าน** — เดิมขึ้น "แนบสลิปไม่สำเร็จ" แล้วกดวนได้ไม่จบ เพราะไฟล์วิ่งผ่าน body ของ function ซึ่งตันที่ 4.5MB ก่อนถึงโค้ดเรา |
| AB02 | ข้อความใต้หัวข้ออัปโหลด | ตัวเลขเพดานตรงกับที่ระบบบังคับจริง (อ่านจาก `uploadMaxSize`) ไม่ใช่เลขที่พิมพ์ไว้ |
| AB03 | แนบไฟล์ที่ใหญ่เกินเพดานจริง | ข้อความบอก **ขนาดที่เกิน** ไม่ใช่ "ลองใหม่อีกครั้ง" ลอย ๆ (ลองใหม่กับไฟล์เดิมไม่มีวันสำเร็จ) |
| AB04 | แนบ PDF | ผ่าน (นโยบายรับ PDF ตาม FR-2.5) |

---

### กลุ่ม AC — Browser QA: มือถือจริง (ทำจากเดสก์ท็อปไม่ได้)

> 🛑 **ต้องเป็นเครื่องจริง ไม่ใช่ device toolbar ของ DevTools** — `env(safe-area-inset-*)` คืน 0 บนเดสก์ท็อป Chrome เสมอ ปัญหาชนิดนี้จึงมองไม่เห็นจากเครื่อง dev

| # | เครื่อง | Expected |
|---|---------|----------|
| **AC01** | iPhone ที่มีแถบ home indicator — หน้า **guest** | 🛑 ปุ่มล่างสุดไม่โดนแถบ home ทับ (เดิมจอที่ล็อกอินแล้วใส่ inset ถูก แต่จอแรกที่ผู้ซื้อทุกคนเจอไม่มีเลย) |
| AC02 | เครื่องเดียวกัน — หน้าที่ล็อกอินแล้ว | เหมือนกัน (ของเดิมถูกอยู่แล้ว ใช้เป็นตัวเทียบ) |
| AC03 | เปิดกล่องยืนยัน/แจ้งปัญหาแล้วลากนิ้ว | หน้าข้างหลังไม่เลื่อนตาม |
| AC04 | หมุนจอแนวนอน/แนวตั้งขณะพิมพ์คำตอบรีวิวค้างไว้ (ฝั่งร้าน) | ข้อความที่พิมพ์ไม่หาย/ไม่สลับไปอีกชุด |

---

### กลุ่ม AD — คำบนหน้าจอ (จาก `/impeccable clarify`)

| # | เคส | Expected |
|---|-----|----------|
| AD01 | หน้ากรอก OTP ทั้งเส้นทาง | เรียกสิ่งเดียวกันด้วยคำเดียว — **"รหัส OTP"** ทุกจุด (เดิมจอเดียวเรียก 3 ชื่อ: "รหัสความปลอดภัย" / "SMS" / "รหัส") |
| AD02 | ทำให้เกิด error ซ้ำ 2 ครั้งในเซสชันเดียว (เช่นแนบไฟล์ผิดชนิด 2 รอบ) | ข้อความปิดท้ายเหมือนกัน — ไม่ใช่ "ลองอีกครั้ง" ครั้งหนึ่งแล้ว "กรุณาลองใหม่อีกครั้ง" อีกครั้ง |
| AD03 | ตัดเน็ตแล้วกดบันทึก | ได้ข้อความที่พูดถึง **สัญญาณ** (คนละชุดกับ error ทั่วไปโดยตั้งใจ) |
| AD04 | กดคัดลอกเลขพัสดุในเบราว์เซอร์ที่ไม่ให้สิทธิ์คลิปบอร์ด | บอกทางที่ทำได้จริง ("กดค้างที่เลขพัสดุ…") ไม่ใช่ชวนให้กดใหม่ |
| AD05 | ยืนยันรับของสำเร็จ | ข้อความไม่มีคำลงท้ายที่ระบุเพศผู้พูด (ระบบเป็นคนพูด ไม่ใช่พนักงานคนหนึ่ง) |
...
| — | 2026-08-11 | **ยังไม่รัน** — implementation จบแล้ว (22 คอมมิตขึ้น prod) และเพิ่มกลุ่ม X–AD ตามของที่สร้างหลังเอกสารนี้ + ที่ `/impeccable critique` จับได้ | — |

🛑 **ข้อสังเกตที่ควรอ่านก่อนเริ่มเดินเคส:** `N01`/`N04` (เทียบสถานะพัสดุสองฝั่ง) มีอยู่ในเอกสารนี้
ตั้งแต่วันแรก — ถ้าเคยถูกรันจริง มีโอกาสสูงที่จะสะดุดกับ timeline ที่แสดงผิด ซึ่งกลายเป็น P0
ที่ `/impeccable critique` ไปเจอทีหลังตอนขึ้น prod แล้ว **ปัญหาไม่ใช่ "ไม่มีเทสเคส" แต่คือ
"เขียนไว้แล้วไม่มีใครเดิน"** — เอกสารที่ไม่ถูกรันมีค่าเท่ากับไม่มี

รอบทดสอบจริงครั้งแรกต้องเกิด **หลัง** implementation ของ TFR-001..014 เสร็จ — เริ่มจากกลุ่ม A–G (unit/integration `[blocker]`) ก่อนเสมอ เพราะเป็น dependency ของกลุ่ม H–AD ทั้งหมด (โดยเฉพาะกลุ่ม G ที่ต้องผ่านก่อนกลุ่ม U ถึงจะมีความหมาย — ถ้า SSOT ยังไม่รวม การเทียบ label สองฝั่งจะ fail แน่นอนโดยไม่ต้องรอ)

---

## 6. สรุป (Summary)

เอกสารนี้กำหนดเคสทดสอบของฟีเจอร์ 00041 แบ่งเป็น unit `[blocker]` (กลุ่ม A–B), integration `[blocker]` (กลุ่ม C–G), API ตรงไม่ผ่าน UI (กลุ่ม H–I), และ browser QA (กลุ่ม J–W และ X–AD ที่เพิ่ม 2026-08-11)
...
- boundary ของ maxLength คำตอบร้าน (1000 ตัวอักษร) — **มีเคสแล้วที่ AA06 แต่ยังไม่รัน**
- `shopRepliedByUserId` ถูกบันทึกทุกครั้งแต่ไม่มีจอไหนแสดง ⇒ **ทดสอบไม่ได้จากหน้าจอเลย** (ต้องดูที่ฐานข้อมูล) — ร้านที่มีพนักงานหลายคนตรวจย้อนหลังไม่ได้ว่าใครพูดแทนแบรนด์
- หนี้คอนทราสต์ระดับธีมที่ critique วัดไว้ (`text.disabled` 2.30:1 ใน 41 จุด · ชิป `variant='tonal'` 1.83–3.51:1 · ปุ่ม MUI default 38px) — user เคาะว่าไม่แตะรอบนี้ จึงยัง **fail ตามเกณฑ์ AA ของ PRODUCT.md** อยู่ ไม่ใช่ผ่าน
