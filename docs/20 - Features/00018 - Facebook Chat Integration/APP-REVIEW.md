...
ส่วนที่เหลือทั้งหมดที่คลิปต้องใช้ (ช่องทางการขาย · กล่องข้อความ · โปรไฟล์สาธารณะ) ไม่ถูก gate ด้วย role

**ความเสี่ยงที่เคยกังวลแล้วตกไป:** โควตา admin ของ Business Package ถูกเช็ค **ตอน accept ลิงก์เชิญเท่านั้น**
(`invite-link.service.ts:158-166`) ส่วนการเข้าถึงรายวันใช้ `canAccessShop` → `isShopMember` ซึ่ง
**ไม่แตะ subscription เลย** ⇒ ถ้าแพ็กเกจหมดอายุระหว่างที่ Meta ตรวจ (รอบก่อนใช้ 12 วัน) `metareview`
จะยังเข้าร้านได้ตามปกติ
