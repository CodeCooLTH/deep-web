...
| FR-ADREF-09 | แบนเนอร์แสดง **ข้อความโฆษณาจริง** (ดึงจากโพสต์ด้วย `post_id`) ไม่ใช่ `ad_title` ซึ่งเป็นชื่อ ad ภายใน Ads Manager + ลิงก์ "ดูโฆษณา" ไปโพสต์จริง | เธรดจากโฆษณา → เห็นข้อความโปรฯ ที่ลูกค้าเห็น ไม่ใช่ "video v3" |
| FR-ADREF-10 | **รายการแชท** แสดงชิป `ad_id.…` (ไอคอน Meta, ตัดข้อความ, รหัสเต็มใน tooltip) แบบ Business Suite | เปิด /inbox → แถวที่มาจากโฆษณามีชิป |
| FR-ADREF-11 | **แผงลูกค้าขวา** มีส่วน "ป้ายกำกับจาก Meta": `ad_id.<id>` + `messenger_ads`/`instagram_ads` — **ระบบเติมเอง แก้ไม่ได้** (ต่างจาก tag ของ CRM ที่ร้านตั้งเอง จึงไม่มีปุ่มลบ) | เปิดแผงขวาของเธรดจากโฆษณา → เห็น 2 ชิป |
