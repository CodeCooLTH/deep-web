...
| **ปักหมุด/ซ่อน/ปิดงานต่อเธรด** (S-7, spec §8.1) | FR-FBC-13 | BR-FBC-14, 15, 16 |
| **แท็ก/โน้ตภายใน/tab ออเดอร์ในแผงขวา** (S-8, spec §8.1) | FR-FBC-14 | BR-FBC-17, 18, 19 |
| **ข้อความสำเร็จรูป / AI ช่วยร่าง / เครื่องมือ composer** (2026-07-23) | FR-FBC-15, 16, 17 | BR-FBC-23, 24, 25, 26, 27 |
| PII / ความปลอดภัย token/webhook | (ครอบทุก FR ข้างต้น) | BR-FBC-20, 21, 22 |

> ✅ **PRD sync gap ปิดแล้ว (2026-07-23):** ตอนเขียน BRD ฉบับแรก S-7/S-8 ยังไม่มีรหัส FR ใน PRD
> v1.0 (OQ-FBC-04) — ตอนนี้ PRD มี `FR-FBC-13`/`FR-FBC-14` ครอบแล้ว traceability FR→BR→AC
> จึงต่อกันครบ. รอบเดียวกันเพิ่ม `FR-FBC-15..17` (composer productivity) ซึ่ง **โค้ดขึ้น prod
> ก่อนเอกสาร** — เป็นหนี้ Hard Rule 11 ที่ back-fill ในรอบนี้ ไม่ใช่ลำดับการทำงานที่ถูกต้อง
...
### 3.6 ข้อความสำเร็จรูป + AI ช่วยร่างคำตอบ (FR-FBC-15/16/17)

- ✅ ข้อความสำเร็จรูปที่ร้าน A สร้าง ต้องไม่ปรากฏให้ร้าน B เห็นหรือแก้ได้ แม้รู้ `id` (ทดสอบด้วยการยิง API ตรงด้วย session ของอีกร้าน)
- ✅ พนักงานทุกคนของร้านเดียวกันเห็นและใช้ชุดข้อความสำเร็จรูปชุดเดียวกัน (เป็นสมบัติของร้าน ไม่ใช่ของรายบุคคล)
- ✅ สร้างข้อความสำเร็จรูปที่ไม่มีทั้งข้อความและรูป → ถูกปฏิเสธพร้อมเหตุผล
- ✅ ข้อความสำเร็จรูปที่มีรูปแนบ ต้องเห็น **รูปจริง** ในแผงเลือก ไม่ใช่แค่สัญลักษณ์ว่า "มีรูป"
- ✅ กดเลือกข้อความสำเร็จรูป → เนื้อหาไปอยู่ใน **ช่องพิมพ์** (แก้ต่อได้) ไม่ใช่ส่งออกทันที
- ✅ AI ต้องเสนอร่าง **3 แบบที่ต่างกันจริง** และทุกร่างต้องผ่านการกดเลือก+ส่งโดยคนเสมอ — ไม่มีเส้นทางใดในระบบที่ส่งข้อความ AI ออกไปหาลูกค้าเองได้
- ✅ UI ต้องมีข้อความกำกับว่าเป็นคำแนะนำจาก AI และให้ตรวจทานก่อนส่ง
- ✅ ขอ AI ร่างในเธรดของร้านอื่น (ยิง API ตรงด้วย conversation id ที่ไม่ใช่ของร้านตัวเอง) → `404` ไม่ leak ว่าเธรดนั้นมีจริง
- ✅ ยังไม่ตั้งค่า AI (ไม่มี API key) → ผู้ใช้เห็นข้อความว่าใช้ไม่ได้ชั่วคราว ไม่ใช่หน้าจอพัง และปุ่มอื่นในช่องพิมพ์ยังใช้ได้ปกติ
- ✅ กดปุ่ม AI รัว ๆ เกินโควตา → ระบบตอบว่าใช้ถี่เกินไป (429) แทนการยิง Gemini ต่อเนื่อง
- ✅ ลูกค้าส่ง วิดีโอ/เสียง/ไฟล์ มาทาง Messenger/IG → เปิดดู/ฟัง/ดาวน์โหลดได้จากในเธรด Deep โดยไม่ต้องเปิดแอป Messenger
- ✅ แผง "ข้อความสำเร็จรูป" กับแผง "AI ช่วยร่างคำตอบ" เปิดพร้อมกันไม่ได้ (เปิดได้ทีละแผง) — กันบังพื้นที่อ่านข้อความ
...
stateDiagram-v2
  ...
  note right of WindowOpen
    ช่องพิมพ์เปิด + แบนเนอร์เตือนเมื่อเหลือ ≤ 4 ชม. (BR-FBC-11, §6.5)
  end note
...
- แบนเนอร์เวลาที่เหลือของ 24h window ต้องเตือน **ก่อน** หมดเวลา ไม่ใช่แค่ตอนหมดแล้ว — แต่เตือน
  เฉพาะตอน "เหลือน้อยจนต้องรีบ" (เกณฑ์ปัจจุบัน ≤ 4 ชม.) ไม่ใช่แสดงค้างตลอดเวลาที่หน้าต่างเปิดอยู่
  (แก้ 2026-07-26 ตามคำสั่ง user หลังใช้งานจริง: แถบค้างตลอดเวลารบกวนสายตาโดยไม่ให้ข้อมูลที่ต้อง
  ลงมือทำ เพราะเคสปกติเหลือเวลาเยอะอยู่แล้ว — ดู [[EXTENSIONS-2026-07-26]] E6)
...
### 8.8 ข้อความสำเร็จรูปและ AI ช่วยร่างคำตอบ (FR-FBC-15/16/17 — เพิ่ม 2026-07-23)

- **BR-FBC-23 (ข้อความสำเร็จรูปเป็นสมบัติของร้าน):** `QuickMessage` ผูกกับ `shopId` ไม่ใช่ `userId` — พนักงานทุกคนของร้านเห็น/ใช้/แก้ชุดเดียวกัน และทุก query/mutation ต้อง scope ด้วย `shopId` ใน `WHERE` เสมอ (ไม่ใช่ดึงมาแล้วค่อยเช็คทีหลัง) ร้านอื่นเข้าถึงไม่ได้แม้รู้ `id`
- **BR-FBC-24 (ต้องมีข้อความหรือรูปอย่างน้อยหนึ่งอย่าง):** ข้อความสำเร็จรูปที่ว่างทั้งข้อความและรูปสร้างไม่ได้ — รูปอย่างเดียวได้ (เช่น รูปรีวิว/โปรโมชัน), ข้อความอย่างเดียวได้
- **BR-FBC-25 (AI ร่างเท่านั้น ห้ามส่งเอง):** ผลลัพธ์จาก AI ต้องลงไปที่ช่องพิมพ์ให้คนตรวจ/แก้แล้วกดส่งเองเสมอ — ห้ามมี code path ใดส่งข้อความจาก AI ออกไปหาลูกค้าโดยไม่มีการกดของมนุษย์ และ UI ต้องกำกับว่าเป็นข้อความที่ AI ร่าง
- **BR-FBC-26 (AI ห้ามแต่งข้อเท็จจริงทางการค้า):** system prompt ต้องห้าม AI แต่งราคา/สต็อก/เงื่อนไข/คำสัญญาที่ยืนยันไม่ได้ และห้ามร่างข้อความขอ OTP/รหัสผ่าน/ข้อมูลบัตร — ถ้าไม่รู้ข้อมูลจริงให้ร่างเป็นการถามกลับอย่างสุภาพ
- **BR-FBC-27 (บริบทของ AI มาจากเซิร์ฟเวอร์ตามสิทธิ์เท่านั้น):** transcript ที่ส่งให้ผู้ให้บริการ AI ต้องอ่านจาก DB ฝั่งเซิร์ฟเวอร์หลังตรวจ ownership `{conversationId, shopId}` แล้ว — ห้ามรับ transcript จาก client (กันปลอมบริบท/ยัด prompt) ส่งเท่าที่จำเป็น (ข้อความล่าสุดจำนวนจำกัด, แทนรูป/การ์ดสินค้าด้วย placeholder) และโน้ตภายใน (BR-FBC-18) ใช้เป็นบริบทได้แต่ AI ต้องไม่อ้างถึงตรง ๆ ว่ามีโน้ต
