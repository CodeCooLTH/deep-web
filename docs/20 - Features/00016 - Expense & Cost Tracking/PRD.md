...
🛑 **Open Gate — ห้าม implement ก่อนยืนยัน:** `IShipOrderRow` (`src/lib/iship/client.ts:318-341`) ที่ `syncShipmentStatuses()` ใช้ **ไม่มีฟิลด์ราคาประกาศไว้เลย** (มีแค่ `status/status_name/updated_at/settlement_at/cod_amount/cod_fee/delivered_at`) — payload ที่ยืนยันแล้วว่ามี `price` คือ payload ของ **webhook** (`API.md` ของ 00022 บรรทัด 236-243) ซึ่งเป็นคนละ endpoint คนละทิศทาง

ต้องยิง `query_orders`/`get_order` จริงกับพัสดุที่ปิดแล้ว 2-3 ใบ อ่าน raw JSON ทั้งก้อน (ไม่ใช่ตาม type ที่ประกาศไว้) ก่อนเขียน SRS — ด่านเดียวกับ `docs/conventions/external-payload-schema.md` (บทเรียน 23 ออเดอร์ตำบล/อำเภอสลับ)
...
#### FR-EXP-22 (Should — รอ OQ-1): เก็บ `cod_fee` เป็นต้นทุนของคำสั่งซื้อด้วย

`cod_fee` อยู่ใน payload ที่ `syncShipmentStatuses()` อ่านอยู่แล้ว ต้นทุนเพิ่มต่ำมากถ้าเก็บพร้อม `carrierPrice` — **แต่ยังไม่ล็อกเป็น Must เพราะเสี่ยงหักซ้ำซ้อน** (ดู OQ-1)
...
- **OQ-1** 🛑 `cod_fee` รวมค่าส่ง (`carrierPrice`) ไว้แล้วหรือแยกกัน? ถ้ารวมไว้แล้ว การหักทั้งคู่ = หักซ้ำ ต้องยืนยันกับพัสดุ COD จริงที่ settle แล้วก่อนล็อก FR-EXP-22
