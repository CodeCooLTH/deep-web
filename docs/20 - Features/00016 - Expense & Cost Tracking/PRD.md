...
#### FR-EXP-18: ค่าส่งจริงเข้าเป็น "ค่าใช้จ่าย" ลดกำไรสุทธิ (D-EXT-10 — แทนที่มติเดิมที่ให้หักจากกำไรขั้นต้น)

🛑 **`GROSS_PROFIT_FORMULA` ไม่เปลี่ยน** — ค่าส่งไม่ใช่ต้นทุนของตัวสินค้า กำไรขั้นต้นยังเป็น `ยอดที่ลูกค้าจ่าย − ต้นทุนสินค้า` เหมือนเดิมทุกประการ ผลพลอยได้คือภาระของ HR16 เรื่อง "ต้องประกาศบนหน้าจอว่าความหมายเปลี่ยน" หมดไปด้วย เพราะไม่มีตัวเลขไหนเปลี่ยนความหมาย

`NET_PROFIT_FORMULA` = `ยอดขาย − ต้นทุนสินค้า − ค่าใช้จ่าย` **ก็ไม่ต้องแก้ถ้อยคำ** — ค่าส่งจริงเข้าไปอยู่ใน "ค่าใช้จ่าย" หมวดค่าขนส่ง ซึ่งเป็นหมวดที่มีอยู่แล้ว (`SHIPPING`) เพียงแต่เดิมมีแหล่งเดียวคือที่ร้านพิมพ์เอง ตอนนี้มีสองแหล่งคือพิมพ์เอง + คำนวณจากพัสดุจริง

**ไม่สร้างแถว `Expense`** (D-EXT-11) — `sumOrders()`/รายงานกำไร-ขาดทุนอ่าน `OrderShipment.carrierPrice` + `codFee` ของพัสดุ active ในช่วงเวลานั้นโดยตรง แล้วบวกเข้ากับ `totalExpense` ที่มาจากตาราง `Expense`

**บนการ์ดออเดอร์รายใบยังต้องเห็นค่าส่งของใบนั้น** — คำร้องตั้งต้นของผู้ใช้คือ "กำไรรายใบผิดเพราะไม่นับค่าส่ง" การย้ายไปเป็นค่าใช้จ่ายระดับช่วงเวลาอย่างเดียวโดยไม่แสดงอะไรบนใบ = ยังตอบคำร้องไม่ครบ ให้แสดงเป็นบรรทัดแยกใต้กำไรขั้นต้น (รูปแบบที่แน่ชัดรอ `safepay-ux` ตาม HR8)

สถานะที่ต้องแยกให้ขาด **3 แบบ** (ไม่ใช่ binary มี/ไม่มี):
1. ไม่มีพัสดุ iShip เลย (ส่งเองผ่าน `ShipmentTracking` หรือยังไม่ส่ง) → ไม่มีค่าส่งให้แสดง พฤติกรรมเดิมทุกประการ ไม่มีธง
2. มีพัสดุ iShip แต่ยังไม่รู้ราคาจริง (ขนส่งยังไม่เข้ารับ) → ขึ้นธง "ยังไม่ทราบค่าส่งจริง" **แยกจาก `hasMissingCost` เดิม** (คนละสาเหตุ คนละข้อความ)
3. มีราคาจริงแล้ว → แสดงจริง และเข้าค่าใช้จ่ายในรายงาน
