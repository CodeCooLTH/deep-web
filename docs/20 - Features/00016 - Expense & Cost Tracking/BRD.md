...
|---|---|---|---|
| **D-EXT-7** | วิธีหักค่าส่ง | **หักจากกำไรขั้นต้นตรง ๆ และเปลี่ยนนิยาม `GROSS_PROFIT_FORMULA`** ไม่สร้างบรรทัดที่สาม | ตรงกับที่ร้านเข้าใจคำว่า "กำไรใบนี้" ที่สุด แต่ตัวเลขเดือนก่อนจะขยับ ⇒ ภาระ HR16: **ต้องบอกผู้ใช้บนหน้าจอ** + ไล่ทุก surface ผ่าน `src/lib/order-profit-presentation.ts` |
...
| รหัส | ข้อความ |
|---|---|
| **BR-EXP-18-01** | กำไรขั้นต้นหักค่าส่ง **จริง** เท่านั้น (`carrierPrice`) — ราคาประเมิน (`estimatedPrice`) ไม่มีผลต่อตัวเลขกำไร ใช้เป็นข้อมูลอ้างอิงเท่านั้น |
| **BR-EXP-18-02** | ออเดอร์ที่ไม่มีพัสดุ iShip เลย — สูตรไม่เปลี่ยนพฤติกรรมจากเดิม และไม่มีธงเตือนใด ๆ (กัน regression) |
...
| **BR-EXP-20-01** (Open Gate) | การเขียน `carrierPrice` ผ่าน polling ต้องยืนยันชื่อฟิลด์กับ payload จริงของ `query_orders`/`get_order` ก่อน — **ห้าม assume จาก payload ของ webhook** (`docs/conventions/external-payload-schema.md`) |
