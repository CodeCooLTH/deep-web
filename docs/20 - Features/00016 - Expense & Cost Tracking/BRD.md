...
**Acceptance Criteria:**
- [ ] `[FR-EXP-05-AC-01]` ระบบมีหมวดหมู่ให้เลือกทั้งหมด 7 หมวด: `RENT`(ค่าเช่า), `PACKAGING`(ค่าแพ็คเกจ/บรรจุภัณฑ์), `ADVERTISING`(ค่าโฆษณา), `SHIPPING`(ค่าขนส่ง), `SALARY`(เงินเดือน), `UTILITIES`(สาธารณูปโภค), `OTHER`(อื่นๆ)
...
สำหรับ technical specification (architecture/API/data/NFR) ดู SRS ของโมดูลนี้ (ยังไม่จัดทำ — รอ user review เอกสารนี้ก่อน)
