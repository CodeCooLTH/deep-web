...
#### 12.2.1 TFR-018b — กำไรรายออเดอร์บน **หน้ารายการ** `/seller/orders` — 🛑 ถอดออกแล้ว 2026-08-09

> ## สถานะ: **ไม่ได้อยู่บน prod แล้ว** (ขึ้น 2026-08-08 · ถอด 2026-08-09)
>
> user สั่งถอด: *"งั้นเอาออกจาก order list ก่อนดีกว่า เด๋วผมคิดก่อน"* →
> *"ให้มีอยู่ในรายงานอย่างเดียวก็พอ"*
>
> **สาเหตุคือหน้าตา ไม่ใช่ตรรกะ** — ตัวเลข/สิทธิ์/เกณฑ์ถูกต้องหมดตั้งแต่รอบแรก แต่ผ่าน
> การออกแบบ 3 รอบแล้ว user ยังบอกว่า "ไม่สวย" ทุกรอบ (คำเต็ม → ไอคอน+ตัวเลข →
> caret+ตัวเลข) ข้อสังเกตที่ได้ตอนท้าย: **ทุกแบบล้วนระบายตัวเลขให้เป็นเขียว `#065f46`
> ซึ่งเป็นเขียวเข้มหม่น จัดวางยังไงก็ยังหม่น** — ทางที่ยังไม่ได้ลองคือปล่อยตัวเลขเป็นสีเข้ม
> ปกติแล้วให้ของเล็ก ๆ ที่มีสี (จุด/แท่ง/caret) บอกทิศทางแทน
>
> **แบบที่เสนอไว้รอ user ตัดสิน 7 แบบ:** `docs/superpowers/specs/2026-08-09-order-profit-line-variants.html`
> (เปิดด้วยเบราว์เซอร์ — มีทั้งการ์ดมือถือและตารางเดสก์ท็อป ใช้สี/ขนาด/ฟอนต์จริงจากแอป)
>
> **สิ่งที่ยังอยู่และไม่ได้ถูกแตะ:** การ์ดกำไรรายใบในหน้า order detail (TFR-018 ข้างบน) ·
> `src/lib/order-profit.ts` · `src/lib/order-profit-presentation.ts` (`presentOrderProfit`
> + `PROFIT_TONE` — หน้า detail ใช้อยู่) · รายงานกำไร-ขาดทุนและ `/sales` ทั้งหมด
>
> **สิ่งที่ถูกลบไปพร้อมกัน (ถ้าจะทำใหม่ต้องเขียนใหม่):** `OrderProfitInline.tsx` ·
> `profitColumnCaption()` · `presentOrderProfitCompact()` · field `OrderRow.profit`
>
> ข้างล่างนี้เก็บไว้เป็น **บันทึกของสิ่งที่เคยทำและข้อตัดสินที่ได้มา** เพื่อไม่ต้องเริ่มจากศูนย์
> ถ้า user เคาะแบบใหม่ — ไม่ใช่คำอธิบายของสิ่งที่อยู่บนจอตอนนี้
