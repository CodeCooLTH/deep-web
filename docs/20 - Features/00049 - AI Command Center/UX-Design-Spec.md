...
- **เฉพาะคอลัมน์รอเคาะ:** ปุ่ม "เคาะ 'พร้อมขึ้น'" เต็มความกว้าง แยกจาก `⋮`
  🛑 **คอลัมน์นี้ถือ 2 สถานะ ต้องแยกให้เห็นด้วยตา** (SRS §1.4 · D-10):
  - `awaitingApproval: true` (รอคุณกด) → แถบ `border-s-3 border-primary` + **ปุ่มเคาะ**
  - `awaitingApproval: false` (เคาะแล้ว รอ `auto-merge.yml`) → **ไม่มีปุ่ม** แทนด้วยแถวสถานะ
    `text-default-500` + ไอคอน `clock-play`: *"อนุมัติแล้ว — จะขึ้นเมื่อด่านผ่านครบ"* และ**ไม่มีแถบ accent**
    (สีธีมสงวนไว้กับ "สิ่งที่รอคุณอยู่" เท่านั้น — One Voice · ใบที่กดแล้วไม่ต้องการความสนใจอีก)
  🛑 **เลข "รอเคาะ" บนแถบสถานะบนนับเฉพาะ `awaitingApproval: true`** ไม่ใช่ `count` ของคอลัมน์
  (ไม่งั้นเลขจะไม่ลดลงหลังกด แล้วผู้ใช้จะกดซ้ำ) — ยังคงกฎ "มาจาก symbol เดียว" ของ §3 ข้อ 1
