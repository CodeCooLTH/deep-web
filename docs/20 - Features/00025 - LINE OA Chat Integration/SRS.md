...
### TFR-LINE-09b: การ์ดคำสั่งซื้อแบบ Flex + รับ event `postback` — เพิ่ม 2026-08-11

**การ์ดคำสั่งซื้อ (ขาออก)**

เดิมร้านกด "ส่งการ์ดออเดอร์" แล้วลูกค้า LINE ได้ **ข้อความเปล่า 3 บรรทัดกับลิงก์ดิบ** ขณะที่ฝั่งร้านเห็นการ์ดสวยงามในระบบเรา — ตอนนี้ลูกค้าได้ Flex bubble จริง (ชื่อรายการ · "และอีก n รายการ" · ยอดสุทธิ · ปุ่ม `uri`)

- ตัวประกอบคือ `buildLineFlexOrderCard()` (`src/lib/line/flex-order-card.ts`) — **pure** รับค่าที่ฟอร์แมตแล้ว คืน JSON
- 🛑 **การฟอร์แมตเงินห้ามอยู่ในไฟล์นั้น (HR16)** — ผู้เรียกส่ง `totalText` ที่ผ่านสูตรกลางมาแล้ว ไม่งั้นยอดบนการ์ดที่ลูกค้าเห็นเพี้ยนจากยอดบนหน้าจอร้านได้โดยไม่มีอะไรฟ้อง
- 🛑 **`altText` คือสิ่งเดียวที่ลูกค้าเห็นในรายการแชทและ notification** (flex ไม่ถูกเรนเดอร์ที่นั่น) ต้องมีทั้งชื่อของและยอดเงิน · จำกัด 1500 ตัวอักษร · `LineAdapter` ปฏิเสธ flex ที่ altText ว่างตั้งแต่ต้นทาง
- 🛑 **`uri` action รับเฉพาะ https** — dev ที่ `NEXT_PUBLIC_BUYER_URL` เป็น http จะถอยไปใช้ข้อความลิงก์เดิมอัตโนมัติ ดีกว่าส่งไม่ออกเลย
- **Messenger/IG ไม่เปลี่ยนพฤติกรรมเลย** ยังได้ลิงก์ข้อความเหมือนเดิม — `MetaAdapter` โยน error ถ้ามีใครเผลอส่ง part ชนิด `flex` ไป (ห้ามถอยเงียบไปเป็นข้อความเปล่า) และ `text` ยังถูกส่งเสมอเพราะเป็น `ChatMessage.body` ที่ร้านเห็นในประวัติ
- สีบนการ์ดยึด `.impeccable/design.json` **ฝั่งผู้ซื้อ** (`#7367F0` / ink `#2F2B3D` / slate `#808390`) เพราะการ์ดไปโผล่ในแอป LINE ของลูกค้า ไม่ใช่ในหน้าจอร้าน — ห้ามใช้ `paces-primary` น้ำเงินซึ่งเป็นสีของ surface ฝั่งผู้ขาย (HR7)
- "และอีก n รายการ" นับจาก `_count.items` ของออเดอร์ **ไม่ใช่ `items.length`** ที่ `take:1` ไว้แล้ว (จะได้ 1 เสมอ)

**event `postback` (ขาเข้า)**

- เกิดเมื่อลูกค้าแตะปุ่มที่เราส่งไป (Flex button ชนิด postback · Quick Reply · Rich Menu) — ingest เป็นข้อความขาเข้าผ่าน `ingestLineTextMessage` เส้นทางเดียวกับข้อความปกติ
- 🛑 **dedupe ด้วย `webhookEventId` ไม่ใช่ `message.id`** (postback ไม่มี) — ใส่ prefix `pb:` กันชนกับ message id จริงในคอลัมน์เดียวกัน. ใช้ timestamp หรือ data แทนไม่ได้: ลูกค้ากดปุ่มเดิมสองครั้งจะถูกตีเป็นซ้ำแล้วการกดครั้งที่สองหายเงียบ ทั้งที่เป็นการกดที่ตั้งใจ
- 🛑 **postback มี `replyToken` มาด้วย** — การแตะปุ่มจึงเปิดหน้าต่างตอบฟรี 60 วินาทีเหมือนการพิมพ์ข้อความทุกประการ ทิ้ง event นี้ = เสียทั้งสัญญาณและโอกาสตอบฟรี
- payload ไม่ครบ (ไม่มี `data` หรือ `webhookEventId`) → ข้ามเงียบ ไม่ล้มทั้ง request (เหมือน event ชนิดอื่นที่ยังไม่รองรับ)
- ข้อความที่บันทึกมาจาก `describeLinePostback()` (`src/lib/line/postback.ts`) — **ห้ามคืนสตริงว่างไม่ว่าอินพุตเพี้ยนแค่ไหน** บับเบิลว่างในเธรดคือสิ่งที่ผู้ขายตีความไม่ได้เลย (หลักการเดียวกับ placeholder ของสื่อที่ mirror ไม่ผ่าน)

**การ์ดสินค้า (เพิ่ม 2026-08-11)** — `type=PRODUCT` เปิดให้ **ทุกช่องทาง**: LINE = Flex · Messenger/IG = Generic Template

- 🛑 **ก่อนรอบนี้ไม่มีปุ่มไหนในระบบส่ง `type=PRODUCT` เลยสักปุ่ม** — แผง "เลือกสินค้า" ทั้ง 3 โหมด (`image`/`detail`/`stock`) **เติมข้อความ+รูปลงช่องพิมพ์** ไม่ได้ส่งการ์ด โดยหัวไฟล์ `ProductPickerPanel.tsx` ให้เหตุผลไว้เองว่า *"การ์ดสินค้าส่งได้เฉพาะเธรด Deep — Messenger/IG คืน 400"* ⇒ เปิด backend อย่างเดียวจะได้โค้ดที่ไม่มีใครเรียก จึงเพิ่ม **โหมดที่ 4 "ส่งการ์ดสินค้า" ที่ส่งออกทันที** (3 โหมดเดิมไม่เปลี่ยน — เหตุผล "คนตรวจก่อนกดส่ง" ยังจริง)
- ตัวส่งอยู่ใน `useSellerChatThread.sendProductCard()` ไม่ใช่ใน component — ฮุกเป็นเจ้าของ `sending` และการ refetch อยู่แล้ว
- 🛑 **ไม่มีบับเบิล optimistic ต่างจาก `sendSticker` โดยตั้งใจ** — การ์ดประกอบที่ server ทั้งใบ (อ่านสินค้า + แปลงรูปตามช่องทาง) client เดารูปร่างล่วงหน้าไม่ได้ เดาแล้วให้ refetch มาแทนจะทำให้การ์ดกระพริบเปลี่ยนรูปต่อหน้าผู้ขาย = อ่านเหมือนส่งสองครั้ง
- **รูปเป็นคนละไฟล์ต่อช่องทาง:** LINE ครอบ 1:1 เองจากรูป 1024 JPEG · Messenger/IG ได้รูป **จัตุรัส 1080×1080 แบบ `cover`** (`lib/meta/card-image.ts`)

- 🔄 **แก้ 2026-08-11 รอบสอง (user สั่ง: "ผมชอบรูปเต็ม ๆ แบบนี้")** — รอบแรกทำเป็น 1.91:1 + `contain` โดยอ้างประโยคในเอกสาร Meta *"Messenger scales or crops photos that are not 1.91:1"* แล้วตีความว่าถูกบังคับ. **ภาพการ์ด Facebook จริงที่ user ส่งมาแสดงรูปสูงกว่า 1.91:1 และเห็นเต็มใบ** ⇒ ไม่ได้ถูกบังคับ ⇒ จัตุรัส + cover ได้ทั้งเต็มกรอบและเสียเนื้อรูปน้อยกว่ามาก (แนวตั้ง 3:4 เสีย ~25% แทน ~72% ถ้า cover บนกรอบ 1.91:1)
- 🛑 **กรอบนี้ต้องตรงกับการ์ดในแอปผู้ขาย** (`ProductCardBubble` + `OwnProductCardCarousel` = `aspect-square` + `object-cover`) — ผู้ขายกับลูกค้าต้องเห็นรูปถูกครอปเหมือนกัน มีเทส `[blocker]` ตรึง **อัตราส่วน** ไว้ (ไม่ใช่เทียบกับค่าคงที่ตัวเอง ซึ่งจะเขียวตลอดแม้มีคนเปลี่ยนกรอบ — จับได้ตอน mutation)
- ยังต้อง `flatten` พื้นขาว: PNG โปร่งใสที่ถูก cover แล้วยังมี alpha → JPEG จะได้พื้นดำ
- เพดาน Meta (ยืนยันกับเอกสาร 2026-08-11): elements ≤10 · **title ≤80** · **subtitle ≤80** · ปุ่ม ≤3 — เกินแล้ว Meta ตัดเองแบบไม่บอก จึงตัดเองที่ `truncateForMeta()` และพยายามตัดที่ช่องว่างคำ (บทเรียน `image_grid` ที่ user เจอบน prod: ตัดคากลางคำ "เพิ่มความนุ่")
- ไม่มีรูป → **ไม่ใส่คีย์ `image_url` เลย** ไม่ใช่ส่งค่าว่าง (Meta ตีเป็น payload ผิดรูปแล้วตกทั้งข้อความ)

- เนื้อหายึด **การ์ดในแอป** (`ProductCardBubble`): รูป · ชื่อ · ราคา · ป้าย "หยุดขายแล้ว" และ **ไม่มีปุ่ม** เหมือนกัน — ระบบไม่มีหน้าสาธารณะของสินค้ารายชิ้นให้ลิงก์ไป (`/a/[id]` คือประมูล สินค้าโผล่เฉพาะในกริดหน้าร้าน) ใส่ปุ่มที่พาไปหน้าร้านรวม = affordance ที่ไม่ตรงกับสิ่งที่ผู้ใช้คาดจากการกด
- 🛑 **รูปต้องแปลงเป็น JPEG ก่อนเสมอ** (`resolveLineFlexImageUrl`) — Flex รับแค่ JPEG/PNG แต่รูปสินค้าในระบบเป็น webp/gif ได้ **ห้ามถอยไปใช้ไฟล์ต้นฉบับ** เพราะลูกค้าจะเห็นกรอบว่างซึ่งแย่กว่าไม่มีรูป → แปลงไม่ผ่าน = ส่งการ์ดแบบไม่มี hero
- 🛑 **ด่าน cross-shop (FR-CTX-07) เดิมอยู่ใน `chat.service.sendMessage()` ซึ่งครอบเฉพาะ DEEP** — ช่องทางนอกไม่เคยต้องใช้เพราะถูก 400 ทิ้งก่อน เส้นทางใหม่วิ่งผ่าน `sendOutboundMessage` ที่**ไม่ผ่าน sendMessage** จึงต้องมีด่านของตัวเองที่ route โดยใช้ `getProductById()` ตัวเดียวกัน. ถอดเงื่อนไขนี้ออกแล้ว `tsc` ยังเขียวและไม่มีเทสไหนแดง → มีเทสสแกนซอร์สคุมทั้งสองเส้นทาง
- ราคาบนการ์ดใช้ `formatBaht` — การ์ดในแอปถูกเปลี่ยนมาใช้ตัวเดียวกันในรอบนี้ (สูตรเดิมให้ `฿1,290.5` ขณะที่ `formatBaht` ให้ `฿1,290.50` = ราคาชิ้นเดียวกันอ่านคนละแบบระหว่างจอร้านกับแอปลูกค้า — HR16)
- `ChatMessage` ฝั่งเราเก็บ `type='PRODUCT'` + `productRefId` และ `body=null` ตรงกับที่ DEEP ทำ (live-join การ์ดตอนอ่าน) ไม่งั้นร้านจะเห็นทั้งการ์ดและข้อความดิบซ้อนกัน
- **หนี้ที่รู้ตัว:** เส้นทาง DEEP มี idempotent-guard (ส่งสินค้าชิ้นเดิมซ้ำติดกัน → คืนแถวเดิม ไม่ insert ซ้ำ) แต่เส้นทาง LINE **ยังไม่มี** — กดซ้ำจะเสียโควตาจริง 1 ข้อความ

**ที่ยังไม่มีตัวส่ง:** รอบนี้ไม่มีปุ่มชนิด postback ถูกส่งออกจริงสักปุ่ม (การ์ดออเดอร์ใช้ `uri`) — ตัวรับมาก่อนโดยตั้งใจ เพราะลำดับกลับกันแปลว่าลูกค้ากดแล้วเงียบหาย

**อ้างอิง:** FR-LINE-04, BR-LINE-16
