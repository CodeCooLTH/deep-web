...
**สิ่งที่ผู้ขายเห็น (S-14b · ปรับ 2026-08-10 ตาม user):** สถานะโควตาอยู่ **บนปุ่มส่ง** ไม่ใช่แคปชันใต้ช่องพิมพ์อีกต่อไป — ปุ่มอ่านว่า `ส่ง · ฟรี` (อยู่ในหน้าต่าง reply ใบนี้ไม่หักโควตา) / `ส่ง · 290/300` (ใบนี้หักโควตา) / `ส่ง` เปล่า ๆ (ไม่จำกัด · อ่านยอดไม่สำเร็จ · โควตาหมดซึ่งปุ่มถูกปิดอยู่แล้ว) พร้อม `ring-warning` เมื่อ tone เป็น warning
