...
#### TC-47: การ์ดสินค้าไปถึงลูกค้า LINE เป็นการ์ดจริง
**Given** เธรด LINE · สินค้าที่มีรูปในร้านนั้น
**When** ผู้ขายเลือกสินค้าจากแผงเลือกสินค้าแล้วส่ง
**Then** ลูกค้าเห็นการ์ด (รูป 1:1 · ชื่อ · ราคา) และ notification ขึ้น "สินค้า: {ชื่อ} · {ราคา}"
**Then (ต่อ)** ฝั่งร้านเห็นเป็นการ์ดสินค้าในเธรดเหมือนช่องทาง DEEP ไม่ใช่ข้อความดิบซ้อนการ์ด
**ระดับ:** Unit ([blocker]) + Manual บนเครื่องจริง

#### TC-48: สินค้ารูปเป็น webp / ไม่มีรูป
**Given** สินค้าที่รูปแรกเป็น `.webp` (Flex รับแค่ JPEG/PNG)
**Then** ระบบแปลงเป็น JPEG ให้เอง — ลูกค้าเห็นรูปปกติ
**When (ต่อ)** สินค้าที่ไม่มีรูปเลย / แปลงรูปไม่สำเร็จ
**Then** ส่งการ์ดแบบ**ไม่มีรูป** ยังอ่านชื่อ+ราคาได้ครบ — ห้ามขึ้นกรอบรูปว่าง
**ระดับ:** Unit ([blocker]) + Manual

#### TC-49: สินค้าหยุดขาย
**Then** การ์ดขึ้นป้าย "หยุดขายแล้ว" (คำ ไม่ใช่สีอย่างเดียว) · สินค้าที่ยังขายอยู่ต้องไม่มีป้ายนั้น
**ระดับ:** Unit ([blocker])

#### TC-50 [ห้ามข้าม] (security): ส่งการ์ดสินค้าของร้านอื่น
**Given** ยิง API ตรงด้วย `productRefId` ของสินค้าร้านอื่น เข้าเธรด LINE ของร้านเรา
**Then** ได้ 400 "ไม่พบสินค้านี้ในร้าน" และ **ไม่มีแถวใดถูกเขียน ไม่มี request ไป LINE**
**Then (ต่อ)** ทดสอบซ้ำบนช่องทาง DEEP → ต้องได้ `PRODUCT_NOT_IN_SHOP` เหมือนเดิม
**ระดับ:** Integration
**เหตุผลที่ต้องเข้ม:** ด่านเดิมอยู่ใน `chat.service` ซึ่งครอบเฉพาะ DEEP — เส้นทาง LINE ไม่ผ่านตรงนั้น ถอดด่านออกแล้ว `tsc` ยังเขียวและไม่มีเทสไหนแดง

#### TC-51: ราคาบนการ์ดต้องตรงกันสองจอ
**Given** สินค้าราคา 1,290.50 บาท
**Then** จอร้าน (การ์ดในแอป) กับที่ลูกค้าเห็นในแอป LINE ต้องอ่านว่า `฿1,290.50` **เหมือนกันเป๊ะ**
**ระดับ:** Manual (เทียบสองจอ)

#### TC-52: Messenger/IG ยังไม่รองรับการ์ดสินค้า
**Then** ส่ง `type=PRODUCT` เข้าเธรด Messenger/IG → 400 "ช่องทางนี้ยังไม่รองรับการ์ดสินค้า" เหมือนเดิม
**ระดับ:** Integration
...
| FR-LINE-04 | BR-LINE-16 | TC-10, TC-11, TC-16, TC-36…TC-41 (ไฟล์แนบ), TC-42…TC-46 (Flex ออเดอร์ + postback), TC-47…TC-52 (การ์ดสินค้า) |
