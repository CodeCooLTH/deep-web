...
## ส่วนขยาย 2026-09-10 — เพดานเวลา · คำที่ถูกต้อง · กู้คืนอัตโนมัติ

> ที่มา: ทดสอบซื้อจริงบน TestFlight แล้วเจอ "แอปไม่ตอบสนอง" ทั้งที่ StoreKit ยังทำงานปกติ
> · PR deep-web #59 + deep-mobile-seller #3 · OTA runtime 1.0.0 ปล่อย 2026-09-10

| รหัส | เคส | คาดหวัง | สถานะ |
|---|---|---|---|
| TC-IAP-53 | กดซื้อแล้ว **ปล่อยแผ่นของ Apple ค้างไว้เกิน 90 วินาที** ค่อยยืนยัน | ต้องได้สิทธิ์ตามปกติ · **ห้ามขึ้น "แอปไม่ตอบสนอง"** | ✅ เทสอัตโนมัติ (`iap-client.test.ts`) · ⬜ ยังไม่กดจริง |
| TC-IAP-54 | เพดานค่าปริยาย `purchase`/`restore` ≥ 5 นาที และยาวกว่า `products` | ปักหมุดด้วยเทส `[blocker]` | ✅ mutation 4 แบบแดงครบ |
| TC-IAP-55 | ข้อความเมื่อ `TIMEOUT` **ตอนกดซื้อ** | ชี้ไป "กู้คืนการซื้อ" · **ห้ามมีคำว่า "ลองใหม่"** | ✅ `iap-failure-message.test.ts` |
| TC-IAP-56 | ข้อความเมื่อ `TIMEOUT` **ตอนขอราคา** | ยังเป็น "แอปไม่ตอบสนอง" (คนละเคส) | ✅ |
| TC-IAP-57 | ซื้อสำเร็จแล้ว **ปิดแอปทันทีก่อนสิทธิ์เข้า** → เปิดแอปใหม่ | **สิทธิ์เข้าเองโดยไม่ต้องกดอะไร** | ⬜ **ยังไม่เคยทดสอบบนเครื่องจริง** |
| TC-IAP-58 | ธุรกรรมกู้คืนมาถึงตอนหน้าเว็บยังโหลดไม่เสร็จ (cold start) | เข้าคิวแล้วส่งตอน `onLoadEnd` ไม่หล่นหาย | ✅ ด่านสแกนซอร์ส (`iap-recovery-wired.test.ts`) · ⬜ ยังไม่กดจริง |
| TC-IAP-59 | ยืนยันหลายใบพร้อมกัน โดยใบแรกถูกปฏิเสธ (4xx) | ใบที่เหลือ **ต้องยังได้รับการยืนยัน** ไม่หยุดทั้งชุด | ✅ `iap-recovery.test.ts` |
| TC-IAP-60 | เซิร์ฟเวอร์ตอบ 5xx ระหว่างกู้คืน | **ห้ามปิดธุรกรรม** ปล่อยให้ StoreKit ส่งกลับมารอบหน้า | ✅ |
| TC-IAP-61 | กู้คืนไม่สำเร็จสักใบ | **เงียบ** ไม่เด้ง error ใส่ผู้ใช้ที่ไม่ได้สั่งอะไร | ✅ |
| TC-IAP-62 | ลิงก์ "จัดการหรือยกเลิกการสมัคร" ในแอป | เปิดหน้าการสมัครรับของ Apple ได้จริง | ⬜ **สงสัยว่าไม่ทำงาน** — เป็นลิงก์นอกโดเมนที่ถูกเตะเข้า in-app browser ซึ่งไม่รองรับ universal link · ยังไม่ยืนยัน |

### 🛑 สิ่งที่ยังไม่เคยยืนยันบนเครื่องจริง

TC-IAP-57 · TC-IAP-58 · TC-IAP-62 — และ **TC-IAP-49 (Sign in with Apple บน iPad)
ซึ่งเป็นข้อที่ Apple ตีกลับมาเอง** ยังคงค้างจากรอบก่อนหน้า

เทสอัตโนมัติทั้งหมดในตารางนี้เป็น **ฟังก์ชันบริสุทธิ์ + สแกนซอร์ส** — ยืนยันได้ว่า "กติกาถูก
และด่านไม่หายไป" แต่ยืนยันไม่ได้ว่า "สะพานระหว่างแอปกับเว็บต่อกันติดจริงบนเครื่อง"
