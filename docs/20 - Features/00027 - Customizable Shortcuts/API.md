...
| 401 | `UNAUTHORIZED` | ไม่มี session |
| 404 | `SHOP_NOT_FOUND` | ไม่มี active shop |
| 409 | `MIN_REQUIRED` | เหลือรายการเดียว (รวม unavailable) — ถอดแล้วจะเหลือ 0 |
...
⚠️ **ดู [[SRS]] §3 TFR-006 / กรณีค้างให้ user เคาะ:** ตอนนี้ `MIN_REQUIRED` นับรวมรายการ `unavailable` ด้วย — ถ้ารายการสุดท้ายที่เหลือคือรายการที่ "ใช้ไม่ได้แล้ว" endpoint นี้ยัง reject การถอด ทั้งที่การ์ดว่างอยู่แล้วในสถานการณ์นั้น (ดูตอนท้ายชุดเอกสารนี้)
