...
`IMAGE_GRID_COUNT_OUT_OF_RANGE` (validation ก่อนเรียก service, `400`), fallback "ตกไปส่งทีละใบ" เมื่อ
Meta ปฏิเสธ template `image_grid` โดยเฉพาะ (คนละเหตุผลจากหน้าต่างเวลา — ไม่เปลี่ยน)
...
`sendTextMessage`/`sendAttachmentMessage`/`sendImageGridMessage` (`src/lib/facebook/graph.ts`) **ไม่ถูก
แก้ไขแม้แต่บรรทัดเดียว** ในฟีเจอร์นี้ — ทั้งสามฟังก์ชันรับพารามิเตอร์ `tag?: string` อยู่แล้วและประกอบ
body ตามรูปแบบนี้เหมือนกันทุกฟังก์ชัน:
