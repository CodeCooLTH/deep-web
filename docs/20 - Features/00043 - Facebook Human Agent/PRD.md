...
### สถานะ ณ 2026-08-11 — **ยังตอบไม่ได้ (OPEN)** แต่ได้ข้อมูลเพิ่มแล้ว

**ทดสอบจริงบน prod แล้ว** (ตั้ง `META_HUMAN_AGENT_TEST_PSIDS` + redeploy + กดส่งในเธรดที่พ้น 24 ชม.):

| สิ่งที่พิสูจน์ได้ | ผล |
|---|---|
| allow-list รายเธรดทำงาน | ✅ แถบขึ้น "ยังตอบเองได้ถึง 2569-08-16 18:32:04" = `lastInboundAt + 7 วัน` เป๊ะ |
| tag ถูกแนบและยิงถึง Meta จริง | ✅ Meta ตอบ `(#100) Cannot tag messages with 'HUMAN_AGENT' without prior approval.` — พูดถึงชื่อ tag ตรง ๆ (ถ้าโค้ดไม่ติด tag จะได้ error คนละแบบ) |
| BR-HA-14 (พยายามส่งก่อน ให้ Meta ตัดสิน) | ✅ ไม่ block ฝั่งเราเอง บันทึก FAILED + แสดงเหตุผลครบ |

🛑 **แต่การทดลองนี้คุมตัวแปรไม่ครบ — ห้ามใช้สรุปว่า "Standard Access ไม่พอ"**
Meta DevTools MCP ยืนยันว่า **ฟีเจอร์ "Human Agent" ไม่เคยถูกเพิ่มเข้า use case ของแอปเลยสักครั้ง**
(privileges 17 รายการ ไม่มีตัวนี้) ⇒ สิ่งที่พิสูจน์ได้คือ **"ไม่เคยขอสิทธิ์ = ถูกปฏิเสธ"** ซึ่งเป็นคนละ
คำถามกับ **"ขอแล้วรออนุมัติ = ถูกปฏิเสธหรือไม่"** — ต้องทำ §11.3 ข้อ 1 (เพิ่มเข้า use case) ก่อน
แล้วทดสอบซ้ำ จึงจะได้คำตอบที่คุมตัวแปรครบ

**ข้อมูลเพิ่มที่ได้จาก MCP รอบเดียวกัน:**
- ✅ **Business Verification ผ่านแล้ว** (เงื่อนไขหนึ่งของฟีเจอร์นี้)
- 🛑 **ยื่น App Review ใบใหม่ไม่ได้ตอนนี้** — มีใบค้างอยู่ 5 permission (`pages_messaging`,
  `pages_show_list`, `pages_manage_metadata`, `pages_read_engagement`, `business_management`)
  ซึ่ง**ยังขาด screencast ทั้ง 5 ตัว** ⇒ ต้องปิดใบนั้นก่อนถึงจะยื่นขอ `human_agent` ได้
- เอกสารทางการ (`features-reference/human-agent`) **ไม่ระบุ tier** Standard/Advanced เลย เขียนแค่ว่า
  ต้องผ่าน App Review + business verification ก่อนแตะ live data — จึงยังตีความปิดประเด็นไม่ได้

**ผลต่อ Roadmap:** กอง 1 จบแล้ว · กอง 2 โค้ดจบและ allow-list ใช้งานได้จริง แต่ยังพิสูจน์ "เส้นทาง
สำเร็จ" ไม่ได้ · กอง 3 บล็อกที่ใบ App Review ค้าง ไม่ใช่ที่โค้ด
