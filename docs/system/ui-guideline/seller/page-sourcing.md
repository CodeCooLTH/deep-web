...
| `/seller/verification` (level cards) | **Compose from** `app/(admin)/apps/ecommerce/(sellers)/seller-details/components/SellerStatisticCard.tsx` (+ `SellerContact.tsx`) | ไม่มี template verification-workflow ตรง. ใช้ `SellerStatisticCard` เป็น card primitive: 3 instance (L1/L2/L3), แต่ละอันมีชื่อ level, status chip (pending/approved/rejected), description, upload CTA |
