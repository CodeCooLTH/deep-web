...
- **`useAuth` ใน `MainLayout`** — Paces theme ใช้ `useAuth` hook ของตัวเอง (ไม่ใช่ NextAuth). เวอร์ชัน SafePay ต้องเปลี่ยนเป็น `useSession()` จาก `next-auth/react` พร้อม redirect ไป `/auth/sign-in` เมื่อ session ไม่มี
...
- **🛑 seller nav ใช้ path สั้น ห้ามมี `/seller` prefix (REVERSED 2026-05-16 — retro `2026-05-16-seller-path-prefix-removal.md`, commits `068bcc5`+`accf9af`)** — ทุก `router.push/replace`, server `redirect()`, `<Link href>`, `cancelHref`/`retryHref`, `signOut({callbackUrl})`, menu url, `getLoginPath('seller')` ใน `(paces)/seller/**` ที่ชี้ route ใต้ seller tree → **เขียน path สั้น** (`/dashboard`,`/orders`,`/products`,`/auth/sign-in`) **ห้ามใส่ `/seller` prefix**. `src/proxy.ts` seller block จัดการครบ: rewrite short→internal `/seller/*` (cold GET + client nav) **+ 301 redirect URL เก่า `/seller/*`→short** (bookmark/พิมพ์ตรง). QA Phase นี้ยืนยัน client `router.push`/server `redirect()`/form submit → address bar สะอาดไม่มี `/seller/`. **เห็น `/seller/` ใน nav = บั๊ก ต้อง strip** (เดิม Phase B เคยบังคับ explicit `/seller/*` — ยกเลิกแล้ว, สาเหตุ Phase B B7 คือ proxy ตอนนั้นยังไม่มี backward-compat + nav ปนทั้งสองแบบ; ตอนนี้ proxy รับ short path ครบ). ข้อยกเว้น: `/u/[username]` `/o/[token]` = buyer-domain public route → absolute buyer URL ผ่าน `resolveBuyerBaseUrl()`; `/api/*` subdomain-agnostic; `?query`-only relative ปล่อยได้
