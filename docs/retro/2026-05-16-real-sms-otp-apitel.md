...
### RC3 — module-level Map ไม่ใช่ cross-route singleton (commit `82666cb`)
หลัง user ใส่ key จริง + ถอด test-bypass → real OTP login 401 ทุกครั้ง. reproduce: `storeOtp` ใน `/api/otp/send` กับ `verifyOtp` ใน `/api/auth/[...nextauth]` (และ `/api/otp/verify`) เป็นคนละ module instance — Next.js bundle แต่ละ route handler แยกกัน → `const otpStore = new Map()` ระดับ module ไม่ shared → store ว่างตอน verify → 401. **test-account bypass (`123456` เช็ก `TEST_ACCOUNTS` const ก่อนแตะ store) บังบั๊กนี้ไว้ตลอด dev cycle** — ทุก QA/smoke ที่ผ่านมาใช้ test path เลยไม่เคยโดน.
→ **Lesson (repo-wide):** state ที่ mutable + ต้อง share ข้าม request/route ใน Next.js App Router **ห้าม**เป็น `const x = new Map()` ระดับ module — ต้อง `globalThis` singleton (pattern `src/lib/prisma.ts`). และ **test-bypass / mock ที่ลัด real path = ตัวบังบั๊กของ real path** — ต้องมีอย่างน้อย 1 เทสที่เดิน real path จริง (ไม่แตะ bypass) ก่อน claim feature complete. fix: ย้าย otpStore + otpRequestTimestamps → globalThis. verified: send→NextAuth callback = 200 + session จริง.
