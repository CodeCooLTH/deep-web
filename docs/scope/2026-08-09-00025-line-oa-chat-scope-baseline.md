...
| หัวข้อ | รายละเอียด |
|---|---|
| **ทำ** | badge ช่องทาง LINE บนเธรด · Quota Meter ("เหลือ x/y ข้อความเดือนนี้") · แถบ/ตัวชี้สถานะหน้าต่างตอบฟรี (**แก้ 2026-08-10: นับถอยหลังวินาทีบนปุ่มส่งแล้ว** — ข้อห้ามที่แท้จริงคือห้ามมีตัวเร่งความเครียด ไม่ใช่ห้ามตัวเลข · 🛑 `AC-005-05` ที่เคยอ้างไว้ตรงนี้เป็น ID ของ feature 00019 ไม่ใช่ของ 00025 ของจริงคือ bullet ใต้ `FR-LINE-05` ซึ่งขอให้แสดง "เหลือเวลาเท่าไร" ด้วย) · ตัวชี้ว่าข้อความไหนตอบด้วยระบบอัตโนมัติ (ใช้ pattern เดิมของ 00023) · หมายเหตุถาวรในเธรด LINE ว่า "ข้อความที่ตอบจากแอป LINE OA จะไม่ปรากฏที่นี่" |
...
| 2026-08-10 | **S-7b (ใหม่) — แสดงรูปสติกเกอร์ LINE จริงในเธรด ทับ OOS-08 บางส่วน** (`0d7ca4c1` prod แล้ว) | user สั่งตรง: *"สติ๊กเกอร์ ต้องทำให้สมบูรณ์นะ ต้องโหลด sticker ไลน์มาแสดงได้ ส่งได้ รับได้ เห็นในจอ"* — OOS-08 เดิมตัดทั้งขาเข้าขาออก มติใหม่เปิดทั้งคู่. ขาเข้าเสร็จแล้ว: mirror รูปจาก `stickershop.line-scdn.net` (URL นี้**ไม่อยู่ในเอกสารทางการ** จึงห้ามเก็บ URL ลง DB ต้อง mirror ทันทีที่รับ) | user |
| 2026-08-10 | 🛑 **มติทิศทาง — LINE คือ "ทางเข้าของออเดอร์" ไม่ใช่ "ที่แทนแอปแชท"** | user สรุปเองหลังเห็นข้อจำกัดครบ: *"พนักงานจะคุยจบใน Deep หรือกลับไปคุยในแอป LINE ก็ไม่พังอะไร เพราะสิ่งที่เราต้องการคือออเดอร์ ไม่ใช่บทสนทนา"*. **ผลกับ scope:** ขาเข้าคือส่วนที่ต้องครบ 100% (เห็นทุกอย่างที่ลูกค้าส่งมา) · ขาออกทำเท่าที่แพลตฟอร์มให้ ไม่ต้องไล่ให้เท่าแอป LINE · 🛑 **S-12 (ตอบอัตโนมัติ) ชะลอ** เพราะ LINE ไม่มี echo → ถ้าพนักงานตอบในแอป บอทจะตอบซ้ำ **และเราตรวจไม่ได้เลย** (ต่างจาก Messenger ที่มี echo) | user |
| 2026-08-10 | **S-18 (ใหม่) — ส่งสติกเกอร์ + ตอบกลับแบบอ้างข้อความ (quote)** | user สั่งทับข้อเสนอของ Controller ที่ให้ตัดทิ้ง: *"ใน Deep ก็ต้องส่ง Sticker ได้ครับ ถึงแม้จะพื้นฐานก็ตาม ต้อง reply ข้อความได้"* — เหตุผลที่รับได้: พนักงานที่ตอบใน Deep ไม่ควรรู้สึกว่าเครื่องมือด้อยกว่าจนต้องหนีกลับไปแอป LINE ซึ่งจะพาออเดอร์หนีไปด้วย (ขัดกับมติทิศทางแถวบน). **ถูกกว่าที่ประเมินไว้มาก** — `quoteToken` มาใน webhook ขาเข้าอยู่แล้วและเก็บใน `rawMessage` ตั้งแต่ S-6 · UI ตอบกลับมีครบแล้วจาก Deep/Messenger (`ChatThread.tsx:2211`) เหลือแค่ต่อสาย. 🛑 **เพดานที่แก้ไม่ได้: ส่งได้เฉพาะสติกเกอร์ในรายการที่ LINE ประกาศ ไม่ใช่ตัวที่ร้านซื้อ** — ux ออกแบบ notice ถาวรในแผงให้แล้ว | user |
