...
| **S-7** | **Command Center home** — `CommandCenter` + `CommandTile` (RSC) render ใน `dashboard/page.tsx` ฝั่ง `lg:hidden` | ที่ ≤1024px: เปิด `/dashboard` เจอ command center (greeting+trust, 2 hero tile, quick-access grid 6 tile, ลิงก์รายงานเต็ม) ตั้งแต่ paint แรก ไม่ flash; ที่ ≥1024px เจอ dashboard เดิม. ทุก tile เป็นลิงก์ไปหน้าเป้าหมายถูกต้อง, badge live แสดงค่าจริง (หรือ label เฉย ๆ ถ้า data ขาด). spec: `docs/superpowers/specs/2026-06-07-seller-mobile-command-center-design.md` | TODO |
| **S-8** | **CreateFab** (client) — FAB speed-dial มุมขวาล่างบน command center | ที่ ≤1024px: FAB น้ำเงินมุมขวาล่าง; กด → กาง 3 ปุ่ม (สร้างออเดอร์/สินค้า/หมวดหมู่) + backdrop dim; กด ×/backdrop/ESC ปิด; แต่ละปุ่มลิงก์ถูก route; touch ≥44px; a11y aria-expanded + focus trap | TODO |
| **S-9** | **Command center data** — service count functions ที่ยังไม่มี (pendingOrderCount, productActiveCount, customerCount, ฯลฯ) | RSC `dashboard/page.tsx` ประกอบ `CommandCenterData` ครบทุก field; service ใหม่เป็น read-only count (ไม่แตะ schema); badge ทุกตัวแสดงค่าตรงกับ DB | TODO |
...
| 2026-06-07 | **รับเข้า S-7/S-8/S-9 (Command Center)** | user ขอเปลี่ยนทิศ: mobile ต้องเป็น command center launcher (FAB speed-dial) ไม่ใช่แค่ responsive-fix. brainstorm + mockup V2 อนุมัติ. OOS-5 (redesign) บางส่วนถูก promote เข้า scope ในรูป command center home (ไม่ใช่ redesign หน้าปลายทาง) | Controller (shinobu22 approve mockup) |
