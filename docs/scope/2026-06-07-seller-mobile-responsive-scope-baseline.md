...
| **S-8** | **Mobile topbar** (render ที่ layout-level, ทุกหน้า seller mobile <lg) — **ไม่มี hamburger/sidebar** (revised 2026-06-07 ดู Change Log) | dashboard = identity mode (avatar + ชื่อร้าน truncate + tier chip Deep tier SSOT + bell dot); หน้าย่อย = back button (deep-link safe: history>1 ? back : /dashboard) + ชื่อหน้า (จาก sellerMenuItems) + bell; topbar โผล่ทุกหน้า mobile; ไม่มี sidebar/offcanvas/hamburger ที่ <lg | DONE |
...
| 2026-06-07 | **BUILD S-7..S-13 เสร็จ + V4 visual polish** | command center V3 build (T1-T9) + V4 polish (คุม tone สี 4 กลุ่ม, card treatment, highlight bar, tier chip Deep tier SSOT). QA MCP pass. spec `2026-06-07-command-center-v4-polish-design.md` | Controller (shinobu22 approve mockup v4) |
| 2026-06-07 | **revise S-8: เอา hamburger/sidebar ออกทุกหน้า mobile → Seller Mobile Shell** | user สั่ง: mobile (<lg) ไม่มี hamburger/sidebar; topbar เดียวสม่ำเสมอทุกหน้า (dashboard=identity, หน้าย่อย=back+ชื่อหน้า); nav ผ่าน command center hub + back. แก้ root cause padding dead-zone (769-1023px sidebar margin ค้าง). plan `2026-06-07-seller-mobile-shell-build.md`. **S-8 acceptance เดิม (hamburger เปิด offcanvas) ถูกแทนที่** | Controller (shinobu22) |
